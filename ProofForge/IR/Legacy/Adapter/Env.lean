...
def adaptContextRead (field : ProofForge.IR.ContextField) :
  ...
  | .gasPrice => .ok (.host ProofForge.Target.HostOps.Evm.gasPriceSig.id .u64)
  | .baseFee => .ok (.host ProofForge.Target.HostOps.Evm.baseFeeSig.id .u64)
  | .coinbase => .ok (.host ProofForge.Target.HostOps.Evm.coinbaseSig.id .hash)
