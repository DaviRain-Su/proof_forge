...
partial def lowerStmt (ctx : Ctx) (env : LocalTypes) (returns : ValueType)
  ...
  | .effect (.eventEmitIndexed name _ _) => err (indexedEventUnsupportedMessage name)
  | .assert cond _ => do
...
/- EmitWat capability surface: the wasmNear profile plus `controlConditional`
    `controlBoundedLoop`, storage arrays, fixed arrays, and structs. Crosscall
    remains rejected with a NEAR Promise-specific diagnostic. -/
