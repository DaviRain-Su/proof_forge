...
def HostABI.soroban : HostABI := {
  ...
  returnData := "value_return"  -- retained NEAR import (Soroban uses set_return_data, TODO)
  ...
  randomSeed := "random_seed"                -- retained
  prepaidGas := "prepaid_gas"               -- retained (Soroban has compute_budget)
  usedGas := "used_gas"                     -- retained
