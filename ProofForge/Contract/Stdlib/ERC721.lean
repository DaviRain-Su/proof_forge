...
def initialized : ScalarRef :=
  ProofForge.Contract.Source.Legacy.slot "erc721Initialized" .u64

def mintAuthority : ScalarRef :=
  /- Portable callers are represented by the canonical u64 identity handle. -/
  ProofForge.Contract.Source.Legacy.slot "erc721MintAuthority" .u64
...
contract_mixin ERC721Mixin do
  ...
  query ownerOf (tokenId : .u64) returns(.u64) do
    ...
    do ProofForge.Contract.Source.Legacy.requireNonZero (ProofForge.Contract.Source.Legacy.ref tokenOwner) "invalid token";
    return tokenOwner;
  ...
  entry transferFrom (holder : .address, recipient : .address, tokenId : .u64) do
    ...
    do ProofForge.Contract.Source.Legacy.requireEq (ProofForge.Contract.Source.Legacy.ref tokenOwner)
      (ProofForge.Contract.Source.Legacy.ref holder) "wrong from";
    do ProofForge.Contract.Source.Legacy.requireEq (ProofForge.Contract.Source.Legacy.ref operator)
      (ProofForge.Contract.Source.Legacy.ref holder) "not authorized";
  ...
  entry safeTransferFrom (holder : .address, recipient : .address, tokenId : .u64) do
    ...
    do ProofForge.Contract.Source.Evm.checkErc721Received
      ...
      (ProofForge.Contract.Source.Legacy.ref recipient)
      (ProofForge.Contract.Source.Legacy.ref tokenId);

  entry mint (recipient : .address, tokenId : .u64) do
    do ProofForge.Contract.Source.Legacy.requireEq caller
      (ProofForge.Contract.Source.Legacy.read mintAuthority) "not mint authority";
    do ProofForge.Contract.Source.Legacy.requireNonZero (ProofForge.Contract.Source.Legacy.ref recipient) "zero recipient";
    let existing : .u64 := mapRead tokenOwners tokenId;
    do ProofForge.Contract.Source.Legacy.requireEq (ProofForge.Contract.Source.Legacy.ref existing) (u64 0) "token exists";
    do mapWrite tokenOwners tokenId recipient;
    emit Transfer indexed #[
  ...
  entry burn (tokenId : .u64) do
    ...
    let tokenOwner : .u64 := mapRead tokenOwners tokenId;
    do ProofForge.Contract.Source.Legacy.requireEq (ProofForge.Contract.Source.Legacy.ref tokenOwner)
      (ProofForge.Contract.Source.Legacy.ref who) "not owner";
