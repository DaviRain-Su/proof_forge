...
| `TASK-D1-04` | pending | 完整 statement/expression/place/pattern；local fn与qualified external call/schedule分流 | grammar/legacy decoder有大量 unary/binary、let/assert/revert/emit/if/for等；V1 Loader 现已从 Syntax 直接构造 `.call/.schedule ExternalCallExprV1`、最小 `.if_/.for_` control-flow slice、`.localCall` / `.constructor` / bare-base `.place (.index ...)` expression slice、prefix `-`/`~`/`!` `.unary` expression slice、core statement slice（annotated/omitted-type `.let_`、simple-component `.assign`、valued/value-less `.return_`、`.assert_`/assert-else）以及 `.revert`/`.emit` statement slice，保留 raw qualified components、escaped embedded-dot raw components、args/index/source order、unary operator/operand order、nested/dangling-else ownership、statement/source order、raw iterator/binder/target/condition/error/event identity、revert/emit argument count/order/nested expression shape与 loop endpoint order；`decodeNameV1` 已在 ProgramV1 Syntax decoder 层拒绝多 component Lean Name，whole-escaped dotted identifier 仍是一个 raw component，bare `A.x` expression/assignment 以及 unescaped qualified revert/emit name 在 field-place/D2 error-event lookup 尚未迁移前 fail closed；并拒绝 one-component/reserved/legacy string、malformed core/if/for/call-like/index/revert/emit、reserved iterator/callee/path/base/binder/target/error/event、qualified index base 和非法/越界 exact decimal bound；仍缺 match/field-place/pattern、D2 error/event lookup、typing/effect、Semantic rollback、event runtime与target behavior等完整迁移。shared grammar 仍保留 string-call shape 用于稳定拒绝，不能写成 string grammar 已删除 | `ProgramV1ExternalStatements`、`ProgramV1ControlFlow`、`ProgramV1ExpressionForms`、`ProgramV1UnaryExpressions`、`ProgramV1CoreStatements` 与 `ProgramV1RevertEmitStatements` 是聚焦 source→ProgramV1 suites；`Tests/Language/*Statements.lean`、operator suites（含 CheckedNeg/BitwiseNot/LogicalNot/Grouping）与 legacy expression suites 仍固定旧 Source AST | **旧轨实现 + 新轨子集** | formal 仍 pending；继续迁移剩余 statement/expression/place/pattern、旧 statement/operator suites 与 legacy decoder/grammar reader；未达到删除门槛，不删除旧代码 |
