...
| `TASK-D1-04` | pending | 完整 statement/expression/place/pattern；local fn与qualified external call/schedule分流 | grammar/legacy decoder有大量 unary/binary、let/assert/revert/emit/if/for等；V1 Loader 现已从 Syntax 直接构造 `.call/.schedule ExternalCallExprV1` 与最小 `.if_/.for_` control-flow slice，保留 raw qualified components、args order、nested/dangling-else ownership、statement/source order、raw iterator identity与 loop endpoint order，并拒绝 one-component/reserved/legacy string、malformed if/for、reserved iterator 和非法/越界 exact decimal bound；仍缺 match/field-place/pattern 等完整迁移。shared grammar 仍保留 string-call shape 用于稳定拒绝，不能写成 string grammar 已删除 | `ProgramV1ExternalStatements` 与 `ProgramV1ControlFlow` 是聚焦 source→ProgramV1 suites；`Tests/Language/*Statements.lean`、operator suites 仍固定旧 Source AST | **旧轨实现 + 新轨子集** | formal 仍 pending；继续迁移剩余 statement/expression/place/pattern、旧 statement/operator suites 与 legacy decoder/grammar reader；未达到删除门槛，不删除旧代码 |
...
2. 完整迁移 statement/expression/call/schedule和 `TST-SRC-005`。
