...
| `TASK-D1-04` | pending | 完整 statement/expression/place/pattern；local fn与qualified external call/schedule分流 | grammar/legacy decoder有大量 unary/binary、let/assert/revert/emit/if/for等；V1 Loader 现已从 Syntax 直接构造 `.call/.schedule ExternalCallExprV1`、最小 `.if_/.for_` control-flow slice、以及 `.localCall` / `.constructor` / bare-base `.place (.index ...)` expression slice，保留 raw qualified components、escaped embedded-dot raw components、args/index/source order、nested/dangling-else ownership、statement/source order、raw iterator identity与 loop endpoint order，并拒绝 one-component/reserved/legacy string、malformed if/for/call-like/index、reserved iterator/callee/path/base、qualified index base 和非法/越界 exact decimal bound；仍缺 match/field-place/pattern 等完整迁移。shared grammar 仍保留 string-call shape 用于稳定拒绝，不能写成 string grammar 已删除 | `ProgramV1ExternalStatements`、`ProgramV1ControlFlow` 与 `ProgramV1ExpressionForms` 是聚焦 source→ProgramV1 suites；`Tests/Language/*Statements.lean`、operator suites 与 legacy expression suites 仍固定旧 Source AST | **旧轨实现 + 新轨子集** | formal 仍 pending；继续迁移剩余 statement/expression/place/pattern、旧 statement/operator suites 与 legacy decoder/grammar reader；未达到删除门槛，不删除旧代码 |
