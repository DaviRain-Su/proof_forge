...
- Snapshot commit: `78cf189e477a8c7f1595443e9806048c27325cbe`
...
| Task | Formal | 新设计要求 | 实际代码与产品接线 | 现有测试事实 | Engineering | 迁移与旧代码处理 |
|---|---|---|---|---|---|---|
| `TASK-D2-01` | pending | 完整name/type checker、pure-fn call graph、invariant/proof-reference source binding | 新增 `ProofForgeV2/Typed/ModelV1.lean` 与 `NameResolutionV1.lean` 直接消费 `ValidatedSourceV1`，构建 13 类声明 source-order 序数表与显式作用域名解析；新增 `ProofForgeV2/Typed/TypeCheckV1.lean` 在 name resolution 之后对 literal/place（含 struct field chain 与 Array/Map index）/unary/binary/constructor/localCall 做精确 `TypeV1` 推导与宽度边界检查，mixed-type/unknown/非法 place 按 source order 给出 expected-vs-actual 诊断，struct constructor 按 declaration order 检查 field count/types 并产生 `.named structName`，enum variant constructor 检查 payload count/types 并产生 `.named enumName`，local fn call 检查 callee 解析、arg count 与 param types 并传播 fn result 类型，entry/view/state/const/local/param 作为 callee 保留 wrong-category/local-as-function 诊断；整数字面量必须携带预期整数类型并按宽度精确范围检查、无默认宽度；不做 statement/effect | 新增 `Tests/Typed/NameResolutionV1.lean` 覆盖作用域解析与错误类别诊断；新增 `Tests/Typed/TypeCheckExpressionsV1.lean` 覆盖运算符结果类型、struct field chain 与 Array/Map index、整数字面量边界、mixed-type 二元拒绝与 expected-vs-actual source order、一元类型门、bool/整数 分离；新增 `Tests/Typed/TypeCheckCallsV1.lean` 覆盖 struct/enum constructor 与 local fn call 的 exact/nested/wrong-count/wrong-type/wrong-category、source-order 诊断优先级以及 match expression 继续 fail closed | **新轨子集** | 继续扩展完整 type/effect checker；迁移全部 declaration 和 fn/proof binding |
