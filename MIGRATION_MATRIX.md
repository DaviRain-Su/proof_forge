...
- Snapshot commit: `0f26b61ad7e9e6fea960cbb5d203ac3f3d82140f`
...
| `TASK-D1-07` | pending | stable DiagnosticV1 code/schema/order/redaction和source origin | 新增 `ProofForgeV2/Core/DiagnosticV1.lean`：closed `DiagnosticCodeV1` enum 覆盖 `PF-SRC-INVALID`/`PF-BOUND-001`/`PF-TOOLCHAIN-MISSING`/`PF-TOOLCHAIN-MISMATCH`/`PF-TARGET-NOT-IMPLEMENTED`/`PF-OUTPUT-ATOMICITY`/`PF-INTERNAL`，`DiagnosticV1` 使用 `Array SourceOrigin`，提供 `toCanonicalJson`/`renderHuman` 与 `compare`/`sortAndDedupe`；当前 producer 仍返回 alpha `CompileError`，尚未接线 | `Tests/Core/DiagnosticV1.lean` 固定 wire 唯一性、human golden、canonical JSON 精确性与 round-trip、origin JCS codec round-trip、排序全序/传递/`sortAndDedupe` 幂等去重、absolute path redaction | **可复用地基** | 实现typed diagnostic bundle、稳定priority/origin/redaction和human/JSON renderer；迁移测试后移除String error seam |
