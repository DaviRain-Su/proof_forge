...
| `TASK-D1-03` | pending | 13种 Phase-1 declaration、完整 type/default elaboration | legacy decoder覆盖 state/struct/enum/const/event/error/init/entry/view/fn/invariant/requires/proof；V1 decoder只实现 state/init/entry/view，type只实现 primitive、Bytes、Field、单层 Option子集 | `DeclarationAcceptance`包装大量 legacy Source AST suites；ProgramV1 full-tag golden是constructed value，不是source parser coverage | **旧轨实现 + 新轨子集** | 将完整 declaration直接实现到唯一 ProgramV1 decoder并迁移 tests；不得经 legacy AST投影 |
| `TASK-D1-04` | pending | 完整 statement/expression/place/pattern；local fn与qualified external call/schedule分流 | grammar/legacy decoder有大量 unary/binary、let/assert/revert/emit/if/for等；V1 decoder有部分 expr和简单 stmt，缺 if/for/match/field-place/qualified call/schedule。旧 surface仍是 `call "string"` | `Tests/Language/*Statements.lean`、operator suites固定旧 Source AST；没有完整 source→ProgramV1 parser suite | **旧轨实现 + 新轨子集** | 按 ProgramV1构造完整迁移；采用 `call QualifiedId(...)`/`schedule QualifiedId(...)`，删除 string-call reader而非兼容 |
