...
- Snapshot commit: `70d01487e06b64fa08354e3d6c7c63ae415e7332`
...
| Task | Formal | 新设计要求 | 实际代码与产品接线 | 现有测试事实 | Engineering | 迁移与旧代码处理 |
|---|---|---|---|---|---|---|
| `TASK-D2-01` | pending | 完整name/type checker、pure-fn call graph、invariant/proof-reference source binding | 新增 `ProofForgeV2/Typed/ModelV1.lean` 与 `NameResolutionV1.lean` 直接消费 `ValidatedSourceV1`，构建 13 类声明 source-order 序数表（state/struct/enum/const/event/error/init/entry/view/fn/invariant/extension/proof）与显式作用域名解析：body 内 params 优先于 state、let 按 source order 遮蔽、for/match binders 局部、local call 仅限 fn、named type 仅限 struct/enum、revert/emit/error/event/invariant 精确分类；不做 body type checking，不改动现有 alpha Typed 路径 | 新增 `Tests/Typed/NameResolutionV1.lean` 覆盖 declaration-table ordinals、params/let 遮蔽、match/for binder 泄漏、fn-only local call、struct/enum-only named types、unknown/wrong-category diagnostics source order | **新轨子集** | 继续扩展 `ModelV1`/`NameResolutionV1` 到完整 type/effect checker；迁移全部 declaration 和 fn/proof binding |
