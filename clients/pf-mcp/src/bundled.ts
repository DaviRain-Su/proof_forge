  ...
  "targets": [
    ...
    },
    {
      ...
      "localDev": {
        "offlineInterpret": null,
        "chainLike": null,
        "deploy": "official psyup deploy / psy_user_cli / WebIDE — not pf deploy",
        "frontend": "official psy-template + @psy-protocol/* (not PF template yet)",
        "dpnArtifact": "{program}.dpn.json from pf build -t psy"
      },
      "honesty": [
        "canonical DPN package only from PF (profile psy-dpn-v1, deployable=false)",
        "no Psy-lang source, Dargo, local VM, UPS, proof, or network lane inside PF",
        "hand-off to official psyup/dargo/psy_user_cli/SDK/wallet for deploy & UX",
        "config endpoints drift — refresh config.psy-protocol.xyz",
        "engineering only — not formal/hermetic/mainnet evidence"
      ],
      "ecosystem": {
        "docs": [
