...
export const CATALOG_JSON = {
  ...
  "notes": [
    ...
    "NEAR frontend skeleton: templates/near-dapp-ui (near-api-js ecosystem; not shipped wallet).",
    "CosmWasm frontend skeleton: templates/cosmwasm-dapp-ui (cosmjs ecosystem).",
    "NEAR frontend: docs/product/18-near-dapp-frontend.md + templates/near-dapp-ui.",
    "CosmWasm frontend: docs/product/17-cosmwasm-dapp-frontend.md + templates/cosmwasm-dapp-ui."
  ],
  "networksRef": {
  ...
  "targets": [
    ...
    },
    {
      ...
      "pfSurface": {
        ...
        "test": "pf test -t near → scripts/pf_near_test.sh (artifact fast-path when *.wasm; else corpus; skip-clean if tools missing)",
        "deploy": "pf deploy -t near save-only; --broadcast refused",
        "network": "catalog-only (near.local.sandbox / near.testnet); no product broadcast",
        "mcpTools": [
          "pf_list_targets",
          "pf_doctor",
          "pf_install",
        ...
        "run": "pf run -t near -- <method> [u64…] → one-shot near-sandbox (engineering)",
        "doctor": "pf setup --target near (near-sandbox, wat2wasm, pf_near_test/run scripts)",
        "scaffoldUi": "pf scaffold-ui --template near-dapp"
      },
      "frontendClients": [
        {
          ...
          "shippedByProofForge": false,
          "template": "templates/near-dapp-ui"
        }
      ],
      "localDev": {
        "offlineInterpret": null,
        "chainLike": "near-sandbox engineering (host-heavy); scripts/near_runtime_test.sh fifteen suites; pf deploy save-only"
      },
      "honesty": [
        "Engineering Wasm + near-sandbox only — not formal Reference↔sandbox",
        "Sync call / sync native·token transfer permanent fail-closed (Promise async)",
        "Public testnet/mainnet broadcast not a product path",
        "Proof-bearing vault (ADR-0042) is Reference-verified + runtime observed, not formal target refinement",
        "CI: ordinary near-runtime job (path-filtered; Tool Lock near-sandbox)"
      ],
      "agentGuide": "docs/product/near-agent-cheatsheet.md",
      "syncAsyncGuide": "docs/product/near-sync-async-api.md",
      "parityRoadmap": "docs/plan/near-parity-roadmap.md",
      "demoExamples": [
        "Examples/PoseTransform.lean",
        "Examples/BlockHeightCheck.lean",
        "Examples/UnixTimeCheck.lean",
        "Examples/ConstAnswer.lean",
        "Examples/StateCell.lean",
        "Examples/VerifiedVaultPF.lean"
      ],
      "frontendTemplate": "templates/near-dapp-ui",
      "frontendGuide": "docs/product/18-near-dapp-frontend.md"
    },
    {
      "id": "noir",
      "implemented": true,
    ...
    },
    {
      ...
      "localDev": {
        ...
        "frontend": "templates/psy-dapp-ui + official window.psy / @psy-protocol/*",
        "dpnArtifact": "{program}.dpn.json from pf build -t psy",
        "abi": "scripts/psy_dpn_to_abi.py → *.abi.json (also pf deploy/test)",
        "diff": "just psy-dpn-diff (official simulate vs session)"
      },
      "honesty": [
    ...
    },
    {
      ...
      "pfSurface": {
        ...
        "test": "pf test -t cosmwasm → scripts/pf_cosmwasm_test.sh (artifact fast-path when *.wasm; else corpus)",
        "deploy": "pf deploy -t cosmwasm save-only; --broadcast refused",
        "network": "catalog-only (cosmwasm.local.mock); no product broadcast",
        "mcpTools": [
          "pf_list_targets",
          "pf_doctor",
          "pf_install",
        ...
        "doctor": "pf setup --target cosmwasm (wat2wasm, cosmwasm-check, pf_cosmwasm_test)",
        "scaffoldUi": "pf scaffold-ui --template cosmwasm-dapp",
        "run": "not in v0 — use pf test -t cosmwasm (JSON ABI + cosmwasm-vm mock)"
      },
      "frontendClients": [
        {
          ...
          "shippedByProofForge": false,
          "template": "templates/cosmwasm-dapp-ui"
        }
      ],
      "localDev": {
        "offlineInterpret": null,
        "chainLike": "cosmwasm-vm mock + optional wasmd Docker rung; pf deploy save-only"
      },
      "honesty": [
        "Engineering Wasm + cosmwasm-vm only — not formal Reference↔host",
        "Generic sync call permanent fail-closed; pf.assets bank/CW20 use SubMsg reply_on=never (error-propagating)",
        "schedule = same-tx SubMsg never (not cross-tx async)",
        "Public chain broadcast refused in pf v0",
        "Scalar const + Bytes N state/return open; Map return and invariants remain fail-closed",
        "CI: ordinary cosmwasm-runtime job (path-filtered; cosmwasm-vm mock)"
      ],
      "agentGuide": "docs/product/cosmwasm-agent-cheatsheet.md",
      "demoExamples": [
        "Examples/StateCell.lean",
        "Examples/ConstAnswer.lean",
        "Examples/BlockHeightCheck.lean",
        "Examples/TipJar.lean",
        "Examples/TokenJar.lean",
        "runtime-tests/cosmwasm/fixtures/BytesRet.lean",
        "runtime-tests/cosmwasm/fixtures/CallerGate.lean",
        "Examples/UnixTimeCheck.lean",
        "Examples/PoseTransform.lean"
      ],
      "frontendTemplate": "templates/cosmwasm-dapp-ui",
      "frontendGuide": "docs/product/17-cosmwasm-dapp-frontend.md"
    },
    {
      "id": "ton",
      "implemented": true,
    ...
    },
    {
      ...
      "honesty": [
        "ADR-0044 S0: .rs recipe only; zero-tool finalize; deployable=false; no stellar-cli/Wasm/auth/TTL claim"
      ]
    },
    {
      "id": "icp",
      ...
      "pfSurface": {
        ...
        "network": "none",
        "test": "pf test / local --target icp → scripts/pf_icp_test.sh (PocketIC 15.0.0 host-optional; skip-clean if POCKET_IC_BIN missing)",
        "mcpTools": [
          "pf_list_targets",
          "pf_doctor",
          "pf_install",
      ...
      "localDev": {
        "offlineInterpret": null,
        "chainLike": "PocketIC StateCell engineering gate (not mainnet)"
      },
      "honesty": [
        "ADR-0047 ICP-3: wat2wasm Finalize → .wasm; PocketIC 15.0.0 host-optional StateCell gate",
        "sync call and portable emit fail closed; async workflow advertised only (no Plan shape yet)",
        "heap globals only — no upgrade/stable/timers/HTTP outcalls/management canister",
        "source-only maturity; deployable=true means Wasm artifact present, not subnet/mainnet readiness",
        "not accepted Phase 1 / formal / hermetic / Stage-0"
      ]
    },
    {
      "id": "openvm",
      ...
      "honesty": [
        "openvm-guest-source-v1 zero-tool guest source + catalog only",
        "no ELF/VmExe/prove/verify toolchain install",
        "not accepted Phase 1; not verifiable-workload"
      ]
    }
  ]
} as const;
...
export const NETWORKS_JSON = {
  ...
  "networks": [
    ...
    },
    {
      ...
      "targetFamily": "near",
      "displayName": "NEAR near-sandbox (local engineering)",
      "env": "local",
      "chainId": null,
      "nativeCurrency": {
        "name": "NEAR",
      ...
      "explorers": [],
      "policy": "local-only",
      "pfProductBroadcast": "refused",
      "related": {
        "runtimeScript": "scripts/near_runtime_test.sh",
        "guide": "docs/product/near-agent-cheatsheet.md",
        "parityRoadmap": "docs/plan/near-parity-roadmap.md"
      },
      "honesty": [
        "Engineering sandbox only — not testnet/mainnet",
        "Default port illustrative; near_runtime_test.sh picks an ephemeral port",
        "pf deploy --broadcast refused for NEAR public nets in v0"
      ]
    },
    {
      "id": "near.testnet",
      "targetFamily": "near",
      "displayName": "NEAR testnet (catalog only)",
      "env": "testnet",
      "chainId": null,
      "nativeCurrency": {
        "name": "NEAR",
      ...
      "policy": "public-testnet-catalog-only",
      "pfProductBroadcast": "refused",
      "related": {
        "guide": "docs/product/near-agent-cheatsheet.md"
      },
      "honesty": [
        "Catalog entry for agent RPC discovery only",
        "pf deploy --broadcast refused — no product key path",
        "Not formal / not mainnet"
      ]
    },
    {
      "id": "cosmwasm.local.mock",
      "targetFamily": "cosmwasm",
      "displayName": "CosmWasm cosmwasm-vm mock (local engineering)",
      "env": "local",
      "chainId": null,
      "nativeCurrency": {
        "name": "stake",
      ...
      "pfProductBroadcast": "refused",
      "related": {
        "runtimeScript": "scripts/cosmwasm_runtime_test.sh",
        "guide": "docs/product/cosmwasm-agent-cheatsheet.md"
      },
      "honesty": [
        "Engineering mock only — not wasmd chain / mainnet",
        "pf deploy --broadcast refused"
      ]
    },
    {
      "id": "cosmwasm.testnet.catalog-only",
      "targetFamily": "cosmwasm",
      "displayName": "CosmWasm testnet (catalog only)",
      "env": "testnet",
      "chainId": null,
      "nativeCurrency": {
        "name": "stake",
      ...
      "honesty": [
        "Catalog placeholder only — no product key path",
        "Not formal / not mainnet"
      ]
    }
  ],
  "ecosystems": {
    ...
    "near": {
      "displayName": "NEAR",
      "defaultLocal": "near.local.sandbox",
      "notes": "Wasm via proof-forge-next --target near; runtime = locked near-sandbox."
    }
  }
} as const;
