//! Spawn `scripts/pf_ton_test.sh` (@ton/sandbox engineering corpus).
...
pub fn resolve_ton_test_script() -> PfResult<PathBuf> {
    if let Ok(p) = std::env::var("PROOF_FORGE_TON_TEST_SCRIPT") {
        let pb = PathBuf::from(&p);
        if pb.is_file() {
            return Ok(pb);
        }
        return Err(PfError::Tool(format!(
            "PROOF_FORGE_TON_TEST_SCRIPT set but not a file: {p}"
        )));
    }
    if let Some(root) = compiler::resolve_package_root() {
        if let Some(p) = script_if_file(root.join("scripts/pf_ton_test.sh")) {
            return Ok(p);
        }
    }
    if let Ok(root) = std::env::var("PROOF_FORGE_ROOT") {
        if let Some(p) = script_if_file(PathBuf::from(&root).join("scripts/pf_ton_test.sh")) {
            return Ok(p);
        }
    }
    let cwd = std::env::current_dir()?;
    if let Some(p) = script_if_file(cwd.join("scripts/pf_ton_test.sh")) {
        return Ok(p);
    }
    let mut dir = cwd;
    loop {
        if let Some(p) = script_if_file(dir.join("scripts/pf_ton_test.sh")) {
            return Ok(p);
        }
        if !dir.pop() {
            break;
    ...
    Err(PfError::Tool(
        "scripts/pf_ton_test.sh not found (install engineering bundle or monorepo)".into(),
    ))
}
...
pub fn run_sandbox_test(artifact_dir: &Path) -> PfResult<TestOutcome> {
    ...
    let script = resolve_ton_test_script()?;
    ...
    let output = cmd
        .output()
        .map_err(|e| PfError::Tool(format!("spawn pf_ton_test.sh: {e}")))?;
