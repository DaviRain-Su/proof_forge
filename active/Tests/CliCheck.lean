...
def main : IO UInt32 := do
  ...
  let psyReport : ProofForge.Cli.Check.Report := {
    targetId := "psy-dpn"
    fixture? := some "counter"
  }
  match ProofForge.Cli.Check.checkFixture ProofForge.Target.psyDpn
      "psy-dpn" "counter" none psyReport with
  | .error err => throw <| IO.userError s!"Psy Counter check failed: {err}"
  | .ok checked =>
      require (!ProofForge.Cli.Check.hasErrors checked)
        "Psy Counter fixture should require only its implemented capability subset"
      require (checked.validation.contains ("capabilities", "passed"))
        "Psy Counter capability validation should pass"
