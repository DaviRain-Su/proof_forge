...
| D-057 | 2026-07-15 | **接受 Lean/Rust 所有权边界为延期设计；在主三链 cutover 稳定前不开启生产级 Rust 编译后端。** Lean 拥有产品源到 checked Core + CapabilityPlan（以及可选形式化）。Rust 现在可拥有证据 runner，之后可在 dual-run 下可选地拥有 plan/render。只交换版本化制品（artifact JSON、实验性 Core export）；不交换 Lean 对象指针，也不做多链 SDK 巨型二进制。Phase 0（制品契约）可作为文档/harness 卫生推进；Phase 1+ 的 Core export 与 Rust `buildFromCore` 等 cutover 安静后、在隔离分支/worktree 中进行。 | 对齐 Canonical Core / backend-interface 接缝；保全形式化叙事；避免双前端；防止 SDK lock 冲突。规格：[lean-rust-boundary](../superpowers/specs/2026-07-15-lean-rust-boundary-design.zh.md)（[EN](../superpowers/specs/2026-07-15-lean-rust-boundary-design.md)）、[artifact-contract-v1](../superpowers/specs/2026-07-15-artifact-contract-v1.zh.md)、[core-export-v0](../superpowers/specs/2026-07-15-core-export-v0-draft.zh.md)。 |
| D-058 | 2026-07-15 | **在没有现成外部库、也没有明确 sourcegen 战略之前，不投资用 Rust 重做主三链机器级 IR 发射（sBPF `.s`、WAT，或平行 Yul 打印机）。** 产品 codegen 仍在 Lean：EVM → Yul → `solc`；Solana → sBPF AST → `.s` → `sbpf`；NEAR/Wasm-host → Wasm AST → WAT → `wat2wasm`。Rust 可保留：(1) 已有链 SDK 的执行证据 runner；(2) 实验性 Seam A **只读** Core 包 load/inspect/observe sketch（非产品编译）；(3) Stylus `rust-sdk` 仅作 **差分 oracle**（D-053）。**范围外/延期：** LR-3 默认 Rust lower；仅为“Lean 拼字符串难”而用 Rust 重写 Solana/Wasm 打印机；对象指针 FFI 融堆。仅在有书面决策（指明现成库栈或有意的 **源码生成** 后端如 Pinocchio/near-sdk、dual-run 维度与回滚）时重开。 | 产品路径审计显示没有可插拔的现成 Rust sBPF/WAT lowerer；用 Rust 重做 AST→文本只是重复成本。EVM 已有成熟 handoff（`solc`）。对齐 D-026/D-031/D-057。PR #105 Seam A 成功仅覆盖 inspect/export。 |
