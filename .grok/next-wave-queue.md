...
| F-TYPEKEY-BYTES | in_progress | Isolated SPEC `typeKey` byte-form encoder + WireV1 unit tests. Do **not** install as a structure gate. **Not** formal TASK-D2-06 |
...
1. Track A is exhausted. Track B is exhausted. Track F through `F-TYPEKEY-USAGE-GAP` is inventoried. Next engineering row is `F-TYPEKEY-BYTES` (isolated SPEC `typeKey` byte form; not a structure gate). Goal must **not** auto-close formal IDs. Never Track C. Unused TypeKey rejection / decoder-side rank / SPEC-honesty ADR / EV binding / C-3 remain product/formal decisions.
