# Goal slice — B-ctx: ContextRead Plan fail-closed tests
...
## Objective
Ensure all targets Plan-fail ContextRead with stable tests/matrix rows.
...
```json
{
  "slice_id": "B-ctx",
  "milestone": "D2",
  "objective": "Ensure all targets Plan-fail ContextRead with stable tests/matrix rows.",
  "dependencies": ["N-2"],
  "allowed_paths": ["ProofForgeV2/Targets/", "Tests/", "docs/research/12-target-coverage-matrix.md", "docs/engineering-backlog.md"],
  ...
  "commit_message": "slice(B-ctx): contextread-plan-fail-closed-tests",
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"B-ctx","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
