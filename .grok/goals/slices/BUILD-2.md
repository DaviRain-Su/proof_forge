# Goal slice — BUILD-2: Drop worker from default build
...
## Objective
Remove proof_forge_frontend_worker_v1 from default `just build` / test-fast / dev-check chain. Add explicit recipe if ci/Frontend tests need the worker. Product CLI uses in-process Loader.
...
```json
{
  "slice_id": "BUILD-2",
  "milestone": "D2",
  "objective": "Remove proof_forge_frontend_worker_v1 from default `just build` / test-fast / dev-check chain. Add explicit recipe if ci/Frontend tests need the worker. Product CLI uses in-process Loader.",
  "dependencies": ["BUILD-1"],
  "allowed_paths": ["justfile", "docs/engineering-backlog.md", ".github/workflows/ci.yml"],
  "focused_checks": ["just build", "just test-fast"],
  ...
  "commit_message": "slice(BUILD-2): drop-worker-from-default-build",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Do not delete lakefile lean_exe; only default path."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"BUILD-2","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
...
## Notes
Do not delete lakefile lean_exe; only default path.
