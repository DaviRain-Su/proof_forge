# Goal slice — D5-plus: Solana/NEAR/Noir target completion milestones
...
## Objective
Drive non-formal maturity: Solana already ELF+Mollusk; NEAR runtime; Noir prove — as separate sub-slices after C-1/C-4. This meta-item only updates backlog split if still monolithic.
...
```json
{
  "slice_id": "D5-plus",
  "milestone": "D2",
  "objective": "Drive non-formal maturity: Solana already ELF+Mollusk; NEAR runtime; Noir prove \u2014 as separate sub-slices after C-1/C-4. This meta-item only updates backlog split if still monolithic.",
  "dependencies": ["C-1", "C-4"],
  "allowed_paths": ["docs/engineering-backlog.md", "docs/targets/"],
  ...
  "commit_message": "slice(D5-plus): solana/near/noir-target-completion-milestones",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Prefer splitting not one mega PR."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"D5-plus","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
...
## Notes
Prefer splitting not one mega PR.
