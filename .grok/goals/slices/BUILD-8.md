# Goal slice — BUILD-8: Remove dead supervised frontend residue if any

> Paste into `/goal` or run via master queue. Detailed BUILD/N-A2 also in parent prompts.

## ID
`BUILD-8`

## Phase
`build` · milestone hint `D1` · shared_cutover `True`

## Objective
Audit remaining SafeOpen/supervisor/worker product path references after 2026-08-01 removal. Delete only dead product-required code if safe; keep Protocol/Worker modules if tests need them. Align comments with RECOVERY.

## Dependencies (must be done or explicitly waived)
DOC-1

## Allowed path prefixes
```
ProofForgeV2/Frontend/
ProofForgeV2/CLI/
Tests/Frontend/
docs/engineering-backlog.md
AGENTS.md
```

## Focused checks
```
just test-fast
just dev-check
```

## Standard verification
```
just docs-check
just dev-check   # when product/docs touched
just ci          # before treating slice complete if product code changed
git diff --check
```
Skip ci only for pure docs research slices; still run docs-check.

## Engineering-slice args skeleton
```json
{
  "slice_id": "BUILD-8",
  "milestone": "D1",
  "objective": "Audit remaining SafeOpen/supervisor/worker product path references after 2026-08-01 removal. Delete only dead product-required code if safe; keep Protocol/Worker modules if tests need them. Align comments with RECOVERY.",
  "dependencies": ["DOC-1"],
  "allowed_paths": ["ProofForgeV2/Frontend/", "ProofForgeV2/CLI/", "Tests/Frontend/", "docs/engineering-backlog.md", "AGENTS.md"],
  "focused_checks": ["just test-fast", "just dev-check"],
  "verification_commands": ["just docs-check", "just dev-check", "just ci", "git diff --check"],
  "deletion_zero_patterns": ["planFromAlpha", "alphaResidualOf"],
  "shared_cutover": true,
  "commit_message": "slice(BUILD-8): remove-dead-supervised-frontend-residue-if-any",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Do not reintroduce supervisor as product path."
}
```

## Workflow hooks
After implementation is green and still uncommitted (or use engineering-slice for full pipeline):

1. Record BASE=`git rev-parse HEAD` before edits if self-implementing.
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"BUILD-8","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
3. Or full: workflow `proof-forge-engineering-slice` with milestone/objective/allowed_paths from this file.
4. One local commit; never push.

## Global forbidden
- No push, force, amend of published commits, secrets, network package install
- No new formal TASK-*/TST-*/EV-*/freeze objects
- No just governance-check / release-check / Stage-0
- No fallback/dual-reader/second ProgramV1 decoder
- No formal D1–D4 completion claims
- ProofForgeV2 changes ⇒ just sbom-package-files-refresh
- Update docs/engineering-backlog.md status for this ID when done


## Notes
Do not reintroduce supervisor as product path.
