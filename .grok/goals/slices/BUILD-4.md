# Goal slice — BUILD-4: Split CI source-core jobs
...
## Objective
Split GitHub Actions source-core so lean product tests / target smoke / solana-runtime can fail independently while sharing Lake cache. Keep docs job separate. No formal/release jobs.
...
```json
{
  "slice_id": "BUILD-4",
  "milestone": "D2",
  "objective": "Split GitHub Actions source-core so lean product tests / target smoke / solana-runtime can fail independently while sharing Lake cache. Keep docs job separate. No formal/release jobs.",
  "dependencies": ["BUILD-1", "BUILD-2"],
  "allowed_paths": [".github/workflows/ci.yml", "justfile", "docs/engineering-backlog.md"],
  ...
  "commit_message": "slice(BUILD-4): split-ci-source-core-jobs",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Do not require full just ci in Actions redesign validation if too heavy; document."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"BUILD-4","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
...
## Notes
Do not require full just ci in Actions redesign validation if too heavy; document.
