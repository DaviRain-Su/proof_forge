# Goal slice — BUILD-3: Shrink interpreter test exes
...
## Objective
Investigate and reduce ~170MB supportInterpreter shard linking cost: merge light shards, optional interpreter only where needed, or document why impossible. Prefer measurable wall-time/disk win without breaking tests.
...
```json
{
  "slice_id": "BUILD-3",
  "milestone": "D2",
  "objective": "Investigate and reduce ~170MB supportInterpreter shard linking cost: merge light shards, optional interpreter only where needed, or document why impossible. Prefer measurable wall-time/disk win without breaking tests.",
  "dependencies": ["BUILD-1"],
  "allowed_paths": ["justfile", "lakefile.lean", "Tests/Shards/", "docs/engineering-backlog.md"],
  ...
  "commit_message": "slice(BUILD-3): shrink-interpreter-test-exes",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Measure before/after sizes."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"BUILD-3","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
