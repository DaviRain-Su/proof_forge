# Goal slice — N-4: Aggregate callable results + ABI
...
## Objective
Allow struct/enum returns from entry/view/fn and target ABI encoding. Large multi-target; may split EVM-first with others FAIL-CLOSED.
...
```json
{
  "slice_id": "N-4",
  "milestone": "D2",
  "objective": "Allow struct/enum returns from entry/view/fn and target ABI encoding. Large multi-target; may split EVM-first with others FAIL-CLOSED.",
  "dependencies": ["N-3"],
  ...
  "shared_cutover": true,
  "commit_message": "slice(N-4): aggregate-callable-results-+-abi",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Prefer EVM-first if size explodes."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"N-4","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
...
## Notes
Prefer EVM-first if size explodes.
