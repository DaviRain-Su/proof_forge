# Goal slice — BUILD-7: Lake -j in just build
...
## Objective
Ensure lake build uses full core count on CI/local via lake -j$(nproc) or documented default; no semantic change.
...
```json
{
  "slice_id": "BUILD-7",
  "milestone": "D2",
  "objective": "Ensure lake build uses full core count on CI/local via lake -j$(nproc) or documented default; no semantic change.",
  "dependencies": [],
  "allowed_paths": ["justfile", "docs/engineering-backlog.md"],
  "focused_checks": ["just build"],
  ...
  "commit_message": "slice(BUILD-7): lake--j-in-just-build",
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"BUILD-7","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
