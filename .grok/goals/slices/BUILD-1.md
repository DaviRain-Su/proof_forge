# Goal slice — BUILD-1: Parallel test shards
...
## Objective
Make `just test` run the 9 proof-forge-next-tests-shard-* processes with bounded parallelism (env PROOF_FORGE_TEST_JOBS, default 4, 1=serial). Fail-fast with shard name on non-zero. Keep single lake build of all shards. No Lean semantic changes.
...
```json
{
  "slice_id": "BUILD-1",
  "milestone": "D2",
  "objective": "Make `just test` run the 9 proof-forge-next-tests-shard-* processes with bounded parallelism (env PROOF_FORGE_TEST_JOBS, default 4, 1=serial). Fail-fast with shard name on non-zero. Keep single lake build of all shards. No Lean semantic changes.",
  "dependencies": [],
  "allowed_paths": ["justfile", "docs/engineering-backlog.md"],
  "focused_checks": ["just build", "PROOF_FORGE_TEST_JOBS=2 just test"],
  ...
  "commit_message": "slice(BUILD-1): parallel-test-shards",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. May touch scripts if needed for wrapper."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"BUILD-1","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
...
## Notes
May touch scripts if needed for wrapper.
