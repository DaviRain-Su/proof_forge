# Goal slice — N-A2: Multi-arm same-outer constructor match
...
## Objective
Allow Normalize to refine match arms sharing the same outer constructor when safe; fail-closed on unsafe. Tests + matrix update. See also .grok/goals/prompt-n-a2.md for detail.
...
```json
{
  "slice_id": "N-A2",
  "milestone": "D2",
  "objective": "Allow Normalize to refine match arms sharing the same outer constructor when safe; fail-closed on unsafe. Tests + matrix update. See also .grok/goals/prompt-n-a2.md for detail.",
  "dependencies": [],
  "allowed_paths": ["ProofForgeV2/Semantic/", "ProofForgeV2/Typed/", "Tests/", "docs/engineering-backlog.md", "docs/research/12-target-coverage-matrix.md", "supply-chain/"],
  "focused_checks": ["lake build ProofForgeV2", "just test-fast"],
  ...
  "shared_cutover": true,
  "commit_message": "slice(N-A2): multi-arm-same-outer-constructor-match",
  "constraints": "Serial shared-core if Normalize/Typed/Semantic. Never push. Never formal. Update engineering-backlog.md. Shared core SERIAL."
}
```
...
2. Optional: workflow `proof-forge-one-slice` with args
   `{"slice":"N-A2","base_commit":"<BASE>","task_prompt":"<one-line objective>","changed_files":[...]}`
