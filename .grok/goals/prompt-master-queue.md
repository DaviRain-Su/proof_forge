...
> **RETIRED（historical seed）。** 不要从此入口恢复执行，也不要改开
> `prompt-next-wave`（那个也已退役，`next-wave-runner` 会 refuse-exit）。
> 活索引：[`.grok/README.md`](../README.md)。
> 日常下一刀：[`docs/plan/completeness-phased-roadmap.md`](../../docs/plan/completeness-phased-roadmap.md)。
...
从 **第一个仍 pending / 可做** 起（当前常见 **INV-1**；**C-5** 为 ongoing 不阻塞后续）**继续 drain**。
