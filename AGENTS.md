...
| Active development slice | 无；**D1-PA-116 / TASK-D1-01 pre-acceptance** 已以 `EV-20260724-0003` 收口；正式 TASK 仍为 pending |
| Next development slice | 未冻结；正在对 `ProgramV1` decoder residual 做只读审计并选择单一 slice。完整 root decoder 前不得切换 shared DSL/Loader/Lean-command/export v2，不得建立 legacy→ProgramV1、dual reader、第二套 quoted decoder 或 fallback |
