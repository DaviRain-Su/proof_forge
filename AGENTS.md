...
| Active development slice | **D1-PA-96 / TASK-D1-01 pre-acceptance**：ProgramV1 self-contained supporting records `ParamV1`/`FieldDeclV1`/`EnumVariantV1` + canonical encoder；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA96 后优先冻结完整 Pattern table，再把包含 forward tags 的 Place/Expr/Stmt/Block/arms/ExternalCall mutual spine 作为完整闭集裁决；theorem/visibility alpha mapping、decoder、root/hash 与 NodeId 分开处理。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
