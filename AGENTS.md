...
| Active development slice | **D1-PA-100 / TASK-D1-01 pre-acceptance**：PA99 7-type SCC 的完整 25-tag kernel-total codec + independent Python oracle；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA100 收口后再裁决 remaining body-bearing ProgramItem records，complete ProgramItem sum/root、theorem/visibility alpha mapping、decoder、root/hash 与 NodeId 分开处理，禁止由 checkpoint 自动递增。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
