...
| Active development slice | **D1-PA-92 / TASK-D1-01 pre-acceptance**：cursor-based `WireDecodeV1` scalar/collection/NFC-String decoder foundation；完成面已冻结，下一步提交真实 decoder RED；明确排除尚未裁决的 Ident/qualified/tag/model |
| Next development slice | 未冻结；D1-PA-92 收口后先落成 raw escaped component carrier，再处理 `ProofDecl.theorem` QualifiedId 与 visibility model；NodeId slice 另需逐字固定 JCS keys。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 wire/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
