...
| Active development slice | **D1-PA-84 / TASK-D1-05 pre-acceptance**：export declaration FQN 与 reconstructed payload `qualifiedName` exact binding；完成面已冻结，下一步提交 tests-only RED；正式 `TASK-D1-05` 仍为 pending |
| Next development slice | 未冻结；D1-PA-84 收口后再按 D1-05 residual audit 选择单一 seam；`PF-EXPORT-003` 保留给 CLI/Loader 零候选选择，禁止由 checkpoint 自动递增 |
