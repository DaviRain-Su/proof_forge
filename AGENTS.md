...
| Active development slice | **D1-PA-86 / TASK-D1-05 pre-acceptance**：把已冻结的 `TST-SRC-006/007` surface 收敛为单一 tests-only `ProgramExportAcceptance` 可执行入口；完成面已冻结，下一步提交 characterization/golden harness；正式 `TASK-D1-05` 仍为 pending |
| Next development slice | 未冻结；D1-PA-86 收口后只做 formal task close/split/block 复核；在 `TASK-D1-05` 未正式 done 前禁止进入依赖它的 D1-06 |
