...
| Active development slice | 无；D1-PA-35 less-than `<` Source-only carrier 已以 `EV-20260718-0021` 完成 development 收口 |
| Next development slice | less-or-equal `<=` 为下一 Source-only candidate，尚未冻结；必须先核准与 `<`/`<<`/`=` 的 token integrity、单一 migration 与 mixed-chain rejection |
