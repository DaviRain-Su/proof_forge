...
| Active development slice | 无；**D1-PA-120 / TASK-D1-01 pre-acceptance prerequisite** 已以 `EV-20260724-0007` 收口；NodeId preimage现使用source raw identity，正式 TASK仍pending |
| Next development slice | 未冻结；只读审计完整ProgramV1 NodeId explicit-preorder traversal、production SHA truncation与collision/duplicate-visit residual。不得自动切换shared DSL/Loader/Lean-command/export v2，不建立legacy→ProgramV1、dual reader、第二套quoted decoder或fallback |
