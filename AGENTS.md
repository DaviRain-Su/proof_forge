...
| Active development slice | **D1-PA-115 / TASK-D1-01 pre-acceptance**：spine-dependent declaration decoder 已由 `bdaff3ce` 冻结，下一步提交只因 production module 缺失而失败的 tests-only RED；未改变正式 TASK 状态 |
| Next development slice | 未冻结；D1-PA-115 收口后再对 `ProgramItemV1` decoder residual 做只读审计并选择单一 slice，禁止由 checkpoint 自动递增。完整 root decoder 前不得切换 shared DSL/Loader/Lean-command/export v2，不得建立 legacy→ProgramV1、dual reader、第二套 quoted ProgramV1 decoder 或 fallback |
| Phase 1 targets | `evm`, `solana`, `near`, `noir` |
| Design-only targets | `cosmwasm`, `soroban`, `icp`, `openvm`, `aleo`, `psy` |
| Known blocker | 无；formal D0 收口仍按既有冻结包推进，D1 工作只可形成明确标注的 pre-acceptance development evidence |
