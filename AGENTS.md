...
| Engineering slice | structured `call QualifiedId(args)` / `schedule QualifiedId(args)`、direct `.if_` / bounded `.for_`、local function call / qualified constructor expression / bare-base index access、ProgramV1 core statements（let/assign/return/assert）以及 direct ProgramV1 `revert`/`emit` statement shape tests 已进入 ProgramV1 Loader；bare `A.x` expression/assignment 与 unescaped qualified revert/emit name 在 field-place/lookup 迁移前 fail closed；下一步仍是 match/field-place/pattern、剩余 expression/place forms 和旧 statement/operator tests 迁移；替代验证前不删旧代码 |
