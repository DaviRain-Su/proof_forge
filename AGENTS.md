...
| Active development slice | 无；**D1-PA-105 / TASK-D1-01 pre-acceptance** 已以 `EV-20260719-0103` 收口为 complete (development)，正式 `TASK-D1-01` 仍 pending |
| Next development slice | 未冻结；先只读裁决 theorem/visibility alpha mapping、decoder与 full-tree resource validator 中的单一最小闭合面。sourceHash 与 NodeId 必须继续分开处理，禁止由 checkpoint 自动递增。不得复用 alpha bucket validator充当 v1 oracle、把 D2 name/type/theorem lookup塞入 set validation、修改 alpha sourceHash、提前发布 decoder/hash/NodeId API 或据 development evidence 关闭 pending `TASK-D1-01` |
