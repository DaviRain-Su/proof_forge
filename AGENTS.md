...
| Active development slice | **D1-PA-20 / TASK-D1-04 pre-acceptance**：bounded `let name [ : Type ] := Expr` Source statement carrier；Source-only 完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；D1-PA-20 收口后再按 statement/expression residual audit 选择单一 slice，禁止由 checkpoint 自动递增 |
