...
| Active development slice | **D1-PA-90 / TASK-D1-01 pre-acceptance**：tests-only `SourceBoundsAcceptance` direct unit harness；完成面已冻结，下一步实现 256/257、100000/100001 与 identity exact-boundary assertions，并复用现有 `just dsl-negative` integration |
| Next development slice | 未冻结；D1-PA-90 收口后按 TASK-D1-01 residual audit 选择单一 slice；不得包装混合 `ProgramSyntax`/`Loader`、在 resident test 重复 20,000-state/16 MiB 路径或据 development evidence 关闭 pending `TASK-D1-01` |
