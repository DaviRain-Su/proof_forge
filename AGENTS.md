...
| Active development slice | 无；**D1-PA-92 / TASK-D1-01 pre-acceptance** cursor-based `WireDecodeV1` scalar/collection/NFC-String decoder foundation 已以 `EV-20260719-0090` 收口；正式 `TASK-D1-01` 仍为 pending |
| Next development slice | 未冻结；优先做 raw escaped component carrier audit，再处理 `ProofDecl.theorem` QualifiedId 与 visibility model；NodeId slice 另需逐字固定 JCS keys。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 wire/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
