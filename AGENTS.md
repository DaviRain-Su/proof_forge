...
| Active development slice | **D1-PA-83 / TASK-D1-05 pre-acceptance**：post-decode cross-row exported Source identity duplicate/split-brain rejection；完成面已冻结，下一步提交 tests-only RED；正式 `TASK-D1-05` 仍为 pending |
| Next development slice | 未冻结；D1-PA-83 收口后再审计 single-row declaration/payload identity binding 等剩余 seam，禁止由 checkpoint 自动递增 |
