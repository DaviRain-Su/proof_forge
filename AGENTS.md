...
| Active development slice | **D1-PA-93 / TASK-D1-01 pre-acceptance**：`SourceNameComponentV1` raw Lean `Name.str` carrier + wire encode/decode；完成面已冻结，下一步提交 RED→GREEN；正式 `TASK-D1-01` 仍为 pending |
| Next development slice | 未冻结；PA-93 收口后再冻结 model/`ProofDecl.theorem`/visibility 或 NodeId JCS keys 中的单一 slice。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
