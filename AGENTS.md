...
| Active development slice | **D1-PA-117 / TASK-D1-01 pre-acceptance**：`ProgramV1` tagged-value decoder 已由 `75d68d96` 冻结；下一步提交只因 production module 缺失而失败的 tests-only RED；正式 TASK 仍为 pending |
| Next development slice | 未冻结；D1-PA-117 收口后再只读审计 canonical root decoder residual。完整 root decoder 前不得切换 shared DSL/Loader/Lean-command/export v2，不得建立 legacy→ProgramV1、dual reader、第二套 quoted decoder 或 fallback |
