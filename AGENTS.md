...
| Active development slice | **D1-PA-98 / TASK-D1-01 pre-acceptance**：七个 spine-independent declaration records + total canonical encoders；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA98 收口后再裁决包含 forward tags 的 Place/Expr/Stmt/Block/arms/ExternalCall mutual spine，禁止由 checkpoint 自动递增；remaining body-bearing ProgramItem、complete ProgramItem sum/root、theorem/visibility alpha mapping、decoder、root/hash 与 NodeId 分开处理。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
