...
| Active development slice | 无；**D1-PA-94 / TASK-D1-01 pre-acceptance** 已以 development evidence 收口；正式 `TASK-D1-01` 仍为 pending |
| Next development slice | 未冻结；正在对 `ProofDecl.theorem` source QualifiedId、visibility 与完整 ProgramV1 model 做双路依赖/拆分审计，只能冻结一个单一 slice；NodeId 仍依赖完整 model/path inventory。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
