...
| Active development slice | **D1-PA-127 / TASK-D1-01 pre-acceptance prerequisite**（in_progress；冻结commit `0b179181`）；PA125 base的Bool/Option noncanonical marker 10-case negative descriptor，正式 TASK仍pending |
| Next development slice | 未选择；PA127收口前不得并入其他residual。不得自动切换shared DSL/Loader/Lean-command/export v2，不建立legacy→ProgramV1、dual reader、第二套quoted decoder或fallback |
