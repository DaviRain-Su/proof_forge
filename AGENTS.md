...
| Engineering slice | structured `call QualifiedId(args)` / `schedule QualifiedId(args)` 与 direct `.if_` / bounded `.for_` 已进入 ProgramV1 Loader；下一步仍是 match/field-place/pattern、剩余 expression/place forms 和旧 statement/operator tests 迁移；替代验证前不删旧代码 |
