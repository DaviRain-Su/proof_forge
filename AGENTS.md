...
| Active development slice | **D1-PA-95 / TASK-D1-01 pre-acceptance**：ProgramV1 leaf AST 的五个完整 constructor family（Visibility/Type/Literal/UnaryOp/BinaryOp，共 38 tags）+ canonical encoder；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA95 收口后再冻结 ProgramV1 recursive spine model/encoder；theorem QID、visibility alpha mapping、decoder、root/hash 与 NodeId 分开裁决。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
