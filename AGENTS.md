...
| Active development slice | **D1-PA-106 / TASK-D1-01 pre-acceptance**：bounded tag reader + complete nonrecursive `VisibilityV1`/`LiteralV1`/`UnaryOpV1`/`BinaryOpV1` decoder（27 tags）；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA106 收口后优先只读裁决 budget-threaded recursive `TypeV1` decoder。不得在 bare `CursorV1` 上先递归分配再事后检查，full Program decoder、alpha mapping、sourceHash、NodeId与target继续分开，禁止由 checkpoint 自动递增或据 development evidence 关闭 pending `TASK-D1-01` |
