...
| Active development slice | **D1-PA-104 / TASK-D1-01 pre-acceptance**：canonical root encoder（moduleName ‖ programIdentity ‖ Program）+ strict raw identity/name join + independent Python oracle；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA104 收口后再只读裁决 `SPEC-LANG-001` declaration-set validation 的单一闭合面。theorem/visibility alpha mapping、decoder、full-tree resource validator、sourceHash 与 NodeId 必须继续分开处理，禁止由 checkpoint 自动递增。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布 decoder/hash/NodeId API 或据 development evidence 关闭 pending `TASK-D1-01` |
