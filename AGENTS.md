...
| Engineering slice | structured `call QualifiedId(args)` / `schedule QualifiedId(args)` 已进入 ProgramV1 Loader；下一步仍是剩余 statement/expression/place/pattern（如 if/for/match/field-place）和旧 tests/negatives 迁移；替代验证前不删旧代码 |
