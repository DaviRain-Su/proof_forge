...
| Active development slice | **D1-PA-116 / TASK-D1-01 pre-acceptance**：no-wrapper `ProgramItemV1` decoder 已由 `8e487f4f` 冻结；下一步提交只因 production module 缺失而失败的 tests-only RED；正式 TASK 仍为 pending |
| Next development slice | 未冻结；D1-PA-116 收口后再只读审计 `ProgramV1` decoder residual；完整 root decoder 前不得切换 shared DSL/Loader/Lean-command/export v2，不得建立 legacy→ProgramV1、dual reader、第二套 quoted decoder 或 fallback |
