...
| Active development slice | 无；**D1-PA-110** complete recursive `PatternV1` decoder 已完成 development GREEN（`EV-20260720-0004`），正式 `TASK-D1-01` 状态未改变 |
| Next development slice | `decodeCanonicalSourceAstBytesV1` prerequisite 的下一窄 decoder family，须另行审计并冻结；完整 root decoder 前不得切换 shared DSL/Loader/Lean-command/export v2，不得建立 legacy→ProgramV1、dual reader、第二套 quoted ProgramV1 decoder 或 fallback |
