...
| Active development slice | 无；**D1-PA-95 / TASK-D1-01 pre-acceptance** 已以 development evidence 收口；正式 `TASK-D1-01` 仍为 pending |
| Next development slice | 未冻结；正在对 ProgramV1 recursive spine 的完整依赖图和 model/encoder 拆分做双路审计；theorem QID、visibility alpha mapping、decoder、root/hash 与 NodeId 分开裁决。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
