...
| Active development slice | **D1-PA-101 / TASK-D1-01 pre-acceptance**：全部六个 spine-dependent declaration records + total codecs + independent Python oracle；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA101 收口后再裁决 complete 13-alternative `ProgramItemV1` sum/codec。Program root、set-level validation、theorem/visibility alpha mapping、decoder、root/hash 与 NodeId 必须继续分开处理，禁止由 checkpoint 自动递增。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
