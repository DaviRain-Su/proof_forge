...
| Active development slice | 无；D1-PA-37 greater-than `>` Source-only carrier 已以 `EV-20260718-0023` 完成 development 收口 |
| Next development slice | greater-or-equal `>=` 为下一 Source-only candidate，尚未冻结；它是 CompareExpr 最后一个 residual，完成后运行批量 `just ci` checkpoint |
