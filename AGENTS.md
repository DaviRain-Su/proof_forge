...
| Active development slice | **D1-PA-94 / TASK-D1-01 pre-acceptance**：source-only `SourceQualifiedNameV1`（1..256）+ QualifiedId（2..256）context validation、raw-array wire encode/decode 与 module/program strict-prefix join；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA94 收口后才可在 `ProofDecl.theorem`/visibility model 与完整 ProgramV1 model 中裁决一个单一 slice；NodeId 仍依赖完整 model/path inventory。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
