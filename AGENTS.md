...
| Active development slice | **D1-PA-26 / TASK-D1-04 pre-acceptance**：bare `assert Expr` Source-only statement carrier；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；D1-PA-26 收口后再按 statement/expression residual audit 选择单一 slice，禁止由 checkpoint 自动递增 |
