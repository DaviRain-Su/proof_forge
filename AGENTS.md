...
| Active development slice | **D1-PA-99 / TASK-D1-01 pre-acceptance**：完整 7-type/25-constructor mutual spine model + kernel-total proof-carrying `DecidableEq`；完成面已冻结，下一步提交 RED |
| Next development slice | 未冻结；PA99 收口后候选为同一 SCC 的 25-tag total codec，必须单独冻结；remaining body-bearing ProgramItem、complete ProgramItem sum/root、theorem/visibility alpha mapping、decoder、root/hash 与 NodeId 分开处理。不得把 alpha `Source.Program` 投影伪装成 v1、修改 alpha sourceHash、提前发布完整 ProgramV1 root/hash API 或据 development evidence 关闭 pending `TASK-D1-01` |
