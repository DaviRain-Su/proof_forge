...
| `wasm-arbitrum-stylus` | Surface v2 / Legacy adapter → canonical contract → `StylusPlan` → pinned Rust SDK → Wasm | Research bootstrap | `just stylus-public-route`; emits Rust crate, Solidity ABI, TypeScript client, intermediate Wasm, artifact/deploy metadata; no final/deployable claim until `cargo stylus check` and direct HostIO Wasm differential evidence |
