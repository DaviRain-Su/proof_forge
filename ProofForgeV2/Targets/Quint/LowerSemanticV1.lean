...
Named Struct/Enum **state** flattens to UInt64/Int64 leaves (`p_x`/`p_y`,
`m_tag`/`m_p0`); named return and nested named stay fail closed.
...
private def quintTypeClosureWording : PilotTypeClosureWording where
  ...
  unsupportedShapeDetail :=
    "only anonymous UInt64, Int64, UInt8 (Bytes element), Bool, Unit, Principal (9-leaf wire identity; pf.assets dst packs the same leaves), named Struct/Enum UInt64/Int64 leaf flatten (return stays fail closed), Array UInt64 N state flatten, Option UInt64 2-leaf flatten, Map UInt64 UInt64 cap-8 flatten, and Bytes N (N UInt64 low-8 leaves) are supported (narrow Int/Field/aggregates fail closed)"
...
private def decodePrincipalLiteralLeavesV1 (bytes : ByteArray) :
  ...
  unless bytes.size ≥ 4 do
    planError
      "unsupported Quint semantic shape: Principal literal valueBytes too short"
  let len :=
    (bytes.get! 0).toNat + (bytes.get! 1).toNat * 256 +
      (bytes.get! 2).toNat * 65536 + (bytes.get! 3).toNat * 16777216
  unless bytes.size == 4 + len do
    planError
      "unsupported Quint semantic shape: Principal literal valueBytes length framing mismatch"
  unless 1 ≤ len do
    planError
      "unsupported Quint semantic shape: Principal body shorter than 1 byte"
  unless len ≤ principalMaxPayloadBytesV1 do
    planError
      s!"unsupported Quint semantic shape: Principal longer than {principalMaxPayloadBytesV1} bytes (identity bound)"
...
private def isArrayValue (v : TypedExpr) : Bool :=
  isAggregateValue v && !v.isOption && !v.isMap && !v.isNamed && v.ty != .principal
...
/-- T4: 9 unsigned UInt64 identity leaves. `ty.principal` keeps pf.assets
    `transfer` dst packing; leaves are the T10/T12 wire identity. -/
...
/-- Flatten named Struct/Enum into ordered leaf names. Leaves are UInt64 or
    Int64 only; nested named / containers / Bool / Field fail closed. -/
...
private def viewAggregateLeafIsIntV1
  ...
  | some { shape := .bytes len, name := none, .. } =>
      ...
      unless 1 ≤ n && n ≤ 8 do
        planError
          s!"unsupported Quint semantic shape: Bytes N view return must be 1..8 (got {n})"
...
/-- Physical PlanState leaves after Array/Option/named flatten. `leavesOf[logicalId]`
    ...
    `name_tag`/`name_p0`, or `{state}_{field}`). -/
...
private def makeStateLayoutV1
  ...
  for st in data.logicalState do
    ...
    if types.isNamedAggregate st.typeId then
      requirePublicUInt64OrInt64OrFieldOrPrincipalOrNamedState quintPlanErr types st
...
private def lowerLiteral
  ...
  if isInt64Type types typeId then
    ...
    planError "unsupported Quint semantic shape: literal type is outside UInt64/Int64/UInt32/UInt8/Bool/Principal"
  ...
  | .eq | .ne | .lt | .le | .gt | .ge => do
      if isPrincipalValue lhs && isPrincipalValue rhs then
        unless op == .eq || op == .ne do
          planError "unsupported Quint semantic shape: Principal comparison only supports eq/ne"
        let mut acc : Expr :=
  ...
  for instr in block.instructions do
        ...
        let ret? ← match cBlock.terminator with
          | .return_ value =>
              ...
              | some vid =>
                  ...
                  | none =>
                      planError "unsupported Quint semantic shape: return value undefined"
          | .revert .. =>
              planError
                "unsupported Quint semantic shape: pureFn revert during inline is outside Q0"
        ...
        | some vd =>
            if isAnonymousMapTypeIdV1 data.types typeId then
              ...
              let some decl := data.types[typeId.toNat]? |
                planError "unsupported Quint semantic shape: construct TypeDecl missing"
              match decl.shape with
              | .struct fields => do
                  unless ctorIdx.toNat == 0 do
                    planError
                      "unsupported Quint semantic shape: struct construct ctorIdx must be 0"
                  unless argIds.size == fields.size do
                    planError
                      "unsupported Quint semantic shape: struct construct arity mismatch"
                  let mut leafExprs : Array Expr := #[]
                  let mut nodes : Nat := 0
                  for i in [0:argIds.size] do
                    let some argId := argIds[i]? |
                      planError "unsupported Quint semantic shape: struct construct arg missing"
                    let some field := fields[i]? |
                      planError "unsupported Quint semantic shape: struct construct field missing"
                    unless isUInt64Type types field.typeId || isInt64Type types field.typeId do
                      planError
                        "unsupported Quint semantic shape: named Struct field must be UInt64 or Int64"
                    let av ← match envLookup acc.env argId with
                      | some v => pure v
                      | none => planError "unsupported Quint semantic shape: construct arg undefined"
                    unless !isAggregateValue av &&
                      ...
                      planError
                        "unsupported Quint semantic shape: struct construct args must be scalar UInt64/Int64"
              ...
              | .enum variants => do
                  ...
                  let some variant := variants[vi]? |
                    planError
                      "unsupported Quint semantic shape: enum construct variant out of range"
                  unless argIds.size == variant.payloadTypes.size do
                    planError
                      "unsupported Quint semantic shape: enum construct arity mismatch"
                  let mut maxPay : Nat := 0
                  for v in variants do
                    if v.payloadTypes.size > maxPay then maxPay := v.payloadTypes.size
                  ...
                  for argId in argIds do
                    ...
                    unless !isAggregateValue av &&
                        (av.ty == .uint64 || av.ty == .int64) do
                      planError
                        "unsupported Quint semantic shape: enum construct payload must be scalar UInt64/Int64"
                    leafExprs := leafExprs.push av.expr
                    nodes := nodes + av.expandedNodes
              ...
              | _ =>
                  planError
                    "unsupported Quint semantic shape: named construct requires Struct or Enum"
            else
              let n ← match ← arrayUInt64LenV1 data.types types typeId (numericTy == .int64) with
                | some n => pure n
                | none =>
                    planError
                      "unsupported Quint semantic shape: construct admits only Array UInt64 N, Bytes N, Option UInt64, Map UInt64 UInt64, or named Struct/Enum on Quint"
              unless ctorIdx == 0 do
    ...
    | .variantTag baseId => do
        ...
        | some vd =>
            ...
            unless isOptionValue bv || isNamedValue bv do
              planError
                "unsupported Quint semantic shape: variantTag base must be Option UInt64 or named Enum"
    ...
    | .variantPayload baseId variantIndex payloadIndex => do
        ...
        | some vd =>
            ...
            unless isOptionValue bv || isNamedValue bv do
              planError
                "unsupported Quint semantic shape: variantPayload base must be Option UInt64 or named Enum"
            ...
            else if variantIndex.toNat == 0 && payloadIndex.toNat == 0 &&
                bv.leaves.size == 1 then
              planError
                "unsupported Quint semantic shape: variantPayload of empty Enum variant is empty"
            let payloadIdx :=
              if isOptionValue bv then 1 else 1 + payloadIndex.toNat
            let some payload := bv.leaves[payloadIdx]? |
              planError
                "unsupported Quint semantic shape: variantPayload payload leaf missing"
            acc := { acc with env := envInsert acc.env vd.valueId {
              ty := numericTy
              expr := payload
              expandedNodes := 1
    ...
    | .fieldGet baseId fieldIndex => do
        match instr.result with
        | none => planError "unsupported Quint semantic shape: fieldGet must produce a value"
        | some vd =>
            let bv ← match envLookup acc.env baseId with
              | some v => pure v
              | none => planError "unsupported Quint semantic shape: fieldGet base undefined"
            unless isNamedValue bv do
              planError
                "unsupported Quint semantic shape: fieldGet base must be a named Struct"
            let i := fieldIndex.toNat
            unless i < bv.leaves.size do
              planError "unsupported Quint semantic shape: fieldGet leaf index out of range"
            let some leaf := bv.leaves[i]? |
              planError "unsupported Quint semantic shape: fieldGet leaf missing"
            acc := { acc with env := envInsert acc.env vd.valueId {
              ty := numericTy
              expr := leaf
              expandedNodes := 1
    ...
    | .fieldSet baseId fieldIndex valueId => do
        match instr.result with
        | none => planError "unsupported Quint semantic shape: fieldSet must produce a value"
        | some vd =>
            let bv ← match envLookup acc.env baseId with
              | some v => pure v
              | none => planError "unsupported Quint semantic shape: fieldSet base undefined"
            unless isNamedValue bv do
              planError
                "unsupported Quint semantic shape: fieldSet base must be a named Struct"
            let vv ← match envLookup acc.env valueId with
              | some v => pure v
              | none => planError "unsupported Quint semantic shape: fieldSet value undefined"
            unless !isAggregateValue vv && (vv.ty == .uint64 || vv.ty == .int64) do
              planError
                "unsupported Quint semantic shape: fieldSet value must be scalar UInt64/Int64"
            let i := fieldIndex.toNat
            unless i < bv.leaves.size do
              planError "unsupported Quint semantic shape: fieldSet leaf index out of range"
            let newLeaves := bv.leaves.set! i vv.expr
            acc := { acc with
              env := envInsert acc.env vd.valueId
...
private def decodeSwitchCaseValue
    (types : QuintTypeClosureV1) (typeId : TypeIdV1) (bytes : ByteArray) :
    CompileResult UInt64 := do
  if isBoolType types typeId then
    let b ← decodeBoolLiteralBit quintPlanErr "Quint" bytes
    pure (if b then 1 else 0)
  else if isUInt32Type types typeId then
    decodeUInt32LiteralLe quintPlanErr "Quint" bytes
  else if isUInt64Type types typeId || isInt64Type types typeId then
    decodeUInt64LiteralLe quintPlanErr "Quint" bytes
  else
    planError
      "unsupported Quint semantic shape: switch case type is outside UInt64/Int64/UInt32/Bool"
...
private partial def emitRegion
  ...
  match fuel with
  | 0 =>
      planError "unsupported Quint semantic shape: CFG walk fuel exhausted"
  | fuel' + 1 => do
      let some block := callable.blocks[blockId]? |
        planError "unsupported Quint semantic shape: missing CFG block"
      unless block.id.toNat == blockId do
        planError "unsupported Quint semantic shape: block id must match index"
      let acc ←
      ...
      | .branch condId thenT elseT => do
          unless thenT.args.isEmpty && elseT.args.isEmpty do
            planError
              "unsupported Quint semantic shape: branch targets must carry empty args"
          let cond ← match envLookup acc.env condId with
            | some v => requireTy v .bool "branch condition"
            | none =>
                planError "unsupported Quint semantic shape: branch condition undefined"
          let (preStmts, acc) := flushRegion layout acc
          ...
          if thenJoin == some elseT.blockId.toNat then
            ...
            | some j1, some j2 => do
                unless j1 == j2 do
                  planError
                    "unsupported Quint semantic shape: branch arms converge on divergent joins"
      ...
      | .switch scrutId cases defaultTarget => do
          let some defaultT := defaultTarget |
            planError
              "unsupported Quint semantic shape: switch must carry a default target"
          unless !cases.isEmpty do
            planError "unsupported Quint semantic shape: switch cases must be nonempty"
          let scrutVal ← match envLookup acc.env scrutId with
            | some v => pure v
            | none =>
                planError "unsupported Quint semantic shape: switch scrutinee undefined"
          unless defaultT.args.isEmpty do
            planError
              "unsupported Quint semantic shape: switch default args / block-param phi are outside Q0"
          if isPrincipalValue scrutVal then
            ...
            for switchCase in cases do
              unless switchCase.target.args.isEmpty do
                planError
                  "unsupported Quint semantic shape: switch case args / block-param phi are outside Q0"
              let caseLeaves ← decodePrincipalLiteralLeavesV1 switchCase.valueBytes
              unless caseLeaves.size == scrutVal.leaves.size do
                planError
                  "unsupported Quint semantic shape: String match case leaf count mismatch"
              let mut cond : Expr :=
              ...
              | .join j, some j0 =>
                  unless j == j0 do
                    planError
                      "unsupported Quint semantic shape: switch arms converge on divergent joins"
            ...
            planError "unsupported Quint semantic shape: switch scrutinee must be scalar"
          else
      ...
      | .revert .. =>
          planError
            "unsupported Quint semantic shape: revert in CFG arms is outside Q0"
    ...
    if isAnonymousOptionTypeIdV1 typeDecls p.typeId then
      ...
      | some { shape := .option elTid, name := none, .. } =>
          unless isUInt64Type types elTid || isInt64Type types elTid do
            planError
              s!"{owner} Option param payload must be UInt64 or Int64"
          ...
          isPrincipal := isPrincipal.push false |>.push false
      ...
      unless p.visibility == .public_ do
        planError s!"parameter '{p.name}' is not public"
      ...
      | some { shape := .array elTid len, .. } =>
          unless isUInt64Type types elTid || isInt64Type types elTid do
            planError
              s!"parameter '{p.name}' in {owner} Array element must be UInt64 or Int64"
          let n := len.toNat
          unless 1 ≤ n && n ≤ 8 do
            planError
      ...
      | some { shape := .map keyTid valTid, name := none, .. } =>
          unless (isUInt64Type types keyTid && isUInt64Type types valTid) ||
            ...
            planError
              s!"parameter '{p.name}' in {owner} Map must be Map UInt64 UInt64 or Map Int64 Int64"
      ...
      | _ =>
          planError
            "unsupported Quint semantic shape: nested/unknown container params stay fail closed"
    else if isInt64Type types p.typeId then
      env := envInsert env p.valueId {
        ty := .int64
        expr := .param i
        expandedNodes := 1
...
private def lowerCallableBody
    ...
    CompileResult
      (Array String × Array Bool × Array Check × Array (Nat × Expr) ×
        Array PfAssetsOp × Option TypedExpr × Bool × Bool × Array Statement) := do
  let owner :=
...
private def makePlanFromSemanticDataV1
  ...
  for c in data.constants do
    unless isInt64Type types c.typeId || isUInt64Type types c.typeId ||
      ...
      planError
        "unsupported Quint semantic shape: constant is not admitted UInt64/Int64/UInt32/Bool/String"
  ...
  for callable in data.callables do
    ...
    | .pureFn => do
        ...
        unless body.isEmpty do
          planError s!"pureFn '{name}' control flow is outside Q0"
    ...
    | .initializer => do
        ...
        unless body.isEmpty do
          planError "unsupported Quint semantic shape: initializer control flow is outside Q0"
        if endedRevert then
    ...
    | .entry => do
        ...
        if !body.isEmpty then
          ...
          entries := entries.push {
            ...
            assetOps, result? := none, leaves := #[], leafIsInt := #[]
        ...
        entries := entries.push {
          actionIndex := entryActionIndex
          name, params, paramIsPrincipal, resultKind := rk, checks, stores
          assetOps, result?, leaves, leafIsInt
          terminalRevert := endedRevert
        }
    | .view => do
        let name ← match callable.name with
        ...
        let (params, paramIsPrincipal, checks, stores, assetOps, ret?, endedRevert,
          ...
          lowerCallableBody data types idx callable numericTy layout
            ...
            (initialStateDefaults := false)
        unless body.isEmpty do
          planError s!"view '{name}' control flow is outside Q0"
        if endedRevert then
    ...
    | .invariant => do
        ...
        let (params, _paramIsPrincipal, checks, stores, assetOps, ret?, endedRevert,
          ...
          lowerCallableBody data types idx callable numericTy layout
            ...
            (initialStateDefaults := false)
        unless body.isEmpty do
          planError s!"invariant '{name}' control flow is outside Q0"
        if endedRevert then
