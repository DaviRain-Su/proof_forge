...
private partial def emitRegion
  ...
  | fuel' + 1 => do
      ...
      | .switch scrutId cases defaultTarget => do
          ...
          if isPrincipalValue scrutVal then
            ...
            for switchCase in cases do
              ...
              unless caseLeaves.size == scrutVal.leaves.size do
                planError
                  "unsupported ICP semantic shape: String match case leaf count mismatch"
              let cond := Expr.principalEq scrutVal.leaves caseLeaves
          ...
          let scrut ← requireScalar scrutVal "switch scrutinee"
          let (preStmts, acc) := flushRegion layout acc
  ...
  for p in callable.params do
    ...
    if isAnonymousOptionTypeIdV1 typeDecls p.typeId then
      ...
      | some { shape := .option elTid, name := none, .. } =>
          ...
          unless isIdentifier tagName do
            planError s!"parameter '{tagName}' in {owner} is not a safe identifier"
          unless isIdentifier pName do
            planError s!"parameter '{pName}' in {owner} is not a safe identifier"
          names := names.push tagName |>.push pName
          kinds := kinds.push .integer |>.push .integer
      ...
      | some { shape := .bytes len, .. } =>
          ...
          for j in [0:n] do
            ...
            unless isIdentifier leafName do
              planError s!"parameter '{leafName}' in {owner} is not a safe identifier"
            names := names.push leafName
            kinds := kinds.push .integer
            leafExprs := leafExprs.push (.param i)
            i := i + 1
...
private def makePlanFromSemanticDataV1
  ...
  for c in data.constants do
    unless isInt64Type types c.typeId || isUInt64Type types c.typeId ||
        isUInt32Type types c.typeId || isStringType types c.typeId do
      planError
        "unsupported ICP semantic shape: constant is not admitted UInt64/Int64/UInt32/String"
