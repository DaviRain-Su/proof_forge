...
inductive HostImport where
  ...
  | currentAccountId
  /-- ADR-0031 S5 / CAP-X-BYTES-NEAR: host `sha256(value_len, value_ptr, register_id)`.
      Present when an entry lowers the exact
      `pf.crypto.sha256(UInt256) -> UInt256` leaf and/or the exact
      `pf.crypto.sha256Bytes(Bytes N) -> UInt256` leaf. This is a host
      syscall, not a promise or synchronous cross-contract call. -/
  | sha256
...
structure StorageLayout where
  ...
  /-- Dense `ConstantId` → pre-decoded scalar plan word. Parallel arrays keep
      `StorageLayout` free of a forward ref to `NearValueKindV1`. Empty keeps
      historical no-constant Plans byte-identical. Not part of planDigest. -/
...
inductive Statement where
  ...
  /-- CAP-X-BYTES-NEAR: exact `pf.crypto.sha256Bytes(Bytes N) -> UInt256`.
      `inputLeaves` are the N UInt8 Bytes leaves in source order (N ∈ 1..64);
      `resultTemp` binds `.sha256Result` (same four-limb UInt256 layout as
      `.sha256Precompile`). Independent Plan node from the UInt256 leaf. -/
  | sha256BytesHost (inputLeaves : Array Expr) (resultTemp : Nat)
...
/-- CAP-X-BYTES-NEAR packing cap: WAT writes N Bytes leaves as consecutive
    `i32.store8` at `valueOffset`, then `read_register` overlays the 32-byte
    digest on the same scratch. 64 matches the already-admitted Principal
    body packing window and `maxParams` (Bytes param N>64 already FC).
    Larger N is named fail-closed even though one Wasm page could hold it. -/
def maxSha256BytesLenV1 : Nat := 64
...
/-- Host import set driven by schedule and/or transferAsync and/or token
    ...
    are independent. `usesSha256` covers both the UInt256 leaf and the
    CAP-X-BYTES Bytes-N leaf (same host import). -/
...
/-- Dense tag for `StorageLayout.constantKinds` (keeps layout free of
    `NearValueKindV1` at structure-definition time). -/
private def nearConstantKindTagV1 : NearValueKindV1 → Nat
  | .uint64 => 0
  | .uint32 => 1
  | .uint16 => 2
  | .uint8 => 3
  | .bool => 4
  | .int64 => 5
  | .uint128 | .uint256 => 0  -- unused for constants pilot

private def nearConstantKindOfTagV1 (tag : Nat) : Option NearValueKindV1 :=
  match tag with
  | 0 => some .uint64
  | 1 => some .uint32
  | 2 => some .uint16
...
/-- Decode one source `ConstantV1` into a NEAR plan surface word + kind tag.
...
private def decodeNearConstantSlotV1
    (types : NearTypeClosureV1) (typeId : TypeIdV1) (bytes : ByteArray) :
    CompileResult (NearValueKindV1 × UInt64) := do
  if let some bitWidth := types.intWidthOf typeId then
    ...
    unless bitWidth ≤ 64 && isNearBodyUintWidth bitWidth do
      throw <| .planInvariant .near
        s!"unsupported NEAR semantic shape: UInt{bitWidth} constant is outside the NEAR scalar const pilot"
    let kind ← match uintKindOfWidthV1 bitWidth with
      | some k => pure k
      | none =>
          throw <| .planInvariant .near
            s!"unsupported NEAR semantic shape: UInt{bitWidth} constant is not admitted"
    ...
    let boolTypeId ← match types.boolTypeId with
      ...
      | none =>
          throw <| .planInvariant .near
            "unsupported NEAR semantic shape: Bool type is missing for Bool constant"
    unless typeId == boolTypeId do
      throw <| .planInvariant .near
        "unsupported NEAR semantic shape: constant is not admitted UInt width, Int width, Bool, or String"
...
private def makeNearConstantTableV1
    (types : NearTypeClosureV1) (constants : Array ConstantV1) :
  ...
  for i in [0:constants.size] do
    let some c := constants[i]? |
      throw <| .planInvariant .near
        "unsupported NEAR semantic shape: constant table hole"
    unless c.id.toNat == i do
      throw <| .planInvariant .near
        "unsupported NEAR semantic shape: Constant id does not match declaration order"
    let (kind, value) ← decodeNearConstantSlotV1 types c.typeId c.valueBytes
    typeIds := typeIds.push c.typeId
    kinds := kinds.push (nearConstantKindTagV1 kind)
...
private def lowerBlockInstructionsV1
  ...
  for instruction in block.instructions do
    ...
    | .constant constantId, some result =>
        -- Scalar const table: same plan surface as inline `.literal`.
        let some typeId := layout.constantTypeIds[constantId.toNat]? |
          throw <| .planInvariant .near
            "unsupported NEAR semantic shape: Constant references an unknown constant id"
        unless result.typeId == typeId do
          throw <| .planInvariant .near
            "unsupported NEAR semantic shape: Constant result typeId must match the declaration"
        if types.isString typeId then
          let some bytes := layout.constantPayloads[constantId.toNat]? |
            throw <| .planInvariant .near
              "unsupported NEAR semantic shape: String constant payload is missing"
          ...
          let some kindTag := layout.constantKinds[constantId.toNat]? |
            throw <| .planInvariant .near
              "unsupported NEAR semantic shape: Constant kind table is incomplete"
          let some value := layout.constantValues[constantId.toNat]? |
            throw <| .planInvariant .near
              "unsupported NEAR semantic shape: Constant value table is incomplete"
          let kind ← match nearConstantKindOfTagV1 kindTag with
            | some k => pure k
            | none =>
                throw <| .planInvariant .near
    ...
    | .externalCall _effectId callee argIds, some result =>
        ...
        if isPfCryptoSha256BytesCalleeV1 components then
          unless argIds.size == 1 && types.uintWidthOf result.typeId == some 256 do
            throw <| .planInvariant .near
              s!"unsupported NEAR semantic shape: pf.crypto.sha256Bytes requires exactly one Bytes N argument and UInt256 result"
          let some inputId := argIds[0]? |
            throw <| .planInvariant .near
              "unsupported NEAR semantic shape: pf.crypto.sha256Bytes Bytes argument is missing"
          let inputRoot ← currentValueWithArmsV1 values blockEntry segmentStart
            armReadables inputId
          unless inputRoot.isAggregate && inputRoot.leafByteWidth == 1 do
            throw <| .planInvariant .near
              s!"unsupported NEAR semantic shape: pf.crypto.sha256Bytes requires exactly one Bytes N argument and UInt256 result"
          let n := inputRoot.leafExprs.size
          unless 1 ≤ n && n ≤ maxSha256BytesLenV1 do
            throw <| .planInvariant .near
              s!"unsupported NEAR semantic shape: pf.crypto.sha256Bytes Bytes N exceeds NEAR packing cap {maxSha256BytesLenV1} (WAT valueOffset consecutive i32.store8 overlay the 32-byte digest register; N>64 is named fail-closed)"
          let _ ← consumeSegmentRootsV1 values blockEntry segmentStart argIds
          let resultTemp := result.valueId.toNat
          body := body.push (.sha256BytesHost inputRoot.leafExprs resultTemp)
          values := ← appendResultValueV1 result.typeId values result {
    ...
    | .externalCall _effectId callee argIds, none =>
        ...
        if isPfCryptoCalleeV1 components then
          ...
          else if isPfCryptoSha256BytesCalleeV1 components then
            throw <| .planInvariant .near
              "unsupported NEAR semantic shape: pf.crypto.sha256Bytes requires exactly one Bytes N argument and UInt256 result"
  ...
  | .switch scrutId cases defaultTarget =>
      ...
      if scrutVal.isAggregate then
        ...
        unless scrutLeaves.size == expectedStringLeaves do
          throw <| .planInvariant .near
            "unsupported NEAR semantic shape: switch on non-String aggregate is outside the NEAR pilot"
        ...
        for switchCase in cases do
          ...
          unless caseLeaves.size == scrutLeaves.size do
            throw <| .planInvariant .near
              "unsupported NEAR semantic shape: String match case leaf count mismatch"
...
partial def statementsUseNativeDepositV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256Precompile .. | .sha256BytesHost .. | .keccak256Host .. => false
...
partial def statementsUseAttachedDepositValueV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256BytesHost leaves _ => leaves.any exprUsesAttachedDepositValueV1
...
partial def statementsUseTimestampV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256BytesHost leaves _ => leaves.any exprUsesTimestampV1
...
partial def statementsUseBlockIndexV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256BytesHost leaves _ => leaves.any exprUsesBlockIndexV1
...
partial def statementsUseAccountBalanceV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256BytesHost leaves _ => leaves.any exprUsesAccountBalanceV1
...
partial def statementsUseCallerV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256BytesHost leaves _ => leaves.any exprUsesCallerV1
...
partial def statementsUseSelfV1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    ...
    | .sha256BytesHost leaves _ => leaves.any exprUsesSelfV1
...
partial def statementsUseSha256V1 (statements : Array Statement) : Bool :=
  statements.any fun statement =>
    match statement with
    | .sha256Precompile .. | .sha256BytesHost .. => true
    | .ifThenElse _ thenBody elseBody =>
...
private def makePlanFromSemanticDataV1
  ...
  -- Scalar const table (UInt/Int/Bool) plus String 9-leaf payloads: decode
  -- once, thread via layout for body `Op.Constant`. Empty table keeps
  -- historical Plan bytes. Multiword / aggregate / Principal constants fail
  -- closed inside makeNearConstantTableV1.
  let (constTypeIds, constKinds, constValues, constPayloads) ←
    makeNearConstantTableV1 types source.constants
  let storage := {
    storage0 with
      pfAssetsDeclared
