/-
  NEAR Plan engineering canonical schema + digest (T9d / M5).

  planDigest = domainSeparatedSha256(
    "pf.near-plan.engineering.v1",
    encodeEngineeringNearPlanBytesV1(plan))
...
private def encodeNatAsU32le (count : Nat) : Except String ByteArray := do
  unless count ≤ UInt32.size - 1 do
    throw "near plan u32 length is not representable"
...
private partial def encodeExpr (expr : Expr) : Except String ByteArray := do
  ...
  -- ADR-0031 S5: handle for a preceding dedicated SHA-256 statement. Tag 51
...
def engineeringNearPlanDigestV1 (plan : Plan) : Except String Digest := do
  let bytes ← encodeEngineeringNearPlanBytesV1 plan
  domainSeparatedSha256 engineeringNearPlanDomainV1 bytes
