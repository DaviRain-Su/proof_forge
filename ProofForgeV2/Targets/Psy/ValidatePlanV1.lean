...
private def validateExprNodes (expr : Expr) : Option Nat :=
  ...
  | .ctxUserPublicKeyHash | .ctxSessionProofTreeRoot => some 1
  ...
  | .imtSet k v =>
      ...
      | _, _ => none
  | .imtGetExternal c k =>
      match validateExprNodes c, validateExprNodes k with
      | some a, some b => some (a + b + 1)
      | _, _ => none
  | .imtGetOther u c k | .imtContainsOther u c k =>
      match validateExprNodes u, validateExprNodes c, validateExprNodes k with
      | some a, some b, some d => some (a + b + d + 1)
      | _, _, _ => none
...
private partial def validateWideExpr
  ...
  | .ctxUserId | .ctxContractId | .ctxCheckpointId | .ctxNonce | .ctxCallerContractId
  | .ctxUserPublicKeyHash | .ctxSessionProofTreeRoot => pure ()
  ...
  | .imtGetExternal c k => do
      validateWideExpr defined c
      validateWideExpr defined k
  | .imtGetOther u c k | .imtContainsOther u c k => do
      validateWideExpr defined u
      validateWideExpr defined c
      validateWideExpr defined k
