...
/-- External CALL/schedule arguments are unsigned ABI words. The lowering-owned
    width vector is authoritative for otherwise width-erased leaves such as `.temp`; reject
    every expression root that is intrinsically Bool, signed, or Field. -/
private def exprIsExternalUIntCompatibleV1
...
private def validExternalUIntArgWidthsV1
...
private partial def checkPlanStatementsV1
  ...
  for statement in statements do
    ...
    | .externalCallResult callee args result boundAddress =>
        ...
        unless validExternalUIntArgWidthsV1 args result.argBitWidths do
          throw <| .planInvariant .evm
            "unsupported EVM semantic shape: external call arguments must be UInt8, UInt16, UInt32, UInt64, UInt128, or UInt256"
        unless callee.size ≥ 2 do
          throw <| .planInvariant .evm
    ...
    | .keccak256Opcode input _resultTemp =>
        if isView then
          throw <| .planInvariant .evm
            s!"{owner} calls pf.crypto.keccak256 in a view/pureFn context"
        if isConstructor then
          throw <| .planInvariant .evm
            "constructor cannot call pf.crypto.keccak256"
        unless exprIsExternalUIntCompatibleV1 fns input do
          throw <| .planInvariant .evm
