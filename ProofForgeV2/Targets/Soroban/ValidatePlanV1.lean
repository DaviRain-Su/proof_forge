...
private partial def inferExprType
  ...
  | .arith _ lhs rhs => do
      ...
      unless lhsTy == .uint64 && rhsTy == .uint64 do
        planError s!"Soroban plan {what} arithmetic operands must be UInt64"
      pure (.uint64, remaining)
  | .compare op lhs rhs => do
      ...
      | .eq | .ne =>
          unless lhsTy == rhsTy && (lhsTy == .uint64 || lhsTy == .bool) do
            planError s!"Soroban plan {what} equality operands must share UInt64/Bool type"
      | .lt | .le | .gt | .ge =>
          unless lhsTy == .uint64 && rhsTy == .uint64 do
            planError s!"Soroban plan {what} ordering operands must be UInt64"
      pure (.bool, remaining)
...
private def validateExpr
  ...
  let (actual, _) ←
    inferExprType e what paramCount stateCount maxExprNodes
...
def validatePlan (plan : Plan) : CompileResult Unit := do
  ...
  for ent in plan.entries do
    ...
    for ck in ent.checks do
      exprBudget ←
        validateCheck ck ent.params.size plan.states.size exprBudget
    exprBudget ←
