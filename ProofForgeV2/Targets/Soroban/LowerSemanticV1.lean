...
private def viewAggregateLeafIsIntV1
  ...
  | some { shape := .array elTid len, name := none, .. } =>
      unless isUInt64Type types elTid do
        planError
          "unsupported Soroban semantic shape: Array view return element must be UInt64"
      let n := len.toNat
      unless 1 ≤ n && n ≤ 8 do
        planError
...
private def emptyBodyAccum (env : ValueEnv) (overlay : StateOverlay) : BodyAccum :=
  { env, overlay, checks := #[], opCount := 0, sha256Sites := #[], nextTemp := 0 }
...
private def makePlanFromSemanticDataV1
  ...
  for c in data.constants do
    unless isInt64Type types c.typeId || isUInt64Type types c.typeId ||
      ...
      planError
        "unsupported Soroban semantic shape: constant is not admitted UInt64/Int64/UInt32/Bool"
