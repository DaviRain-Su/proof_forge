...
/-!
# Ton ValidatePlanV1 — plan canonicity

Validates the public target-owned Ton Plan before recipe lowering.
-/
...
private def validateStorageLayout (limits : ResourceLimits)
  ...
  unless layout.markerKey == layoutMarkerKey && layout.markerValue != 0 &&
    ...
    throw <| .planInvariant .ton "Ton storage initialization/layout policy is not canonical"
  ...
  for index in [0:layout.fields.size] do
    ...
    -- BL-14: public UInt{8,16,32,64,128,256}/Int{8,16,32,64} leaves
    -- (1/2/4/8/16/32). UInt256 is one 32-byte cell; Int128/256 still FC.
    let admittedWidth :=
  ...
  if hasDuplicates (layout.fields.map (·.name)) ||
    ...
    throw <| .planInvariant .ton "state field names/keys must be unique and distinct from the marker"
  unless layout.markerValue == layoutMarker layout.fields do
    throw <| .planInvariant .ton "state layout marker is not bound to the canonical KV schema"
...
private def validateParams (limits : ResourceLimits) (owner : String)
  ...
  for index in [0:params.size] do
    ...
    -- BL-14: UInt{8,16,32,64,128,256}/Int{8,16,32,64} (1/2/4/8/16/32).
    -- UInt256 admitted as one 32-byte cell; Int128/256 still FC.
    let admittedWidth :=
  ...
  if hasDuplicates (params.map (·.name)) then
    throw <| .planInvariant .ton s!"parameter names in {owner} must be unique"
...
private partial def checkMethodStatementsV1
  ...
  for statement in statements do
    if closed then
      throw <| .planInvariant .ton s!"method has a statement after return"
    match statement with
    | .store store =>
        if isView then
          throw <| .planInvariant .ton s!"view method writes state"
        if isPureFn then
          throw <| .planInvariant .ton s!"pureFn body writes state"
        unless store.fieldIndex < layout.fields.size do
          throw <| .planInvariant .ton s!"method stores to an unknown KV field"
    ...
    | .storeAtomic leaves =>
        ...
        unless leaves.size > 0 do
          throw <| .planInvariant .ton "atomic store must have at least one leaf"
    ...
    | .emitEvent eventIndex args =>
        if isView then
          throw <| .planInvariant .ton "view method emits an event"
        if isPureFn then
          throw <| .planInvariant .ton s!"pureFn body emits an event"
        unless eventIndex < eventCount do
          throw <| .planInvariant .ton "method emits an unknown event"
        unless args.size == eventFieldCounts[eventIndex]! do
          throw <| .planInvariant .ton "method event argument count mismatch"
        for arg in args do
          unless exprIsUInt64CompatibleV1 fns arg do
            throw <| .planInvariant .ton "method event arguments must be UInt64 expressions"
    ...
    | .revertError errorIndex args =>
        ...
        unless args.size == errorFieldCounts[errorIndex]! do
          throw <| .planInvariant .ton "method error argument count mismatch"
        for arg in args do
          unless exprIsUInt64CompatibleV1 fns arg do
            throw <| .planInvariant .ton "method error arguments must be UInt64 expressions"
          total ← addPlanExprNodes limits layout params fns total arg
          methodTemps ← addMethodExprTemps limits layout params fns methodTemps arg
        total := total + 1
        closed := true
...
private def validateMethod (limits : ResourceLimits) (layout : StorageLayout)
  ...
  unless isIdentifier method.name && method.name != "memory" do
    throw <| .planInvariant .ton s!"method '{method.name}' is not a safe export name"
  if isInitializer then
  ...
  unless method.exactInputLen == exactInputLenOfParams method.params do
    throw <| .planInvariant .ton s!"method '{method.name}' raw input length is not canonical"
  if method.body.size > limits.maxBodyStatements || (!isInitializer && method.body.isEmpty) then
    throw <| .planInvariant .ton s!"method '{method.name}' has an invalid body size"
...
private def validateFnBinding (limits : ResourceLimits) (layout : StorageLayout)
  ...
  unless isIdentifier fn.name && fn.name != "memory" do
    throw <| .planInvariant .ton s!"pureFn '{fn.name}' is not a safe identifier"
  validateParams limits s!"pureFn '{fn.name}'" fn.params
  if fn.body.isEmpty || fn.body.size > limits.maxBodyStatements then
    throw <| .planInvariant .ton s!"pureFn '{fn.name}' has an invalid body size"
  let (total, _, closed) ← checkMethodStatementsV1
...
def validatePlan (plan : Plan) : CompileResult Unit := do
  ...
  unless plan.targetDescriptor == descriptor &&
    ...
    throw <| .planInvariant .ton "Ton Plan descriptor/schema/profile policies are not canonical"
  unless isIdentifier plan.programName do
    throw <| .planInvariant .ton s!"program name '{plan.programName}' is not a safe identifier"
  ...
  if plan.entries.isEmpty || plan.entries.size > plan.resourceLimits.maxEntries then
    throw <| .planInvariant .ton "entry count is outside the profile limits"
  if plan.fns.size > plan.resourceLimits.maxEntries then
    throw <| .planInvariant .ton "pureFn count is outside the profile limits"
  ...
  if hasDuplicates (methods.map (·.name)) then
    throw <| .planInvariant .ton "Ton export names must be unique"
  if hasDuplicates (plan.fns.map (·.name)) then
    throw <| .planInvariant .ton "Ton pureFn names must be unique"
  let exportAndFnNames := methods.map (·.name) ++ plan.fns.map (·.name)
  if hasDuplicates exportAndFnNames then
    throw <| .planInvariant .ton "Ton export and pureFn names must not collide"
