/-
  ...
  Deleted product commands: `build-counter`, `describe-target`, `network`
  (use `build Examples/StateCell.lean --module Examples.StateCell`, `inspect`,
  and target-owned external deployment tooling). Build flag `--network` remains
  a usage error because there is no network registry.
...
/-- Parsed `local` trailing flags (host-heavy package-script wrapper).
    `mode` empty means the target's registered local default (`runtime` for
    the admitted Solana/EVM lanes). Remaining arguments after product flags
    (and optional `--`) are forwarded to the script. -/
...
Deleted: `build-counter`, `describe-target`, `network` (use build + inspect,
or target-owned external deployment tooling).
...
partial def parseBuildArgsExcept (args : List String) (options : BuildOptions := {}) :
  ...
  | "--callee-output" :: value :: rest =>
      ...
      if options.calleeOutputs.contains value then
        throw s!"duplicate --callee-output '{value}'"
      parseBuildArgsExcept rest
        { options with calleeOutputs := options.calleeOutputs.push value }
  | "--callee-output" :: [] => throw "missing --callee-output value"
  | value :: rest =>
...
def inspectBindingOutputAuthorityV1
  ...
  unless manifestAfter.outputSetDigest == manifest.outputSetDigest do
    throw <| IO.userError
      "call-bind: callee output changed while it was being inspected"
  let sourceHash ← match digestFromBareHex "sourceHash" manifest.sourceHash with
    | .ok value => pure value
    | .error error => throw <| IO.userError error
  let semanticHash ← match digestFromBareHex "semanticHash" manifest.semanticHash with
    | .ok value => pure value
    | .error error => throw <| IO.userError error
  pure {
