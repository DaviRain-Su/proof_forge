...
private def usage : String :=
  ...
  "  proof-forge-next build <source.pf|.lean> --module <Lean.Name> --target <target> [-o <dir>] [--program <Name>] [--root <dir>] [--profile <id>] [--language-version <semver>] [--minimum-evidence <grade>] [--bindings <path>] [--binding-evidence <path>] [--resource-limit <stage>.<field>=<n>]... [--json]\n" ++
  "\n" ++
  "Notes:\n" ++
...
private unsafe def buildSource (options : BuildOptions) : IO Unit := do
  ...
  -- Solana/CosmWasm identity metadata remains outside this slice.
...
private def resolveLocalScriptV1 (target mode : String) :
  ...
  | "near" =>
      ...
      | "runtime" =>
          pure (m, "scripts/pf_near_test.sh",
            "host-heavy near-sandbox; artifact fast-path when PF_NEAR_ARTIFACT_DIR has *.wasm, else full corpus; Promise=async; sync call FC; not ordinary ci; not formal")
      | _ =>
  ...
  | "cosmwasm" =>
      ...
      match m with
      | "runtime" =>
          pure (m, "scripts/pf_cosmwasm_test.sh",
            "host-heavy cosmwasm-vm mock corpus; sync call FC; schedule=SubMsg reply_on=never; not ordinary ci; not formal")
      | _ =>
          throw s!"unsupported local mode '{m}' for target cosmwasm (want runtime)"
  | "ton" =>
      let m := if mode.isEmpty then "runtime" else mode
      match m with
      | "runtime" =>
          pure (m, "scripts/pf_ton_test.sh",
...
private def renderHostHeavyJsonV1
  ...
  let publicArgs := redactHostHeavyArgsV1 args
  let argJson := publicArgs.map PfJson.string
  let publicStdout := redactHostHeavyTextV1 args stdout
  let publicStderr := redactHostHeavyTextV1 args stderr
  renderPfJcs <|
    PfJson.object #[
      ...
      ("notes", .string notes),
      ("scriptStdout", .string publicStdout),
...
private def runLocal (options : LocalOptions) : IO Unit := do
  ...
  if options.json then
    ...
    let publicStdout := redactHostHeavyTextV1 options.scriptArgs output.stdout
    let publicStderr := redactHostHeavyTextV1 options.scriptArgs output.stderr
