...
namespace ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualityPreservationV1
...
theorem preservationTheorem_of_subjectBodyV1
    ...
    (hsubject : subjectData =
      ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1
        qualifiedName state0Name state1Name depositName depositParameterName withdrawName withdrawParameterName viewName invariantName)
  ...
  have legal : ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.StructureLegalV1
      qualifiedName state0Name state1Name depositName depositParameterName withdrawName withdrawParameterName viewName invariantName :=
    ⟨hnameShape, hstate0Name, hstate1Name, hdepositName, hdepositParameterName, hwithdrawName, hwithdrawParameterName,
      hviewName, hinvariantName, hstate01, hdepositWithdraw, hdepositView, hdepositInvariant,
      hwithdrawView, hwithdrawInvariant, hviewInvariant⟩
  have hstructure : validateSemanticProgramStructureV1 data = .ok () := by
    simpa [data] using
      ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.structureV1
        qualifiedName state0Name state1Name depositName depositParameterName withdrawName withdrawParameterName viewName invariantName legal
  have hinvert : RootFieldInvertV1 data := by
    simpa [data] using
      ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.rootFieldInvertV1
        qualifiedName state0Name state1Name depositName depositParameterName withdrawName withdrawParameterName viewName invariantName
        hstate0Name hstate1Name hdepositName hdepositParameterName hwithdrawName
  ...
  have hinitializerReturned :
    ...
    exact (evalInvariantV1_returnedTrue_iff_two_state_bytes_eq program data 0 0
      ...
      zero8BytesV1 4 1 0 0 1 (some invariantName) .public_ .public_
    ...
    simpa [initial, data, zero8BytesV1] using
      initialLogicalStateV1_double_uint64_eq_ok program data
        ...
        (by
          simp [data,
            ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
            ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1,
            show
  ...
  have hbaseWithInitializer :
    ...
    | ready callable overlay context isInitializer =>
        ...
        have hindexBound : invocation.callableId.toNat < 5 := by
        ...
        | 1 =>
            ...
            have hcallable : callable = addParameterTwoReturnCallableV1 1
                (some depositName) depositParameterName 1 0 1 .public_ := by
              simpa [data,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1]
                using hlookup1.symm
        ...
        | 2 =>
            ...
            have hcallable : callable = guardedSubParameterTwoUnitCallableV1 2
                (some withdrawName) withdrawParameterName 1 0 2 0 1 .public_ := by
              simpa [data,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1]
                using hlookup2.symm
        ...
        | 3 =>
            ...
            have hcallable : callable = viewLoadCallableV1 3 (some viewName) 1 0 := by
              simpa [data,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1]
                using hlookup3.symm
            ...
            contradiction
        | 4 =>
            have hlookup4 := hlookup
            rw [hindex] at hlookup4
            have hcallable : callable = twoStateCompareInvariantCallableV1 4 (some invariantName) 1 0 0 1 .eq .public_ (some 5) := by
              simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1] using hlookup4.symm
            subst callable
            change false = true at hkind
            contradiction
        | n + 5 => omega
  ...
  have hreturned : PreservationReturnedCallablesV1 program 0 admitted := by
    ...
    | ⟨1, _⟩ =>
      intro pre invocation responses vault overlay context isInitializer post value effects
        hcallableId _hconforms heval hgate hstep
      have hisInitializer : isInitializer = false := by
        ...
        have hkFalse : (data.callables[1].kind == CallableKindV1.initializer) = false := by
          change (CallableKindV1.entry == CallableKindV1.initializer) = false
          decide
        exact hk.trans hkFalse
      have htypeBound : 1 < data.types.size := by
        simp [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
          ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.typesV1]
        ...
        | mk cid args ctx =>
          ...
          | n + 1 =>
            have hsize : args.size = 1 := by
              simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1,
                addParameterTwoReturnCallableV1] using harity
            simp [Array.getElem?_eq_none, hsize]
      have hdecode := (gateInvocation_ready_noninit_decode admitted pre invocation
        data.callables[1] overlay context (by simpa [hisInitializer] using hgate)).1
      rw [hadmittedData] at hdecode
      have hoverlay : overlay.size = 2 := by
        have hs := decodeLogicalStateValuesV1_size data pre overlay hdecode
        simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1] using hs
      obtain ⟨before0, before1, hoverlayEq⟩ : ∃ b0 b1, overlay = #[b0, b1] := by
        exact ⟨overlay[0]'(by omega), overlay[1]'(by omega), Array.ext (by simp [hoverlay]) (by
          intro i hi hj
          match i with | 0 => simp | 1 => simp | n + 2 => omega)⟩
      have hbefore : before0 = before1 := by
        have hpreInitialized : pre.initialized = true :=
        ...
        have hiff := evalInvariantV1_returnedTrue_iff_two_state_bytes_eq program data 0 0
          invariantName pre #[before0, before1] before0 before1 4 1 0 0 1
      ...
      obtain ⟨sumBytes, hpostEncode⟩ :=
        stepReferenceSliceV1_ready_add_parameter_two_returned_post_encode
          ...
          depositParameterName 1 (some depositName) invocation.context context responses vault value
          effects hadmittedData rfl rfl rfl hcanonical (by
            change gateInvocation admitted pre invocation = .ready
              (addParameterTwoReturnCallableV1 1 (some depositName) depositParameterName 1 0 1 .public_)
      ...
      have hpostInitialized : post.initialized = true :=
        LogicalStateV1.initialized_of_encodeLogicalStateValuesV1 data true
          #[sumBytes, sumBytes] post hpostEncode
      exact (evalInvariantV1_returnedTrue_iff_two_state_bytes_eq program data 0 0
        invariantName post #[sumBytes, sumBytes] sumBytes sumBytes 4 1 0 0 1
        (some invariantName) .public_ .public_ .public_ state0Name state1Name
    ...
    | ⟨2, _⟩ =>
      ...
      have hisInitializer : isInitializer = false := by
        ...
        have hkFalse : (data.callables[2].kind == CallableKindV1.initializer) = false := by
          change (CallableKindV1.entry == CallableKindV1.initializer) = false
          decide
        exact hk.trans hkFalse
      have htypeBound : 1 < data.types.size := by
        simp [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
          ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.typesV1]
      obtain ⟨argument, harg⟩ := gateInvocation_ready_uint64_argumentV1 admitted pre
        invocation data.callables[2] overlay context isInitializer 0 (by change 0 < 1; decide) 1
        (by rfl) (data.types[1]'htypeBound) (by simpa [hadmittedData]) (by rfl) hgate
      have harity := gateInvocation_ready_arity admitted pre invocation data.callables[2]
        overlay context isInitializer hgate
      have hinvocation : invocation = {
          callableId := 2
          args := #[{ typeId := 1, valueBytes := encodeU64le argument }]
        ...
        | mk cid args ctx =>
          ...
          | n + 1 =>
            have hsize : args.size = 1 := by
              simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
                ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1,
                guardedSubParameterTwoUnitCallableV1] using harity
            simp [Array.getElem?_eq_none, hsize]
      have hdecode := (gateInvocation_ready_noninit_decode admitted pre invocation
        data.callables[2] overlay context (by simpa [hisInitializer] using hgate)).1
      rw [hadmittedData] at hdecode
      have hoverlay : overlay.size = 2 := by
        have hs := decodeLogicalStateValuesV1_size data pre overlay hdecode
        simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1] using hs
      ...
      have hbefore : before0 = before1 := by
        have hpreInitialized : pre.initialized = true :=
          (gateInvocation_ready_noninit_decode admitted pre invocation data.callables[2]
            overlay context (by simpa [hisInitializer] using hgate)).2.1
        have hiff := evalInvariantV1_returnedTrue_iff_two_state_bytes_eq program data 0 0
          invariantName pre #[before0, before1] before0 before1 4 1 0 0 1
          (some invariantName) .public_ .public_ .public_ state0Name state1Name
      ...
      have hpostEncode :
        ...
        simpa [differenceBytes] using
          stepReferenceSliceV1_ready_guarded_sub_parameter_two_unit_returned_post_encode
            admitted pre post data before0 (encodeU64le argument) 1 0 2 state0Name
            ...
            rfl rfl rfl rfl rfl hcanonical (by
              change gateInvocation admitted pre invocation = .ready
                (guardedSubParameterTwoUnitCallableV1 2 (some withdrawName)
                  withdrawParameterName 1 0 2 0 1 .public_)
                  overlay context isInitializer at hgate
              rw [hinvocation, hoverlayEq, hisInitializer] at hgate
              exact hgate) (by rw [hinvocation] at hstep; exact hstep)
      have hdecodePost := decodeLogicalStateValuesV1_of_encodeLogicalStateValuesV1 data true
      ...
      have hpostInitialized : post.initialized = true :=
        LogicalStateV1.initialized_of_encodeLogicalStateValuesV1 data true
          #[differenceBytes, differenceBytes] post hpostEncode
      exact (evalInvariantV1_returnedTrue_iff_two_state_bytes_eq program data 0 0
        invariantName post #[differenceBytes, differenceBytes] differenceBytes differenceBytes 4 1 0 0 1
        (some invariantName) .public_ .public_ .public_ state0Name state1Name
        hvalidate hpostInitialized hdecodePost rfl rfl rfl rfl rfl rfl rfl).2 rfl
    | ⟨3, _⟩ =>
      intro pre invocation responses vault overlay context isInitializer post
      ...
      have hisInitializer : isInitializer = false := by
        have hkind := (gateInvocation_ready_callable_lookup admitted pre invocation
          data.callables[3] overlay context isInitializer hgate).2
        have hkind' : isInitializer = ((viewLoadCallableV1 3 (some viewName) 1 0).kind == CallableKindV1.initializer) := by
          simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
            ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1] using hkind
        change isInitializer = false at hkind'
        exact hkind'
      have hdecode := gateInvocation_ready_decodeV1 admitted pre invocation data.callables[3]
        overlay context isInitializer hgate
      rw [hadmittedData] at hdecode
      have hoverlayNonempty : 0 < overlay.size := by
        have hs := decodeLogicalStateValuesV1_size data pre overlay hdecode
        have : overlay.size = 2 := by simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1] using hs
        omega
      ...
      change gateInvocation admitted pre invocation = .ready
        (viewLoadCallableV1 3 (some viewName) 1 0) overlay context isInitializer at hgate
      rw [hisInitializer] at hgate
      have hpostEq := postEqPre_of_readyViewLoadV1 admitted pre post invocation data overlay
        loadedBytes 1 0 state0Name 3 (some viewName) context responses vault value effects
    ...
    | ⟨n + 5, hlt⟩ =>
      have : n + 5 < 5 := by
        simpa [data, ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.subjectDataV1,
          ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualitySubjectV1.callablesV1] using hlt
      omega
  exact preservationTheoremV1_of_callableObligationsV1 program 0 admitted hordinal
    hadmit hbase hreturned

end ProofForgeV2.Semantic.InitializerDepositWithdrawViewEqualityPreservationV1
