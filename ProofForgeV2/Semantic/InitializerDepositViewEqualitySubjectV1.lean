...
/-!
  ...
  a two-slot UInt64-zero initializer, a unary additive entry, a nullary UInt64
  view, and a two-state equality invariant. Names remain parameters; execution remains solely in
  `ReferenceMachineV1`.
-/

namespace ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1
...
def callablesV1 (depositName parameterName viewName invariantName : String) : Array CallableV1 := #[
  initializerStoreZeroTwoCallableV1 0 1 2,
  addParameterTwoReturnCallableV1 1 (some depositName) parameterName 1 0 1 .public_,
  viewLoadCallableV1 2 (some viewName) 1 0,
  twoStateCompareInvariantCallableV1 3 (some invariantName)
    1 0 0 1 .eq .public_ (some 5)
]

def subjectDataV1
    (qualifiedName : QualifiedName)
    (state0Name state1Name depositName parameterName viewName invariantName : String) :
  ...
  events := #[]
  errors := #[]
  callables := callablesV1 depositName parameterName viewName invariantName
  invariants := #[{ id := 0, name := invariantName, callableId := 3 }]
...
theorem exactAtRoot_invariantsV1 (invariantName : String) :
    ExactMidOffsetInvertAtV1 (encodeArray encodeInvariantDeclV1)
      ...
      #[{ id := 0, name := invariantName, callableId := 3 }] 1 :=
  exactAt_array_one_of_exactAtV1 encodeInvariantDeclV1 decodeInvariantDeclV1
    maxTableElements (by decide)
    ({ id := 0, name := invariantName, callableId := 3 } : InvariantDeclV1) 1
...
theorem rootFieldInvertV1
    ...
    RootFieldInvertV1
      (subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
        invariantName) := by
  refine {
    qualifiedName := ?_
    types := ?_
  ...
  · simpa [subjectDataV1, callablesV1] using
      exactAt_initializerAddViewEqualityCallableTableV1 0 1 2 3 depositName parameterName
        viewName invariantName 1 2 0 0 1 hdepositName hparameterName hviewName
        hinvariantName
...
structure StructureLegalV1
    (qualifiedName : QualifiedName)
    (state0Name state1Name depositName parameterName viewName invariantName : String) : Prop where
...
theorem callablesValueBytesV1 (depositName parameterName viewName invariantName : String) :
    validateCallablesValueBytesV1 typesV1
      (callablesV1 depositName parameterName viewName invariantName) maxCanonicalProgramBytes =
...
theorem callableSignaturesV1
    (depositName parameterName viewName invariantName : String)
    (hdepositView : depositName ≠ viewName)
    (hdepositInvariant : depositName ≠ invariantName)
    (hviewInvariant : viewName ≠ invariantName) :
    validateCallableSignaturePhasesV1 typesV1
      (callablesV1 depositName parameterName viewName invariantName) = .ok () := by
  ...
  all_goals
    simp [typesV1, callablesV1, initializerStoreZeroTwoCallableV1,
      ...
      hdepositInvariant', hInitInit, hEntryInit, hViewInit,
      hInvInit, hInitInv, hEntryInv, hViewInv, hInvInv, hPublic,
      Pure.pure, Except.pure, Bind.bind, Except.bind]

theorem invariantDeclarationJoinV1
    (depositName parameterName viewName invariantName : String) :
    validateInvariantDeclarationJoinV1 (callablesV1 depositName parameterName viewName invariantName)
      #[{ id := 0, name := invariantName, callableId := 3 }] = .ok () := by
  have hInitInv : ((.initializer : CallableKindV1) == .invariant) = false :=
    by decide
  have hViewInv : ((.view : CallableKindV1) == .invariant) = false :=
    by decide
...
theorem declarationIdentifierNamesV1
  ...
  have hparameter := validateIdentifierNameV1_eq_ok_of_common _ hparameterName
...
private theorem initializerCfgV1
  ...
  let data := subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
    invariantName
...
private theorem viewCfgV1
    ...
    validateCallableCfgShape
      ...
      (subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
        invariantName) = .ok () := by
  let callable := viewLoadCallableV1 2 (some viewName) 1 0
  let data := subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
    invariantName
  refine validateCallableCfgShape_eq_ok_of_phases callable typesV1.size
    typesV1 data #[true] ?_ ?_ ?_ ?_ ?_
...
private theorem entryCfgV1
    ...
    (state0Name state1Name depositName parameterName viewName invariantName : String) :
    validateCallableCfgShape
      (addParameterTwoReturnCallableV1 1 (some depositName) parameterName
        1 0 1 .public_)
      typesV1.size typesV1
      (subjectDataV1 qualifiedName state0Name state1Name depositName
        parameterName viewName invariantName) = .ok () := by
  ...
  let data := subjectDataV1 qualifiedName state0Name state1Name depositName
    parameterName viewName invariantName
  refine validateCallableCfgShape_eq_ok_of_phases callable typesV1.size
    typesV1 data #[true] ?_ ?_ ?_ ?_ ?_
  · rfl
  · rfl
...
private theorem invariantCfgV1
    ...
    validateCallableCfgShape
      ...
      (subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
        invariantName) = .ok () := by
  let callable := twoStateCompareInvariantCallableV1 3 (some invariantName)
    1 0 0 1 .eq .public_ (some 5)
  let data := subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
    invariantName
  refine validateCallableCfgShape_eq_ok_of_phases callable typesV1.size
...
theorem genericCfgPhasesV1
    ...
    validateGenericCfgPhasesV1
      (subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName
        invariantName) = .ok () := by
  apply validateGenericCfgPhasesV1_four_eq_ok
    (subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName invariantName)
    (initializerStoreZeroTwoCallableV1 0 1 2)
    (addParameterTwoReturnCallableV1 1 (some depositName) parameterName
      1 0 1 .public_)
    (viewLoadCallableV1 2 (some viewName) 1 0)
    (twoStateCompareInvariantCallableV1 3 (some invariantName)
      1 0 0 1 .eq .public_ (some 5))
  · rfl
  · exact initializerCfgV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  · exact entryCfgV1 qualifiedName state0Name state1Name depositName parameterName
      viewName invariantName
  · exact viewCfgV1 qualifiedName state0Name state1Name depositName parameterName viewName invariantName
  · exact invariantCfgV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  · rfl

def closureMembersV1 : Array Bool := #[false, false, false, true]

theorem invariantClosurePhasesV1
    (depositName parameterName viewName invariantName : String) :
    validateInvariantClosurePhasesV1
      (callablesV1 depositName parameterName viewName invariantName) = .ok closureMembersV1 := by
...
theorem invariantFuelPhasesV1
    ...
    validateInvariantFuelPhasesV1
      (callablesV1 depositName parameterName viewName invariantName) closureMembersV1 = .ok () := by
  rfl
...
theorem cfgInvariantPhasesV1
  ...
  apply validateCfgInvariantPhasesV1_eq_ok
    (subjectDataV1 qualifiedName state0Name state1Name depositName parameterName viewName invariantName)
    closureMembersV1
  · exact genericCfgPhasesV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  · simpa [subjectDataV1] using
      invariantClosurePhasesV1 depositName parameterName viewName invariantName
  · simpa [subjectDataV1] using
      invariantFuelPhasesV1 depositName parameterName viewName invariantName

private def usageClosureFixtureV1 : SemanticProgramDataV1 :=
  subjectDataV1
    { components := { head := "Proof", tail := #["Forge"] } }
    "s0" "s1" "deposit" "param" "view" "inv"

private def initializerDepositCoreRootsV1 : Array TypeIdV1 :=
  collectCoreTypeSlotRootsV1 usageClosureFixtureV1

private theorem collectCoreTypeSlotRootsV1_initializerDeposit_fixture :
    collectCoreTypeSlotRootsV1 usageClosureFixtureV1 = initializerDepositCoreRootsV1 := rfl

private theorem foldBoolUInt64UnitCoreRootsV1_initializerDeposit_fixture :
    foldBoolUInt64UnitCoreRootsV1 initializerDepositCoreRootsV1 = #[true, true, true] := by
  dsimp [initializerDepositCoreRootsV1, usageClosureFixtureV1, subjectDataV1,
    collectCoreTypeSlotRootsV1, foldBoolUInt64UnitCoreRootsV1]
  simp [typesV1, callablesV1, initializerStoreZeroTwoCallableV1,
    addParameterTwoReturnCallableV1, viewLoadCallableV1,
    twoStateCompareInvariantCallableV1, Array.foldl, Array.set!, Array.replicate]
  rfl

private theorem anonymousTypeUsageBitmapV1_initializerDeposit_fixture :
  ...
  rw [collectCoreTypeSlotRootsV1_initializerDeposit_fixture,
    foldBoolUInt64UnitCoreRootsV1_initializerDeposit_fixture]

private theorem anonymousTypeUsageBitmapV1_initializerDeposit_names_irrel
    (qualifiedName : QualifiedName)
    (state0Name state1Name depositName parameterName viewName invariantName : String) :
  ...
  simp [anonymousTypeUsageBitmapV1, subjectDataV1, usageClosureFixtureV1, typesV1,
    ...
    viewLoadCallableV1, twoStateCompareInvariantCallableV1, collectCoreTypeSlotRootsV1]
...
private theorem usageClosureV1
  ...
  · rw [anonymousTypeUsageBitmapV1_initializerDeposit_names_irrel qualifiedName state0Name
      state1Name depositName parameterName viewName invariantName,
    anonymousTypeUsageBitmapV1_initializerDeposit_fixture]
...
theorem structureV1
    (qualifiedName : QualifiedName)
    (state0Name state1Name depositName parameterName viewName invariantName : String)
    (legal : StructureLegalV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName) :
  ...
  · exact structurePreludeV1 qualifiedName state0Name state1Name depositName parameterName viewName
  ...
  · exact usageClosureV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  · simpa [data, subjectDataV1] using namedTypeNamesV1
  · exact constantsValueBytesV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  · simpa [data, subjectDataV1] using
      callablesValueBytesV1 depositName parameterName viewName invariantName
  · exact constantNamesV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  · simpa [data, subjectDataV1] using
  ...
  · simp [data, subjectDataV1, validateInterfaceFieldNameUniquenessV1,
      Pure.pure, Except.pure, Bind.bind, Except.bind]
  · simpa [data, subjectDataV1] using
      callableSignaturesV1 depositName parameterName viewName invariantName
        legal.hdepositView legal.hdepositInvariant legal.hviewInvariant
  · simpa [data, subjectDataV1] using
      invariantDeclarationJoinV1 depositName parameterName viewName invariantName
  · exact declarationIdentifierNamesV1 qualifiedName state0Name state1Name
      depositName parameterName viewName invariantName legal.hstate0Name
      legal.hstate1Name legal.hdepositName legal.hparameterName legal.hviewName
      legal.hinvariantName
  · exact cfgInvariantPhasesV1 qualifiedName state0Name state1Name depositName parameterName viewName
      invariantName
  ...
  · exact (emptyOperationRequirementsV1 qualifiedName state0Name state1Name
      depositName parameterName viewName invariantName).1
  · exact (emptyOperationRequirementsV1 qualifiedName state0Name state1Name
      depositName parameterName viewName invariantName).2.1
  · exact (emptyOperationRequirementsV1 qualifiedName state0Name state1Name
      depositName parameterName viewName invariantName).2.2
...
end ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1
