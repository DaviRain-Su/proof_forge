...
private def compareByteArrayLexFinishV1
    (left right : ByteArray) : Ordering :=
  if left.size < right.size then .lt
  else if left.size > right.size then .gt
  else .eq

private def compareByteArrayLexLoopFuelV1
    (left right : ByteArray) (n i : Nat) : Nat → Ordering
  | 0 => compareByteArrayLexFinishV1 left right
  | fuel + 1 =>
      if i < n then
        let bl := left.get! i
        let br := right.get! i
        ...
        else compareByteArrayLexLoopFuelV1 left right n (i + 1) fuel
      else
        compareByteArrayLexFinishV1 left right
...
/-- Internal production loop for unsigned byte-array lexicographic order.
    ...
    comparator remains the sole ordering authority. The structural fuel is
    exactly the number of bytes remaining in the compared prefix. -/
def compareByteArrayLexLoopV1 (left right : ByteArray) (n i : Nat) : Ordering :=
  compareByteArrayLexLoopFuelV1 left right n i (n - i)
...
theorem compareByteArrayLexLoopV1_eq_next (left right : ByteArray) (n i : Nat)
  ...
  unfold compareByteArrayLexLoopV1
  rw [show n - i = (n - (i + 1)) + 1 by omega]
  simp [compareByteArrayLexLoopFuelV1, hi, heq]
...
theorem compareByteArrayLexLoopV1_eq_lt (left right : ByteArray) (n i : Nat)
  ...
  unfold compareByteArrayLexLoopV1
  rw [show n - i = (n - (i + 1)) + 1 by omega]
  simp [compareByteArrayLexLoopFuelV1, hi, hlt]
...
theorem compareByteArrayLexLoopV1_eq_gt (left right : ByteArray) (n i : Nat)
  ...
  unfold compareByteArrayLexLoopV1
  rw [show n - i = (n - (i + 1)) + 1 by omega]
  simp [compareByteArrayLexLoopFuelV1, hi, hnot, hgt]
