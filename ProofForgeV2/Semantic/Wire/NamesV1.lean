...
/-- Exact declaration/field names are checked on a private UTF-8 sort so
    ...
    non-quadratic at the wire table limit. Tables of at most four names use an
    equivalent bounded pair scan, keeping the canonical small generated
    families transparent to kernel certificates without changing acceptance. -/
...
def checkUniqueDeclarationNamesV1 (names : Array String) :
  ...
  if names.size ≤ 4 then
    if names[0]! == names[1]! then return ← err .duplicate
    if names.size ≥ 3 then
      if names[0]! == names[2]! then return ← err .duplicate
      if names[1]! == names[2]! then return ← err .duplicate
    if names.size == 4 then
      if names[0]! == names[3]! then return ← err .duplicate
      if names[1]! == names[3]! then return ← err .duplicate
      if names[2]! == names[3]! then return ← err .duplicate
    return
