...
/-!
  ...
  | B-SC-DEC | Parametric `decode (simpleClosureWireBytesV1 p) = ok (materialize p)` | **Partial**: sole Encode builder; `SemanticProgramFieldsOkV1` inversion; production mid-offset NFC string/u32/u16/magic/finish; empty tables; QN induction under Legal; strong packaging `decode_of_simpleClosure_fields_ok_of_taggedBody` (sole encode `hfields` + residual tagged body). **Exact residual:** `decodeSemanticProgramDataTagged_of_simpleClosure_field_bytes` — nine-field tagged body encode→decode for TypeDecl×2 / empty×4 / Callable×2 / InvariantDecl / Requirements. |
