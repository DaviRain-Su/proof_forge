...
/-!
  ...
  | B-SC-ENC | Parametric `encode (materialize p) = ok (simpleClosureWireBytesV1 p)` | **Closed** in `SimpleClosureEncodeFieldsV1`: sole production body `encodeSemanticProgramDataBodyV1`; legal-only `encodeSimpleClosure_of_legal` / `EncodeSimpleClosureGoalV1`; fixed fields and total size close without body/field/size premises. |
  | B-SC-DEC | Parametric `decode (simpleClosureWireBytesV1 p) = ok (materialize p)` | **Closed under `SimpleClosureParamsLegalV1`** in `SimpleClosureDecodeComposeV1`: production QN encode/parser join, root tag, fixed TypeDecl/empty/Callable/InvariantDecl/Requirements leaves, exact nine-field cursor composition, and framing; no intermediate decoder premises. |
