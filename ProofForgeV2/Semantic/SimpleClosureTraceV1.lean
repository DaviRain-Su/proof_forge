...
/-!
  ...
  | B-SC-ENC | Parametric `encode (materialize p) = ok (simpleClosureWireBytesV1 p)` | **Partial** in `SimpleClosureEncodeV1`: sole production body `encodeSemanticProgramDataBodyV1` (root = gates then body; no second field composition); wireBytes owner = body of materialize; identifier→encodeString + QN encode under legal; packaging legal+body-ok⇒root=wireBytes. Residual = discharge body-ok (`∃ b, encode body = ok b`) from legal alone (fixed types/callables/invariants/requirements + size cap). |
  | B-SC-DEC | Parametric `decode (simpleClosureWireBytesV1 p) = ok (materialize p)` | **Partial**: sole Encode builder; `SemanticProgramFieldsOkV1` inversion; production mid-offset NFC string/u32/u16/magic/finish; QN induction under Legal; fixed TypeDecl/empty/Callable/InvariantDecl/Requirements leaf decoders; strong packaging `decode_of_simpleClosure_fields_ok_of_taggedBody`. Residual = root nine-field cursor composition plus QN encoder-byte/parser join. |
