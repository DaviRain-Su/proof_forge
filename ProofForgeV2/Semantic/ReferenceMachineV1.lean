      ...
      isInitializer = (callable.kind == CallableKindV1.initializer) ∧
        invocation.args.size = callable.params.size ∧
          invocationArgumentsCanonicalV1 admitted.data callable invocation =
            true ∧
          decodeLogicalStateValuesV1 admitted.data pre = .ok overlay := by
...
/-- Every ready production gate exposes the exact logical-state decode that
    supplied its execution overlay. This projection covers both the distinct
    initializer lifecycle state and ordinary initialized calls; it does not
    decode state independently of `gateInvocation`. -/
theorem gateInvocation_ready_decodeV1
    (admitted : AdmittedReferenceSliceV1)
    (pre : LogicalStateV1)
    (invocation : InvocationV1)
    (callable : CallableV1)
    ...
    decodeLogicalStateValuesV1 admitted.data pre = .ok overlay := by
  exact
    (gateInvocation_ready_details admitted pre invocation callable overlay
      context isInitializer hgate).2.2.2.2
...
theorem gateInvocation_ready_argument
  ...
  have hcanonical := hdetails.2.2.2.1
...
private theorem maxEffectIdInCallable_eq_zero_of_literal_return_shape
    (callable : CallableV1)
    (typeId : TypeIdV1)
    (valueBytes : ByteArray)
    (hparams : callable.params = #[])
...
private theorem evalBinary_le_uint64_success
    (data : SemanticProgramDataV1) (uint64TypeId boolTypeId : TypeIdV1)
    (lhs rhs : ByteArray)
    (htypeU : data.types[uint64TypeId.toNat]? = some {
      id := uint64TypeId, name := none, shape := .uint 64 })
    (htypeB : data.types[boolTypeId.toNat]? = some {
    ...
    (hcanLhs : validateValueBytesV1 data.types uint64TypeId lhs = .ok ())
    (hcanRhs : validateValueBytesV1 data.types uint64TypeId rhs = .ok ())
    (hle : leBytesToNatV1 lhs ≤ leBytesToNatV1 rhs) :
    evalBinary data .le
      { typeId := uint64TypeId, valueBytes := lhs }
      { typeId := uint64TypeId, valueBytes := rhs }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 1 } := by
  have hvcL : valueCanonical data
      { typeId := uint64TypeId, valueBytes := lhs } = true := by
    simp [valueCanonical, hcanLhs]
  have hvcR : valueCanonical data
  ...
  have hbool : isBoolType data boolTypeId = true := by
    simp [isBoolType, shapeOf, htypeB]
  have hwidth : uintWidth data uint64TypeId = some 64 := by
    simp [uintWidth, shapeOf, htypeU]
  change leBytesToNat lhs ≤ leBytesToNat rhs at hle
  simp [evalBinary, hvcL, hvcR, hbool, hwidth, intWidth, shapeOf, htypeU,
    hle]

private theorem evalBinary_le_uint64_failure
    (data : SemanticProgramDataV1) (uint64TypeId boolTypeId : TypeIdV1)
    (lhs rhs : ByteArray)
    (htypeU : data.types[uint64TypeId.toNat]? = some {
      id := uint64TypeId, name := none, shape := .uint 64 })
    ...
    (hcanLhs : validateValueBytesV1 data.types uint64TypeId lhs = .ok ())
    (hcanRhs : validateValueBytesV1 data.types uint64TypeId rhs = .ok ())
    (hnotle : ¬ leBytesToNatV1 lhs ≤ leBytesToNatV1 rhs) :
    evalBinary data .le
      { typeId := uint64TypeId, valueBytes := lhs }
      { typeId := uint64TypeId, valueBytes := rhs }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 0 } := by
  have hvcL : valueCanonical data
      { typeId := uint64TypeId, valueBytes := lhs } = true := by
    simp [valueCanonical, hcanLhs]
  have hvcR : valueCanonical data
  ...
  have hwidth : uintWidth data uint64TypeId = some 64 := by
    simp [uintWidth, shapeOf, htypeU]
  change ¬ leBytesToNat lhs ≤ leBytesToNat rhs at hnotle
  simp [evalBinary, hvcL, hvcR, hbool, hwidth, intWidth, shapeOf, htypeU,
    hnotle]

private theorem evalBinary_sub_uint64_success
    (data : SemanticProgramDataV1) (uint64TypeId : TypeIdV1)
    (lhs rhs : ByteArray)
    (htypeU : data.types[uint64TypeId.toNat]? = some {
      id := uint64TypeId, name := none, shape := .uint 64 })
    (hcanLhs : validateValueBytesV1 data.types uint64TypeId lhs = .ok ())
    (hcanRhs : validateValueBytesV1 data.types uint64TypeId rhs = .ok ())
    (hle : leBytesToNatV1 rhs ≤ leBytesToNatV1 lhs) :
    evalBinary data .sub
      { typeId := uint64TypeId, valueBytes := lhs }
      { typeId := uint64TypeId, valueBytes := rhs }
      uint64TypeId = .ok {
        typeId := uint64TypeId
        valueBytes := natToLeBytesV1
          (leBytesToNatV1 lhs - leBytesToNatV1 rhs) 8 } := by
  have hvcL : valueCanonical data
      { typeId := uint64TypeId, valueBytes := lhs } = true := by
    simp [valueCanonical, hcanLhs]
  have hvcR : valueCanonical data
  ...
  have hwidth : uintWidth data uint64TypeId = some 64 := by
    simp [uintWidth, shapeOf, htypeU]
  change leBytesToNat rhs ≤ leBytesToNat lhs at hle
  simp [evalBinary, hvcL, hvcR, hwidth, intWidth, shapeOf, htypeU,
    Nat.not_lt.mpr hle, uintByteLen, leBytesToNatV1, natToLeBytesV1]
...
private theorem runMachine_nullary_stateLoad_return
    ...
    let v0 : ReferenceValueV1 :=
      { typeId := uint64TypeId, valueBytes := loadedBytes }
    let mk (env : Array (Option ReferenceValueV1)) (idx : Nat) : MachineV1 := {
  ...
  have hexec0 :
    ...
    have hty : (uint64TypeId != uint64TypeId) = false := by
      simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate, hloaded, hty]
    have hstore :=
...
/-- Closed nullary view body (`literal; return`). Any fuel of the form
    `fuel + 2` ends in `.returned` with the literal payload; overlay and
    response cursor are unchanged. Responses and vault are carried but never
    read by this body. -/
private theorem runMachine_nullary_literal_return
    (data : SemanticProgramDataV1)
    (pre : LogicalStateV1)
    (overlay : Array ByteArray)
    (typeId : TypeIdV1)
    ...
    let v0 : ReferenceValueV1 := { typeId, valueBytes }
    let mk (env : Array (Option ReferenceValueV1)) (idx : Nat) : MachineV1 := {
      data, pre, callable := litCallable, isInitializer := false,
      context, overlay, env,
      effects := #[],
      occCounts := Array.replicate (maxEffectIdInCallable litCallable + 1) 0,
      responseCursor := 0, responses, loopCounts := #[],
      blockId := 0, instrIdx := idx, frames := #[],
      vaultNative, vaultToken
    }
  ...
  have hblocks : litCallable.blocks = #[{
      id := 0
      params := #[]
      instructions := #[{
        result := some { valueId := 0, typeId }
  ...
  have hresult : litCallable.result =
      { typeId, visibility := .public_ } := by rfl
  have hvc : valueCanonical data v0 = true := by
    simp [valueCanonical, v0, hcan]
  have heff : maxEffectIdInCallable litCallable = 0 :=
    maxEffectIdInCallable_eq_zero_of_literal_return_shape litCallable typeId
      valueBytes (by rfl) (by rfl)
  have hs0 : (0 : ValueIdV1).toNat < (emptyEnv 1).size := by
    simp [emptyEnv]
  let e1 := (emptyEnv 1).set 0 (some v0) hs0
  have hset0 : envSet (emptyEnv 1) 0 v0 = some e1 := by
  ...
  have hexec0 :
      execInstruction (mk (emptyEnv 1) 0)
        { result := some { valueId := 0, typeId }
          op := .literal typeId valueBytes } =
        .next (mk e1 0) := by
    have hty : (typeId != typeId) = false := by
      simp [BEq.beq, bne]
    have hcan' : valueCanonical data
        { typeId, valueBytes } = true := hvc
    simp only [mk, execInstruction, hty, ↓reduceIte, hcan', Bool.not_true,
      Bool.false_eq_true]
    exact storeResult_envSet (mk (emptyEnv 1) 0) 0 v0 e1
      (by simpa [mk, v0] using hvc) hset0
  have step0 : runMachine false (fuel + 2) (mk (emptyEnv 1) 0) =
      runMachine false (fuel + 1) (mk e1 1) := by
    have hfu : fuel + 2 = (fuel + 1).succ := by omega
    rw [hfu, runMachine]
  ...
  have hret :
      execTerminator (mk e1 1) (.return_ (some 0)) =
        .done (mk e1 1) (.returned (some v0)) := by
    have hunit : isUnitType data litCallable.result.typeId = false := by
      simp [isUnitType, shapeOf, hresult, htypeB]
    have hbeq : (v0.typeId != litCallable.result.typeId) = false := by
      simp [v0, hresult, BEq.beq, bne]
    have hcond :
        (v0.typeId != litCallable.result.typeId || !valueCanonical data v0) =
          false := by
      simp [hbeq, hvc]
    simp only [mk, execTerminator, hg0]
    simp only [hunit, Bool.false_eq_true, ↓reduceIte, hcond, ↓reduceIte]
...
theorem runMachine_get_finalize_returned
  ...
  have hrun :=
    runMachine_nullary_stateLoad_return data pre #[countBytes] countBytes
      uint64TypeId stateId stateName callableId viewName fuel responses
      vaultNative vaultToken context htypeU hstate (by simp [hstateId]) hcan
...
theorem stepReferenceSliceV1_ready_get_nonempty_responses_traps
  ...
  have hrun :=
    runMachine_nullary_stateLoad_return data pre #[countBytes] countBytes
      ...
      vault.native vault.token context htypeU hstate (by simp [hstateId]) hcan
  rcases hrun with ⟨e1, _hset, hrunEq⟩
  let mGet : MachineV1 := {
    data, pre, callable := getCallable, isInitializer := false,
    context, overlay := #[countBytes], env := emptyEnv 1,
...
/-- A successful nullary UInt64 `stateLoad; return` view leaves the complete
    logical state unchanged, for an overlay of any arity. The overlay and its
    loaded element come only from the production ready gate; arbitrary context,
    responses, vault, result, and effects remain on the sole Reference step. -/
theorem stepReferenceSliceV1_ready_viewLoad_returned_post_eq_pre
    (admitted : AdmittedReferenceSliceV1)
    (pre post : LogicalStateV1)
    (invocation : InvocationV1)
    (data : SemanticProgramDataV1)
  ...
  obtain ⟨_hcandidate, _heffects, hencodePostRaw⟩ :=
    finalize_returned_implies_encodeV1 finalMachine
      (.returned (some loaded)) pre post value effects hstep'
  have hencodePost :
      encodeLogicalStateValuesV1 data pre.initialized overlay = .ok post := by
    simpa [finalMachine, initialMachine] using hencodePostRaw
  have hencodePre :
...
/-- A successful nullary Bool `literal; return` view leaves the complete
    logical state unchanged, for an overlay of any arity. The overlay comes
    only from the production ready gate; arbitrary context, responses, vault,
    result, and effects remain on the sole Reference step. -/
theorem stepReferenceSliceV1_ready_literalReturn_returned_post_eq_pre
    (admitted : AdmittedReferenceSliceV1)
    (pre post : LogicalStateV1)
    (invocation : InvocationV1)
    (data : SemanticProgramDataV1)
  ...
  let litValue : ReferenceValueV1 := { typeId, valueBytes }
  have hdecodeAdmitted :
      decodeLogicalStateValuesV1 admitted.data pre = .ok overlay :=
    gateInvocation_ready_decodeV1 admitted pre invocation litCallable overlay
      context false (by simpa [litCallable] using hgate)
  have hdecode : decodeLogicalStateValuesV1 data pre = .ok overlay := by
    simpa [hadmittedData] using hdecodeAdmitted
  have hnull :=
    stepReferenceSliceV1_ready_nullary_eq admitted pre invocation responses vault
      litCallable overlay context false (by simpa [litCallable] using hgate)
        (by rfl)
  have hmax : maxValueIdInCallable litCallable = 0 :=
    maxValueIdInCallable_eq_zero_of_single_result_zero litCallable typeId
      (.literal typeId valueBytes) (.return_ (some 0)) (by rfl) (by rfl)
  have hmaxEffect : maxEffectIdInCallable litCallable = 0 :=
    maxEffectIdInCallable_eq_zero_of_literal_return_shape litCallable typeId
      valueBytes (by rfl) (by rfl)
  have hrun :=
    runMachine_nullary_literal_return data pre overlay typeId valueBytes
      callableId viewName 999998 responses vault.native vault.token context
      htypeB hcan
  rcases hrun with ⟨finalEnv, _hset, hrunEq⟩
  let initialMachine : MachineV1 := {
    data
    pre
    callable := litCallable
    isInitializer := false
    context
    overlay
    env := emptyEnv 1
  ...
  have hrunExact :
      runMachine false 1000000 initialMachine =
        (0, finalMachine, CandidateV1.returned (some litValue)) := by
    have hfuel : (999998 : Nat) + 2 = 1000000 := by decide
    simpa [hfuel, initialMachine, finalMachine, litCallable, litValue] using
      hrunEq
  have hstepAsFinalize :
      stepReferenceSliceV1 admitted pre invocation responses vault =
        finalize (runMachine false 1000000 initialMachine).2.1
          (runMachine false 1000000 initialMachine).2.2 pre := by
    simpa [initialMachine, hadmittedData, hmax, hmaxEffect, litCallable] using
      hnull
  have hstep' := hstep
  rw [hstepAsFinalize, hrunExact] at hstep'
  obtain ⟨_hcandidate, _heffects, hencodePostRaw⟩ :=
    finalize_returned_implies_encodeV1 finalMachine
      (.returned (some litValue)) pre post value effects hstep'
  have hencodePost :
      encodeLogicalStateValuesV1 data pre.initialized overlay = .ok post := by
    simpa [finalMachine, initialMachine] using hencodePostRaw
  have hencodePre :
...
/-- Execute an exact straight-line two-state binary invariant body. The proof
    runs the sole production invariant machine; `hbinary` comes directly from
    production `evalBinary` and does not interpret an expression independently. -/
private theorem runInvariantCallableV1_binary_two_state_bytes
    (data : SemanticProgramDataV1)
    (state : LogicalStateV1)
    (overlay : Array ByteArray)
    (leftBytes rightBytes : ByteArray)
    ...
    (hbitBool : bit = 0 ∨ bit = 1)
    (hbinary : evalBinary data op
      { typeId := valueTypeId, valueBytes := leftBytes }
      { typeId := valueTypeId, valueBytes := rightBytes }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 bit }) :
  ...
  have hrootMax : maxValueIdInCallable root = 2 :=
    maxValueIdInCallable_eq_two_of_three_results root
      ...
      (.binary op 0 1) (.return_ (some 2)) (by rfl) (by rfl)
  have hrootSteps : root.invariantSteps = some 5 := by rfl
  have hrootKind : root.kind = .invariant := by rfl
  have hrootParams : root.params = #[] := by rfl
  have hrootLoops : root.loopBounds = #[] := by rfl
  ...
  have hexec2 :
    ...
    have hg1 : envGet e2 1 = some v1 := by
      have h1 : e2[(1 : ValueIdV1).toNat]? = some (some v1) :=
        Array.getElem?_set_self hs1
      simp [envGet, e2, h1, UInt32.toNat]
    have heq' : evalBinary data op v0 v1 boolTypeId = .ok v2 := by
      simpa [v0, v1, v2] using hbinary
  ...
  have hfalseBytes : (encodeU8 0 == encodeU8 0) = true := by decide
  rcases hbitBool with rfl | rfl <;> simp [htyEq, htrueBytes,
    hfalseTrueBytes, hfalseBytes, v2]
...
theorem runInvariantCallableV1_returnedTrue_iff_two_state_bytes_eq
  ...
  have hbinary := evalBinary_eq_same_type_bytes data valueTypeId boolTypeId
    leftBytes rightBytes bit htypeB hbit
  have hrun :=
    runInvariantCallableV1_binary_two_state_bytes data state overlay leftBytes
      ...
      rightStateName .eq bit hinitialized hdecode htypeB hleftState hrightState
      hleftOverlay hrightOverlay hroot hcanLeft hcanRight hcanBit
      (by simp [bit]) hbinary
  rw [hrun]
  simp [bit, byteArray_beq_eq_true_iff]

private theorem evalBinary_ne_same_type_bytes
    (data : SemanticProgramDataV1)
    (valueTypeId boolTypeId : TypeIdV1)
    (leftBytes rightBytes : ByteArray)
    (bit : UInt8)
    (htypeB : data.types[boolTypeId.toNat]? = some {
      id := boolTypeId, name := none, shape := .bool })
    (hbit : bit = if leftBytes == rightBytes then 0 else 1) :
    evalBinary data .ne
      { typeId := valueTypeId, valueBytes := leftBytes }
      { typeId := valueTypeId, valueBytes := rightBytes }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 bit } := by
  have htid : (valueTypeId == valueTypeId) = true := by simp [BEq.beq]
  have hbool : isBoolType data boolTypeId = true := by
    simp [isBoolType, shapeOf, htypeB]
  simp only [evalBinary, htid, hbool, Bool.and_self, Bool.not_true,
    Bool.false_eq_true, ↓reduceIte, bytesEqual]
  rw [hbit]
  split <;> rfl

/-- The exact lowered two-state inequality invariant returns true precisely
    when the production-decoded canonical field payloads differ. -/
theorem runInvariantCallableV1_returnedTrue_iff_two_state_bytes_ne
    (data : SemanticProgramDataV1) (state : LogicalStateV1)
    (overlay : Array ByteArray) (leftBytes rightBytes : ByteArray)
    (rootId : CallableIdV1) (valueTypeId boolTypeId : TypeIdV1)
    (leftStateId rightStateId : StateIdV1) (rootName : Option String)
    (rootVisibility leftVisibility rightVisibility : VisibilityV1)
    (leftStateName rightStateName : String)
    (hinitialized : state.initialized = true)
    (hdecode : decodeLogicalStateValuesV1 data state = .ok overlay)
    (htypeB : data.types[boolTypeId.toNat]? = some { id := boolTypeId, name := none, shape := .bool })
    (hleftState : data.logicalState[leftStateId.toNat]? = some { id := leftStateId, name := leftStateName, typeId := valueTypeId, visibility := leftVisibility })
    (hrightState : data.logicalState[rightStateId.toNat]? = some { id := rightStateId, name := rightStateName, typeId := valueTypeId, visibility := rightVisibility })
    (hleftOverlay : overlay[leftStateId.toNat]? = some leftBytes)
    (hrightOverlay : overlay[rightStateId.toNat]? = some rightBytes)
    (hroot : data.callables[rootId.toNat]? = some {
      id := rootId
      ...
      blocks := #[{
        ...
        params := #[]
        instructions := #[
          { result := some { valueId := 0, typeId := valueTypeId }, op := .stateLoad leftStateId },
          { result := some { valueId := 1, typeId := valueTypeId }, op := .stateLoad rightStateId },
          { result := some { valueId := 2, typeId := boolTypeId }, op := .binary .ne 0 1 }]
        terminator := .return_ (some 2)
      }]
      loopBounds := #[]
      invariantSteps := some 5
    }) :
    runInvariantCallableV1 data rootId state = .returnedTrue ↔ leftBytes ≠ rightBytes := by
  have hcanLeft := validateValueBytesV1_of_decodeLogicalStateValuesV1_getElem data state overlay hdecode
    leftStateId.toNat { id := leftStateId, name := leftStateName, typeId := valueTypeId, visibility := leftVisibility }
    leftBytes hleftState hleftOverlay
  have hcanRight := validateValueBytesV1_of_decodeLogicalStateValuesV1_getElem data state overlay hdecode
    rightStateId.toNat { id := rightStateId, name := rightStateName, typeId := valueTypeId, visibility := rightVisibility }
    rightBytes hrightState hrightOverlay
  let bit : UInt8 := if leftBytes == rightBytes then 0 else 1
  have hbit : bit = if leftBytes == rightBytes then 0 else 1 := rfl
  have hcanBit : validateValueBytesV1 data.types boolTypeId (encodeU8 bit) = .ok () := by
    by_cases heqBytes : leftBytes == rightBytes
    · rw [show bit = 0 by simp [bit, heqBytes]]
      exact validateValueBytesV1_bool_eq_ok data.types boolTypeId
        { id := boolTypeId, name := none, shape := .bool } 0 htypeB rfl (Or.inl rfl)
    · rw [show bit = 1 by simp [bit, heqBytes]]
      exact validateValueBytesV1_bool_eq_ok data.types boolTypeId
        { id := boolTypeId, name := none, shape := .bool } 1 htypeB rfl (Or.inr rfl)
  have hbinary := evalBinary_ne_same_type_bytes data valueTypeId boolTypeId leftBytes rightBytes bit htypeB hbit
  have hrun := runInvariantCallableV1_binary_two_state_bytes data state overlay leftBytes rightBytes
    rootId valueTypeId boolTypeId leftStateId rightStateId rootName rootVisibility leftVisibility
    rightVisibility leftStateName rightStateName .ne bit hinitialized hdecode htypeB hleftState
    hrightState hleftOverlay hrightOverlay hroot hcanLeft hcanRight hcanBit
    (by simp [bit]) hbinary
  rw [hrun]
  simp [bit, byteArray_beq_eq_true_iff]
...
/-! ### Two-slot zero initializer packaging

    This exact lifecycle body writes canonical UInt64 zero to slots 0 and 1,
    then returns Unit. It runs only through the sole production machine with
    `isInitializer = true`; the pre-initialization carrier remains a logical
    lifecycle state rather than an initialized business state.
-/

private theorem runMachine_initializer_store_zero_two_return_unit
    (data : SemanticProgramDataV1)
    (pre : LogicalStateV1)
    ...
    (vaultToken : Array (ByteArray × UInt64) := #[])
    (context : Array ContextInputV1 := #[])
    (_htypeU : data.types[uint64TypeId.toNat]? = some {
      id := uint64TypeId, name := none, shape := .uint 64 })
    ...
    (hcanZero :
      validateValueBytesV1 data.types uint64TypeId zero8BytesV1 = .ok ()) :
    let initializerCallable : CallableV1 := {
      id := callableId
      kind := .initializer
      name := none
      params := #[]
    ...
    ∃ finalEnv,
      runMachine false (fuel + 5) {
        data
        pre
        callable := initializerCallable
        isInitializer := true
        context
        ...
        effects := #[]
        occCounts :=
          Array.replicate (maxEffectIdInCallable initializerCallable + 1) 0
        responseCursor := 0
        responses
        loopCounts := #[]
        blockId := 0
        ...
        (0, {
          ...
          isInitializer := true
          context
          overlay := #[zero8BytesV1, zero8BytesV1]
          env := finalEnv
          effects := #[]
          occCounts :=
            Array.replicate (maxEffectIdInCallable initializerCallable + 1) 0
  ...
  let zeroValue : ReferenceValueV1 := {
    typeId := uint64TypeId
    valueBytes := zero8BytesV1
  }
  have hcanonical : valueCanonical data zeroValue = true := by
    simp [zeroValue, valueCanonical, hcanZero]
  have hcanonicalLiteral :
      valueCanonical data {
        typeId := uint64TypeId
        valueBytes := zero8BytesV1
      } = true := by
    simpa [zeroValue] using hcanonical
  have hblocks : initializerCallable.blocks = #[{
      id := 0
      params := #[]
      instructions := #[
        { result := some { valueId := 0, typeId := uint64TypeId },
  ...
  have hresult : initializerCallable.result.typeId = unitTypeId := by rfl
  have henv0 : (0 : ValueIdV1).toNat < (emptyEnv 2).size := by
    simp [emptyEnv, UInt32.toNat]
  let env1 := (emptyEnv 2).set 0 (some zeroValue) henv0
  have hset0 : envSet (emptyEnv 2) 0 zeroValue = some env1 := by
    simpa [env1] using
      envSet_of_lt (emptyEnv 2) (0 : ValueIdV1) zeroValue henv0
  have henv1 : (1 : ValueIdV1).toNat < env1.size := by
    simp [env1, emptyEnv, UInt32.toNat]
  let env2 := env1.set 1 (some zeroValue) henv1
  have hset1 : envSet env1 1 zeroValue = some env2 := by
    simpa [env2] using envSet_of_lt env1 (1 : ValueIdV1) zeroValue henv1
  have hoverlay0 :
      (0 : Nat) < (#[before0, before1] : Array ByteArray).size := by simp
  let overlay1 :=
    (#[before0, before1] : Array ByteArray).set 0 zero8BytesV1 hoverlay0
  have hoverlay1 : (1 : Nat) < overlay1.size := by simp [overlay1]
  let overlay2 := overlay1.set 1 zero8BytesV1 hoverlay1
  have hoverlay2 : overlay2 = #[zero8BytesV1, zero8BytesV1] := by
    ...
    | index + 2 =>
        ...
        have hright :
            (#[zero8BytesV1, zero8BytesV1] : Array ByteArray)[index + 2]? =
              none := by
          apply Array.getElem?_eq_none
          simp
  ...
  let mk (env : Array (Option ReferenceValueV1))
    ...
    env
    effects := #[]
    occCounts := Array.replicate (maxEffectIdInCallable initializerCallable + 1) 0
    responseCursor := 0
    responses
    loopCounts := #[]
    blockId := 0
  ...
  have hexecute0 :
      execInstruction (mk (emptyEnv 2) #[before0, before1] 0)
          { result := some { valueId := 0, typeId := uint64TypeId },
            op := .literal uint64TypeId zero8BytesV1 } =
        .next (mk env1 #[before0, before1] 0) := by
    have htype : (uint64TypeId != uint64TypeId) = false := by
      simp [BEq.beq, bne]
    simp only [mk, execInstruction, htype, ↓reduceIte, hcanonicalLiteral,
      Bool.not_true, Bool.false_eq_true]
    exact storeResult_envSet (mk (emptyEnv 2) #[before0, before1] 0) 0
      zeroValue env1 hcanonical hset0
  have step0 :
      runMachine false (fuel + 5) (mk (emptyEnv 2) #[before0, before1] 0) =
        runMachine false (fuel + 4) (mk env1 #[before0, before1] 1) := by
    have hfuel : fuel + 5 = (fuel + 4).succ := by omega
    rw [hfuel, runMachine]
    simp [mk, hblocks, hexecute0]
  have hget0 : envGet env1 0 = some zeroValue := by
    have hvalue : env1[(0 : ValueIdV1).toNat]? = some (some zeroValue) :=
      Array.getElem?_set_self henv0
    simp [envGet, env1, hvalue, UInt32.toNat]
  have hexecute1 :
      execInstruction (mk env1 #[before0, before1] 1)
          { result := none, op := .stateStore 0 0 } =
        .next (mk env1 overlay1 1) := by
    have hstate0' : data.logicalState[(0 : StateIdV1).toNat]? = some {
        id := 0, name := state0Name, typeId := uint64TypeId,
        visibility := .public_ } := by
      simpa using hstate0
    have htype : (zeroValue.typeId != uint64TypeId) = false := by
      simp [zeroValue, BEq.beq, bne]
    simp only [mk, execInstruction, hstate0', hget0, htype, hcanonical,
    ...
    simp [overlay1, zeroValue, UInt32.toNat]
  have step1 :
      runMachine false (fuel + 4) (mk env1 #[before0, before1] 1) =
        runMachine false (fuel + 3) (mk env1 overlay1 2) := by
    have hfuel : fuel + 4 = (fuel + 3).succ := by omega
    rw [hfuel, runMachine]
    simp [mk, hblocks, hexecute1]
  have hexecute2 :
      execInstruction (mk env1 overlay1 2)
          { result := some { valueId := 1, typeId := uint64TypeId },
            op := .literal uint64TypeId zero8BytesV1 } =
        .next (mk env2 overlay1 2) := by
    have htype : (uint64TypeId != uint64TypeId) = false := by
      simp [BEq.beq, bne]
    simp only [mk, execInstruction, htype, ↓reduceIte, hcanonicalLiteral,
      Bool.not_true, Bool.false_eq_true]
    exact storeResult_envSet (mk env1 overlay1 2) 1 zeroValue env2
      hcanonical hset1
  have step2 :
      runMachine false (fuel + 3) (mk env1 overlay1 2) =
        runMachine false (fuel + 2) (mk env2 overlay1 3) := by
    have hfuel : fuel + 3 = (fuel + 2).succ := by omega
    rw [hfuel, runMachine]
    simp [mk, hblocks, hexecute2]
  have hget1 : envGet env2 1 = some zeroValue := by
    have hvalue : env2[(1 : ValueIdV1).toNat]? = some (some zeroValue) :=
      Array.getElem?_set_self henv1
    simp [envGet, env2, hvalue, UInt32.toNat]
  have hexecute3 :
      execInstruction (mk env2 overlay1 3)
          { result := none, op := .stateStore 1 1 } =
        .next (mk env2 overlay2 3) := by
    ...
    have htype : (zeroValue.typeId != uint64TypeId) = false := by
      simp [zeroValue, BEq.beq, bne]
    simp only [mk, execInstruction, hstate1', hget1, htype, hcanonical,
      Bool.not_true, Bool.false_eq_true, ↓reduceIte]
    have hbound : (1 : StateIdV1).toNat < overlay1.size := by
      simp [overlay1, UInt32.toNat]
    simp only [hbound, ↓reduceDIte]
    simp [overlay2, zeroValue, UInt32.toNat]
  have step3 :
      runMachine false (fuel + 2) (mk env2 overlay1 3) =
        runMachine false (fuel + 1) (mk env2 overlay2 4) := by
    have hfuel : fuel + 2 = (fuel + 1).succ := by omega
    rw [hfuel, runMachine]
    simp [mk, hblocks, hexecute3]
  have hreturn :
      execTerminator (mk env2 overlay2 4) (.return_ none) =
        .done (mk env2 overlay2 4) (.returned none) := by
    have hunit : isUnitType data initializerCallable.result.typeId = true := by
      simp [isUnitType, shapeOf, hresult, htypeUnit]
    simp only [mk, execTerminator, hunit, ↓reduceIte]
  have stepReturn :
      runMachine false (fuel + 1) (mk env2 overlay2 4) =
        (0, mk env2 overlay2 4, CandidateV1.returned none) := by
  ...
  have hrun :
      runMachine false (fuel + 5) (mk (emptyEnv 2) #[before0, before1] 0) =
        (0, mk env2 overlay2 4, CandidateV1.returned none) := by
    rw [step0, step1, step2, step3, stepReturn]
  simpa [hoverlay2, mk] using hrun

/-- Invert a successful Reference step for the exact nullary two-slot zeroing
    initializer. The conclusion is the production initialized-state encoding;
    callers do not supply or reconstruct an expected post carrier. -/
theorem stepReferenceSliceV1_ready_initializer_store_zero_two_returned_post_encode
    (admitted : AdmittedReferenceSliceV1)
    (pre post : LogicalStateV1)
    (invocation : InvocationV1)
    (data : SemanticProgramDataV1)
  ...
  have hnull :=
    stepReferenceSliceV1_ready_nullary_eq admitted pre invocation responses vault
      initializerCallable #[before0, before1] context true
        (by simpa [initializerCallable] using hgate) (by rfl)
  have hmax : maxValueIdInCallable initializerCallable = 1 := by
    simp [maxValueIdInCallable, initializerCallable]
  have hmaxEffect : maxEffectIdInCallable initializerCallable = 0 := by
    simp [maxEffectIdInCallable, initializerCallable]
  have hrun :=
    runMachine_initializer_store_zero_two_return_unit data pre before0 before1
      uint64TypeId unitTypeId state0Name state1Name callableId 999995 responses
      vault.native vault.token context htypeU htypeUnit hstate0 hstate1 hcanZero
  ...
  have hrunExact :
      runMachine false 1000000 initialMachine =
        (0, finalMachine, CandidateV1.returned none) := by
    have hfuel : (999995 : Nat) + 5 = 1000000 := by decide
    simpa [hfuel, initialMachine, finalMachine, initializerCallable] using
      hrunEq
  have hstepAsFinalize :
      stepReferenceSliceV1 admitted pre invocation responses vault =
        finalize (runMachine false 1000000 initialMachine).2.1
          (runMachine false 1000000 initialMachine).2.2 pre := by
    simpa [initialMachine, hadmittedData, hmax, hmaxEffect,
      initializerCallable] using hnull
  have hstep' := hstep
  rw [hstepAsFinalize, hrunExact] at hstep'
  obtain ⟨_hcandidate, _heffects, hencodePost⟩ :=
    finalize_returned_implies_encodeV1 finalMachine (.returned none) pre post
      value effects hstep'
  simpa [finalMachine, hadmittedData] using hencodePost
...
/-- Successful private trace for the exact unary two-slot guarded checked-sub
    body. Both guards have already evaluated to canonical Bool true; execution
    remains entirely in the sole Reference machine. -/
private theorem runMachine_guarded_sub_parameter_two_unit
    (data : SemanticProgramDataV1) (pre : LogicalStateV1)
    (before0 before1 argumentBytes difference0 difference1 : ByteArray)
    (uint64TypeId boolTypeId unitTypeId : TypeIdV1)
    (state0Name state1Name parameterName : String)
    (callableId : CallableIdV1) (entryName : Option String) (fuel : Nat)
    (responses : ExternalResponsesV1 := #[]) (vaultNative : UInt64 := 0)
    (vaultToken : Array (ByteArray × UInt64) := #[])
    ...
    (hstate1 : data.logicalState[1]? = some {
      id := 1, name := state1Name, typeId := uint64TypeId,
      visibility := .public_ })
    (hcanBefore0 : validateValueBytesV1 data.types uint64TypeId before0 = .ok ())
    (hcanBefore1 : validateValueBytesV1 data.types uint64TypeId before1 = .ok ())
    (hcanArgument : validateValueBytesV1 data.types uint64TypeId argumentBytes = .ok ())
    (hcanTrue : validateValueBytesV1 data.types boolTypeId (encodeU8 1) = .ok ())
    (hcanDifference0 :
      validateValueBytesV1 data.types uint64TypeId difference0 = .ok ())
    (hcanDifference1 :
      validateValueBytesV1 data.types uint64TypeId difference1 = .ok ())
    (hle0 : evalBinary data .le
      { typeId := uint64TypeId, valueBytes := argumentBytes }
      { typeId := uint64TypeId, valueBytes := before0 }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 1 })
    (hle1 : evalBinary data .le
      { typeId := uint64TypeId, valueBytes := argumentBytes }
      { typeId := uint64TypeId, valueBytes := before1 }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 1 })
    (hsub0 : evalBinary data .sub
      { typeId := uint64TypeId, valueBytes := before0 }
      { typeId := uint64TypeId, valueBytes := argumentBytes }
      uint64TypeId = .ok { typeId := uint64TypeId, valueBytes := difference0 })
    (hsub1 : evalBinary data .sub
      { typeId := uint64TypeId, valueBytes := before1 }
      { typeId := uint64TypeId, valueBytes := argumentBytes }
      uint64TypeId = .ok { typeId := uint64TypeId, valueBytes := difference1 }) :
    let guardedSubCallable : CallableV1 := {
      ...
      blocks := #[{
        ...
        { result := some { valueId := 5, typeId := uint64TypeId }, op := .stateLoad 0 },
        { result := some { valueId := 6, typeId := uint64TypeId }, op := .binary .sub 5 0 },
        { result := none, op := .stateStore 0 6 },
        { result := some { valueId := 7, typeId := uint64TypeId }, op := .stateLoad 1 },
        { result := some { valueId := 8, typeId := uint64TypeId }, op := .binary .sub 7 0 },
        { result := none, op := .stateStore 1 8 }]
        terminator := .return_ none }]
      loopBounds := #[], invariantSteps := none }
    let argument : ReferenceValueV1 :=
      { typeId := uint64TypeId, valueBytes := argumentBytes }
    let hslot0 : (0 : ValueIdV1).toNat < (emptyEnv 9).size := by
      simp [emptyEnv, UInt32.toNat]
    let boundEnv := (emptyEnv 9).set 0 (some argument) hslot0
    ∃ finalEnv,
      runMachine false (fuel + 13) {
        data, pre, callable := guardedSubCallable, isInitializer := false, context
        overlay := #[before0, before1], env := boundEnv, effects := #[]
        occCounts := Array.replicate (maxEffectIdInCallable guardedSubCallable + 1) 0
        responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := 0
        frames := #[], vaultNative, vaultToken } =
      (0, {
        data, pre, callable := guardedSubCallable, isInitializer := false, context
        overlay := #[difference0, difference1], env := finalEnv, effects := #[]
        occCounts := Array.replicate (maxEffectIdInCallable guardedSubCallable + 1) 0
        responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := 12
        frames := #[], vaultNative, vaultToken }, CandidateV1.returned none) := by
  let guardedSubCallable : CallableV1 := {
    id := callableId, kind := .entry, name := entryName
    params := #[{
      valueId := 0
  ...
  let argument : ReferenceValueV1 :=
    { typeId := uint64TypeId, valueBytes := argumentBytes }
  let beforeValue0 : ReferenceValueV1 :=
    { typeId := uint64TypeId, valueBytes := before0 }
  let beforeValue1 : ReferenceValueV1 :=
    { typeId := uint64TypeId, valueBytes := before1 }
  let trueValue : ReferenceValueV1 :=
    { typeId := boolTypeId, valueBytes := encodeU8 1 }
  let differenceValue0 : ReferenceValueV1 :=
    { typeId := uint64TypeId, valueBytes := difference0 }
  let differenceValue1 : ReferenceValueV1 :=
    { typeId := uint64TypeId, valueBytes := difference1 }
  let hslot0 : (0 : ValueIdV1).toNat < (emptyEnv 9).size := by
    simp [emptyEnv, UInt32.toNat]
  let boundEnv := (emptyEnv 9).set 0 (some argument) hslot0
  have hblocks : guardedSubCallable.blocks = #[{
      id := 0, params := #[], instructions := #[
      { result := some { valueId := 1, typeId := uint64TypeId }, op := .stateLoad 0 },
      { result := some { valueId := 2, typeId := boolTypeId }, op := .binary .le 0 1 },
      { result := none, op := .assert_ 2 none #[] },
      ...
      { result := some { valueId := 8, typeId := uint64TypeId }, op := .binary .sub 7 0 },
      { result := none, op := .stateStore 1 8 }]
      terminator := .return_ none }] := by rfl
  have hresult : guardedSubCallable.result.typeId = unitTypeId := by rfl
  have hvcArgument : valueCanonical data argument = true := by
    simp [valueCanonical, argument, hcanArgument]
  have hvcBefore0 : valueCanonical data beforeValue0 = true := by
    simp [valueCanonical, beforeValue0, hcanBefore0]
  have hvcBefore1 : valueCanonical data beforeValue1 = true := by
    simp [valueCanonical, beforeValue1, hcanBefore1]
  have hvcTrue : valueCanonical data trueValue = true := by
    simp [valueCanonical, trueValue, hcanTrue]
  have hvcDifference0 : valueCanonical data differenceValue0 = true := by
    simp [valueCanonical, differenceValue0, hcanDifference0]
  have hvcDifference1 : valueCanonical data differenceValue1 = true := by
    simp [valueCanonical, differenceValue1, hcanDifference1]
  have hle0' : evalBinary data .le argument beforeValue0 boolTypeId = .ok trueValue := by
    simpa [argument, beforeValue0, trueValue] using hle0
  have hle1' : evalBinary data .le argument beforeValue1 boolTypeId = .ok trueValue := by
    simpa [argument, beforeValue1, trueValue] using hle1
  have hsub0' : evalBinary data .sub beforeValue0 argument uint64TypeId =
      .ok differenceValue0 := by
    simpa [beforeValue0, argument, differenceValue0] using hsub0
  have hsub1' : evalBinary data .sub beforeValue1 argument uint64TypeId =
      .ok differenceValue1 := by
    simpa [beforeValue1, argument, differenceValue1] using hsub1
  have hbool : isBoolType data boolTypeId = true := by
    simp [isBoolType, shapeOf, htypeB]
  have hvcTrueConcrete : valueCanonical data {
      typeId := boolTypeId
      valueBytes := ByteArray.empty.push 1
    } = true := by
    simpa [trueValue, encodeU8] using hvcTrue
  have htrueByte : (ByteArray.empty.push (1 : UInt8)).get! 0 = 1 := by decide
  have hget0 : envGet boundEnv 0 = some argument := by
    simp [envGet, boundEnv, hslot0, UInt32.toNat]
  have hs1 : (1 : ValueIdV1).toNat < boundEnv.size := by
    simp [boundEnv, emptyEnv, UInt32.toNat]
  let e1 := boundEnv.set 1 (some beforeValue0) hs1
  have hset1 : envSet boundEnv 1 beforeValue0 = some e1 := by
    simpa [e1] using envSet_of_lt boundEnv 1 beforeValue0 hs1
  have hs2 : (2 : ValueIdV1).toNat < e1.size := by
    simp [e1, boundEnv, emptyEnv, UInt32.toNat]
  let e2 := e1.set 2 (some trueValue) hs2
  have hset2 : envSet e1 2 trueValue = some e2 := by
    simpa [e2] using envSet_of_lt e1 2 trueValue hs2
  have hs3 : (3 : ValueIdV1).toNat < e2.size := by
    simp [e2, e1, boundEnv, emptyEnv, UInt32.toNat]
  let e3 := e2.set 3 (some beforeValue1) hs3
  have hset3 : envSet e2 3 beforeValue1 = some e3 := by
    simpa [e3] using envSet_of_lt e2 3 beforeValue1 hs3
  have hs4 : (4 : ValueIdV1).toNat < e3.size := by
    simp [e3, e2, e1, boundEnv, emptyEnv, UInt32.toNat]
  let e4 := e3.set 4 (some trueValue) hs4
  have hset4 : envSet e3 4 trueValue = some e4 := by
    simpa [e4] using envSet_of_lt e3 4 trueValue hs4
  have hs5 : (5 : ValueIdV1).toNat < e4.size := by
    simp [e4, e3, e2, e1, boundEnv, emptyEnv, UInt32.toNat]
  let e5 := e4.set 5 (some beforeValue0) hs5
  have hset5 : envSet e4 5 beforeValue0 = some e5 := by
    simpa [e5] using envSet_of_lt e4 5 beforeValue0 hs5
  have hs6 : (6 : ValueIdV1).toNat < e5.size := by
    simp [e5, e4, e3, e2, e1, boundEnv, emptyEnv, UInt32.toNat]
  let e6 := e5.set 6 (some differenceValue0) hs6
  have hset6 : envSet e5 6 differenceValue0 = some e6 := by
    simpa [e6] using envSet_of_lt e5 6 differenceValue0 hs6
  have hs7 : (7 : ValueIdV1).toNat < e6.size := by
    simp [e6, e5, e4, e3, e2, e1, boundEnv, emptyEnv, UInt32.toNat]
  let e7 := e6.set 7 (some beforeValue1) hs7
  have hset7 : envSet e6 7 beforeValue1 = some e7 := by
    simpa [e7] using envSet_of_lt e6 7 beforeValue1 hs7
  have hs8 : (8 : ValueIdV1).toNat < e7.size := by
    simp [e7, e6, e5, e4, e3, e2, e1, boundEnv, emptyEnv, UInt32.toNat]
  let e8 := e7.set 8 (some differenceValue1) hs8
  have hset8 : envSet e7 8 differenceValue1 = some e8 := by
    simpa [e8] using envSet_of_lt e7 8 differenceValue1 hs8
  have hov0 : (0 : Nat) < (#[before0, before1] : Array ByteArray).size := by simp
  let overlay1 := (#[before0, before1] : Array ByteArray).set 0 difference0 hov0
  have hov1 : (1 : Nat) < overlay1.size := by simp [overlay1]
  let overlay2 := overlay1.set 1 difference1 hov1
  have overlay2_eq : overlay2 = #[difference0, difference1] := by
    refine Array.ext_getElem? (fun index => ?_)
    match index with
    | 0 =>
        have hne : (1 : Nat) ≠ 0 := by decide
    ...
    | index + 2 =>
        have hleft : overlay2[index + 2]? = none := by
          apply Array.getElem?_eq_none
          simp [overlay2, overlay1]
        have hright : (#[difference0, difference1] : Array ByteArray)[index + 2]? = none := by
          apply Array.getElem?_eq_none
          simp
        exact hleft.trans hright.symm
  let mk (env : Array (Option ReferenceValueV1)) (overlay : Array ByteArray)
  ...
  have hstate1' : data.logicalState[(1 : StateIdV1).toNat]? = some {
      id := 1, name := state1Name, typeId := uint64TypeId,
      visibility := .public_ } := by simpa using hstate1
  have hexec0 : execInstruction (mk boundEnv #[before0, before1] 0)
      { result := some { valueId := 1, typeId := uint64TypeId }, op := .stateLoad 0 } =
      .next (mk e1 #[before0, before1] 0) := by
    have hloaded : (#[before0, before1] : Array ByteArray)[(0 : StateIdV1).toNat]? =
        some before0 := by simp
    have hty : (uint64TypeId != uint64TypeId) = false := by simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate0', hloaded, hty]
    simpa [mk, e1, beforeValue0] using storeResult_envSet
      (mk boundEnv #[before0, before1] 0) 1 beforeValue0 e1 hvcBefore0 hset1
  have hget1 : envGet e1 1 = some beforeValue0 := by simp [envGet, e1, UInt32.toNat]
  have hget0e1 : envGet e1 0 = some argument := by
    simp [envGet, e1, boundEnv, hslot0, UInt32.toNat]
  have hexec1 : execInstruction (mk e1 #[before0, before1] 1)
      { result := some { valueId := 2, typeId := boolTypeId }, op := .binary .le 0 1 } =
      .next (mk e2 #[before0, before1] 1) := by
    simp only [mk, execInstruction, hget0e1, hget1, hvcArgument, hvcBefore0,
      Bool.not_true, Bool.or_false, Bool.false_eq_true, ↓reduceIte, hle0', fromEval]
    simpa [mk, e2, trueValue] using storeResult_envSet
      (mk e1 #[before0, before1] 1) 2 trueValue e2 hvcTrue hset2
  have hget2 : envGet e2 2 = some trueValue := by simp [envGet, e2, UInt32.toNat]
  have hexec2 : execInstruction (mk e2 #[before0, before1] 2)
      { result := none, op := .assert_ 2 none #[] } =
      .next (mk e2 #[before0, before1] 2) := by
    simp only [mk, execInstruction, hget2, hbool, hvcTrueConcrete, Bool.not_true,
      Bool.false_eq_true, ↓reduceIte]
    simp [trueValue, byteAt?, encodeU8, htrueByte]
    exact ⟨hbool, hvcTrueConcrete⟩
  have hexec3 : execInstruction (mk e2 #[before0, before1] 3)
      { result := some { valueId := 3, typeId := uint64TypeId }, op := .stateLoad 1 } =
      .next (mk e3 #[before0, before1] 3) := by
    have hloaded : (#[before0, before1] : Array ByteArray)[(1 : StateIdV1).toNat]? =
        some before1 := by simp
    have hty : (uint64TypeId != uint64TypeId) = false := by simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate1', hloaded, hty]
    simpa [mk, e3, beforeValue1] using storeResult_envSet
      (mk e2 #[before0, before1] 3) 3 beforeValue1 e3 hvcBefore1 hset3
  have hget3 : envGet e3 3 = some beforeValue1 := by simp [envGet, e3, UInt32.toNat]
  have hget0e3 : envGet e3 0 = some argument := by
    simp [envGet, e3, e2, e1, boundEnv, hslot0, UInt32.toNat]
  have hexec4 : execInstruction (mk e3 #[before0, before1] 4)
      { result := some { valueId := 4, typeId := boolTypeId }, op := .binary .le 0 3 } =
      .next (mk e4 #[before0, before1] 4) := by
    simp only [mk, execInstruction, hget0e3, hget3, hvcArgument, hvcBefore1,
      Bool.not_true, Bool.or_false, Bool.false_eq_true, ↓reduceIte, hle1', fromEval]
    simpa [mk, e4, trueValue] using storeResult_envSet
      (mk e3 #[before0, before1] 4) 4 trueValue e4 hvcTrue hset4
  have hget4 : envGet e4 4 = some trueValue := by simp [envGet, e4, UInt32.toNat]
  have hexec5 : execInstruction (mk e4 #[before0, before1] 5)
      { result := none, op := .assert_ 4 none #[] } =
      .next (mk e4 #[before0, before1] 5) := by
    simp only [mk, execInstruction, hget4, hbool, hvcTrueConcrete, Bool.not_true,
      Bool.false_eq_true, ↓reduceIte]
    simp [trueValue, byteAt?, encodeU8, htrueByte]
    exact ⟨hbool, hvcTrueConcrete⟩
  have hexec6 : execInstruction (mk e4 #[before0, before1] 6)
      { result := some { valueId := 5, typeId := uint64TypeId }, op := .stateLoad 0 } =
      .next (mk e5 #[before0, before1] 6) := by
    have hloaded : (#[before0, before1] : Array ByteArray)[(0 : StateIdV1).toNat]? =
        some before0 := by simp
    have hty : (uint64TypeId != uint64TypeId) = false := by simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate0', hloaded, hty]
    simpa [mk, e5, beforeValue0] using storeResult_envSet
      (mk e4 #[before0, before1] 6) 5 beforeValue0 e5 hvcBefore0 hset5
  have hget5 : envGet e5 5 = some beforeValue0 := by simp [envGet, e5, UInt32.toNat]
  have hget0e5 : envGet e5 0 = some argument := by
    simp [envGet, e5, e4, e3, e2, e1, boundEnv, hslot0, UInt32.toNat]
  have hexec7 : execInstruction (mk e5 #[before0, before1] 7)
      { result := some { valueId := 6, typeId := uint64TypeId }, op := .binary .sub 5 0 } =
      .next (mk e6 #[before0, before1] 7) := by
    simp only [mk, execInstruction, hget5, hget0e5, hvcBefore0, hvcArgument,
      Bool.not_true, Bool.or_false, Bool.false_eq_true, ↓reduceIte, hsub0', fromEval]
    simpa [mk, e6, differenceValue0] using storeResult_envSet
      (mk e5 #[before0, before1] 7) 6 differenceValue0 e6 hvcDifference0 hset6
  have hget6 : envGet e6 6 = some differenceValue0 := by simp [envGet, e6, UInt32.toNat]
  have hexec8 : execInstruction (mk e6 #[before0, before1] 8)
      { result := none, op := .stateStore 0 6 } = .next (mk e6 overlay1 8) := by
    have hty : (differenceValue0.typeId != uint64TypeId) = false := by
      simp [differenceValue0, BEq.beq, bne]
    simp only [mk, execInstruction, hstate0', hget6, hty, hvcDifference0,
      Bool.not_true, Bool.false_eq_true, ↓reduceIte, hov0, ↓reduceDIte]
    simp [mk, overlay1, differenceValue0, UInt32.toNat]
  have hloaded1 : overlay1[(1 : StateIdV1).toNat]? = some before1 := by
    simp [overlay1, UInt32.toNat]
  have hexec9 : execInstruction (mk e6 overlay1 9)
      { result := some { valueId := 7, typeId := uint64TypeId }, op := .stateLoad 1 } =
      .next (mk e7 overlay1 9) := by
    have hty : (uint64TypeId != uint64TypeId) = false := by simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate1', hloaded1, hty]
    simpa [mk, e7, beforeValue1] using storeResult_envSet
      (mk e6 overlay1 9) 7 beforeValue1 e7 hvcBefore1 hset7
  have hget7 : envGet e7 7 = some beforeValue1 := by simp [envGet, e7, UInt32.toNat]
  have hget0e7 : envGet e7 0 = some argument := by
    simp [envGet, e7, e6, e5, e4, e3, e2, e1, boundEnv, hslot0, UInt32.toNat]
  have hexec10 : execInstruction (mk e7 overlay1 10)
      { result := some { valueId := 8, typeId := uint64TypeId }, op := .binary .sub 7 0 } =
      .next (mk e8 overlay1 10) := by
    simp only [mk, execInstruction, hget7, hget0e7, hvcBefore1, hvcArgument,
      Bool.not_true, Bool.or_false, Bool.false_eq_true, ↓reduceIte, hsub1', fromEval]
    simpa [mk, e8, differenceValue1] using storeResult_envSet
      (mk e7 overlay1 10) 8 differenceValue1 e8 hvcDifference1 hset8
  have hget8 : envGet e8 8 = some differenceValue1 := by simp [envGet, e8, UInt32.toNat]
  have hexec11 : execInstruction (mk e8 overlay1 11)
      { result := none, op := .stateStore 1 8 } = .next (mk e8 overlay2 11) := by
    have hty : (differenceValue1.typeId != uint64TypeId) = false := by
      simp [differenceValue1, BEq.beq, bne]
    have hbound : (1 : StateIdV1).toNat < overlay1.size := by
      simp [overlay1, UInt32.toNat]
    simp only [mk, execInstruction, hstate1', hget8, hty, hvcDifference1,
      Bool.not_true, Bool.false_eq_true, ↓reduceIte, hbound, ↓reduceDIte]
    simp [mk, overlay2, differenceValue1, UInt32.toNat]
  have hreturn : execTerminator (mk e8 overlay2 12) (.return_ none) =
      .done (mk e8 overlay2 12) (.returned none) := by
    have hunit : isUnitType data guardedSubCallable.result.typeId = true := by
      simp [isUnitType, shapeOf, hresult, htypeUnit]
    simp only [mk, execTerminator, hunit, ↓reduceIte]
  have step0 : runMachine false (fuel + 13) (mk boundEnv #[before0, before1] 0) =
      runMachine false (fuel + 12) (mk e1 #[before0, before1] 1) := by
    rw [show fuel + 13 = (fuel + 12).succ by omega, runMachine]; simp [mk, hblocks, hexec0]
  have step1 : runMachine false (fuel + 12) (mk e1 #[before0, before1] 1) =
      runMachine false (fuel + 11) (mk e2 #[before0, before1] 2) := by
    rw [show fuel + 12 = (fuel + 11).succ by omega, runMachine]; simp [mk, hblocks, hexec1]
  have step2 : runMachine false (fuel + 11) (mk e2 #[before0, before1] 2) =
      runMachine false (fuel + 10) (mk e2 #[before0, before1] 3) := by
    rw [show fuel + 11 = (fuel + 10).succ by omega, runMachine]; simp [mk, hblocks, hexec2]
  have step3 : runMachine false (fuel + 10) (mk e2 #[before0, before1] 3) =
      runMachine false (fuel + 9) (mk e3 #[before0, before1] 4) := by
    rw [show fuel + 10 = (fuel + 9).succ by omega, runMachine]; simp [mk, hblocks, hexec3]
  have step4 : runMachine false (fuel + 9) (mk e3 #[before0, before1] 4) =
      runMachine false (fuel + 8) (mk e4 #[before0, before1] 5) := by
    rw [show fuel + 9 = (fuel + 8).succ by omega, runMachine]; simp [mk, hblocks, hexec4]
  have step5 : runMachine false (fuel + 8) (mk e4 #[before0, before1] 5) =
      runMachine false (fuel + 7) (mk e4 #[before0, before1] 6) := by
    rw [show fuel + 8 = (fuel + 7).succ by omega, runMachine]; simp [mk, hblocks, hexec5]
  have step6 : runMachine false (fuel + 7) (mk e4 #[before0, before1] 6) =
      runMachine false (fuel + 6) (mk e5 #[before0, before1] 7) := by
    rw [show fuel + 7 = (fuel + 6).succ by omega, runMachine]; simp [mk, hblocks, hexec6]
  have step7 : runMachine false (fuel + 6) (mk e5 #[before0, before1] 7) =
      runMachine false (fuel + 5) (mk e6 #[before0, before1] 8) := by
    rw [show fuel + 6 = (fuel + 5).succ by omega, runMachine]; simp [mk, hblocks, hexec7]
  have step8 : runMachine false (fuel + 5) (mk e6 #[before0, before1] 8) =
      runMachine false (fuel + 4) (mk e6 overlay1 9) := by
    rw [show fuel + 5 = (fuel + 4).succ by omega, runMachine]; simp [mk, hblocks, hexec8]
  have step9 : runMachine false (fuel + 4) (mk e6 overlay1 9) =
      runMachine false (fuel + 3) (mk e7 overlay1 10) := by
    rw [show fuel + 4 = (fuel + 3).succ by omega, runMachine]; simp [mk, hblocks, hexec9]
  have step10 : runMachine false (fuel + 3) (mk e7 overlay1 10) =
      runMachine false (fuel + 2) (mk e8 overlay1 11) := by
    rw [show fuel + 3 = (fuel + 2).succ by omega, runMachine]; simp [mk, hblocks, hexec10]
  have step11 : runMachine false (fuel + 2) (mk e8 overlay1 11) =
      runMachine false (fuel + 1) (mk e8 overlay2 12) := by
    rw [show fuel + 2 = (fuel + 1).succ by omega, runMachine]; simp [mk, hblocks, hexec11]
  have step12 : runMachine false (fuel + 1) (mk e8 overlay2 12) =
      (0, mk e8 overlay2 12, CandidateV1.returned none) := by
    rw [show fuel + 1 = fuel.succ by omega, runMachine]; simp [mk, hblocks, hreturn]
  refine ⟨e8, ?_⟩
  rw [step0, step1, step2, step3, step4, step5, step6, step7, step8, step9,
    step10, step11, step12]
  simp [mk, overlay2_eq, guardedSubCallable]

/-- The first false guard in the exact guarded checked-sub body reverts before
    either state slot is written. -/
private theorem runMachine_guarded_sub_parameter_two_first_assertion_failure
    (data : SemanticProgramDataV1) (pre : LogicalStateV1)
    (before0 before1 argumentBytes : ByteArray)
    (uint64TypeId boolTypeId unitTypeId : TypeIdV1)
    (state0Name parameterName : String)
    (callableId : CallableIdV1) (entryName : Option String) (fuel : Nat)
    (responses : ExternalResponsesV1 := #[]) (vaultNative : UInt64 := 0)
    (vaultToken : Array (ByteArray × UInt64) := #[])
    (context : Array ContextInputV1 := #[])
    ...
    (hcanBefore0 : validateValueBytesV1 data.types uint64TypeId before0 = .ok ())
    (hcanArgument : validateValueBytesV1 data.types uint64TypeId argumentBytes = .ok ())
    (hcanFalse : validateValueBytesV1 data.types boolTypeId (encodeU8 0) = .ok ())
    (hleFalse : evalBinary data .le
      { typeId := uint64TypeId, valueBytes := argumentBytes }
      { typeId := uint64TypeId, valueBytes := before0 }
      boolTypeId = .ok { typeId := boolTypeId, valueBytes := encodeU8 0 }) :
    let guardedSubCallable : CallableV1 := {
      id := callableId, kind := .entry, name := entryName
      params := #[{
        valueId := 0
    ...
    ∃ finalEnv,
      ...
      (0, {
        data, pre, callable := guardedSubCallable, isInitializer := false, context
        overlay := #[before0, before1], env := finalEnv, effects := #[]
        occCounts := Array.replicate (maxEffectIdInCallable guardedSubCallable + 1) 0
        responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := 2
        frames := #[], vaultNative, vaultToken },
        CandidateV1.reverted (.standard .assertionFailed)) := by
  let guardedSubCallable : CallableV1 := {
    id := callableId, kind := .entry, name := entryName
    params := #[{
      valueId := 0
  ...
  let beforeValue0 : ReferenceValueV1 :=
    { typeId := uint64TypeId, valueBytes := before0 }
  let falseValue : ReferenceValueV1 :=
    { typeId := boolTypeId, valueBytes := encodeU8 0 }
  let hslot0 : (0 : ValueIdV1).toNat < (emptyEnv 9).size := by
    simp [emptyEnv, UInt32.toNat]
  let boundEnv := (emptyEnv 9).set 0 (some argument) hslot0
  ...
  have hvcBefore0 : valueCanonical data beforeValue0 = true := by
    simp [valueCanonical, beforeValue0, hcanBefore0]
  have hvcFalse : valueCanonical data falseValue = true := by
    simp [valueCanonical, falseValue, hcanFalse]
  have hleFalse' : evalBinary data .le argument beforeValue0 boolTypeId =
      .ok falseValue := by
    simpa [argument, beforeValue0, falseValue] using hleFalse
  have hbool : isBoolType data boolTypeId = true := by
    simp [isBoolType, shapeOf, htypeB]
  have hvcFalseConcrete : valueCanonical data {
      typeId := boolTypeId
      valueBytes := ByteArray.empty.push 0
    } = true := by
    simpa [falseValue, encodeU8] using hvcFalse
  have hfalseByte : (ByteArray.empty.push (0 : UInt8)).get! 0 = 0 := by decide
  have hs1 : (1 : ValueIdV1).toNat < boundEnv.size := by
    simp [boundEnv, emptyEnv, UInt32.toNat]
  let e1 := boundEnv.set 1 (some beforeValue0) hs1
  have hset1 : envSet boundEnv 1 beforeValue0 = some e1 := by
  ...
  have hs2 : (2 : ValueIdV1).toNat < e1.size := by
    simp [e1, boundEnv, emptyEnv, UInt32.toNat]
  let e2 := e1.set 2 (some falseValue) hs2
  have hset2 : envSet e1 2 falseValue = some e2 := by
    simpa [e2] using envSet_of_lt e1 2 falseValue hs2
  let mk (env : Array (Option ReferenceValueV1)) (idx : Nat) : MachineV1 := {
    data, pre, callable := guardedSubCallable, isInitializer := false, context
    overlay := #[before0, before1], env, effects := #[]
    occCounts := Array.replicate (maxEffectIdInCallable guardedSubCallable + 1) 0
    responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := idx
    frames := #[], vaultNative, vaultToken }
  have hstate0' : data.logicalState[(0 : StateIdV1).toNat]? = some {
  ...
  have hexec0 : execInstruction (mk boundEnv 0)
    ...
    have hty : (uint64TypeId != uint64TypeId) = false := by simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate0', hloaded, hty]
    simpa [mk, e1, beforeValue0] using
      storeResult_envSet (mk boundEnv 0) 1 beforeValue0 e1 hvcBefore0 hset1
  have hget1 : envGet e1 1 = some beforeValue0 := by simp [envGet, e1, UInt32.toNat]
  have hget0 : envGet e1 0 = some argument := by
    simp [envGet, e1, boundEnv, hslot0, UInt32.toNat]
  have hexec1 : execInstruction (mk e1 1)
      { result := some { valueId := 2, typeId := boolTypeId }, op := .binary .le 0 1 } =
      .next (mk e2 1) := by
    simp only [mk, execInstruction, hget0, hget1, hvcArgument, hvcBefore0,
      Bool.not_true, Bool.or_false, Bool.false_eq_true, ↓reduceIte, hleFalse', fromEval]
    simpa [mk, e2, falseValue] using
      storeResult_envSet (mk e1 1) 2 falseValue e2 hvcFalse hset2
  have hget2 : envGet e2 2 = some falseValue := by simp [envGet, e2, UInt32.toNat]
  have hexec2 : execInstruction (mk e2 2)
      { result := none, op := .assert_ 2 none #[] } =
      .done (mk e2 2) (.reverted (.standard .assertionFailed)) := by
    simp only [mk, execInstruction, hget2, hbool, hvcFalseConcrete,
      Bool.not_true, Bool.false_eq_true, ↓reduceIte]
    simp [falseValue, byteAt?, encodeU8, hfalseByte]
    exact ⟨hbool, hvcFalseConcrete⟩
  have step0 : runMachine false (fuel + 3) (mk boundEnv 0) =
      runMachine false (fuel + 2) (mk e1 1) := by
    rw [show fuel + 3 = (fuel + 2).succ by omega, runMachine]; simp [mk, hblocks, hexec0]
  have step1 : runMachine false (fuel + 2) (mk e1 1) =
      runMachine false (fuel + 1) (mk e2 2) := by
    rw [show fuel + 2 = (fuel + 1).succ by omega, runMachine]; simp [mk, hblocks, hexec1]
  have step2 : runMachine false (fuel + 1) (mk e2 2) =
      (0, mk e2 2, CandidateV1.reverted (.standard .assertionFailed)) := by
    rw [show fuel + 1 = fuel.succ by omega, runMachine]; simp [mk, hblocks, hexec2]
  refine ⟨e2, ?_⟩
  rw [step0, step1, step2]
...
/-- Invert a successful sole Reference step for the exact unary guarded
    ...
    return; a true guard computes the unique checked-sub post encoding. -/
theorem stepReferenceSliceV1_ready_guarded_sub_parameter_two_unit_returned_post_encode
    (admitted : AdmittedReferenceSliceV1) (pre post : LogicalStateV1)
    (data : SemanticProgramDataV1) (beforeBytes argumentBytes : ByteArray)
    (uint64TypeId boolTypeId unitTypeId : TypeIdV1)
    (state0Name state1Name parameterName : String)
  ...
  let invocation : InvocationV1 := {
    callableId, args := #[argument], context := invocationContext }
  let hslot0 : (0 : ValueIdV1).toNat < (emptyEnv 9).size := by
    simp [emptyEnv, UInt32.toNat]
  let boundEnv := (emptyEnv 9).set 0 (some argument) hslot0
  let initialMachine : MachineV1 := {
    ...
    responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := 0
    frames := #[], vaultNative := vault.native, vaultToken := vault.token }
  have hmax : maxValueIdInCallable guardedSubCallable = 8 := by
    simp [maxValueIdInCallable, guardedSubCallable]
  have hmaxEffect : maxEffectIdInCallable guardedSubCallable = 0 := by
    simp [maxEffectIdInCallable, guardedSubCallable]
  have hgate' : gateInvocation admitted pre invocation =
      .ready guardedSubCallable #[beforeBytes, beforeBytes] context false := by
    simpa [invocation, guardedSubCallable, argument] using hgate
  have hstepAsFinalize : stepReferenceSliceV1 admitted pre invocation responses vault =
      finalize (runMachine false 1000000 initialMachine).2.1
        (runMachine false 1000000 initialMachine).2.2 pre := by
    unfold stepReferenceSliceV1
    rw [hgate']
    simp [initialMachine, guardedSubCallable, argument, invocation, boundEnv, hslot0,
      hadmittedData, hmax, hmaxEffect, emptyEnv, envSet]
  have hdecodeAdmitted := (gateInvocation_ready_noninit_decode admitted pre invocation
    guardedSubCallable #[beforeBytes, beforeBytes] context hgate').1
  have hdecode : decodeLogicalStateValuesV1 data pre =
      .ok #[beforeBytes, beforeBytes] := by
    simpa [hadmittedData] using hdecodeAdmitted
  have hcanBefore : validateValueBytesV1 data.types uint64TypeId beforeBytes = .ok () := by
    exact validateValueBytesV1_of_decodeLogicalStateValuesV1_getElem data pre
      #[beforeBytes, beforeBytes] hdecode 0
      { id := 0, name := state0Name, typeId := uint64TypeId,
        visibility := .public_ }
      beforeBytes hstate0 (by simp)
  have hcanTrue : validateValueBytesV1 data.types boolTypeId (encodeU8 1) = .ok () := by
    exact validateValueBytesV1_bool_eq_ok data.types boolTypeId
      { id := boolTypeId, name := none, shape := .bool } 1 htypeB rfl
      (Or.inr rfl)
  have hcanFalse : validateValueBytesV1 data.types boolTypeId (encodeU8 0) = .ok () := by
    exact validateValueBytesV1_bool_eq_ok data.types boolTypeId
      { id := boolTypeId, name := none, shape := .bool } 0 htypeB rfl
      (Or.inl rfl)
  by_cases hguard : leBytesToNatV1 argumentBytes ≤ leBytesToNatV1 beforeBytes
  · let differenceBytes := natToLeBytesV1
      (leBytesToNatV1 beforeBytes - leBytesToNatV1 argumentBytes) 8
    have hle := evalBinary_le_uint64_success data uint64TypeId boolTypeId
      argumentBytes beforeBytes htypeU htypeB hcanonical hcanBefore hguard
    have hsub := evalBinary_sub_uint64_success data uint64TypeId beforeBytes
      argumentBytes htypeU hcanBefore hcanonical hguard
    have hcanDifference :
        validateValueBytesV1 data.types uint64TypeId differenceBytes = .ok () := by
      have hsize : differenceBytes.size = 8 := by
        simp [differenceBytes, natToLeBytesV1_size]
      exact validateValueBytesV1_uint64_of_size data.types uint64TypeId
        { id := uint64TypeId, name := none, shape := .uint 64 }
        differenceBytes htypeU rfl hsize
    obtain ⟨finalEnv, hrun⟩ := runMachine_guarded_sub_parameter_two_unit
      data pre beforeBytes beforeBytes argumentBytes differenceBytes differenceBytes
      uint64TypeId boolTypeId unitTypeId state0Name state1Name parameterName
      callableId entryName 999987 responses vault.native vault.token context
      htypeU htypeB htypeUnit hstate0 hstate1 hcanBefore hcanBefore hcanonical
      hcanTrue hcanDifference hcanDifference hle hle (by
        simpa [differenceBytes] using hsub) (by
        simpa [differenceBytes] using hsub)
    let finalMachine : MachineV1 := {
      data, pre, callable := guardedSubCallable, isInitializer := false, context
      overlay := #[differenceBytes, differenceBytes], env := finalEnv, effects := #[]
      occCounts := Array.replicate (maxEffectIdInCallable guardedSubCallable + 1) 0
      responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := 12
      frames := #[], vaultNative := vault.native, vaultToken := vault.token }
    have hrunExact : runMachine false 1000000 initialMachine =
        (0, finalMachine, CandidateV1.returned none) := by
      simpa [initialMachine, finalMachine, guardedSubCallable, argument, boundEnv,
        hslot0] using hrun
    have hstep' : stepReferenceSliceV1 admitted pre invocation responses vault =
        .returned post value effects := by
  ...
  · have hleFalse := evalBinary_le_uint64_failure data uint64TypeId boolTypeId
    ...
    obtain ⟨failureEnv, hrun⟩ :=
      runMachine_guarded_sub_parameter_two_first_assertion_failure data pre
        beforeBytes beforeBytes argumentBytes uint64TypeId boolTypeId unitTypeId
        state0Name parameterName callableId entryName 999997 responses vault.native
        vault.token context htypeB hstate0 hcanBefore hcanonical hcanFalse hleFalse
    let failureMachine : MachineV1 := {
      data, pre, callable := guardedSubCallable, isInitializer := false, context
      overlay := #[beforeBytes, beforeBytes], env := failureEnv, effects := #[]
      occCounts := Array.replicate (maxEffectIdInCallable guardedSubCallable + 1) 0
      responseCursor := 0, responses, loopCounts := #[], blockId := 0, instrIdx := 2
      frames := #[], vaultNative := vault.native, vaultToken := vault.token }
    have hrunExact : runMachine false 1000000 initialMachine =
        (0, failureMachine, CandidateV1.reverted (.standard .assertionFailed)) := by
      simpa [initialMachine, failureMachine, guardedSubCallable, argument, boundEnv,
        hslot0] using hrun
    have hstep' : stepReferenceSliceV1 admitted pre invocation responses vault =
        .returned post value effects := by
      simpa [invocation, argument] using hstep
    rw [hstepAsFinalize, hrunExact] at hstep'
    have hc := (finalize_returned_implies_encodeV1 failureMachine
      (.reverted (.standard .assertionFailed)) pre post value effects hstep').1
    cases hc
...
/-! ### Single-state unary StateCell execution

    These exact traces close the retained one-field UInt64 shapes used by the
    Solana StateCell carrier. They execute the sole Reference machine; they do
    not define a target-owned business transition. -/

/-- Execute the exact unary initializer body `stateStore 0 parameter; return`.
    The invocation binder has already produced the one-slot environment. -/
private theorem runMachine_initializer_store_parameter_one_return_unit
    (data : SemanticProgramDataV1)
    (pre : LogicalStateV1)
    (beforeBytes argumentBytes : ByteArray)
    (uint64TypeId unitTypeId : TypeIdV1)
    (stateName parameterName : String)
    (callableId : CallableIdV1)
    (fuel : Nat)
    (responses : ExternalResponsesV1 := #[])
    (vaultNative : UInt64 := 0)
    ...
    (htypeUnit : data.types[unitTypeId.toNat]? = some {
      id := unitTypeId, name := none, shape := .unit })
    (hstate : data.logicalState[0]? = some {
      id := 0, name := stateName, typeId := uint64TypeId,
      visibility := .public_ })
    (hcanonical :
      validateValueBytesV1 data.types uint64TypeId argumentBytes = .ok ()) :
    let initializerCallable : CallableV1 := {
    ...
    let boundEnv : Array (Option ReferenceValueV1) := #[some argument]
    runMachine false (fuel + 2) {
      data
      pre
      callable := initializerCallable
  ...
  let boundEnv : Array (Option ReferenceValueV1) := #[some argument]
  let mk (overlay : Array ByteArray) (instructionIndex : Nat) : MachineV1 := {
    data
    pre
    callable := initializerCallable
    isInitializer := true
  ...
  have htypeMatch : (argument.typeId != uint64TypeId) = false := by
    simp [argument, BEq.beq, bne]
  have hvalueCanonical : valueCanonical data argument = true := by
    simp [valueCanonical, argument, hcanonical]
  have hoverlay : (0 : Nat) < (#[beforeBytes] : Array ByteArray).size := by simp
  have hstate' : data.logicalState[(0 : StateIdV1).toNat]? = some {
      id := 0, name := stateName, typeId := uint64TypeId,
      visibility := .public_ } := by
    simpa using hstate
  have hexecute :
      execInstruction (mk #[beforeBytes] 0) {
          result := none
          op := .stateStore 0 0
        } = .next (mk #[argumentBytes] 0) := by
    simp only [mk, execInstruction, hstate', hgetArgument, htypeMatch,
      hvalueCanonical, Bool.not_true, Bool.false_eq_true, ↓reduceIte,
      hoverlay, ↓reduceDIte]
    simp [mk, argument, UInt32.toNat]
  have stepStore :
      runMachine false (fuel + 2) (mk #[beforeBytes] 0) =
        runMachine false (fuel + 1) (mk #[argumentBytes] 1) := by
    rw [show fuel + 2 = (fuel + 1).succ by omega, runMachine]
    simp [mk, hblocks, hexecute]
  have hreturn :
      execTerminator (mk #[argumentBytes] 1) (.return_ none) =
        .done (mk #[argumentBytes] 1) (.returned none) := by
    have hunit : isUnitType data initializerCallable.result.typeId = true := by
      simp [isUnitType, shapeOf, hresult, htypeUnit]
    simp only [mk, execTerminator, hunit, ↓reduceIte]
  have stepReturn :
      runMachine false (fuel + 1) (mk #[argumentBytes] 1) =
        (0, mk #[argumentBytes] 1, CandidateV1.returned none) := by
    rw [show fuel + 1 = fuel.succ by omega, runMachine]
    simp [mk, hblocks, hreturn]
  change runMachine false (fuel + 2) (mk #[beforeBytes] 0) =
    (0, mk #[argumentBytes] 1, CandidateV1.returned none)
  exact stepStore.trans stepReturn

/-- Exact successful Reference execution for the retained single-state unary
    initializer. The post-state is produced by the production logical-state
    encoder with the initializer lifecycle bit set. -/
theorem stepReferenceSliceV1_ready_initializer_store_parameter_one_returned
    (admitted : AdmittedReferenceSliceV1)
    (pre post : LogicalStateV1)
    (data : SemanticProgramDataV1)
    (beforeBytes argumentBytes : ByteArray)
    ...
    (hcanonical :
      validateValueBytesV1 data.types uint64TypeId argumentBytes = .ok ())
    (hencode :
      encodeLogicalStateValuesV1 data true #[argumentBytes] = .ok post)
    (hgate :
      gateInvocation admitted pre {
          callableId
          args := #[{ typeId := uint64TypeId, valueBytes := argumentBytes }]
  ...
  have hrun :=
    runMachine_initializer_store_parameter_one_return_unit data pre beforeBytes
      argumentBytes uint64TypeId unitTypeId stateName parameterName callableId
      999998 #[] vault.native vault.token context htypeUnit hstate hcanonical
  have hrunExact :
      runMachine false 1000000 initialMachine =
        (0, finalMachine, CandidateV1.returned none) := by
    have hfuel : (999998 : Nat) + 2 = 1000000 := by decide
    simpa [hfuel, initialMachine, finalMachine, initializerCallable, argument,
      boundEnv] using hrun
  have hmax : maxValueIdInCallable initializerCallable = 0 := by
    simp [maxValueIdInCallable, initializerCallable]
  have hmaxEffect : maxEffectIdInCallable initializerCallable = 0 := by
    simp [maxEffectIdInCallable, initializerCallable]
  have hgate' : gateInvocation admitted pre invocation =
      .ready initializerCallable #[beforeBytes] context true := by
    simpa [invocation, initializerCallable, argument] using hgate
  have hstepAsFinalize :
    ...
    unfold stepReferenceSliceV1
    rw [hgate']
    simp [initialMachine, initializerCallable, invocation, argument, boundEnv,
      hadmittedData, hmax, hmaxEffect, emptyEnv, envSet]
  have hfinalize :
      finalize finalMachine (.returned none) pre =
        .returned post none #[] := by
    apply finalize_returned_of_encode finalMachine pre none post
    · rfl
    · rfl
    · simpa [finalMachine, hadmittedData] using hencode
  change stepReferenceSliceV1 admitted pre invocation #[] vault =
    .returned post none #[]
  rw [hstepAsFinalize, hrunExact]
  exact hfinalize

/-- Execute the exact single-state unary checked-add body through the sole
    Reference machine. -/
private theorem runMachine_add_parameter_one_return
    (data : SemanticProgramDataV1)
    (pre : LogicalStateV1)
    (beforeBytes argumentBytes sumBytes : ByteArray)
    (uint64TypeId : TypeIdV1)
    ...
    (hstate : data.logicalState[0]? = some {
      ...
      visibility := .public_ })
    (hcanBefore :
      validateValueBytesV1 data.types uint64TypeId beforeBytes = .ok ())
    (hcanArgument :
      validateValueBytesV1 data.types uint64TypeId argumentBytes = .ok ())
    (hadd : evalBinary data .add
      { typeId := uint64TypeId, valueBytes := beforeBytes }
    ...
    let argument : ReferenceValueV1 := {
      typeId := uint64TypeId, valueBytes := argumentBytes }
    let hslot0 : (0 : ValueIdV1).toNat < (emptyEnv 4).size := by
      simp [emptyEnv, UInt32.toNat]
    let boundEnv := (emptyEnv 4).set 0 (some argument) hslot0
    ∃ finalEnv,
      runMachine false (fuel + 5) {
        data
        pre
  ...
  have hs1 : (1 : ValueIdV1).toNat < boundEnv.size := by
    simp [boundEnv, emptyEnv, UInt32.toNat]
  let env1 := boundEnv.set 1 (some beforeValue) hs1
  have hset1 : envSet boundEnv 1 beforeValue = some env1 := by
    simpa [env1] using envSet_of_lt boundEnv 1 beforeValue hs1
  have hs2 : (2 : ValueIdV1).toNat < env1.size := by
    simp [env1, boundEnv, emptyEnv, UInt32.toNat]
  let env2 := env1.set 2 (some sumValue) hs2
  have hset2 : envSet env1 2 sumValue = some env2 := by
    simpa [env2] using envSet_of_lt env1 2 sumValue hs2
  have hs3 : (3 : ValueIdV1).toNat < env2.size := by
    simp [env2, env1, boundEnv, emptyEnv, UInt32.toNat]
  let env3 := env2.set 3 (some sumValue) hs3
  have hset3 : envSet env2 3 sumValue = some env3 := by
    simpa [env3] using envSet_of_lt env2 3 sumValue hs3
  let mk (env : Array (Option ReferenceValueV1))
      (overlay : Array ByteArray) (instructionIndex : Nat) : MachineV1 := {
    data
    pre
  ...
  have hstate' : data.logicalState[(0 : StateIdV1).toNat]? = some {
    ...
    simpa using hstate
  have hloadedBefore :
      (#[beforeBytes] : Array ByteArray)[(0 : StateIdV1).toNat]? =
        some beforeBytes := by simp
  have hexecute0 :
      execInstruction (mk boundEnv #[beforeBytes] 0) {
          result := some { valueId := 1, typeId := uint64TypeId }
          op := .stateLoad 0
        } = .next (mk env1 #[beforeBytes] 0) := by
    have htypeMatch : (uint64TypeId != uint64TypeId) = false := by
      simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate', hloadedBefore, htypeMatch]
    simpa [mk, env1, beforeValue] using
      storeResult_envSet (mk boundEnv #[beforeBytes] 0) 1 beforeValue env1
        hvcBefore hset1
  have hgetBefore : envGet env1 1 = some beforeValue := by
    simp [envGet, env1, UInt32.toNat]
  have hgetArgument : envGet env1 0 = some argument := by
    simp [envGet, env1, boundEnv, UInt32.toNat]
  have hexecute1 :
      execInstruction (mk env1 #[beforeBytes] 1) {
          result := some { valueId := 2, typeId := uint64TypeId }
          op := .binary .add 1 0
        } = .next (mk env2 #[beforeBytes] 1) := by
    simp only [mk, execInstruction, hgetBefore, hgetArgument, hvcBefore,
      hvcArgument, Bool.not_true, Bool.or_false, Bool.false_eq_true,
      ↓reduceIte, hadd', fromEval]
    simpa [mk, env2, sumValue] using
      storeResult_envSet (mk env1 #[beforeBytes] 1) 2 sumValue env2 hvcSum
        hset2
  have hgetSum : envGet env2 2 = some sumValue := by
    simp [envGet, env2, UInt32.toNat]
  have hexecute2 :
      execInstruction (mk env2 #[beforeBytes] 2) {
          result := none
          op := .stateStore 0 2
        } = .next (mk env2 #[sumBytes] 2) := by
    have htypeMatch : (sumValue.typeId != uint64TypeId) = false := by
      simp [sumValue, BEq.beq, bne]
    have hoverlay :
        (0 : StateIdV1).toNat < (#[beforeBytes] : Array ByteArray).size := by
      simp [UInt32.toNat]
    simp only [mk, execInstruction, hstate', hgetSum, htypeMatch, hvcSum,
      Bool.not_true, Bool.false_eq_true, ↓reduceIte, hoverlay, ↓reduceDIte]
    simp [sumValue, UInt32.toNat]
  have hloadedSum :
      (#[sumBytes] : Array ByteArray)[(0 : StateIdV1).toNat]? =
        some sumBytes := by simp
  have hexecute3 :
      execInstruction (mk env2 #[sumBytes] 3) {
          result := some { valueId := 3, typeId := uint64TypeId }
          op := .stateLoad 0
        } = .next (mk env3 #[sumBytes] 3) := by
    have htypeMatch : (uint64TypeId != uint64TypeId) = false := by
      simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate', hloadedSum, htypeMatch]
    simpa [mk, env3, sumValue] using
      storeResult_envSet (mk env2 #[sumBytes] 3) 3 sumValue env3 hvcSum hset3
  have hgetResult : envGet env3 3 = some sumValue := by
    simp [envGet, env3, UInt32.toNat]
  have hreturn :
      execTerminator (mk env3 #[sumBytes] 4) (.return_ (some 3)) =
        .done (mk env3 #[sumBytes] 4) (.returned (some sumValue)) := by
    have hunit : isUnitType data addCallable.result.typeId = false := by
      simp [isUnitType, shapeOf, hresult, htypeU]
    have hvalid :
        (sumValue.typeId != addCallable.result.typeId ||
          !valueCanonical data sumValue) = false := by
      simp [sumValue, hresult, hvcSum, BEq.beq, bne]
    simp only [mk, execTerminator, hgetResult, hunit, Bool.false_eq_true,
      ↓reduceIte, hvalid]
  have step0 :
      runMachine false (fuel + 5) (mk boundEnv #[beforeBytes] 0) =
        runMachine false (fuel + 4) (mk env1 #[beforeBytes] 1) := by
    rw [show fuel + 5 = (fuel + 4).succ by omega, runMachine]
    simp [mk, hblocks, hexecute0]
  have step1 :
      runMachine false (fuel + 4) (mk env1 #[beforeBytes] 1) =
        runMachine false (fuel + 3) (mk env2 #[beforeBytes] 2) := by
    rw [show fuel + 4 = (fuel + 3).succ by omega, runMachine]
    simp [mk, hblocks, hexecute1]
  have step2 :
      runMachine false (fuel + 3) (mk env2 #[beforeBytes] 2) =
        runMachine false (fuel + 2) (mk env2 #[sumBytes] 3) := by
    rw [show fuel + 3 = (fuel + 2).succ by omega, runMachine]
    simp [mk, hblocks, hexecute2]
  have step3 :
      runMachine false (fuel + 2) (mk env2 #[sumBytes] 3) =
        runMachine false (fuel + 1) (mk env3 #[sumBytes] 4) := by
    rw [show fuel + 2 = (fuel + 1).succ by omega, runMachine]
    simp [mk, hblocks, hexecute3]
  have stepReturn :
      runMachine false (fuel + 1) (mk env3 #[sumBytes] 4) =
        (0, mk env3 #[sumBytes] 4,
          CandidateV1.returned (some sumValue)) := by
    rw [show fuel + 1 = fuel.succ by omega, runMachine]
    simp [mk, hblocks, hreturn]
  refine ⟨env3, ?_⟩
  change runMachine false (fuel + 5) (mk boundEnv #[beforeBytes] 0) =
    (0, mk env3 #[sumBytes] 4, CandidateV1.returned (some sumValue))
  rw [step0, step1, step2, step3, stepReturn]

/-- The load succeeds and an evaluator failure at the checked add halts before
    the single logical state slot is written. -/
private theorem runMachine_add_parameter_one_eval_error
    (data : SemanticProgramDataV1)
    (pre : LogicalStateV1)
    (beforeBytes argumentBytes : ByteArray)
    (uint64TypeId : TypeIdV1)
  ...
  let envAfterLoad := boundEnv.set 1 (some beforeValue) hs1
  have hset1 : envSet boundEnv 1 beforeValue = some envAfterLoad := by
    simpa [envAfterLoad] using envSet_of_lt boundEnv 1 beforeValue hs1
  let mk (env : Array (Option ReferenceValueV1))
      (instructionIndex : Nat) : MachineV1 := {
    data
    pre
    callable := addCallable
    isInitializer := false
  ...
  have hloaded :
      (#[beforeBytes] : Array ByteArray)[(0 : StateIdV1).toNat]? =
        some beforeBytes := by simp
  have hexecute0 :
      execInstruction (mk boundEnv 0) {
          result := some { valueId := 1, typeId := uint64TypeId }
          op := .stateLoad 0
        } = .next (mk envAfterLoad 0) := by
    have htypeMatch : (uint64TypeId != uint64TypeId) = false := by
      simp [BEq.beq, bne]
    simp only [mk, execInstruction, hstate', hloaded, htypeMatch]
    simpa [mk, envAfterLoad, beforeValue] using
      storeResult_envSet (mk boundEnv 0) 1 beforeValue envAfterLoad hvcBefore
        hset1
  have hgetBefore : envGet envAfterLoad 1 = some beforeValue := by
    simp [envGet, envAfterLoad, UInt32.toNat]
  have hgetArgument : envGet envAfterLoad 0 = some argument := by
    simp [envGet, envAfterLoad, boundEnv, UInt32.toNat]
  have haddError' : evalBinary data .add beforeValue argument uint64TypeId =
      .error failure := by
    simpa [beforeValue, argument] using haddError
  have hexecute1 :
      execInstruction (mk envAfterLoad 1) {
          result := some { valueId := 2, typeId := uint64TypeId }
          op := .binary .add 1 0
        } = .done (mk envAfterLoad 1) failure.toCandidateV1 := by
    simp [mk, execInstruction, hgetBefore, hgetArgument, hvcBefore,
      hvcArgument, haddError', fromEval]
  have step0 :
      runMachine false (fuel + 2) (mk boundEnv 0) =
        runMachine false (fuel + 1) (mk envAfterLoad 1) := by
    rw [show fuel + 2 = (fuel + 1).succ by omega, runMachine]
    simp [mk, hblocks, hexecute0]
  have step1 :
      runMachine false (fuel + 1) (mk envAfterLoad 1) =
        (0, mk envAfterLoad 1, failure.toCandidateV1) := by
    rw [show fuel + 1 = fuel.succ by omega, runMachine]
    simp [mk, hblocks, hexecute1]
  refine ⟨envAfterLoad, ?_⟩
  change runMachine false (fuel + 2) (mk boundEnv 0) =
    (0, mk envAfterLoad 1, failure.toCandidateV1)
  exact step0.trans step1

/-- Exact successful Reference execution for the retained single-state unary
    UInt64 checked-add entry. -/
theorem stepReferenceSliceV1_ready_add_parameter_one_returned
    (admitted : AdmittedReferenceSliceV1)
    (pre post : LogicalStateV1)
    (data : SemanticProgramDataV1)
    (beforeBytes argumentBytes : ByteArray)
    ...
    (hcanArgument :
      validateValueBytesV1 data.types uint64TypeId argumentBytes = .ok ())
    (hnoOverflow :
      leBytesToNatV1 beforeBytes + leBytesToNatV1 argumentBytes < 2 ^ 64)
    (hencode :
  ...
  let hslot0 : (0 : ValueIdV1).toNat < (emptyEnv 4).size := by
    simp [emptyEnv, UInt32.toNat]
  let boundEnv := (emptyEnv 4).set 0 (some argument) hslot0
  have haddCharacterization :=
    evalBinary_add_uint64_characterization data uint64TypeId beforeBytes
      argumentBytes htypeU hcanBefore hcanArgument
  have hadd : evalBinary data .add
      { typeId := uint64TypeId, valueBytes := beforeBytes }
      { typeId := uint64TypeId, valueBytes := argumentBytes }
      uint64TypeId = .ok {
    ...
    have hnotGe :
        ¬ 2 ^ 64 ≤
          leBytesToNatV1 beforeBytes + leBytesToNatV1 argumentBytes :=
      Nat.not_le.mpr hnoOverflow
    simpa [hnotGe, sumBytes] using haddCharacterization
  have hcanSum :
      validateValueBytesV1 data.types uint64TypeId sumBytes = .ok () := by
    have hsize : sumBytes.size = 8 := by
      simp [sumBytes, natToLeBytesV1_size]
    exact validateValueBytesV1_uint64_of_size data.types uint64TypeId
      { id := uint64TypeId, name := none, shape := .uint 64 }
      sumBytes htypeU rfl hsize
  obtain ⟨finalEnv, hrun⟩ :=
    runMachine_add_parameter_one_return data pre beforeBytes argumentBytes
      sumBytes uint64TypeId stateName parameterName callableId entryName 999995
      #[] vault.native vault.token context htypeU hstate hcanBefore
      hcanArgument hadd hcanSum
  let initialMachine : MachineV1 := {
    data
    pre
    callable := addCallable
  ...
  have hrunExact : runMachine false 1000000 initialMachine =
      (0, finalMachine, CandidateV1.returned (some {
        typeId := uint64TypeId, valueBytes := sumBytes })) := by
    have hfuel : (999995 : Nat) + 5 = 1000000 := by decide
    simpa [hfuel, initialMachine, finalMachine, addCallable, argument, boundEnv,
      hslot0] using hrun
  have hmax : maxValueIdInCallable addCallable = 3 := by
    simp [maxValueIdInCallable, addCallable]
  have hmaxEffect : maxEffectIdInCallable addCallable = 0 := by
    simp [maxEffectIdInCallable, addCallable]
  have hgate' : gateInvocation admitted pre invocation =
      .ready addCallable #[beforeBytes] context false := by
    simpa [invocation, addCallable, argument] using hgate
  have hstepAsFinalize :
      stepReferenceSliceV1 admitted pre invocation #[] vault =
        finalize (runMachine false 1000000 initialMachine).2.1
    ...
    unfold stepReferenceSliceV1
    rw [hgate']
    simp [initialMachine, addCallable, invocation, argument, boundEnv, hslot0,
      hadmittedData, hmax, hmaxEffect, emptyEnv, envSet]
  have hinitialized :=
    (gateInvocation_ready_noninit_decode admitted pre invocation addCallable
      #[beforeBytes] context hgate').2.1
  have hfinalize :
      finalize finalMachine (.returned (some {
          typeId := uint64TypeId, valueBytes := sumBytes })) pre =
        .returned post (some {
          typeId := uint64TypeId, valueBytes := sumBytes }) #[] := by
    apply finalize_returned_of_encode finalMachine pre (some {
      typeId := uint64TypeId, valueBytes := sumBytes }) post
    · rfl
    · rfl
    · simpa [finalMachine, hadmittedData, hinitialized, sumBytes] using hencode
  change stepReferenceSliceV1 admitted pre invocation #[] vault =
    .returned post (some {
      typeId := uint64TypeId, valueBytes := sumBytes }) #[]
  rw [hstepAsFinalize, hrunExact]
...
/-- Exact overflow result for the same single-state checked-add entry. The
    Reference machine reverts with arithmeticOverflow before the state store,
    and finalization carries the exact pre-state. -/
theorem stepReferenceSliceV1_ready_add_parameter_one_overflow_reverted
    (admitted : AdmittedReferenceSliceV1)
    (pre : LogicalStateV1)
    (data : SemanticProgramDataV1)
    (beforeBytes argumentBytes : ByteArray)
    ...
    stepReferenceSliceV1 admitted pre {
        ...
        .reverted (.standard .arithmeticOverflow) pre := by
  let addCallable : CallableV1 := {
    id := callableId
    kind := .entry
    name := entryName
  ...
  have haddCharacterization :=
    evalBinary_add_uint64_characterization data uint64TypeId beforeBytes
      argumentBytes htypeU hcanBefore hcanArgument
  have hoverflowGe :
      2 ^ 64 ≤ leBytesToNatV1 beforeBytes + leBytesToNatV1 argumentBytes :=
  ...
  have haddError : evalBinary data .add
      ...
      uint64TypeId =
        .error (.reverted (.standard .arithmeticOverflow)) := by
    simpa [hoverflowGe] using haddCharacterization
  obtain ⟨envAfterLoad, hrun⟩ :=
    runMachine_add_parameter_one_eval_error data pre beforeBytes argumentBytes
      uint64TypeId stateName parameterName callableId entryName 999998 #[]
      vault.native vault.token context hstate hcanBefore hcanArgument
      (.reverted (.standard .arithmeticOverflow)) haddError
  let initialMachine : MachineV1 := {
    data
    pre
    callable := addCallable
  ...
  have hrunExact : runMachine false 1000000 initialMachine =
      (0, failureMachine,
        CandidateV1.reverted (.standard .arithmeticOverflow)) := by
    have hfuel : (999998 : Nat) + 2 = 1000000 := by decide
    simpa [hfuel, initialMachine, failureMachine, addCallable, argument,
      boundEnv, hslot0, LocalFailureV1.toCandidateV1] using hrun
  have hmax : maxValueIdInCallable addCallable = 3 := by
    simp [maxValueIdInCallable, addCallable]
  have hmaxEffect : maxEffectIdInCallable addCallable = 0 := by
    simp [maxEffectIdInCallable, addCallable]
  ...
  have hstepAsFinalize :
    ...
    simp [initialMachine, addCallable, invocation, argument, boundEnv, hslot0,
      hadmittedData, hmax, hmaxEffect, emptyEnv, envSet]
  have hfinalize :
      finalize failureMachine
          (CandidateV1.reverted (.standard .arithmeticOverflow)) pre =
        .reverted (.standard .arithmeticOverflow) pre := by
    rfl
  change stepReferenceSliceV1 admitted pre invocation #[] vault =
