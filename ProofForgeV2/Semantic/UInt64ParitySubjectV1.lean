...
/-- Turn generated body-byte equality plus the production root gates into the
    sole structure-gated root encode.  This is a bridge from the elaborator's
    transparent `subjectBytesV1` body equality to `encodeSemanticProgramDataV1`;
    it does not introduce a second encoder. -/
theorem encode_ok_of_bodyEncode_gatesV1
    (data : SemanticProgramDataV1) (bytes : ByteArray)
    (_hshape : ShapeMatchV1 data)
    ...
    (hbody : bodyEncodeOkV1 data bytes) :
...
/-- Root validation bridge for a generated parity-family subject with no
    caller-supplied transport decode premise.  Decode is derived by the
    production root codec composition from the structure-gated root encode and
    a `RootFieldInvertV1` package. -/
theorem validate_ok_of_encode_invertV1
    (data : SemanticProgramDataV1) (bytes : ByteArray)
    (_hshape : ShapeMatchV1 data)
  ...
  exact validate_of_subjectData_invert data bytes hencode hinvert

/-- Same no-decode validation bridge specialized to the generated carrier
    wrapper. -/
theorem validate_subjectProgram_ok_of_encode_invertV1
    (data : SemanticProgramDataV1) (bytes : ByteArray)
    (hshape : ShapeMatchV1 data)
    (hencode : encodeSemanticProgramDataV1 data = .ok bytes)
    (hinvert : RootFieldInvertV1 data) :
    validateSemanticProgramV1 (programOfEncodeV1 data bytes hencode) = .ok data := by
  exact validate_ok_of_encode_invertV1 data bytes hshape hencode hinvert

/-- End-to-end generated-subject validation bridge from body-byte equality and
    root-field invertibility.  The only byte premise is the generated equality
    against `encodeSemanticProgramDataBodyV1`; root encode and transport decode
    are derived through production codec lemmas. -/
theorem validate_ok_of_bodyEncode_invertV1
    (data : SemanticProgramDataV1) (bytes : ByteArray)
    (hshape : ShapeMatchV1 data)
    (hnameShape : validateProgramQualifiedNameShapeV1 data.qualifiedName = .ok ())
    (htypesSize : checkTableSize data.types.size = .ok ())
    ...
    (hinvert : RootFieldInvertV1 data) :
    validateSemanticProgramV1 ⟨bytes⟩ = .ok data := by
  have hencode := encode_ok_of_bodyEncode_gatesV1 data bytes hshape hnameShape
