...
namespace ProofForgeV2.Semantic.InitializerDepositViewEqualityPreservationV1
...
theorem preservationTheorem_of_subjectBodyV1
    ...
    (hsubject : subjectData =
      ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.subjectDataV1
        qualifiedName state0Name state1Name depositName parameterName viewName invariantName)
  ...
  have legal : ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.StructureLegalV1
      qualifiedName state0Name state1Name depositName parameterName viewName invariantName :=
    ⟨hnameShape, hstate0Name, hstate1Name, hdepositName, hparameterName,
      hviewName, hinvariantName, hstate01, hdepositView, hdepositInvariant, hviewInvariant⟩
  have hstructure : validateSemanticProgramStructureV1 data = .ok () := by
    simpa [data] using
      ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.structureV1
        qualifiedName state0Name state1Name depositName parameterName viewName invariantName legal
  have hinvert : RootFieldInvertV1 data := by
    simpa [data] using
      ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.rootFieldInvertV1
        qualifiedName state0Name state1Name depositName parameterName viewName invariantName
        hstate0Name hstate1Name hdepositName hparameterName hviewName hinvariantName
  ...
  have hadmission : validateReferenceProgramDataAdmissionV1 data = .ok () := by
    simpa [data] using
      ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.referenceAdmissionV1
        qualifiedName state0Name state1Name depositName parameterName viewName invariantName
  ...
  have hbaseWithInitializer :
    ...
    | ready callable overlay context isInitializer =>
        ...
        have hindexBound : invocation.callableId.toNat < 4 := by
  ...
  have hreturned : PreservationReturnedCallablesV1 program 0 admitted := by
    ...
    | ⟨1, _⟩ =>
      ...
      have hoverlay : overlay.size = 2 := by
        ...
        simpa [data, ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.subjectDataV1] using hs
    ...
    | ⟨2, _⟩ =>
      ...
      have hisInitializer : isInitializer = false := by
        ...
        exact hkind'
      have hdecode := gateInvocation_ready_decodeV1 admitted pre invocation data.callables[2]
        overlay context isInitializer hgate
      rw [hadmittedData] at hdecode
      have hoverlayNonempty : 0 < overlay.size := by
        have hs := decodeLogicalStateValuesV1_size data pre overlay hdecode
        have : overlay.size = 2 := by simpa [data, ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.subjectDataV1] using hs
        omega
    ...
    | ⟨n + 4, hlt⟩ =>
      have : n + 4 < 4 := by
        simpa [data, ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.subjectDataV1,
          ProofForgeV2.Semantic.InitializerDepositViewEqualitySubjectV1.callablesV1] using hlt
      omega
  exact preservationTheoremV1_of_callableObligationsV1 program 0 admitted hordinal
    hadmit hbase hreturned

end ProofForgeV2.Semantic.InitializerDepositViewEqualityPreservationV1
