...
structure CertifiedInlineProofV1 where
  ...
  private completePreservingInvariantCoverage_ : Bool
...
/-- Canonical ProgramV1 source digest bound into the audited request. This is
    exposed only from the private certifier carrier so target authorization can
    reject cross-subject certificate reuse. -/
def sourceDigest (c : CertifiedInlineProofV1) : Digest :=
  c.sourceDigest_

/-- Exact retained SemanticProgramV1 digest bound into the audited request. -/
def semanticDigest (c : CertifiedInlineProofV1) : Digest :=
  c.semanticDigest_

/-- True only when the audited obligation set contains exactly one preserving
    row for every validated semantic invariant ordinal/name/root. This is
    evidence consumed by the private target capability mint, not itself a
    materialization capability. -/
def hasCompletePreservingInvariantCoverage (c : CertifiedInlineProofV1) : Bool :=
  c.completePreservingInvariantCoverage_
...
private def generatedSimpleClosureTheoremNameV1 (invName : String) : String :=
  if invName.isEmpty then
    "generatedEmptyV1"
  else
    let head := (String.Pos.Raw.get invName 0).toUpper
    "generated" ++ String.singleton head ++ invName.drop 1 ++ "V1"
...
private def expectedTheoremsForAuditV1
  ...
  for binding in bindings do
    ...
    | .preserving => pure ()
    | .holds =>
        let helperBase := generatedSimpleClosureTheoremNameV1 binding.invariantName
        let helperName :=
...
/-- Exact proof-only invariant-erasure coverage over the validated semantic
    table. Holds rows may coexist, but preserving rows must be a bijection with
    all dense InvariantDecl ordinals and must point at actual invariant roots. -/
private def hasCompletePreservingInvariantCoverageV1
    (data : SemanticProgramDataV1)
    (obligations : Array InlineProofObligationV1) : Bool :=
  if data.invariants.isEmpty then
    false
  else
    let preserving := obligations.filter fun obligation =>
      InlineProofObligationV1.kind obligation == .preserving
    preserving.size == data.invariants.size &&
      data.invariants.all fun invariant =>
        let matching := preserving.filter fun obligation =>
          (InlineProofObligationV1.ordinal obligation).toNat == invariant.id.toNat &&
            InlineProofObligationV1.invariantName obligation == invariant.name
        matching.size == 1 &&
          match data.callables[invariant.callableId.toNat]? with
          | some callable => callable.kind == .invariant
          | none => false

/-- Revalidate the untrusted inventory to answer the high-level no-proof query.
    The audited core repeats this validation before minting. -/
def inlineProofRequiredV1
    (source : ValidatedSourceV1) (theoremInventory : TheoremInventoryV1) :
    Except InlineProofCertifierDetailV1 Bool := do
  let expected ← validateTheoremInventoryBijectionV1 source theoremInventory
  pure (!expected.isEmpty)

/-- Private low-layer audited core. It derives and checks every binding, byte
    comparison, theorem audit and coverage fact before the private mint. -/
private def auditAndCertifyInlineProofCoreV1
    (env : Environment)
    (rawSource : String)
    (source : ValidatedSourceV1)
    ...
    (programSelector : Option String) : InlineProofCertifierOutcomeV1 := Id.run do
  ...
  let semanticData ← match validateSemanticProgramV1
      (CompiledSemanticV1.semanticV1Of compiled) with
    | .ok value => pure value
    | .error _ => return fail .certification .successMint
  let completePreservingInvariantCoverage :=
    hasCompletePreservingInvariantCoverageV1 semanticData obligations
  pure (.certified ⟨success, auditReport, subject.sourceHash,
    subject.semanticHash, completePreservingInvariantCoverage⟩)

private def leanNameFromSelector (selector : String) : Name :=
  if selector.isEmpty then Name.anonymous
  else (selector.splitOn ".").foldl (fun acc part => Name.str acc part) Name.anonymous
...
/-- Sole public certificate mint: preflight, selector gate, in-process
    elaboration of the exact held raw source, then private Environment audit.
    No API accepts an independently supplied `Environment` and raw byte string. -/
unsafe def certifyInlineProofV1
    ...
    (sourcePath : ProjectRelativePath)
    (moduleSelector : String)
    (programSelector : Option String) : IO InlineProofCertifierOutcomeV1 := do
  match inlineProofRequiredV1 source theoremInventory with
  | .error detail => return .failed .obligation detail
  | .ok false => return .noProof
  | .ok true => pure ()
  let mainModule := leanNameFromSelector moduleSelector
  if isForbiddenMainModuleV1 mainModule then
    return .failed .certification .elaborate
  let fileName :=
    match renderProjectRelativePath sourcePath with
    | .ok value => value
    | .error _ => "<inline-proof>"
  let result ← elaborateInlineProofSourceV1
    (ProductParserSessionV1.sessionEnvironment productSession)
    rawSource fileName mainModule
  match result with
  | .error _ => return .failed .certification .elaborate
  | .ok elaborated =>
      return auditAndCertifyInlineProofCoreV1
        (InlineProofElabEnvV1.environment elaborated) rawSource source
        originInventory theoremInventory compiled sourcePath moduleSelector programSelector
