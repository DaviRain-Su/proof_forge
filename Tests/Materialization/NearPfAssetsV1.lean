/-
  Tests.Materialization.NearPfAssetsV1 — ADR-0029 Phase C2 NEAR half binding.
  ...
  Plan-level pins for the pf.assets NEAR lane:
    ...
    * token QNs fail closed (Phase C scope); non-catalog sync calls fail closed
    * host imports: transfer action present iff a transferAsync is lowered;
      no schedule ⇒ no function-call action import
-/
import ProofForgeV2
...
unsafe def testDepositTransferAsyncPlan : IO Unit := do
  ...
  -- (no schedule), promise batch create present (shared by both promise kinds).
...
/-- Token QNs fail closed (Phase C scope). -/
unsafe def testTokenFailClosed : IO Unit := do
  let source :=
    "import ProofForgeV2\n" ++
    "open ProofForgeV2.Language\n" ++
    "program TokenTip where\n" ++
    pfAssetsRequiresBlock ++
    "  state tips : UInt64\n" ++
    "  init(initial : UInt64) do\n" ++
    "    tips := initial\n" ++
    ...
    "  view get() : UInt64 do\n" ++
    "    return tips\n"
  let compiled ← compileSource "<near-pf-assets-token>" "Tests.NearPfAssetsToken" source
  let selection ← liftResult <|
    Targets.BuildSelectionV1.resolveBuildSelectionV1 TargetId.near none
  let capability ← liftResult <|
    Targets.resolveEngineeringRequirementsV1 selection compiled
  match planFromCapability capability with
  | .error (.planInvariant .near msg) =>
      expect (msg.contains "token" || msg.contains "Phase C")
        s!"token QN must cite Phase C scope, got: {msg}"
  | .error e => throw <| IO.userError s!"expected planInvariant, got {e.render}"
  | .ok _ => throw <| IO.userError "token QN must fail closed on NEAR"
