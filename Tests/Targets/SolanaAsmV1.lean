...
/-- L2 / B-SOL-MAP-ELF: Token (multiple Map IndexGet + IndexSet + state stores)
    must fit within 4096 bytes under the ELF profile. Token mint has a match
    with two arms, each doing a Map IndexSet (24 leaves) + scalar state store +
    return. Transfer has nested match with two IndexSet operations. -/
...
/-- L2 / B-SOL-MAP-ELF: A genuinely oversized program must still fail-closed.
    We construct a program with 260 UInt64 state fields and a return expression
    that sums all of them as a left-associative chain. Each state load allocates
    a temp, and each checkedAdd allocates one — 260 loads + 259 adds = 519 temps.
    The Expr depth is 260, which exceeds the 256 depth guard and fails closed
    at Plan lowering — proving that oversized programs are rejected before
    reaching the asm emitter. The frame-budget gate (4096 bytes) remains the
    backstop for any program that passes the depth guard but still overflows
    the SBPF stack (verified by the constant test above). -/
