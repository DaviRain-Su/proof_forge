/-
  Tests.Product.MiniAmmAssetsEvmV1 — ADR-0030 E4 M5 dual ERC-20 product pin.

  Authority: shipped `Examples/MiniAmmAssets.lean` (same source as Solana
  Mollusk `miniamm_assets` + `MiniAmmAssetsSolanaV1`).
    * product `check` / `build --target evm` succeeds
    * inspect exact disk closure
    * Yul emits four ERC-20 `transfer` sites (swap0/swap1 + dual remove)
    * engineering only: Anvil gate is host-optional main-agent smoke
-/
import ProofForgeV2.Compiler.Pipeline
import ProofForgeV2.Core.Common
import Tests.Language.ParserSession

namespace Tests.Product.MiniAmmAssetsEvmV1
...
private def readShipped : IO String := do
  unless ← shippedPath.pathExists do
    throw <| IO.userError "Examples/MiniAmmAssets.lean missing (M5 dual-mint vector)"
...
private def assertShape (text : String) : IO Unit := do
  ...
  expect (containsSubstr text "call pf.assets.token.transfer")
  ...
  expect (containsSubstr text "swap0to1")
    "must declare swap0to1 mint1 payout"
...
private def testCliCheckOk : IO Unit := do
  ...
  let (code, stdout, stderr) ← runCli
    #["check", "Examples/MiniAmmAssets.lean", "--module", "Examples.MiniAmmAssets",
      "--target", "evm"]
  expect (code == 0)
    s!"MiniAmmAssets check --target evm must succeed, exit={code} stderr={stderr} stdout={stdout}"
...
private def testEvmBuildAndInspect : IO Unit := do
  ...
  let outDir := FilePath.mk "build/v2/miniamm-assets-evm-pin"
  ...
  let (code, stdout, stderr) ← runCli
    #["build", "Examples/MiniAmmAssets.lean",
      ...
      "-o", outDir.toString]
  expect (code == 0)
    s!"MiniAmmAssets build --target evm must succeed, exit={code} stderr={stderr} stdout={stdout}"
  expect (containsSubstr stdout "target=evm")
  ...
  unless ← (outDir / "manifest.json").pathExists do
    throw <| IO.userError "MiniAmmAssets EVM must write manifest.json"
  unless ← (outDir / "evidence.json").pathExists do
    throw <| IO.userError "MiniAmmAssets EVM must write evidence.json"
  unless ← (outDir / "MiniAmmAssets.yul").pathExists do
    throw <| IO.userError "MiniAmmAssets EVM must write MiniAmmAssets.yul"
  ...
  expect (containsSubstr manifest "\"artifactProgramName\": \"MiniAmmAssets\"")
    s!"manifest artifact name must be MiniAmmAssets, got={manifest}"
  expect (containsSubstr manifest "MiniAmmAssets.yul")
    s!"manifest files must list MiniAmmAssets.yul, got={manifest}"
  ...
  let yul ← IO.FS.readFile (outDir / "MiniAmmAssets.yul")
  expect (containsSubstr yul "object \"MiniAmmAssets\"")
    "MiniAmmAssets.yul must declare object MiniAmmAssets"
  expect (containsSubstr yul "0xa9059cbb")
    "MiniAmmAssets.yul must emit ERC-20 transfer selector"
  expect (containsSubstr yul "0, 68, 0, 32)")
    "MiniAmmAssets.yul CALL must use 68-byte calldata + 32-byte return buffer"
  expect (containsSubstr yul "tokenAddr")
    "MiniAmmAssets.yul must bind token contract address from mint Principal"
  expect (containsSubstr yul ", 20)")
    "MiniAmmAssets.yul must require Principal len == 20"
  expect (containsSubstr yul "returndatasize()")
    "MiniAmmAssets.yul must read returndatasize for return-value predicate"
  -- Four token.transfer sites: swap0to1, swap1to0, remove mint0, remove mint1.
  let transferSites := countSubstr yul "0xa9059cbb"
  expect (transferSites == 4)
    s!"MiniAmmAssets.yul must emit exactly 4 ERC-20 transfer sites, got={transferSites}"
  unless ← (outDir / "MiniAmmAssets.abi.json").pathExists do
    throw <| IO.userError "MiniAmmAssets EVM must write MiniAmmAssets.abi.json"
  let abi ← IO.FS.readFile (outDir / "MiniAmmAssets.abi.json")
  expect (containsSubstr abi "\"name\":\"swap0to1\"")
    "abi must list swap0to1"
  expect (containsSubstr abi "\"name\":\"removeLiquidity\"")
    "abi must list removeLiquidity"
  expect (containsSubstr abi "\"stateMutability\":\"nonpayable\"")
    "entries must be nonpayable (token transfer carries no ETH)"
  -- inspect re-walks exact disk closure.
  let (iec, istdout, istderr) ← runCli
    #["inspect", outDir.toString, "--json"]
  expect (iec == 0)
    s!"inspect miniamm-assets-evm --json must succeed, exit={iec} stderr={istderr} stdout={istdout}"
  ...
  expect (containsSubstr istdout "\"artifactProgramName\":\"MiniAmmAssets\"")
...
unsafe def run : IO Unit := do
  ...
  IO.println "Tests.Product.MiniAmmAssetsEvmV1: ok"

end Tests.Product.MiniAmmAssetsEvmV1
