/-
  Tests.Product.TipJarEvmV1 — ADR-0029 Phase B2 product vertical.
  ...
  Authority: shipped `Examples/TipJar.lean`.
    ...
    * pins target=evm, default profile, TipJar.yul deposit/transfer structure
...
namespace Tests.Product.TipJarEvmV1
...
private def readShipped : IO String := do
  unless ← shippedPath.pathExists do
    throw <| IO.userError "Examples/TipJar.lean missing (ADR-0029 B2 continuous vector)"
...
private def testCliCheckOk : IO Unit := do
  ...
  let (code, stdout, stderr) ← runCli
    #["check", "Examples/TipJar.lean", "--module", "Examples.TipJar",
      "--target", "evm"]
  expect (code == 0)
    s!"TipJar check --target evm must succeed, exit={code} stderr={stderr} stdout={stdout}"
...
private def testEvmBuildAndInspect : IO Unit := do
  ...
  let outDir := FilePath.mk "build/v2/tipjar-evm"
  ...
  let (code, stdout, stderr) ← runCli
    #["build", "Examples/TipJar.lean",
      ...
      "-o", outDir.toString]
  expect (code == 0)
    s!"TipJar build --target evm must succeed, exit={code} stderr={stderr} stdout={stdout}"
  expect (containsSubstr stdout "target=evm")
    s!"build stdout must pin target=evm, stdout={stdout}"
  unless ← (outDir / "manifest.json").pathExists do
    throw <| IO.userError "TipJar EVM must write manifest.json"
  unless ← (outDir / "evidence.json").pathExists do
    throw <| IO.userError "TipJar EVM must write evidence.json"
  -- Materialized base is TipJar.yul; finalized may add bytecode when solc runs.
  unless ← (outDir / "TipJar.yul").pathExists do
    throw <| IO.userError "TipJar EVM must write TipJar.yul"
  ...
  expect (containsSubstr manifest "TipJar.yul")
    s!"manifest files must list TipJar.yul, got={manifest}"
  ...
  let yul ← IO.FS.readFile (outDir / "TipJar.yul")
  expect (containsSubstr yul "object \"TipJar\"")
    "TipJar.yul must declare object TipJar"
  expect (containsSubstr yul "if iszero(eq(callvalue(),")
    "TipJar.yul must exact-eq callvalue to deposit amount"
  expect (containsSubstr yul "call(gas(),")
    "TipJar.yul must emit full-gas value CALL for transfer"
  expect (containsSubstr yul "if callvalue() { revert(0, 0) }")
    "TipJar.yul view/non-payable path must keep callvalue==0"
  -- ABI payable tip.
  unless ← (outDir / "TipJar.abi.json").pathExists do
    throw <| IO.userError "TipJar EVM must write TipJar.abi.json"
  let abi ← IO.FS.readFile (outDir / "TipJar.abi.json")
  expect (containsSubstr abi "\"stateMutability\":\"payable\"")
    "TipJar.abi tip must be payable"
  -- inspect re-walks exact disk closure.
  let (iec, istdout, istderr) ← runCli
    #["inspect", outDir.toString, "--json"]
  expect (iec == 0)
    s!"inspect tipjar-evm --json must succeed, exit={iec} stderr={istderr} stdout={istdout}"
...
unsafe def run : IO Unit := do
  ...
  IO.println "Tests.Product.TipJarEvmV1: ok"

end Tests.Product.TipJarEvmV1
