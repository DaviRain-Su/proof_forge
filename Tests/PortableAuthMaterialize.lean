...
def main : IO Unit := do
  ...
  -- Direct Product Ownable moved to `just ownable-authoring-cutover`; this
  -- legacy-module test retains only the still-unmigrated RemoteCall path.
  for mod in #[remote] do
