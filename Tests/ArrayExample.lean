...
def main : IO Unit := do
  ...
  require (product.entrypoints.map (·.name) == #["sizeOf3", "getElem", "sumOf3"])
  ...
  require ((operations.filter fun operation => match operation with
      ...
      | _ => false).size == 2)
    "Product ArrayExample Core must allocate exactly two local arrays"
  require ((operations.filter fun operation => match operation with
      | .memoryStore _ _ _ => true
      | _ => false).size == 6)
    "Product ArrayExample Core must initialize all six array elements"
  require ((operations.filter fun operation => match operation with
      | .memoryLoad _ _ => true
      | _ => false).size == 4)
    "Product ArrayExample Core must retain all four array reads"
  ...
  require (nearPlan.functions.size == 3 && nearPlan.layout.scalars.isEmpty)
