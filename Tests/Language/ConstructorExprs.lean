...
unsafe def run : IO Unit := do
  ...
  -- Underscore components reach decoder (not parser); pin Common anonymous diagnostic.
  for (label, expr, expected) in [
      ...
      ("anonymous second component", "A._()",
        "qualified-name component must not be anonymous"),
      ("anonymous first component", "_.B()",
        "qualified-name component must not be anonymous"),
      ("anonymous escaped component", "A.«_»(1)",
        "qualified-name component must not be anonymous")
    ] do
