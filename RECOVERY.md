...
（inspect family + residual 标签；COMP-1-NORMALIZE-RESIDUAL Field/Principal 字面量与 Bytes 嵌套穿透 FC 针；COMP-1-SYS-CAP-L2 attachedValue inspect 族标签 + Targets xrpl/view 针）；部署地址绑定、官方 program catalog 与字面量开放仍 pending。**CodecInvert 九字段工程闭合**（transport invert；不关闭 TASK-D2-06 / TST-SEM-001）。对账见 [`docs/research/28-project-wide-honesty-audit.md`](docs/research/28-project-wide-honesty-audit.md) 与 [`AGENTS.md`](AGENTS.md) Next task。
