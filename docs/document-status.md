...
工程 registry **13 = 13 implemented + 0 design-only**（含 Soroban ADR-0044、OpenVM ADR-0045/0046、ICP ADR-0047、XRPL ADR-0049/0050；host 三键 ADR-0052 proposed、叶未开；ADR-0053 accepted，call-bind Wave 1–4 engineering done：三叶 exact endpoint / missing-row FC、EVM 空账户 void CALL FC、EVM static OutputSet/raw `.bin` identity evidence、Solana 支持子集 outer AccountInfo join + Mollusk；EVM code-at-address、Solana/CW identity 与 unsupported shapes 仍 FC/open）。Quint 是
...
（generic sync FC、`pf.assets` sync admitted、Binary SubMsg same-tx PARTIAL）；TON 工程面为 Tolk + real BoC +
...
leaves 的边界由 **accepted ADR-0036**（现 13+0：含 Soroban ADR-0044、OpenVM
ADR-0045/0046、ICP ADR-0047、XRPL ADR-0049/0050）收口为非 accepted 扩面，
formal lighthouse 为 EVM-first。
日常缺口队列见 [`engineering-backlog.md`](engineering-backlog.md)（完善度勾选 **§12**）。
