...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ArrayExample VM evidence at `0035138e`) | add target-neutral direct map authoring, cut StatusMessage over without a compatibility facade, then expand to GuestBook and later crosscall families |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | ValueVault, Ownable, Pausable, ReentrancyGuard, and ArrayExample primary-triad VM comparisons are verified through `0035138e`; the next evidence family follows direct map cutover |
