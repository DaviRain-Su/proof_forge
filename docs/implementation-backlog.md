...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (direct map authoring at `9f4bd403`) | cut StatusMessage over without a compatibility facade, pin its native evidence, then expand to GuestBook and later crosscall families |
