...
| A-CUT2h | Remove stale Counter `.spec`/`.module` consumers and obsolete backend wrappers | done (verified at `b2d673b4`) | callers use Authored/Canonical or explicit v1-only fixtures; wrappers are deleted; EVM constructor config is target-owned and shared payloads fail closed |
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ValueVault and Ownable direct with native evidence) | rewrite Pausable directly and delete its Legacy stdlib implementation/callers, then migrate ReentrancyGuard, map/collection, event/error, and portable crosscall families |
...
| CMP-2 | Counter native pilot: Solidity EVM, Rust Solana, Rust NEAR | done (verified at `e2834c59`) | closes A-CUT2 with complete primary-triad v1 evidence |
| CMP-3 | ValueVault and representative stateful portable families | in_progress | ValueVault and Ownable primary-triad VM comparisons are verified; Ownable closed at `ce539dce` with 10 steps and all eight dimensions; attach Pausable evidence after its direct cutover |
...
| CMP-CI | Focused local gate, target-specific live lanes, generated fail-closed matrix | pending after CMP-3 | IR-B8/A-CUT5 sign-off |
