...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ValueVault and Ownable direct; ValueVault native-differential verified) | execute the pinned Ownable references on all three native VMs, then migrate map/collection, event/error, and portable crosscall families |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | ValueVault primary-triad VM comparison is verified; Ownable's three references and ten-step scenario are pinned at `6e1df78b` with evidence `none`; execute CMP-3d2 next |
