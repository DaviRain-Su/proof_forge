...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ValueVault direct; CMP-3 native evidence active) | complete the Solidity/near-sdk references and triad runner, then continue the catalog |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | 13-step scenario and independent Solana Rust reference are pinned without semantic promotion; complete the EVM/NEAR references and native VM comparison |
