...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ValueVault and Ownable direct; ValueVault native-differential verified) | execute Ownable native differential next, then migrate map/collection, event/error, and portable crosscall families |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | ValueVault primary-triad VM comparison and Ownable direct compile cutover are verified; execute independent Ownable references next |
