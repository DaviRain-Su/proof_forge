...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (Pausable direct cutover at `50c1c07a`) | complete Pausable native evidence, then rewrite ReentrancyGuard and delete its Legacy implementation before map/collection, event/error, and portable crosscall families |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | ValueVault and Ownable primary-triad VM comparisons are verified; Pausable direct cutover closed at `50c1c07a`; CMP-3e1 now pins independent Pausable references before VM execution |
