...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ValueVault direct; CMP-3 native evidence active) | implement and pass the fail-closed primary-triad ValueVault runner, then continue the catalog |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | 13-step scenario and all three independent references are pinned without semantic promotion; implement the native/ProofForge VM comparison |
