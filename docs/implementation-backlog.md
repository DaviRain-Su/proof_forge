...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ValueVault direct; CMP-3 native evidence active) | add the independent Solana ValueVault reference and close its triad differential, then continue the catalog |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | direct-only triad compile is verified; add the independent Solana Rust reference and native comparison |
