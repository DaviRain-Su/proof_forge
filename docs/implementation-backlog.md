...
| A-CUT3 | Migrate Product/stdlib callers and delete their `Source.Legacy` dependencies | in_progress (ReentrancyGuard references verified at `9772da92`) | complete ReentrancyGuard primary-triad VM evidence before map/collection, event/error, and portable crosscall families |
...
| CMP-3 | ValueVault and representative stateful portable families | in_progress | ValueVault, Ownable, and Pausable primary-triad VM comparisons are verified; ReentrancyGuard is direct-only with pinned references, and CMP-3f2 VM execution is active |
