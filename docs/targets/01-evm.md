...
- **ADR-0030 E4 EVM-first demo**：`Examples/MiniAmm.lean` 以 `context.caller` 作为
  ...
  当前 locked-solc creation bytecode ≈ 5.3 KiB（远低于 EIP-3860 的 49152 B initcode
...
- Token 当前为 **locked-solc engineering finalization**：creation bytecode ≈ 1.4 KiB
  ...
  `sstore`（不再 spill+整表 24/44 次 sstore）。Anvil/mainnet/OZ 产品声明仍以各自 runtime 门为准，不因体积单独阻断；
