...
- **ADR-0030 E4 EVM-first demo**：`Examples/MiniAmm.lean` 以 `context.caller` 作为
  ...
  当前 locked-solc `--optimize` creation bytecode ≈ 3.3 KiB（远低于 EIP-3860 的 49152 B initcode
...
- Token 当前为 **locked-solc engineering finalization**：creation bytecode ≈ 1.0 KiB（`--optimize`）
  ...
  重复 `pf_sload_u64(slot)` 做 CSE（`sstore`/map upsert/`case`/`function` 边界失效）。Anvil/mainnet/OZ 产品声明仍以各自 runtime 门为准，不因体积单独阻断；
