...
- **Option UInt64 / Option Int64 state（BL-31）**：Enum-shaped tag/payload 双 slot；
  ...
  Option/Array/Bytes **param** 已 flatten；Map param 仍 FC（hashed 1-slot）；Option Int64 return 为 2 叶 tag+payload；非 UInt64/Int64 payload 与 nested Option 仍 fail-closed；
- **Array Int64 N**：与 Array UInt8/16/32/64 同 `arrayScalarLeafLayoutV1` 连续 8-byte
  slot；IndexGet/IndexSet 保留 `isInt`；Array Int64 N return 为 N×int64 叶；Array Int8 仍 fail-closed；
...
- engineering planDigest 可绑 BuildIdentity/OutputSet；G4 `evm_anvil_differential.sh` 从产品 CLI
  ...
  **Lighthouse slice-3（engineering）**：StateCell/Accumulator/ArithOps（digest-listed）
  Reference 腿 mint `pf.reference-outcome.v1` digests；digest-listed pass 强制
  shared→Outcome projection equality + sidecar validate（Anvil↛OutcomeWire；非
  formal TST-SEM/C-3）。见 `docs/specs/evm-outcome-adapter-v1.md`。
...
- **`pf.crypto.merkleVerifyKeccak256`（CAP-X-MERKLE-EVM-ANVIL，2026-08-19）**：EVM-only
  unrolled OpenZeppelin sorted-pair `keccak256(0,64)` 链（statement tag 25，D∈1..8，
