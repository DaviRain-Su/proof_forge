...
| CPI (`sol_invoke_signed_c`, `sol_invoke_signed_rust`) | SDK metadata, entry actions, and helper emission exist; System Program transfer/create-account, Memo Program one-word raw payloads, SPL Token helpers, and Token-2022 helpers pack C `SolInstruction`, standard instruction data bytes, `SolAccountMeta[]`, bound `SolAccountInfo[]`, signer seed tables, and decoded scalar entrypoint parameters; System transfer/create-account, Memo, SPL Token `transfer_checked`/`mint_to`/`burn`/`approve`/`revoke`/`set_authority`, and Token-2022 direct CPI for transfer-fee flows plus non-transferable/metadata-pointer/default-account-state/immutable-owner/permanent-delegate/interest-bearing/memo-transfer/transfer-hook initialization have Surfpool/Web3.js live behavior gates; System transfer, System `create_account`, SPL Token `transfer_checked`, SPL Token `mint_to`/`burn`/`approve`/`revoke`, and SPL Token `set_authority` now have checked-in Pinocchio reference contract/manifest gates included in `just solana-light`, plus dual-deploy live-equivalence harnesses gated on Solana rustc availability | Make the Pinocchio live gates pass in CI/local toolchains, add Token-2022 reference coverage, add arbitrary-length Memo payloads, and extend remaining SPL helper live-equivalence |
