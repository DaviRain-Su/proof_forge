...
- **≤8 叶聚合返回**：named Struct/Enum 与 anonymous Array/Option UInt64/Int64 / Bytes N 经单次
  `sol_set_return_data` 发 N×8-byte LE；Map/nested/非标量元素返回仍 fail-closed。
...
- **`context.caller` binding（ADR-0031 S1 / ADR-0030 E3，2026-08-06）**：仅 exact
  ...
  inventory 为 **24 integration test binaries / 418 active tests**（2026-08-13 增
  `sha256_check` 4 测；`block_height` 4 测仍在）。**非** formal/mainnet parity，且不把
...
- **E4 LP state**：Solana 已开 dense `Map Principal UInt64` **cap-4** pilot（每槽
  ...
  Mollusk 执行（这些门**不**入 ordinary ci）。MiniAmmAssets Mollusk dual-mint 应用门
  （`miniamm_assets` 10/10：addLiquidity/swap0to1/removeLiquidity + Token TransferChecked）
  已闭；EVM M5 dual ERC-20 Anvil（strict EIP-3860）亦已闭 — 同一 MiniAmmAssets 源双链 runtime。
...
下一层边界由 [`ADR-0048`](../adr/0048-optional-solana-sbpf-semantics-provider.md) 固定：
...
与stack-store effect contract下闭合完整55步，并导出provider `runFuel` status-zero等式。可执行
fail-closed checker现已同时检查exact production SHA-256与证书消费的全部program lookups，其
成功结果可kernel投影为lookup contract；真实production artifact回归通过该checker。第二个
checker直接认证concrete encoded Loader memory reads，并kernel投影为全部input contracts；真实
