...
>
> 3. **入口与资格不是同一集合**：十三个 target 都可 development build；compiler-local
>    runtime 入口仅 `evm` / `solana` / `near` / `cosmwasm` / `ton` / `icp`；release
>    qualification 必须由 candidate-bound evidence evaluator 产生，当前为 `not-evaluated`。
>    `pf test` 的 developer adapter 集合独立于 compiler-local runtime 集合。
...
| `solana` | explicit-account SVM | accepted Phase 1 implement | `specified` | retained-V1 Plan + SBPF asm → ELF `.so` + Mollusk runtime 差分；static engineering validation label `runtime-validated-alpha`（2026-08-12）；**非** dynamic maturity / formal Stage-0/hermetic | [Solana](02-solana.md) |
...
| `cosmwasm` | Wasm host | engineering implemented (scope ADR open) | `research` | retained-V1 Plan/IR → WAT + locked `wat2wasm`/`cosmwasm-check` 3.0.9；UInt8/16/32、named state、bounded aggregate/Array/Option return；**generic sync FC**（`pf.assets` sync admitted）；async→Binary `SubMsg{reply_on:never}`（same-tx、QN stub）；cosmwasm-vm 28-test mock + wasmd v0.70.3 Docker rung-1；static engineering validation label `wasm-validated-alpha`；**非** dynamic maturity / 主网/formal；**非** accepted Phase 1 范围 | [CosmWasm](04-cosmwasm.md) |
...
| `ton` | TVM Stack-Account | engineering implemented (scope ADR open) | `research` | retained-V1 Plan/IR → Tolk + real BoC；UInt8/16/32/128 与 Int8/16/32/64、named state、bounded view aggregate/Array/Option return；async schedule→`createMessage`（hash dest/value=0/fixed mode，PARTIAL），sync call FC；`@ton/sandbox` 16/16；static engineering validation label `source-only`；**非** dynamic maturity / 主网/formal；**非** accepted Phase 1 范围 | [TON](11-ton.md) · [family](family-tvm-stack-account.md) |
...
`Static dossier ceiling` 列只允许 PRD 定义的 `TargetMaturity` 六个 wire value；`alpha`、
`plan-only`、`source-only`、`implemented` 等不能作为 maturity snapshot 值。代码中的静态
`engineeringValidationLabel` 明确属于另一个工程 inspection 字段。阶段不得跳跃。源程序只有
统一的 `program` 声明，没有 target 类别字段；本表的分类不会进入用户源码。
