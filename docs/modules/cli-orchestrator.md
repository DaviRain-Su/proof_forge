...
> JSON 不含 public `receipts`，也不声明 contained assurance。raw-source product-positive
> certified 仍可 open。下文 supervised receipt 条款是尚未实现的 proposed 契约。
...
controller containment、raw-source product-positive certified 与 formal `TST-PROOF-001` 仍
out of scope。
