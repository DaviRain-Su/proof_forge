...
| 期 | 内容 | 依赖 |
|---|---|---|
| **E1** | token.transfer 四链绑定（EVM/Solana/CW/NEAR-async） | 无（payload 已含 QN） |
| **E2** | payload v1.1.0：balanceOfSelf env-read + Reference/Normalize 接线 | E1 可先并行 |
| **E3** | context.caller Plan 开放（per-target 原子 cutover） | 独立（B-CTX-OPEN） |
| **E4** | MiniAMM 北极星（EVM+Solana 双链 runtime 门） | E1+E2+E3 |
