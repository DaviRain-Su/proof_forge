...
| 恒定乘积 x*y≥k | UInt128/256 宽算术 | **shared + EVM 全宽已有**；Solana/NEAR/CosmWasm 已开 restoring binary long division，Solana 另有 locked-SBPF + Mollusk `WideDiv`/`WideDiv256` 8 个数值/零除回滚测试，并以 `WideDivDispatch` 验证四宽 handler 的长距分派；各 target 其余宽算术边界仍按 dossier fail closed |
| LP 份额 | v1 内部记账（`Map Principal UInt64` state） | **shared Wire/Normalize 已 admit Principal map key**；EVM 与 Solana 均已开 cap-4 dense Principal-key target leaf（occ + 9 Principal leaves + value）；Solana 另固定 Array UInt64 24/44 不得与 dense Map 叶数混淆。两链目前仍仅内部记账，发行真资产（mint）**defer 至 custody/issuance v2** |
...
| **E4** | MiniAMM 北极星（EVM+Solana 双链 runtime 门） | E1+E2+E3 | **in_progress**：Solana UInt128/256 div/mod locked-SBPF+Mollusk、EVM/Solana cap-4 Principal-key Map 与 EVM vault-internal Anvil demo 已交付；仍缺 Solana MiniAMM 产品镜像/Mollusk 应用门、真实 asset movement/remove-liquidity，以及 EVM 无 code-size override 的正常部署闭环 |
...
发射 exact restoring binary long division（`910835aa4`），`WideDiv`/`WideDiv256` 由独立
Rust oracle + Mollusk 固定成功值与除零 `0x1001` 全账户回滚；`WideDivDispatch` 进一步把
四个宽 handler 放入同一产品 ELF，以近距条件 stub + BPF-to-BPF `call` 消除 signed-i16
入口跳转风险，并在 Mollusk 执行最远 `mod256`。Solana cap-4 dense
`Map Principal UInt64`（`aed5a0010`）与 EVM 同构，Array UInt64 24/44 已有类型驱动分派回归，
不再按叶数误判为 Map。EVM `Examples/MiniAmm.lean` + Plan/Yul pins（`06a0bcd1e`）和
host-optional Anvil code-size-override 门已存在。当前仍只做内部 reserve/LP 记账，未走
`pf.assets` transfer/balanceOfSelf、无 remove-liquidity、无 Solana MiniAMM runtime，故仍是
E4 in-progress 前置，不是双链应用级证据，也不是 formal/hermetic/mainnet 证明。
