...
| [ADR-0053](0053-call-bind-v1.md) | Call-bind v1：编译期 opt-in `proof-forge.call-bind.v1`；Wave 1–4 engineering done；三叶 endpoint binding / missing-row FC；EVM static OutputSet/raw `.bin` identity evidence；Solana 支持子集 exact outer AccountInfo join + Mollusk；code-at-address / Solana/CW identity / unsupported shapes 仍 FC/open | accepted |
