...
| D1-PA-62 | TASK-D1-03 | exact `Option Option Field bn254_fr` spelling for existing recursive `option(option(field))` carrier；tag `16→16→2`、transitive `fieldBn254` requirement 与 single existing negative migration | — | freeze-only：Grok post-PA61 residual audit P0=0；RED next；Syntax-only cap ≤32/2；formal TASK 状态不变 | active (development) |
