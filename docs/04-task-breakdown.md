...
| D1-PA-70 | TASK-D1-03 | exact three-layer `Option Option Option PrimitiveAtom` spelling for existing recursive `option(option(option(element)))` carrier；tag `16→16→16→element`、transitive requirements 与 single existing negative migration | `—` | focused RED/GREEN + aggregate/test binary；closed nested Option decoder reuse、depth/non-alias/requirement/support-vs-Plan controls；post-PA69 audits P0=0；package file-set re-pin；按冻结未重复 `just ci` | in_progress (development) |
