...
| D1-PA-60 | TASK-D1-03 | exact `Array Field bn254_fr N` spelling for existing recursive `array(field,length)` carrier；tag `18→2→length`、transitive `fieldBn254` requirement 与 two existing negative migrations | — | freeze-only：Grok post-PA59 residual audit P0=0；RED next；Syntax-only cap ≤32/2；formal TASK 状态不变 | active (development) |
