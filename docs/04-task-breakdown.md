...
| D1-PA-54 | TASK-D1-03 | bounded `Array PrimitiveAtom N` declaration carrier；`Fin 4097` exact length、Type tag `18`、transitive requirements 与 zero migration | — | post-PA53 residual audit 选择最小闭合 type slice；spec freeze before tests-only RED；GREEN 后运行 focused/aggregate/test binary，按冻结不重复 `just ci` | active (development) |
