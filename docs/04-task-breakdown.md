...
| D1-PA-65 | TASK-D1-03 | exact `Option Array Field bn254_fr N` spelling for existing recursive `option(array(field,length))` carrier；tag `16→18→2→length`、transitive `fieldBn254` requirement 与 single existing negative migration | `12fe8e7f`, `a09cbfb5`, `c53457bb`, `9ca87e30` | focused 23-job + 192-job aggregate/test binary；exact named parser、Array Field/length decoder reuse、canonical/requirement/support-vs-Plan controls；Kimi final review pending；package file-set re-pin；按冻结未重复 `just ci` | complete (development) |
