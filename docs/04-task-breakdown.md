...
| D1-PA-59 | TASK-D1-03 | exact `Array Option PrimitiveAtom N` spelling for existing recursive `array(option(element),length)` carrier；tag `18→16→element→length`、transitive requirements 与 two existing negative migrations | — | freeze-only：Grok post-PA58 residual audit P0=0；RED next；Syntax-only cap ≤32/2；formal TASK 状态不变 | active (development) |
