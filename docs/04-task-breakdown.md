...
| D1-PA-74 | TASK-D1-03 | post-PA-73 bounded arbitration 选择 exact `Option Option Option Bytes N` spelling for existing recursive `option(option(option(bytes(length))))` carrier；tag `16→16→16→17→length`、zero requirements 与 single existing negative migration | `—` | tests-only RED then minimal GREEN；single-length/wrapper-depth/Bytes-leaf canonical non-alias、support-vs-Plan/no-artifact controls；production 仅 Syntax ≤32 additions/2 removals；package file-set re-pin；按冻结不重复 `just ci` | in_progress (development) |
