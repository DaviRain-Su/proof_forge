...
| D1-PA-72 | TASK-D1-03 | exact `Array Option Bytes N M` spelling for existing recursive `array(option(bytes(innerLength)),outerLength)` carrier；tag `18→16→17→innerLength→outerLength`、zero requirements 与 single existing negative migration | `—` | tests-only RED then minimal GREEN；dual-length/canonical/zero-requirement/support-vs-Plan/no-artifact controls；production 仅 Syntax ≤32 additions/2 removals；package file-set re-pin；按冻结不重复 `just ci` | in_progress (development) |
