...
| D1-PA-49 | TASK-D1-04 | value-less `returnUnit` Source-only carrier；既有 `returnValue` 不变、Statement tag `6`、zero migration 与 Typed fail-closed | — | deterministic offside return layout/canonical/tag/parser-boundary audits；spec freeze before tests-only RED；GREEN 后运行 focused/aggregate/test binary 与 clean committed statement checkpoint `just ci` | active (development) |
