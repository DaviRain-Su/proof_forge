...
| D1-PA-71 | TASK-D1-03 | exact `Array Option Option PrimitiveAtom N` spelling for existing recursive `array(option(option(element)),length)` carrier；tag `18→16→16→element→length`、transitive requirements 与 single existing negative migration | `—` | tests-only RED then minimal GREEN；15-atom/length/canonical/requirement/support-vs-Plan/no-artifact controls；production 仅 Syntax ≤32 additions/2 removals；package file-set re-pin；按冻结不重复 `just ci` | in_progress (development) |
