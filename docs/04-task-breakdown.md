...
| D1-PA-34 | TASK-D1-04 | not-equal `!=` Source-only carrier；与 `==` 同层 precedence `50` non-associative、Expr tag `15`、single retention migration 与 Typed fail-closed | — | token-integrity/migration/mixed-chain/canonical audits；spec freeze before RED | **active** |
