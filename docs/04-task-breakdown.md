...
| D1-PA-36 | TASK-D1-04 | less-or-equal `<=` Source-only carrier；与既有 comparisons 同层 precedence `50` non-associative、Expr tag `17`、single ordering migration 与 Typed fail-closed | — | frozen tests/production seam；tests-only RED pending | active (development) |
