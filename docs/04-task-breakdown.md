...
| D1-PA-68 | TASK-D1-03 | exact `Option Array Bytes N M` spelling for existing recursive `option(array(bytes(innerLength),outerLength))` carrier；tag `16→18→17→innerLength→outerLength`、zero requirements 与 single existing negative migration | `—` | focused RED/GREEN + aggregate/test binary；dual-length Array Bytes decoder reuse、canonical non-alias/zero-requirement/support-vs-Plan controls；prior independent audits P0=0；package file-set re-pin；按冻结未重复 `just ci` | in_progress (development) |
