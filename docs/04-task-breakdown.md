...
| D1-PA-77 | TASK-D1-03 | post-PA-76 bounded arbitration 选择 exact `Array Array Field bn254_fr N M` spelling for existing recursive `array(array(field,innerLength),outerLength)` carrier；tag `18→18→2→innerLength→outerLength`、single `fieldBn254` requirement 与 single existing negative migration | `—` | tests-only RED then minimal GREEN；fixed Field/dual-length canonical non-alias、requirement/support-rejection/no-artifact controls；production 仅 Syntax ≤36 additions/2 removals；package file-set re-pin；按冻结不重复 `just ci` | in_progress (development) |
