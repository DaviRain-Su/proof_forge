...
| D1-PA-57 | TASK-D1-03 | exact `Option Array PrimitiveAtom N` spelling for existing recursive `option(array(element,length))` carrier；tag `16→18→element→length`、transitive requirements 与 single deferred-negative migration | — | post-PA56 residual audit 选择最小 existing-carrier spelling slice；spec freeze before tests-only RED；GREEN 后运行 focused/aggregate/test binary，按冻结不重复 `just ci` | active (development) |
