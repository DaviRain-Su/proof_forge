...
| D1-PA-66 | TASK-D1-03 | exact `Array Array PrimitiveAtom N M` spelling for existing recursive `array(array(element,innerLength),outerLength)` carrier；tag `18→18→element→innerLength→outerLength`、transitive requirements 与 single existing negative migration | `—` | focused RED/GREEN + aggregate/test binary；15-atom/dual-length decoder reuse、canonical/requirement/support-vs-Plan controls；independent residual audits P0=0；package file-set re-pin；按冻结未重复 `just ci` | in_progress (development) |
