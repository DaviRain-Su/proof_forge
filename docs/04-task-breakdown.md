...
| D1-PA-83 | TASK-D1-05 | PA82 全量 reconstruction 后按 exact payload `qualifiedName` 拒绝 cross-row duplicate/split-brain Source identity；`sourceHash` 只分类同名同/异内容 | — | frozen completion package；下一步 tests-only RED；`PF-EXPORT-001` exact duplicate/conflict diagnostics、decode-before-identity priority、distinct-qname control；不含 declaration/payload single-row binding、independent hash-collision oracle、CLI/Loader、target、contained worker 或正式 `TST-SRC-006/007` closure | frozen (development) |
...
| TASK-D1-05 | `Source.Program` stable attribute export/schema | TASK-D1-03 | — | TST-SRC-006, TST-SRC-007 | EV-20260719-0079, EV-20260719-0080 | pending |
