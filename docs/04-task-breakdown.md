...
| D1-PA-67 | TASK-D1-03 | exact `Option Array Option PrimitiveAtom N` spelling for existing recursive `option(array(option(element),length))` carrier；tag `16→18→16→element→length`、transitive requirements 与 single existing negative migration | `—` | focused RED/GREEN + aggregate/test binary；15-atom/Option/Array/length decoder reuse、canonical non-alias/requirement/support-vs-Plan controls；Grok residual P0=0；EV table must include PA-66;按冻结未重复 `just ci` | in_progress (development) |
