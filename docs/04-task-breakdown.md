...
| D1-PA-89 | TASK-D1-01 | tests-only labeled `SourceWireAcceptance` executable harness for the existing `TST-SRC-001` NodeId/span development surface：exact `SourceIdentity`→`SourceSpan` single-run wrapper；零 production change | `ee1cbc8d`；implementation pending | pending-prep package 已锁定 TASK-D1-01 的两项 formal tests；本切片只包装既有 SRC-001 assertions，总新增 ≤30 行；focused acceptance + aggregate test binary + docs/diff + independent review；不吸收完整 wire golden 或 SRC-002，不运行完整 `just ci`/SBOM，不关闭 formal task | frozen (development) |
