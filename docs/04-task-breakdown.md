...
| D1-PA-63 | TASK-D1-03 | exact `Option Option Bytes N` spelling for existing recursive `option(option(bytes(length)))` carrier；tag `16→16→17→length`、transitive zero requirements 与 single existing negative migration | — | freeze-only：three post-PA62 read-only audits P0=0；RED next；Syntax-only cap ≤32/2；formal TASK 状态不变 | active (development) |
