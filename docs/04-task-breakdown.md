...
| D1-PA-55 | TASK-D1-03 | exact `Option Field bn254_fr` spelling for existing `option(.field)` carrier；tag `16→2`、transitive `fieldBn254` requirement 与 single deferred-negative migration | — | post-PA54 residual audit 选择最小 existing-carrier spelling slice；spec freeze before tests-only RED；GREEN 后运行 focused/aggregate/test binary，按冻结不重复 `just ci` | active (development) |
