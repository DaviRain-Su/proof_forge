...
| D1-PA-87 | TASK-D1-03 | tests-only labeled `DeclarationAcceptance` executable harness for frozen `TST-SRC-004`：exact 16-suite single-run wrapper，保留既有 assertion、suite 相对顺序与 `FrontendParity` 独立执行；零 production change | `0d4b5d31`；implementation pending | pending-prep freeze package 已锁定 exact row/scope/formal-grade doneWhen；本切片只重组 aggregate runner，不新增 declaration/statement/type semantics；focused acceptance + aggregate test binary + docs/diff + independent review；不运行完整 `just ci`/SBOM，不关闭 formal task | frozen (development) |
