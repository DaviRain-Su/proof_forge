...
| D1-PA-94 | TASK-D1-01 | production source-only `SourceQualifiedNameV1` private-constructor carrier；QualifiedId context validation；module/program strict-prefix join；raw component-array encode/decode与 Python fixed oracle | `bf7b3dc0`；implementation pending | 新增 `QualifiedNameV1`≤100、suite≤150；WireCodec/WireDecode/Python 与 registrations authored additions≤80；总 authored≤330（机械 manifest 不计）；固定 1/2/256/257、raw hyphen/open-guillemet、Lean `.str` chain/embedded `.num`、component failure、count-before-child、join prefix/equal/non-prefix 与 fixed wire/roundtrip；禁止 generic caller-selected min-count public API、Common QN validator/render、ProgramV1/theorem/visibility/model/hash/NodeId/alpha/target 改写；focused+aggregate+Python+final single SBOM+docs/diff+independent review；不运行完整 `just ci`，不关闭 formal task | frozen (development) |
