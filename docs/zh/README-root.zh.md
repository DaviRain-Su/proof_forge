...
| `wasm-arbitrum-stylus` | Surface v2 / Legacy adapter → canonical contract → `StylusPlan` → 固定 Rust SDK → Wasm | Research bootstrap | `just stylus-public-route`；生成 Rust crate、Solidity ABI、TypeScript client、中间 Wasm 与 artifact/deploy metadata；在 `cargo stylus check` 和 direct HostIO Wasm differential evidence 完成前不声明 final/deployable |
