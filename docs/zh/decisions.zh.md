...
| D-057 | 2026-07-15 | **接受 Lean/Rust 所有权边界为延期设计；在主三链 cutover 稳定前不开启生产级 Rust 编译后端。** Lean 拥有产品源到 checked Core + CapabilityPlan（以及可选形式化）。Rust 现在可拥有证据 runner，之后可在 dual-run 下可选地拥有 plan/render。只交换版本化制品（artifact JSON、实验性 Core export）；不交换 Lean 对象指针，也不做多链 SDK 巨型二进制。Phase 0（制品契约）可作为文档/harness 卫生推进；Phase 1+ 的 Core export 与 Rust `buildFromCore` 等 cutover 安静后、在隔离分支/worktree 中进行。 | 对齐 Canonical Core / backend-interface 接缝；保全形式化叙事；避免双前端；防止 SDK lock 冲突。规格：[lean-rust-boundary](../superpowers/specs/2026-07-15-lean-rust-boundary-design.md)、[artifact-contract-v1](../superpowers/specs/2026-07-15-artifact-contract-v1.md)、[core-export-v0](../superpowers/specs/2026-07-15-core-export-v0-draft.md)。 |
