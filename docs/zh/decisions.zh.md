...
| D-056 | 2026-07-14 | **Direct Authored/Canonical/target-plan 架构是唯一生产终点；Legacy 只是待删除隔离区，不是兼容层；主三链 direct authoring cutover 必须先于深度 secondary Wasm-host 工作。** 公开路径和已迁移内部路径必须按 `AuthoredContract -> CheckedCanonicalContract -> CapabilityPlan -> target-owned plan` lowering。禁止重新适配回 `ContractSpec` 或 v1 `IR.Module`，禁止 fallback 到 Legacy loader/backend，也禁止同时写入新旧两种表示。将 [PR #104](https://github.com/DaviRain-Su/proof_forge/pull/104) 合入后，才能开启大型 `wasm-stellar-soroban` HostABI/Env/Stellar CLI 或 CosmWasm M3/M4。 | 兼容会长期保留两套所有权模型，并让新工作继续加深旧依赖图。剩余 `Source.Legacy`、`ContractSpec`、v1 `IR.Module` 和 Surface alias 调用方只是显式迁移清单：通过 focused gate 保持行为，将每个调用方直接迁移到新边界，然后删除零调用旧代码。Soroban 深度顺序为 S0 诚实 → S1 去 NEAR 化 HostABI → S2 单一 canonical 路径 → S3 offline crosscall → S4 真实 Env → S5 产品深度（[stellar-soroban.md](../targets/stellar-soroban.md)）。本决议取代 D-052 中将 Legacy adapter 描述为受支持兼容输入的部分。 |
...
| D-058 | 2026-07-15 | **接受 Lean/Rust 所有权边界为延期设计；在没有现成外部库、也没有明确 sourcegen 战略之前，不投资用 Rust 重做主三链机器级 IR 发射（sBPF `.s`、WAT，或平行 Yul 打印机）。** Lean 拥有产品源到 checked Core + CapabilityPlan；产品 codegen 仍在 Lean：EVM → Yul → `solc`；Solana → sBPF AST → `.s` → `sbpf`；NEAR/Wasm-host → Wasm AST → WAT → `wat2wasm`。Rust 可保留已有链 SDK 的执行证据 runner、实验性 Seam A **只读** Core 包 load/inspect/observe sketch，以及 Stylus `rust-sdk` 差分 oracle（D-053）。 | 对齐 Canonical Core / backend-interface 接缝；保全形式化叙事；避免双前端和 SDK lock 冲突。产品路径审计显示没有可插拔的现成 Rust sBPF/WAT lowerer；用 Rust 重做 AST→文本只是重复成本。EVM 已有成熟 handoff（`solc`）。规格：[lean-rust-boundary](../superpowers/specs/2026-07-15-lean-rust-boundary-design.zh.md)（[EN](../superpowers/specs/2026-07-15-lean-rust-boundary-design.md)）、[artifact-contract-v1](../superpowers/specs/2026-07-15-artifact-contract-v1.zh.md)、[core-export-v0](../superpowers/specs/2026-07-15-core-export-v0-draft.zh.md)。PR #105 Seam A 成功仅覆盖 inspect/export；只有在新书面决策说明库栈或有意 sourcegen 后端、dual-run 维度和回滚策略时才重开产品 Rust lower。 |
...
| Wasm 宿主 | `wasm-near`（主路径）、`wasm-cosmwasm` / `wasm-stellar-soroban`（Counter MVP 自定义 bridge）、`wasm-icp-canister`（仅文档候选）；历史 `wasm-cloudflare-workers` 已移除（D-057） | checked Canonical Core → target-owned Wasm-host plan → **`EmitWat`** (Wasm AST → WAT) → `wat2wasm` + 每条链的宿主导入；Rust/CDK 参考实现仅作为差分对比或 bootstrap 工具 (D-031)；Soroban/CosmWasm 深度在 authoring cutover 之后（D-056） |
...
| 源代码生成（历史） | 原 `move-aptos` / `move-sui` 已从 `main` 删除（D-057；归档 `archive/move-cloudflare-2026-07-15`） | 曾为可移植 IR → Move 包源代码 |
...
| Cloudflare Workers 目标（历史，D-057 已移除） | 归档分支 `archive/move-cloudflare-2026-07-15`；见 D-033/D-057 |
...
- D-052 将 Legacy adapter 描述为兼容输入的部分 —— 已被 D-056 取代。Legacy 代码只是待删除迁移清单，不能新增 fallback、adapter-back 或 dual-write 调用方。
