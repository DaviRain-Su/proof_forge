...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（CMP-3 ValueVault） | 先迁移带 CMP-3 证据的 ValueVault，再迁移全部 catalog；持续删除零调用 Legacy 切片 |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | 附着 A-CUT3；先补独立 Solana Rust reference |
