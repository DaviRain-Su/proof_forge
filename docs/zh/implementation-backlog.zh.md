...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ArrayExample direct 完成于 `ccb9221a`；reference 固定于 `b8448961`） | 完成 CMP-3g2 fixed-array VM 证据，再审计并迁移下一个 map/event/crosscall family，不保留兼容 facade |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault、Ownable、Pausable 与 ReentrancyGuard 主三链 VM comparison 均已验证；ArrayExample reference 已在 `b8448961` 固定，CMP-3g2 VM 执行正在进行 |
