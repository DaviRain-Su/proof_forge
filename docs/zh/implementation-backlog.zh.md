...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ValueVault direct 与 native differential 已验证） | 下一步迁移 authorization 代表，然后处理 map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault 主三链 VM comparison 已验证；下一步选择 authorization 代表 |
