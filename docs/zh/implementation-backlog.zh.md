...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ValueVault 与 Ownable 已 direct 且有 native evidence） | 直接重写 Pausable 并删除其 Legacy stdlib 实现/调用方，然后迁移 ReentrancyGuard、map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault 与 Ownable 主三链 VM comparison 已验证；Ownable 在 `ce539dce` 以十步和全部八个 dimension 关闭；Pausable direct cutover 后接入其证据 |
