...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ValueVault 与 Ownable 已 direct；ValueVault native differential 已验证） | 下一步执行 Ownable 原生差分，然后迁移 map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault 主三链 VM comparison 与 Ownable direct compile 切换已验证；下一步执行独立 Ownable reference |
