...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（Pausable direct/VM 证据已完成至 `98e9996f`） | 直接重写 ReentrancyGuard 并删除其 Legacy 实现，再处理 map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault、Ownable 与 Pausable 主三链 VM comparison 均已验证；下一步是 ReentrancyGuard direct cutover 与原生 reference 切片 |
