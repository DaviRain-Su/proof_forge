...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（Pausable direct cutover 已在 `50c1c07a` 完成） | 完成 Pausable 原生证据，然后直接重写 ReentrancyGuard 并删除其 Legacy 实现，再处理 map/collection、event/error 与 portable crosscall family |
...
| CMP-2 | Counter 原生试点：Solidity EVM、Rust Solana、Rust NEAR | done（verified at `e2834c59`） | 以完整主三链 v1 证据关闭 A-CUT2 |
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault 与 Ownable 主三链 VM comparison 已验证；Pausable direct cutover 已在 `50c1c07a` 关闭；CMP-3e1 当前固定独立 Pausable reference，之后才进入 VM 执行 |
...
| CMP-CI | Focused local gate、target-specific live lane 和 fail-closed 生成矩阵 | CMP-3 后 pending | IR-B8/A-CUT5 签署 |
