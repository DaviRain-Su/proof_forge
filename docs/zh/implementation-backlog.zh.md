...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ReentrancyGuard reference 已在 `9772da92` 验证） | 先完成 ReentrancyGuard 主三链 VM 证据，再处理 map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault、Ownable 与 Pausable 主三链 VM comparison 均已验证；ReentrancyGuard 已 direct-only 且 reference 已固定，CMP-3f2 VM 执行正在进行 |
