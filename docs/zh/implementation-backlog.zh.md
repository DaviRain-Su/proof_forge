...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ValueVault 与 Ownable 已 direct；ValueVault native differential 已验证） | 在三个原生 VM 上执行已固定的 Ownable reference，然后迁移 map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault 主三链 VM comparison 已验证；Ownable 的三份 reference 与十步场景已在 `6e1df78b` 固定，evidence 为 `none`；下一步执行 CMP-3d2 |
