...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ReentrancyGuard direct 切换已在 `69499e99` 验证） | 先完成 ReentrancyGuard 独立原生/VM 证据，再处理 map/collection、event/error 与 portable crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault、Ownable 与 Pausable 主三链 VM comparison 均已验证；ReentrancyGuard 已 direct-only，CMP-3f1 原生 reference 正在进行 |
