...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ReentrancyGuard direct/VM 证据已完成至 `b407b493`） | 增加 direct fixed-array authoring，重写 ArrayExample，迁移调用方并删除其 EVM ContractSpec wrapper，再处理后续 map/event/crosscall family |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | ValueVault、Ownable、Pausable 与 ReentrancyGuard 主三链 VM comparison 均已验证；下一步是 ArrayExample fixed-array authoring 与 CMP-3g 证据 |
