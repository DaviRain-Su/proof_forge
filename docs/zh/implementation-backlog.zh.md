...
| A-CUT3 | 迁移 Product/stdlib 调用方并删除其 `Source.Legacy` 依赖 | in_progress（ValueVault 已 direct；CMP-3 native 证据进行中） | 实现并通过 fail-closed 主三链 ValueVault runner，再继续 catalog |
...
| CMP-3 | ValueVault 和代表性 stateful portable family | in_progress | 13 步场景和三份独立 reference 已固定但未语义晋级；实现 native/ProofForge VM comparison |
