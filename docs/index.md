...
分发架构（CLI multi-arch · Author SDK · Host wheel · CI Release）：[`product/05-distribution-and-packages.md`](product/05-distribution-and-packages.md)（`just package-cli` / `package-author-sdk` / `package-host-sdk`；tag `v${VERSION}` → linux-x86_64 + darwin-arm64 + wheels；非 formal Stage-0）。
