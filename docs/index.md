...
分发架构（CLI dist · Lean Author SDK · CI engineering Release）：[`product/05-distribution-and-packages.md`](product/05-distribution-and-packages.md)（`just package-cli` / `package-author-sdk`；tag `v*` / manual `.github/workflows/release-engineering-dist.yml` prerelease/draft assets；非 formal Stage-0）。
