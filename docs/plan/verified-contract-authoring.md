...
- 第十四切首次越过静态边界，但只覆盖 `status` 的 exact MethodIR 子集：
  `MethodSemanticsV1` 定义独立于业务 DSL 的 target recipe machine，执行
  `checkInputLen`、`requireLayout`、`loadState`、`setReturnData`，其余 Operation 一律
  `.unsupportedOperation` fail closed；machine 的 storage/input不可变，只允许 bounded UInt64
  locals 与 return data演进。generic execution theorem从现有 static alignment + initialized KV
  representation推出 exact 8-byte return，使用 shared canonical LE codec roundtrip，而非另造 target
  scalar format；Reference→MethodIR composition theorem再从 sole Reference ready/step 与该真实 target
  execution共同推出 `UInt64ReturnedObservationRelV1`，target success/return/log/promise/storage
  facts不再是调用方提供的 passive premise。capability-scoped theorem还把这一 execution结论接回
...
- `VerifiedVaultPF.deposit(amount)` 现成为第三个 bounded target slice：
  ...
  retained semantic、production `plan.entries[0]` / `ir.methods[1]`、canonical marker/two-field key
  regions及同一次 WAT/ABI emission绑定为单一 proposition。exact recognizer要求一个 8-byte UInt64
  参数、zero-deposit policy、两次 `state + amount` checked add/store及返回更新后的 shares；唯一
  MethodIR/typed-WAT evaluator在同一 storage observation上给出相同 return bytes和两字段 post-state。
  Reference join直接消费既有 returned exact-post theorem，并由 shared UInt64 codec bridge证明
  target checked-add bytes就是 Reference `natToLeBytesV1` bytes。ABI width、nonzero deposit、missing
  layout、first overflow及 second overflow均 fail closed；尤其 second overflow发生在第一条 machine
  store之后，`observeMethodV1` / `observeMethodWATV1`仍只暴露 exact pre-storage，固定 transaction
  rollback边界。没有新增 DSL/business State、Effect、step、lowering、renderer或 evaluator。
- `VerifiedVaultPF.withdraw(amount)` 现成为第四个 bounded target slice：真实 production
  `plan.entries[1]` / `ir.methods[2]` 由 proof-producing recognizer恢复为 exact 双 guard recipe，
  两次 unsigned `amount ≤ state` assert全部位于两次 checked subtraction/store之前，Unit 以自然
  fall-through返回。sole MethodIR lowering与 typed-WAT authority扩展 `i64.sub`、`i64.le_u`和
  `trapIfI64Eqz`，同一 capability carrier绑定 canonical marker/reserves/shares regions及同次
  WAT/ABI emission。成功执行在两级 evaluator产生相同 no-return-data及 two-row subtraction
  post-storage；sole Reference returned theorem现给出 exact canonical
  `natToLeBytesV1 (before - amount) 8` 两行编码，因而与 target checked-sub bytes直接 join。
  first guard failure与“first通过、second失败”均在 capability层证明 MethodIR/typed-WAT trap，
  evaluator-specific错误码虽不同，但 canonical observations exact相等并回滚到 supplied
  pre-storage。没有新增 DSL/business State、Effect、step、Plan/lowering、renderer或 evaluator。

现在已经得到四个 **bounded MethodIR/typed-WAT target slices**：`status()` 的 read-only outcome、
`init()` 的 initialized zero/zero post-storage、`deposit()` 的 checked two-field add及 `withdraw()`
的 guarded checked two-field subtraction均已精确闭合；validated SemanticProgram →
...
recipe semantics。当前只能称 `status()`、`init()`、`deposit()` 与 `withdraw()` 具有 bounded
...
[`ADR-0048`](../adr/0048-optional-solana-sbpf-semantics-provider.md) 已accepted并完成第一接线：
...
`resolveStateCellGetProductionSubjectV1`。`program` elaborator现在从同一次真实elaboration导出
`ElaboratedSourceV1`，而不是在proof lane重新解析UTF-8或维护第二套AST；resolver必须先把该subject
重新送入sole production `validateSourceV1`，再由production canonical encoder重编码并与实际导出的
`.bytes` byte-for-byte绑定。任一module/program identity、declaration validation或canonical bytes
drift均fail closed；成功的`CanonicalSourceBindingV1`携带这些等式与declaration-valid kernel evidence。
绑定后的subject继续沿同一Normalize/compiler→exact Solana profile capability→full-body HandlerIR→
...
source bytes tamper、unsupported、missing与artifact drift仍返回error/false。sound theorem可从任何
kernel-discharge的Boolean恢复exact resolved subject及55步certified carrier。该进展删除了IO与
UTF-8 AST decode作为具体subject的阻塞，但尚未给出该大型计算的无条件kernel equality，所以仍无
...
increment success 70步、overflow 56步）。D5 concrete composition现已覆盖全部四条：四个production
subject都从同一次elaborated source恢复validated Semantic、Reference admission与真实
`stepReferenceSliceV1` outcome；get固定same-state UInt64 return与empty effects，initializer/success保留
logical pre/post state，overflow保留exact pre-state。四个gate分别把get/initializer relation接到各自
55步certificate、successful checked-add relation接到70步certificate、overflow revert/Handler
trap/account snapshot接到56步certificate，并由各自sound theorem实际调用`referenceJoin`返回单一
source-derived Reference→provider carrier。四个sound theorem仍以Boolean checker成功为前提，不能据此
升级为无条件target refinement。
...
| 7 | Per-target refinement | **进行中（NEAR双平台engineering出口已关闭；Solana四条provider certificate与四条concrete composition已闭合）** | NEAR `status/init/deposit/withdraw` 已闭合 selected MethodIR/typed-WAT slices，finalized bytes有 canonical section envelope；rc.1 provider及Darwin GMP/Linux system closure已进入独立 Tool Lock rows。run `31781471216` 的Linux target-smoke与Darwin arm64 locked consumer均成功；结论严格限于identity-bound engineering join，不是一般IR/WAT→Wasm simulation theorem。Solana `get/initialize/increment` 已有kernel Reference→production Plan/HandlerIR join；同一identity-bound production `.s`的四条pinned recipe均有sparse provider certificate：`get`/`initialize(7)`各55步、`increment(41,1)` 70步、overflow 56步。具体subject由同一次`program` elaboration产生AST subject，经sole validator/canonical encoder、production compiler/profile/HandlerIR/emitter/strict parser在纯Lean中fail closed重建。四条recipe现均进一步恢复真实Reference outcome，并分别由Boolean-gated sound theorem组合为source-derived Reference→provider carrier；get固定same-state UInt64 return与account stutter，overflow固定Reference arithmetic revert、provider `0x1001`、账户不变与空return data。尚需无条件kernel-discharge大型production gates，不能据此声明完整provider/ELF/runtime refinement |
...
16. Phase 7 已从 static provenance推进到两级有限 target refinement：production `status` exact
    ...
    **bounded typed-WAT static validator**，检查 i64 constant/local、exact production key annotation、
    ...
    export约束继续来自 canonical Plan validation；证书同样由 complete-module carrier保留。sole
    production renderer的 scratch-local声明现也从完整 Operation tree递归归集，与 if/switch/for body的
    recursive rendering共用同一 typed authority；nested UInt128/256 div/mod及 token transfer不会再产生
...
18. selected `VerifiedVaultPF.deposit(amount)` 已沿同一 authority完成 bounded refinement：真实
    ...
    theorem及 shared codec law join。wrong-width/deposit/layout/first-overflow和 late second-overflow
    failure均固定 no-commit observation，后者明确覆盖第一条 machine store已发生但不可逃逸的 rollback；
19. selected `VerifiedVaultPF.withdraw(amount)` 已沿同一 authority完成 bounded refinement：真实
    production entry 1 / MethodIR 2、exact双 guard-before-write shape及同次 WAT/ABI emission已由
    capability carrier闭合；MethodIR/typed-WAT evaluator成功路径给出相同 Unit fall-through与两行
    checked-sub post-storage，sole Reference theorem则给出同一 canonical subtraction bytes。
    first/second guard failure在 public capability theorem中分别导出两级 trap与 exact canonical
    observation agreement，且 observation post-storage均为 supplied pre-state。下一步仍需
...
32. Solana StateCell的production `.s`已通过strict parser接到exact-pinned sBPF provider；四条pinned
    ...
    漂移均fail closed。get、initializer、increment success与increment overflow又分别从同一次
    source-derived Semantic执行真实Reference step，并将其observation relation接到对应55/55/70/56步
    certificate的`referenceJoin`，形成单一Boolean-gated Reference→provider carrier；get固定same-state
    UInt64 return、empty effects与account stutter，overflow固定Reference arithmetic revert与exact
    pre-state、provider `0x1001`、账户不变及empty return。所有production
    checker的kernel theorem仍以Boolean成功为前提，尚未把大型具体gate归约为无条件
