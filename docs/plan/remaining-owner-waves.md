...
Solana local OutputSet/ELF identity join and caller Plan retention are green;
...
| **ADR-0053** | **Accepted 2026-08-21**；Wave 1–4 + Solana local-output follow-on engineering boundary 已落地 | EVM static identity 不冒充 code-at-address；Solana local identity 不冒充 ProgramData/current ELF；CW identity 与 unsupported Solana shapes FC | Read runtime green as deployment/on-chain code proof/C-3/formal/release; close all B-CALL-SEM |
...
repeatable `--callee-output` directories must be fully inspected deployable Solana
OutputSets. Every bind row carries all three digests and exact-joins callee program
name + manifest source/semantic + raw `{program}.so` SHA-256. The verified identity
is retained on the caller Plan's `callBindProgram` role. Runtime still checks only
expected programId/account key/executable/privileges, so this is not ProgramData,
upgrade-authority, deployment, or on-chain ELF identity evidence.

Open remainder: resolver support keys must not be read as
“cross-platform call done.” EVM on-chain code-at-address, Solana ProgramData/current
ELF identity, CosmWasm identity, target inspect residual, EVM/CW no-table stubs and all unsupported
...
4. **Solana product CPI** — Wave 3 closes the exact outer AccountInfo join only for its stated product subset. Explicit local OutputSet/ELF identity is exact-joined and retained in caller Plan; ProgramData/on-chain code identity, async, generic result-bearing, empty-row/state, multi-callee, and mixed frozen-site stay FC/open.
...
Solana ProgramData/on-chain code identity 或已列 Solana unsupported shape 中经 owner
决策一次选一项；不得把 Wave 4 或 Solana local artifact evidence 写成链上 deployment/code
join，也不能据支持子集声称 cross-platform call complete。Does not close formal D2/D4 or C-3.
