...
> Solana local OutputSet/raw `.so` identity exact join 与 caller Plan retention 也已接线。
> EVM code-at-address、Solana 链上 ProgramData/current ELF、CW identity 与 unsupported
> shape 仍 FC/open。
...
| Solana | Wave 3 支持子集、local OutputSet/ELF identity/Plan retention 与 Token/ATA package-owned binding 已闭合；扩大前先决策 | ProgramData/on-chain code identity、schedule、generic result-bearing、empty-row/state、multi-callee、mixed-site 保持 FC；Mollusk 不进 ordinary `just ci` |
...
unsupported shape、Solana ProgramData/current ELF、CW identity、target inspect static residual
与其它 target family 边界继续 open/FC。Solana local OutputSet/ELF identity 与 Token/ATA
package-owned binding 经代码/规格/磁盘门对账已闭，
