...
| **COMP-1-CALL-SEM-LAND** | 先改 resolver/文档/针，再按 COMP-0 做 EVM 部署地址、Solana 外层账户、CW `contract_addr` | COMP-0-CALL-SEM 人拍 | **partial** — 十三 kind inspect 表面针（human+JSON）+ residual 标签 + ADR-0053 Wave 2 三叶 emit（有表无行 FC）+ Wave 2a 空账户 void CALL FC + ExtFlow/LaterFlow xrpl 针；Wave 2b Solana accounts 仍 pending |
