...
> 已接线；Solana 本地 callee output/ELF identity exact join + caller Plan retention 也已
> 接线。CosmWasm identity、EVM deployment proof、Solana on-chain ProgramData/code
> identity 与 unsupported shape 仍 FC/open。
...
|---|---|---|
| EVM | opt-in pre-placed address + local output/runtime identity 已闭合；下一刀为 deployment proof 或 Bool/Int/Bytes returndata 决策 | 无表 hashed QN stub 保持 PARTIAL；C-3 保持 FC |
| Solana | Wave 3 exact callee/outer AccountInfo join + local output/ELF identity/Plan retention 已闭合；扩大前先决策 | ProgramData/on-chain code identity、schedule、generic result-bearing、empty-row/state、multi-callee、mixed-site 保持 FC；active classic Token/ATA catalog 已固定 ELF；Mollusk 不进 ordinary `just ci` |
...
| **COMP-1-CALL-SEM-LAND** | versioned bind table、三叶 emit 与 Solana 支持子集 outer join；保留各族明确 FC | COMP-0-CALL-SEM 人拍 | **partial** — Wave 1/2/2a/2b/2c + Wave 3 engineering done；EVM local-output identity follow-on done：EVM/CW 精确 endpoint、EVM 显式 output/runtime digest join + CALL-time code hash；Solana local-output identity follow-on done：state-bearing single-callee void-call 的 1..8 rows + executable program outer join、local output/ELF 三 digest exact join + caller Plan retention、program residual 清零与 Mollusk 闭环；EVM deployment proof、Solana on-chain ProgramData/code identity、CW identity、Solana unsupported shapes、target inspect static residual、其它 target families、formal/C-3 仍开 |
