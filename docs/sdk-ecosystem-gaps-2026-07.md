...
| ERC-2612 (permit) | Atomic implementation; compliance remains `experimental` pending requirement-bound evidence | TokenSpec emits canonical `permit(address,address,uint256,uint256,uint8,bytes32,bytes32)`, nonce/domain/deadline checks, low-s/v enforcement, and Approval atomically. `just product-erc20-permit` executes valid, replay, expired, bad-domain, high-s, invalid-v, staging/front-run, and domain-overwrite cases in Foundry | Bind the passing artifact/result digests to the ERC-2612 manifest |
