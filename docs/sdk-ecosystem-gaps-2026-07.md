...
| Pausable | Covered (limited) | `Examples/Product/Pausable.lean` is the sole direct Authored source; `just pausable-authoring-cutover` proves checked Core plus EVM, Solana, NEAR, and shared Soroban Wasm-host planning, while `Examples/Backend/Evm/Contracts/stdlib/Pausable.golden.yul` retains the reviewed EVM output. The retired Legacy stdlib implementation and EVM wrapper are deleted. **Limitation:** pause/unpause have no built-in owner/role auth (compose a later direct Ownable/AccessControl policy) | P1 |
