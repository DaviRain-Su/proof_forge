...
| Bytes N state | **LOWERED** | N=1..8 UInt64 叶存低 8 位；IndexGet/Set 复用 Array 字面量下标；param / entry return / N=0 仍 FC |
...
| Bytes N **view** 叶返回 | **LOWERED** | T8b：N 个 UInt64 低 8 位叶元组；entry Bytes 仍 FC |
...
| irreducible CFG、Field/String | **FAIL-CLOSED** | Principal / Bytes **entry** / Map return 仍 FC |
...
|---|---|---|---|
| **Soroban** | sole `soroban-source-u64-v1` | public UInt64/Bool/Unit Counter/StateCell `.rs`；4-key；zero-tool Finalize `deployable=false`；**CAP-3** `unixTimeSeconds`/`blockHeight`→`env.ledger()`；**CAP-4** `pf.crypto.sha256`→`env.crypto().sha256`（UInt256 LE plumbing）；T3 scalar const 内联 + Bytes N（N UInt64 低 8 位叶）；**T4 Principal 9 叶 identity**（`owner_len`+`w0..w7`，≠ Address）；**T5 named Struct/Enum 叶 flatten**（`p_x`/`p_y`、MaybeMark tag+payload；`symbol_short` 只卡 state 叶）；**T5-Option / T8a-Map 状态**（Lower 已有 Option UInt64 2 叶 + Map UInt64 cap-8；§1d 此前漏记）；**T6 Array/Option/named view 叶返回**（Rust `(u64, …)` 元组）；**T7 entry 叶返回**（同构元组；TON 仍 FC）；**T8b Bytes N view 叶返回**；**T9a/T9b if-diamond + `switchOn`**（Rust if / if-chain；臂内 store）；**T9c `forLoop`**（counted Rust `loop` + bound `panic!`） | auth/TTL/Wasm/stellar-cli；UInt64 ContextRead 四键 named no-host；`pf.crypto.*` / nativeVaultBalance named no-host；不可约 CFG / 臂内 call·emit；Principal / Bytes **entry** / Map return / `self`/`caller` 仍 FC |
| **OpenVM** | default `openvm-guest-source-v1`；opt-in `openvm-guest-elf-v1` | 受控 guest tree（O0 zero-tool）；O1 locked `cargo-openvm` 2.0.1 → ELF+`.vmexe` extras 仍 `deployable=false`；T3 scalar const 内联 + Bytes N；**T4 Principal 9 叶 identity**（≠ host address）；**T5 named Struct/Enum 叶 flatten**；**T5-Option / T8a-Map 状态**（Lower 已有 Option UInt64 2 叶 + Map UInt64 cap-8；§1d 此前漏记）；**T6 Array/Option/named view 叶返回**（guest `(u64, …)` 连续写出；不新开 prove/IO ABI）；**T7 entry 叶返回**（同一 N 叶通道）；**T8b Bytes N view 叶返回**；**T9a/T9b if-diamond + `switchOn`**（guest if / if-chain；臂内 store）；**T9c `forLoop`**（counted guest `loop` + bound `Err`） | keygen/execute/prove/verify；UInt64 ContextRead 四键 / `pf.crypto.*` / nativeVaultBalance named no-host；不可约 CFG / 臂内 call·emit；Principal result/`self` / Bytes **entry** / Map return 仍 FC；caller 仅 init/entry |
...
| Target | Profile | 已开 | 诚实 FC / 非声称 |
|---|---|---|---|
| **XRPL** | default `xrpl-bedrock-source-u64-v1`；opt-in `xrpl-bedrock-wasm-u64-v1` | public UInt64/Bool/Unit Counter/StateCell Bedrock-shaped `{name}.rs`；**const 已 inline**（UInt64/Bool `Op.Constant`）；**T3 Bytes N / Array UInt64 N** state flatten 为 N 个 UInt64 低 8 位叶（`name_0`…，N∈1..8，字面量下标）；精确 4-key（`failure.atomic-rollback` / `state.persistent` / `value.bool` / `value.checked-arithmetic`）；两 profile 共享同一 Plan/IR/base `.rs`；默认 Finalize 零 extras、`deployable=false`；Q1 经 ambient rustc/cargo 在临时 crate 包装 `.rs`，产出 extra `xrpl-build/{program}.wasm`（craft `xrpl-wasm-std` rev `ffbe88da…`，triple `wasm32-unknown-unknown`）；缺 rustc/cargo/wasm target → `PF-TOOLCHAIN-MISSING`；**T4 Principal 9 叶** identity（`len+w0..w7` storage/param/`==`/`!=`，**不是** AccountID）；**T5 named Struct/Enum** 叶 flatten（`p_x`/`p_y`、tag+payload）；**T5-Option** UInt64 2 叶（`name_tag`/`name_p0`）；**T8a-Map** UInt64 cap-8（24 occ/key/val 叶；IndexGet→Option；第 9 次 insert overflow）；**T6 view 叶返回** Rust `(u64,…)` 元组；**T7 entry 叶返回**（同构元组）；**T8b Bytes N view 叶返回**；**T9a/T9b if-diamond + `switchOn`**（朴素 Rust `if` / if-chain；臂内 store）；**T9c `forLoop`**（counted Rust `loop` + bound `Err`） | 不可约 CFG / 臂内 call·emit·revert **仍 FC**；Principal **result / self / caller** **仍 FC**；Bytes **entry** / N=0 / N>8 **仍 FC**；Option/Map **param**、Map **return**、非 UInt64 Option/Map payload **仍 FC**；ContextRead / `pf.crypto.*` / call / schedule / invariant **named FC**（ADR-0052：TIME/CALLER 符号已冻、SHA keep-FC；**叶未开**）；**不**开 CAP-1a/1b/3/4 或 XRPL CAP 叶；**不是** AlphaNet / `ContractCreate` / bedrock CLI / 主网；**不是** Tool Lock rustc；不可部署 |
