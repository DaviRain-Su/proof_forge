...
| stateLoad/stateStore（String） | LOWERED(N4: 9-leaf identity，非 UTF-8 ABI) | LOWERED(B-RET-STR: 同构 9 叶 identity，非 UTF-8 ABI) | LOWERED(B-RET-STR: 同构 9 叶 identity，非 account-id) | LOWERED(B-RET-STR: 同构 9 叶 identity，非 Field) | LOWERED(N4 / PSY-SCALAR-ABI: 9 叶 identity，非 address) | LOWERED(B-RET-STR: 同构 9 叶 identity，非 address) |
...
| constant（`Op.Constant`） | **LOWERED(T3 scalar + B-CONST-STR：String 9 叶 inline；Principal/named aggregate const FC)** | **LOWERED(T3 + B-CONST-STR：String 9 叶 inline；`Cpi*IR` 空表门仍在；Principal const FC)** | **LOWERED(NEAR const slot + B-CONST-STR 9 叶；Principal/UInt128 const FC)** | **LOWERED(T3 + B-CONST-STR：String 9 叶 inline；Principal const FC)** | **LOWERED(UInt8/16/32、Bool、UInt64<p、非负 Int64 + B-CONST-STR 9 叶；显式 VM profile 另开 UInt128 canonical 16B→4×UInt32 limbs；Goldilocks ConstantV1 复用 decoder，source 无 Field literal；负 Int64/UInt64≥p / Principal const FC)** | **LOWERED(ALEO-CONST + B-CONST-STR：`lowerLiteral`/Constant 9 叶 inline；Principal const FC)** |
...
| **match String scrutinee** | LOWERED(N-A1 / N-A1-COPY：9 叶 leaf-wise eq + nested if；Plan `switchOn` 仍仅 UInt64) | LOWERED(N-A1-COPY：同构 9 叶 desugar；非 String aggregate switch FC) | LOWERED(N-A1-COPY：同构 9 叶 desugar；非 String aggregate switch FC) | LOWERED(N-A1-COPY：同构 9 叶 desugar；非 String aggregate switch FC) | LOWERED(N-A1-COPY：同构 9 叶 desugar；非 String aggregate switch FC) | LOWERED(N-A1-COPY：同构 9 叶 desugar；非 String aggregate switch FC) |
...
| **anonymous Array/Map/Option/Bytes result（N-ANON-RESULT）** | LOWERED(Array UInt64/Int64 N≤8 / Option UInt64/Int64 / Bytes N / **B-RET-PRIN** Principal 9 / **B-RET-STR** String 9；Map 仍 FC，hashed 1-slot) | LOWERED(Array UInt64/Int64 N≤8 / Option UInt64/Int64 / Map 24 叶含 Int64 val / Bytes N / **B-RET-PRIN** Principal 9 / **B-RET-STR** String 9；nested FC) | LOWERED(Array UInt64/Int64 N≤8 / Option UInt64/Int64 / Map 24 叶含 Int64 val / Bytes N / **B-RET-PRIN** Principal 9 / **B-RET-STR** String 9；nested FC) | LOWERED(Array UInt64/Int64 N≤8 / Option UInt64/Int64 / Map 24 叶含 Int64 val / Bytes N / **B-RET-PRIN** Principal 9 / **B-RET-STR** String 9；nested FC) | LOWERED(Array UInt64/Int64 N≤8 / Option UInt64/Int64 / Map 24 叶含 Int64 val / Bytes N / **B-RET-PRIN** Principal 9 / **B-RET-STR** String 9；nested FC) | LOWERED(Array UInt64/Int64 N≤8 / Option UInt64/Int64 / Bytes N / Map cap-2 6 叶 UInt64 或 Int64 val / **B-RET-PRIN** Principal 9 / **B-RET-STR** String 9；Int64-key / nested / Map-of-String FC) |
| **match 多臂同构造器** | LOWERED(N-A2) | LOWERED | LOWERED | LOWERED | LOWERED | LOWERED |
| **Principal state/params** | LOWERED(T10: leaf storage; ≠address；**B-RET-PRIN** 9-leaf result) | LOWERED(T12: 9×u64 leaves; ≠32B pubkey；**B-RET-PRIN** 9-leaf result) | LOWERED(T12: 9×KV leaves; ≠account-id；**B-RET-PRIN** 9-leaf result) | LOWERED(T12: 9×u64 inputs; ≠Field；**B-RET-PRIN** 9-leaf result) | LOWERED(9-leaf wire identity；≠network address；**B-RET-PRIN** Principal/String result) | LOWERED(T3: 9×u64 identity leaves；≠address/Field；B-RET-PRIN 9-leaf result) |
...
| 面 | 状态 | 说明 |
|---|---|---|
| public UInt8/16/32/64 state / param / result | **LOWERED** | KV → `env.db_*`；narrow physical slot 仍 8B LE并校验高位；UInt128 2-limb / UInt256 4-limb 已 LOWERED；窄 Int 已 LOWERED |
| checked + − × / % · unary · shift/bitwise/logical · eq/order | **LOWERED** | UInt64 + narrow UInt body guards |
| if / match / bounded for / pureFn | **LOWERED** | 多块 CFG；match 多臂同构造器随 Normalize；**N-A1-COPY** String 9 叶 desugar；Int induction FC |
...
| 面 | 状态 | 说明 |
|---|---|---|
| public UInt8/16/32/64 c4 state / param / result | **LOWERED** | exact cell bit widths + int257 range guards；UInt128/256 单 cell + 窄 Int 已 LOWERED |
| checked 算术 · unary · shift/bitwise/logical · eq/order | **LOWERED** | TVM int257 上显式 declared-width 范围检查 |
| if / match / bounded for / pureFn | **LOWERED** | **N-A1-COPY** String match（entry 可 match，不是 entry 多叶 return）；Int induction 与 aggregate pureFn FC |
...
| anonymous Array/Option/Map view result | **LOWERED** | `Array UInt64/Int64 N`(1..8) / `Option UInt64/Int64` / Bytes N / **B-RET-MAP** `Map UInt64 UInt64` 或 `Map UInt64 Int64` 24 叶 view / **B-RET-STR** String **view** 9 叶；entry 聚合、nested/Int8/Int64-key 仍 FC |
...
## 1c. Quint Q0 executable-model 真实范围（十三个 materializer 之一）
...
| Array/Option/Map state（齐次 UInt64 或 Int64） | **LOWERED** | N=1..8 Array flatten；Option tag+payload；Map cap-8 occ/key/val；元素/payload/key/value 跟随程序级 signedNumeric；无原生 List/Option/Map；错域混用 / N∉1..8 仍 FC |
| Bytes N state | **LOWERED** | N=1..8 UInt64 叶存低 8 位；IndexGet/Set 复用 Array 字面量下标；Bytes **param** N∈1..8 已 flatten（L3）；N=0/N>8 仍 FC |
| scalar `Op.Constant` | **LOWERED** | 经已有 `lowerLiteral`（UInt64/Int64/UInt32/Bool）+ **B-CONST-STR** String 9 叶 inline；named aggregate / Principal const 仍 FC |
| Principal/String state/params | **LOWERED** | T4：9×UInt64 identity 叶（`owner_len`+`w0..w7`）；pf.assets `transfer` 仍按 `.principal` 在 call site 重装；**B-RET-PRIN / B-RET-STR** 9 叶 result 已开；`context.caller`/`self` 仍 FC |
| named Struct/Enum state | **LOWERED** | T5：`p_x`/`p_y` 与 MaybeMark tag+payload 叶 flatten；named/Option/Array/Bytes **param** 已 flatten（L2–L5） |
| Array/Option/named **view** 叶返回 | **LOWERED** | T6：view-only 元组/多 `int`（无原生 List/Option） |
| Bytes N **view+entry** 叶返回 | **LOWERED** | T8b：N 个 UInt64 低 8 位叶元组；N=0/N>8 仍 FC |
| Array/Option/named **entry** 叶返回 | **LOWERED** | T7：与 view 同构的 N 叶元组；TON 仍 FC（async actor 无返回通道） |
| Map UInt64 **entry/view** 24 叶返回 | **LOWERED** | B-RET-MAP：24 occ/key/val 叶元组，不是把 8 叶 cap 改成 24；Map **param** 24 只读叶已 LOWERED（L6）；非域 / 嵌套仍 FC |
| if / match（`Term.branch` / `Term.switch`） | **LOWERED** | T9a/T9b：if-diamond + `switchOn`（整型字面 + Option/Enum tag）+ **N-A1-COPY** String 9 叶 leaf-wise eq + nested if（Plan `switchOn` 仍仅 UInt64）；提前 return 按区域 store；`.qnt` flatten 为 `ite`；缺 default / 非 String aggregate switch / 臂内 call·emit·revert 仍 FC |
| bounded-for（`loopBounds` exact coverage） | **LOWERED** | T9c：`forLoop` + 计数 trap（`.qnt` `for`，非无限 `while`；非 Noir 式展开）；不可约 CFG / 非 loop phi / 臂内 call·emit 仍 FC |
| irreducible CFG、Field | **FAIL-CLOSED** | String state/param/return 已是 9 叶 identity（非 UTF-8 ABI）；Field 仍 FC |
...
|---|---|---|---|
| **Soroban** | sole `soroban-source-u64-v1` | public UInt64/Bool/Unit Counter/StateCell `.rs`；4-key；zero-tool Finalize `deployable=false`；**CAP-3** `unixTimeSeconds`/`blockHeight`→`env.ledger()`；**CAP-4** `pf.crypto.sha256`→`env.crypto().sha256`（UInt256 LE plumbing）；T3 scalar const 内联 + **B-CONST-STR** String 9 叶 + Bytes N（N UInt64 低 8 位叶）；**T4 Principal 9 叶 identity**（`owner_len`+`w0..w7`，≠ Address）；**T5 named Struct/Enum 叶 flatten**（`p_x`/`p_y`、MaybeMark tag+payload；`symbol_short` 只卡 state 叶）；**T5-Option / T8a-Map 状态**（Lower 已有 Option UInt64 2 叶 + Map UInt64 cap-8；§1d 此前漏记）；**T6 Array/Option/named view 叶返回**（Rust `(u64, …)` 元组）；**T7 entry 叶返回**（同构元组；TON 仍 FC）；**T8a-Map B-RET-MAP** entry/view 24 叶返回；**T8b Bytes N view+entry 叶返回**；**T9a/T9b if-diamond + `switchOn`**（Rust if / if-chain；臂内 store）；**T9c `forLoop`**（counted Rust `loop` + bound `panic!`） | auth/TTL/Wasm/stellar-cli；UInt64 ContextRead 四键 named no-host；`pf.crypto.*` / nativeVaultBalance named no-host；不可约 CFG / 臂内 call·emit；**B-RET-PRIN / B-RET-STR** Principal/String 9 叶 result 已开（≠ Address / 非 UTF-8 ABI）；`self` / `caller` 仍 FC；named/Option/Array/Map/Bytes param 已 flatten |
| **OpenVM** | default `openvm-guest-source-v1`；opt-in `openvm-guest-elf-v1` | 受控 guest tree（O0 zero-tool）；O1 locked `cargo-openvm` 2.0.1 → ELF+`.vmexe` extras 仍 `deployable=false`；T3 scalar const 内联 + **B-CONST-STR** String 9 叶 + Bytes N；**T4 Principal 9 叶 identity**（≠ host address）；**T5 named Struct/Enum 叶 flatten**；**T5-Option / T8a-Map 状态**（Lower 已有 Option UInt64 2 叶 + Map UInt64 cap-8；§1d 此前漏记）；**T6 Array/Option/named view 叶返回**（guest `(u64, …)` 连续写出；不新开 prove/IO ABI）；**T7 entry 叶返回**（同一 N 叶通道）；**T8a-Map B-RET-MAP** entry/view 24 叶返回；**T8b Bytes N view+entry 叶返回**；**T9a/T9b if-diamond + `switchOn`**（guest if / if-chain；臂内 store）；**T9c `forLoop`**（counted guest `loop` + bound `Err`） | keygen/execute/prove/verify；UInt64 ContextRead 四键 / `pf.crypto.*` / nativeVaultBalance named no-host；不可约 CFG / 臂内 call·emit；**B-RET-PRIN / B-RET-STR** Principal/String 9 叶 result 已开（≠ host address / 非 UTF-8 ABI）；`self` 仍 FC；named/Option/Array/Map/Bytes param 已 flatten；caller 仅 init/entry |
| **ICP** | sole `icp-wasm-candid-u64-v1` | Counter/StateCell `.wat`+`.did`；locked wat2wasm `{name}.wasm` `deployable=true`；host-optional PocketIC；**CAP-1a** `unixTimeSeconds`→`ic0.time` ns÷10⁹（init/update/query）；**CAP-1b** `caller`→`ic0.msg_caller_size/copy`（ADR-0025 Principal identity；query/view FC）；T3 scalar const 内联 + **B-CONST-STR** String 9 叶 + Bytes N（N extra i64 globals，无 Candid `vec nat8`）；**T4 Principal 9 叶 identity**（9 extra i64 globals，无 Candid `principal`）；**T5 Option UInt64 或齐次 Int64 2 叶 identity**（`o_tag`/`o_p0` extra i64 globals，无 Candid `opt`；混合 UInt64-param OptInt 仍 FC）；**T5 named Struct/Enum 叶 flatten**（extra i64 globals，无 Candid `record`/`variant`）；**T6 view 叶返回 = Candid 位置元组**（`.did` `-> (nat64, nat64) query`；WAT 连续 encode 多个 LE i64；**禁止** `record`/`opt`/`vec`/`variant`）；**T7 entry 叶返回**（同构位置元组 update，非 query）；**T8a Map UInt64 UInt64 或齐次 Map Int64 Int64 cap-8**（24 extra i64 globals，ite/bool mux + assert，无 Candid `map`/`vec`；**B-RET-MAP** entry/view 24 叶位置元组；混合 Map UInt64 Int64 仍 FC）；**T8b Bytes N view+entry 叶返回**（位置元组，无 Candid `vec nat8`）；**T9a/T9b if-diamond + `switchOn`**（Wasm `if` / if-chain；臂内 store；无 Candid `variant` match ABI）；**T9c `forLoop`**（counted Wasm `loop`/`br` + bound `unreachable`） | PocketIC 不进 Finalize；sync+event FC；async advertise-only；`blockHeight`/`attachedValue`/`chainId` / `pf.crypto.*` / nativeVaultBalance named no-host；不可约 CFG / 臂内 call·emit；**B-RET-PRIN / B-RET-STR** Principal/String 9 叶 result 已开（Candid 位置 9-元组，**无** Candid `principal`/`text`）；`self` 仍 FC；named/Option/Array/Map/Bytes param 已 flatten；`caller` 仅 init/entry（CAP-1b） |
...
| Target | Profile | 已开 | 诚实 FC / 非声称 |
|---|---|---|---|
| **XRPL** | default `xrpl-bedrock-source-u64-v1`；opt-in `xrpl-bedrock-wasm-u64-v1` | public UInt64/Bool/Unit Counter/StateCell Bedrock-shaped `{name}.rs`；**齐次 `signedNumeric` Int64**（全程序 UInt64 **或** Int64，混用 FC；Option/Map/Array 元跟域；`get_data`/`set_data` 仍为 8 字节叶）；**const 已 inline**（UInt64/Int64/Bool + **B-CONST-STR** String 9 叶；Principal const FC）；**T3 Bytes N / Array UInt64/Int64 N** state flatten 为 N 个 8 字节叶（`name_0`…，N∈1..8，字面量下标）；精确 4-key（`failure.atomic-rollback` / `state.persistent` / `value.bool` / `value.checked-arithmetic`）；两 profile 共享同一 Plan/IR/base `.rs`；默认 Finalize 零 extras、`deployable=false`；Q1 经 ambient rustc/cargo 在临时 crate 包装 `.rs`，产出 extra `xrpl-build/{program}.wasm`（craft `xrpl-wasm-std` rev `ffbe88da…`，triple `wasm32-unknown-unknown`）；缺 rustc/cargo/wasm target → `PF-TOOLCHAIN-MISSING`；**T4 Principal/String 9 叶** identity（`len+w0..w7` storage/param/`==`/`!=`，**不是** AccountID / 非 UTF-8 ABI）；**B-RET-PRIN / B-RET-STR** 9 叶 result；**T5 named Struct/Enum** 叶 flatten（`p_x`/`p_y`、tag+payload）；**T5-Option** 2 叶（`name_tag`/`name_p0`，payload 跟域）；**T8a-Map** cap-8（24 occ/key/val 叶；IndexGet→Option；第 9 次 insert overflow；key/val 跟域；**B-RET-MAP** entry/view 24 叶返回）；**T6 view 叶返回** Rust `(u64,…)` 元组（named / Array N / Option / Bytes / Map 24）；**T7 entry 叶返回**（同构元组）；**T8b Bytes N view+entry 叶返回**；**T9a/T9b if-diamond + `switchOn`**（朴素 Rust `if` / if-chain；**N-A1-COPY** String 9 叶 leaf-wise eq + nested if；臂内 store）；**T9c `forLoop`**（counted Rust `loop` + bound `Err`） | 不可约 CFG / 臂内 call·emit·revert **仍 FC**；**B-RET-PRIN** Principal 9 叶 result 已开（**不是** AccountID）；`self` / `caller` **仍 FC**；Bytes N=0 / N>8 **仍 FC**；named/Option/Array/Map/Bytes param 已 flatten；窄 Int/Field **仍 FC**；String 为 9 叶 identity（非 UTF-8 ABI）；ContextRead / `pf.crypto.*` / call / schedule / invariant **named FC**（ADR-0052：TIME/CALLER 符号已冻、SHA keep-FC；**叶未开**）；**不**开 CAP-1a/1b/3/4 或 XRPL CAP 叶；**不是** AlphaNet / `ContractCreate` / bedrock CLI / 主网；**不是** Tool Lock rustc；不可部署 |
...
|---|---|---|---|---|
| **N-A1** | String match-switch | **已闭合(EvmStringMatch + N-A1-COPY)**：十三家 Lower 将 `match String` desugar 为 9 叶 leaf-wise eq + nested ifThenElse（Plan `switchOn` 仍仅 UInt64 case）；catch-all fallthrough；非 String aggregate switch 与非 String pattern 仍 fail-closed。不是 UTF-8 ABI | 十三 materializer | EvmStringMatch ✅ + N-A1-COPY ✅ |
...
| **N-A3** | Map/Bytes 穿透元素赋值 | **已闭合(MapBytesAssign)**：TypeCheck/Normalize 单步 `m[k]:=v`/`b[i]:=u8` → IndexSet（load→set→store）；Reference 已有 Map/Bytes step。target 覆盖非均匀：EVM Map+Bytes、Solana Map、NEAR Map 与 fixed Bytes state、Noir Map、Aleo Map+Bytes 已部分 LOWERED，Psy 与其余组合按上表 FAIL-CLOSED；五个 Map-capable target 的 aggregate StateStore snapshot hazard 已修。**Map 整程序 Reference admission（2026-08-07）** 已切 per-canonical-value/helper Wire envelope（无 sampled packing/第二 runtime capacity；empty state default exact 4B；whole-step cumulative work 仍 residual）；**MiniAmm `Map Principal UInt64` Normalize→admit 已通**（runtime upsert 仍 Wire 实际 limits）；**Map 嵌套穿透** `m[k].x:=v` 已开（N-NEST-IDX）；**Bytes 嵌套穿透** 仍 TypeCheck FC（UInt8 叶） | 全 target Normalize；target 见 op 表 | MapBytesAssign + B-SOL-MAP-UPSERT ✅ |
| **N-A4** | Option state | **shared 闭合**：Normalize+Reference default none；十三 materializer 均 Admit `Option UInt64` 2 叶 tag+payload state（T5 补 ICP：两个 extra i64 global，无 Candid `opt`）；**Option UInt64/Int64 param 已 flatten**（十三家 Focus `testOptionParam`：`o_tag`/`o_p0`）；Option-of-String / nested Option 仍 FC；**T8a** 已开 ICP Map UInt64 cap-8（24 i64 globals） | 全 target | OptionState ✅ + B-OPT-STATE ✅ + T5 |
...
| **B-1c** | Aleo 全功能 | **AleoCoverage + H3/NS-1/Bytes/Int64/T14 + G123 + BL-35 + ALEO-I1–I4 + ALEO-INT64-CONTAINERS + ALEO-MAP-PARAM-RET**：标量、named Struct/Enum、Array UInt64 **or Int64**、dense Map cap-2（UInt64 **or Int64** val，6 叶 state/**param/return**）、fixed Bytes N、**Option UInt64 or Int64 state**、Commit 身份透传与 **BLS12-377 Fr** 已 LOWERED；Map aggregate StateStore 以 get-all-before-set two-phase 修复 empty upsert。Plan content digest 已进入 identity chain；产品另发 content-bound query-contract sidecar（bare mappings/views/resultDropped，非 executable query）。bn254/Goldilocks、Int8 containers/Int64-key/nested/Option-of-String/ContextRead/externalCall/schedule/emit/pf.assets 仍 FAIL-CLOSED；named/Option/Array/Bytes/Map param 已 flatten（Map = 6 叶，不是 24 叶 B-RET-MAP）。Principal/String 9-leaf wire identity（≠address / 非 UTF-8 ABI）已 LOWERED；**B-RET-PRIN / B-RET-STR** 9 叶 result 已开；caller→address 仍 FC。Leo 4.0.2 两平台 Tool Lock + locked-only acceptance + opt-in product compile finalization；无 VM/prove/deploy 门 | AleoCoverage + T14 + MapSnapshot + G123 + B-OPT-STATE + ALEO-I1–I4 ✅ |
| **B-1d** | Solana Map/Bytes/Option state | **Map pilot + L2 + BL-29 已闭合**：Map 已进 ELF+Mollusk；named Struct/Enum、fixed Bytes N 与 `Option UInt64` state 已 flatten；`storeAggregate` structural CSE + `storeStateMulti` 固定 pre-store snapshot，峰值 177 temp/1424B，`put_into_empty` 已解除 ignore；Option state 6 项 Mollusk 通过；Option/Array/Map **param** 已 flatten（Map = 24 occ/key/val）；**B-RET-MAP** Map UInt64 UInt64/Int64 entry/view 24 叶返回（不是把 8 叶 cap 改成 24）；**B-RET-PRIN** Principal 9 叶 result 已开（≠ pubkey）；非 UInt64-key / nested / Principal-keyed Map return 仍 FAIL-CLOSED | SolanaMapPilot + B-SOL-MAP-ELF + B-SOL-MAP-UPSERT + L2 + BL-29 ✅ |
| **B-1e** | EVM Map/Bytes/Option state | **闭合(Map pilot + BL-31)**：Array + Bytes + **Map UInt64 cap-8** + `Option UInt64` state 进入 locked-solc engineering finalization（`deployable=true` 仅为制品标志；Token creation bytecode 现约 2.6 KiB，已低于 EIP-3860；chain/Anvil/OZ 产品门另计）；aggregate `storeAtomic` 保证 leaf Expr/sload 全先于 sstore；Option/Array/Bytes param 已 flatten；Map param 仍 FC（hashed 1-slot，不发明 24 叶）；非 UInt64/nested 仍 FC | EvmMapPilot + MapSnapshot + B-EVM-MAP-STACK + BL-31 ✅ |
...
- **EvmStringMatch**（N-A1）：**已闭合** — 十三家 LowerSemantic String match → 9 叶 leaf-wise eq + if chains（Plan `switchOn` 仍仅 UInt64）；EVM 回归 + 十二家 N-A1-COPY
...
- **T10 EVM Principal storage**：**已闭合** — EVM `pilotPrincipalPolicyAdmit` + N4-isomorphic leaf storage（len+8×UInt64）；params/state/eq/ne；非 address；**B-RET-PRIN** 9 叶 result 已开（String result 同 9 叶例外）
- **T12 NEAR/Solana/Noir Principal storage**：**已闭合** — 三 target `pilotPrincipalPolicyAdmit` + 同构 9-leaf layout（Solana account pitch / NEAR 9×KV / Noir 9×u64 inputs）；params/state/eq/ne；非 pubkey/account-id/Field；**B-RET-PRIN** 9 叶 result 已开（十三家；TON 仅 view）；**Aleo T3** 已开同构 9×u64 identity（≠address/Field）；**T4** 已开 TON + Quint/Soroban/OpenVM/ICP 同构 9 叶（ICP 为 9 i64 globals / Candid 位置元组，无 Candid `principal`；caller remap 仍 FC）；**Psy PSY-SCALAR-ABI** wire-identity `len`+8×UInt32（max 32B；非 address；Principal/String result 9 叶）
