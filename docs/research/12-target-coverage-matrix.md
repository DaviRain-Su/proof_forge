...
| if / match（`Term.branch` / `Term.switch`） | **LOWERED** | T9a/T9b：if-diamond + `switchOn`（整型字面 + Option/Enum tag）；提前 return 按区域 store；`.qnt` flatten 为 `ite`；缺 default / 臂内 call·emit·revert 仍 FC |
