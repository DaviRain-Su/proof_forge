...
| **C-2** | Aleo/Psy compiler/VM 验收研究 | **研究闭合（RPT-015 + 2026-08-07 RPT-024）** + **J2/G123/ALEO-I3–I4 follow-up**：EmitIRV1 对齐 Leo 4；Leo 4.0.2 两平台 Tool Lock requiredBy exact 绑定 source+compile profiles；`AleoAcceptance` locked-only 覆盖 corpus，显式 compile profile 的产品 Finalize 生成三 compiler extras、双次 exact bytes 并过 inspect closure。RPT-024 确认 run=解释器、execute/deploy/query 网络依赖、synthesize CRS 未锁、无 snarkOS/snarkVM。Psy 仍无锁定 compiler/VM 门；两者均无 prove/deploy runtime 闭环。Aleo 成熟度为 non-deployable locked compile finalization，**非** hermetic/runtime/formal | AleoPsyResearch + AleoEmissionFix + C-2-pin + ALEO-I3–I4 ✅ |
