...
| Array/Option/Map state（齐次 UInt64 或 Int64） | **LOWERED** | N=1..8 Array flatten；Option tag+payload；Map cap-8 occ/key/val；元素/payload/key/value 跟随程序级 signedNumeric；无原生 List/Option/Map；错域混用 / Map param / N∉1..8 仍 FC |
| Bytes N state | **LOWERED** | N=1..8 UInt64 叶存低 8 位；IndexGet/Set 复用 Array 字面量下标；param / N=0 仍 FC |
...
| Map UInt64 **entry/view** 24 叶返回 | **LOWERED** | B-RET-MAP：24 occ/key/val 叶元组，不是把 8 叶 cap 改成 24；Map param / 非 UInt64 Map 仍 FC |
