...
dylib selected closure，逐文件重验后以最小无业务fixture原生运行真实
parser/checker/instantiator/interpreter。Darwin lane验证平台closure，不重复theorem-heavy、平台无关的
Lean product/Reference编译，也不冒充第二次product join、完整host-profile或hermetic closure；完整
五路径consumer仍由Linux target lane执行。未来新candidate仍是 non-admitted，必须经新的 reviewed
lock变更才能替换上述身份。
...
- `evm-yul-solc-0.8.34-v1`：官方 solc asset（默认；hashed-Map storage）；runtime profile 使用同一 Foundry archive；
  Finalize 使用 standard JSON Yul + optimizer，并同时请求 creation/deployed runtime；
  legacy request 不写 `evmVersion`。
- `evm-yul-solc-0.8.34-cancun-v1`：同一 solc 0.8.34 / Anvil·cast 0.3.0 二进制；Finalize 显式
  standard JSON `evmVersion="cancun"`；runtime 经 `PF_EVM_PROFILE` 启动 `anvil --hardfork cancun`。
