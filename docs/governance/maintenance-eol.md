...
`TargetMaturity` 唯一提升序为 research→specified→prototype→artifact_validated→
local_runtime→network_or_proof_validated，每一级只按该级验收 evidence 提升；不得跳级。
下降立即生效并记录原因。Wasm/ZK family 标签不能替代逐 target 证据。设计档案没有 backend，
无论 dossier maturity 为何都不可 build。`SupportEvidenceGrade` 与 evidence ledger grade 不是
TargetMaturity 的别名。
