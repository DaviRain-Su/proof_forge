...
| 2026-08-06 | **ADR-0032 U1 Solana 统一 materializer 启动**：sole rail = `solana-sbpf-cpi-elf-v1` 吸收 full body；single-account plan/elf 为过渡 shim；P2 首刀 product body UInt64 sub/mul/div/mod；P2 续 full-body hybrid（零 CPI site + multi-block/Map → LowerSemantic+EmitSbpfAsm，callerPrincipalLeaf）；**MiniAmm cpi-elf product pin**（`Tests/Product/MiniAmmSolanaV1` + Finalize `buildFromCapability` recompute）已绿；默认 profile 切换与 MiniAMM Mollusk 仍后续 |
