...
| 2026-08-06 | **ADR-0032 U1 Solana 统一 materializer 启动**：sole rail = `solana-sbpf-cpi-elf-v1` 吸收 full body；single-account plan/elf 为过渡 shim；P2 首刀 product body UInt64 sub/mul/div/mod；P2 续 full-body hybrid（零 CPI site + multi-block/Map → LowerSemantic+EmitSbpfAsm，callerPrincipalLeaf）；**MiniAmm cpi-elf product pin** + **admitCaller multi-account layout** + **MiniAmmHybrid Mollusk 11/11** + **P3-b/c skeleton** + **P3-d 显式 FC 门** 已绿；**真** site+full-body 同 ELF 合成与默认 profile 切换仍后续 |
