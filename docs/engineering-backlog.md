...
| **PF-CLI** | Rust Developer CLI `pf`（ADR-0037） | Aleo-first 闭环 + multi-target build/clean/smoke | **done 工程切片 D0–D6**（2026-08-10）；**Next=D7**：Solana `pf verify` + EVM Anvil `pf test` + 可选 Mollusk wrap——见 [`plan/pf-cli-aleo.md`](plan/pf-cli-aleo.md)；compiler 权威仍在 `proof-forge-next` |
