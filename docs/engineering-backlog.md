...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。ADR-0053 Wave 3 已于 2026-08-21 闭合 Solana 支持子集的 outer AccountInfo join 与产品 runtime 门；Wave 4 已将 EVM optional identity 对到完整重验的 static engineering OutputSet/raw `.bin` digest，并进入 Plan / plan digest / caller OutputSet provenance。ADR-0036 / ADR-0053 同日已由 owner directive accepted。当前进入 accepted 四目标工程 DoD，按 **EVM → Solana → NEAR → Noir** 对账；EVM deployment receipt / block / code-at-address evidence 仍须 owner 冻结，不得由 bind/evidence 两份 caller-supplied input 自证。**不**标 B-CALL-SEM closed：EVM on-chain identity、Solana/CW identity、Solana unsupported shapes、其它 target families、target static inspect、formal/C-3 仍开。剩余人拍项见 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；不开 XRPL TIME/CALLER，不标 formal done）。完善度队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。
...
3. Solana 残差：
   ...
   - Solana identity verification、schedule、generic result-bearing、empty-row/state、multi-callee、mixed frozen-site 继续 FC/open
