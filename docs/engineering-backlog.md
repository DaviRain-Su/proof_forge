...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **DPN-1..7 + G5-WIDE engineering done（2026-08-08）**；**G5 residual open**（Array/Principal/Bytes 金样、hard-require、deletion-gate）：规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md)。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`；完成条件 = Psy **admit 面**在 DPN 上全编码（非全 PSL 语法、非全 DPN op）。已闭合：schema+Counter 金样 → Counter lower → if/match/for → multi-leaf/Option/UInt128 → **mul/div/shift schoolbook/restoring/bit-walk** → Map cap-8 → effects honesty PARTIAL+FC → product dual-write `.dpn.json`+`.psy`。**仍 open**：Array/Principal/Bytes 产品金样、hard-require DPN、`.psy` deletion-gate、psy-runtime DPN-first、method_id 官方 hash |
