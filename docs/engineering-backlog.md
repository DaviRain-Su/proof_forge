...
| **NOIR-IR** | ProgramV1 → ACIR 权威物化（去 sole .nr） | **planned（2026-08-08）**：规划见 [`targets/07-noir-acir-lowering.md`](targets/07-noir-acir-lowering.md)。切片 NOIR-IR-0 规划 → IR-1 金样 → IR-2 MVP；nargo 1.0.0-beta.26；**非** prove/formal |
| **NOIR-IR-0** | 规划 + pin 策略 | **done（2026-08-08）**：`07-noir-acir-lowering.md`；Active 切 Noir ACIR |
| **NOIR-IR-1** | nargo Counter ACIR 金样 + schema | **pending** |
| **NOIR-IR-2** | Plan→ACIR MVP | **pending** |
