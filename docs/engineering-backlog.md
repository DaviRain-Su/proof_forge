...
| **N-ANON-RESULT** | 匿名容器（Array/Map/Option/Bytes）entry/view/fn 返回值 | 共享核 | Normalize/typed Semantic 已接纳四类 anonymous result TypeId；Reference 固定 Array/Bytes/Option/Map canonical valueBytes 与 Array-typed PureCall。**Target ABI 已开 EVM/Solana/NEAR/Noir/Psy/TON(view)/Aleo(entry)**（2026-08-03：`Array UInt64 N`(1..8) 按 named struct 同序 flatten、`Option UInt64` tag+payload 双叶，复用 named 多叶路径零新 tag；Solana Mollusk 16B LE、NEAR sandbox 16B LE、Noir nargo 10 包、Psy psyup、TON locked-tolk .fif/.boc、Aleo leo build 证据；Aleo view-over-state 经 computed-view 门保持 FC、entry 经 Final 评叶 drop 模型承认；TON entry 仍 FC；Bytes/Map/非 UInt64 元素/Int64 容器/嵌套/>8 仍 FC）；CosmWasm 仍以 target-owned Plan invariant 精确 FC（lane 进行中） | **done**（2026-08-03；`ea74d132a` shared + 七 target lanes；非 formal D2/D4） |
