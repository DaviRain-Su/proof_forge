...
| **EXT-ASSETS** | `extension.pf-assets`（L1 portable assets；ADR-0029） | Phase A 工程闭环：payload 冻结 + Typed/Normalize + Quint vault（deposit/transfer）+ 产品 demo `Examples/TipJar.lean` / `Tests.Product.TipJarQuintV1`；**非** formal/主网；Phase B+ Solana/EVM 等 binding 仍 pending | **Phase A done（2026-08-04）**；Phase B–D pending |
