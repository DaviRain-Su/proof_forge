...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **DPN-1..7 + G5-WIDE + G5-AGG + G5-MATRIX + G5-HARD + R-NARROW + R-INT + R-SHIFT-BIT + R-PURE + R-HARD engineering done（2026-08-08）**；**G5 residual closed / allowlist empty**；规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`；完成条件 = Psy **admit 面**在 DPN 上全编码或 plan-FC（非全 PSL 语法、非全 DPN op）。已闭合：schema+Counter → if/match/for → multi-leaf/wide → Map → Array/Principal/Bytes/Struct → effects → dual-write → **§3.2 扫描钉测** → **G5-HARD** → **R-NARROW** → **R-INT** → **R-SHIFT-BIT** → **R-PURE** → **R-HARD**（narrow bitwise/shift + Goldilocks Field DPN + allowlist 恒 false）。**仍 open**：`.psy` deletion-gate（G6）、psy-runtime DPN-first、method_id 官方 hash、optional dargo 字节金样 |
