...
| **N-ANON-RESULT** | 匿名容器（Array/Map/Option/Bytes）entry/view/fn 返回值 | 共享核 | Normalize/typed Semantic 已接纳四类 anonymous result TypeId；Reference 固定 Array/Bytes/Option/Map canonical valueBytes 与 Array-typed PureCall。**Target ABI 已开 EVM+Solana+NEAR**（2026-08-03 `0bae1dd88`/`dfd78ff5b`/`be8842962`：`Array UInt64 N`(1..8) 按 named struct 同序 flatten、`Option UInt64` tag+payload 双叶，复用 named 多叶路径零新 tag；Solana 附 Mollusk 16B LE、NEAR 附 sandbox 16B LE 字节证据；Bytes/Map/非 UInt64 元素/Int64 容器/嵌套/>8 仍 FC）；Noir/Aleo/Psy/CosmWasm/TON 仍以 target-owned Plan invariant 精确 FC | **done**（2026-08-03；`ea74d132a` shared + EVM/Solana/NEAR target lanes；非 formal D2/D4） |
