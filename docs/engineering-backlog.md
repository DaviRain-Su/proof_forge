...
| **ALEO-IR** | ProgramV1 → Aleo Instructions 权威物化（去 sole Leo 源） | **IR-0..IR-6 + G5-MATRIX + G5-HARD done（2026-08-08）**：规划见 [`targets/09-aleo-instructions-lowering.md`](targets/09-aleo-instructions-lowering.md)。官方 Leo→Instructions→AVM；权威 = Instructions IR（对标 Psy DPN）。产品 primary = Instructions；Leo 源 debug-only / compile dual-write；Counter ≡ golden；residual 桶 0（Int64/Field/pureFn true lower + 空 allowlist）。Lane **仍 active**。**Next = IR-7 → RES-CLEAN**（§10 队列）。**仍 open**：full opcode、record、prove；`deployable=false`。**非** formal |
...
| **ALEO-IR-7** | runtime honesty / optional snarkVM | **pending**（Next；可能 evidence MISSING） |
| **ALEO-RES-CLEAN** | residual honesty closeout | **pending** |
