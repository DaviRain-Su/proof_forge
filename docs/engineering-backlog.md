...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。CALL-BIND Wave 1 与 **Wave 2 EVM 叶已开**；下一 coding leaf = Solana callee/outer AccountMeta 或 CosmWasm `contract_addr`（逐叶推进），EVM 空账户 void CALL 仍须独立 Wave 2a 语义决定；inspect residual 暂不改。**不**标 B-CALL-SEM closed。剩余人拍项已排进 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；**不**自动接受 ADR-0036/0051/0052，**不**开 XRPL TIME/CALLER 叶，**不**标 formal done）。完善度分阶段队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。能力层已闭，见 [`docs/plan/capability-layer-tasks.md`](plan/capability-layer-tasks.md)。对账 `research/28-project-wide-honesty-audit.md`。Formal / 产品决策项仍不进 drain。
...
| 2026-08-21 | **ADR-0053 Wave 2 EVM 叶**：CLI decoded table 贯通 Registry → EVM Plan/IR/emit；generic void/result call + schedule exact-QN 命中预置 20B 地址，缺行 `PF-PLAN-INVARIANT`；bound 地址进入 Plan schema/digest、EngineeringBuildIdentity/engineering OutputSet 与 Yul。无 flag 保持历史 tags / hashed-QN Yul。Solana/CosmWasm 仍 parse-only；不做 empty-account、CREATE/CREATE2、code-at-address receipt、NetworkProfile、formal/C-3，不清 inspect residual。 |
