...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **DPN-1..7 + G5-WIDE + G5-AGG + G5-MATRIX + G5-HARD engineering done（2026-08-08）**；**G5 residual open**（residual 族 true DPN lower、full hard-require 删 allowlist、deletion-gate、optional dargo 字节金样）：规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`；完成条件 = Psy **admit 面**在 DPN 上全编码或证据化 residual/Plan FC（非全 PSL 语法、非全 DPN op）。已闭合：schema+Counter → if/match/for → multi-leaf/wide → Map → Array/Principal/Bytes/Struct → effects → dual-write → **§3.2 扫描钉测** → **G5-HARD gated residual policy**。**仍 open**：narrow/Int/pureFn/shl residual DPN lower、full hard-require、`.psy` deletion-gate、psy-runtime DPN-first、method_id 官方 hash |
...
| **PSY-DPN-G5-MATRIX** | §3.2 admit 矩阵扫描 + FC pins | **done（2026-08-08，engineering）**：§3.2 每 Y/P/F 行 DPN 现状列 + §3.2.1 覆盖表；UInt64 checkedMul/Div/Mod + Bool/assert/revert/bit*/const product DPN pins；residual 族（narrow UInt、Int signed、callFn pureFn、UInt64 shl、payload revertError）稳定 `PSY-DPN-G5-MATRIX` 诊断；UInt8 product dual-write 仅 `.psy`；plan-FC 行引用 PsySource/PsyPfAssets；**非** residual 真 lower、非 full hard-require、非 deletion-gate、非 formal |
