...
| **COMP-1-CALL-SEM-LAND** | resolver/文档/针，再按 COMP-0 做 EVM 地址、Solana 外层账户、CW `contract_addr` | **partial** — 十三 kind inspect 表面针 + residual 标签 + ADR-0053 Wave 2 三叶 emit（有表无行 FC）+ Wave 2a 空账户 void CALL FC + Wave 2b Solana 编译期 AccountMeta（非 outer join）；inspect residual 仍 pending |
