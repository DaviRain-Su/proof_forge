...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **DPN-1..7 + G5-WIDE + G5-AGG + G5-MATRIX + G5-HARD + R-NARROW + R-INT + R-SHIFT-BIT + R-PURE engineering done（2026-08-08）**；**G5 residual open**（remaining residual 族 true DPN lower、full hard-require 删 allowlist、deletion-gate、optional dargo 字节金样）：规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`；完成条件 = Psy **admit 面**在 DPN 上全编码或证据化 residual/Plan FC（非全 PSL 语法、非全 DPN op）。已闭合：schema+Counter → if/match/for → multi-leaf/wide → Map → Array/Principal/Bytes/Struct → effects → dual-write → **§3.2 扫描钉测** → **G5-HARD gated residual policy** → **R-NARROW UInt** → **R-INT signed** → **R-SHIFT-BIT UInt64 shl/shr/checkedBitNot** → **R-PURE pureFn/localCall callFn inline**。**仍 open**：narrow bitwise/shift residual DPN lower、full hard-require、`.psy` deletion-gate、psy-runtime DPN-first、method_id 官方 hash |
...
| **PSY-DPN-G5-MATRIX** | §3.2 admit 矩阵扫描 + FC pins | **done（2026-08-08，engineering）**：§3.2 每 Y/P/F 行 DPN 现状列 + §3.2.1 覆盖表；UInt64 checkedMul/Div/Mod + Bool/assert/revert/bit*/const product DPN pins；historical residual 钉测后 **R-NARROW/R-INT/R-SHIFT-BIT/R-PURE 已真 lower**；当前 residual（narrow bitwise/shift）+ payload revertError FC；plan-FC 行引用 PsySource/PsyPfAssets；**非** full hard-require、非 deletion-gate、非 formal |
| **PSY-DPN-R-NARROW** | UInt8/16/32 checked arith → DPN | **done（2026-08-08）**：Felt-carried add/sub/mul/div/mod + param range + unsigned compare；产品 dual-write；narrow bitwise/shift 仍 residual |
...
| **PSY-DPN-R-SHIFT-BIT** | UInt64 shl/shr + checkedBitNot → DPN | **done（2026-08-08）**：`invalidShift: count >= 64` + dargo U32ShiftLeft/Right + CastFelt；checkedBitNot Gte `2^32−1` + Sub mask `2^32−2`；产品 ShiftBit dual-write；**非** formal/execute；narrow bitwise/shift 仍 residual |
...
| **PSY-DPN-G5-HARD** | product honesty：DPN lower fail ≠ silent `.psy`-only | **done（2026-08-08，gated residual policy）**：`EmitIRV1` `isPsyDpnG5HardResidualAllowlistV1` 仅放行 `PSY-DPN-G5-MATRIX` residual 诊断 → transitional `.psy` only；非 allowlist DPN 失败稳定 `PSY-DPN-G5-HARD` fail materialize；`buildFromPlanV1` + `PsyDpnV1` classifier/zero-state/UInt8 residual/Counter dual-write pins；`deployable=false`；**非** full hard-require（删 allowlist）、**非** G6 删 `.psy`、**非** residual 真 DPN lower、**非** formal |
...
| **PSY-DPN-7** | 产品主路径切 DPN | **done（2026-08-08，dual-write transition）**：`EmitIRV1.emitFromIR` 在 Plan→DPN 成功时 dual-write `{name}.dpn.json`（primary package JSON ≡ Counter golden）+ 过渡 `{name}.psy`；G5-HARD residual allowlist 仍可 `.psy` only；Finalize 注记 DPN+`.psy`、zero-tool、`deployable=false`；`PsyDpnV1` pin product dual-write；**非** 删 `.psy`、非 psy-runtime DPN-first、非 UPS/formal |
