...
| **EXT-ASSETS** | `extension.pf-assets`（L1 portable assets；ADR-0029） | Phase A：payload 冻结 + Typed/Normalize + Quint vault + 产品 demo；Phase B：EVM（B2：payable exact callvalue + full-gas value CALL + `interface-standard` 骨架 + Anvil 门真跑）与 Solana（B1：program-owned vault PDA + 幂等 ensure + System CPI 入金 + program-direct 出金 + Mollusk 12/12）native deposit/transfer 已绑定，resolver multi-permit（Quint + EVM×2 + Solana CPI）；token/async 全 target FC；**非** formal/主网 | **Phase A done（2026-08-04）**；**Phase B done（2026-08-05）**；Phase C（CW/NEAR）–D pending |
