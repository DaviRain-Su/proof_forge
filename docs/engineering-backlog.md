...
| **INV-2** | ADR-0034 通用 L1 Preservation 产品实例 | **in progress**（2026-08-07：generic `PreservationTheoremV1` foundation + `ProofKindV1`/三字段 wire/`(inv,kind)` inventory/双 alias/kind-bound protocol+certifier plumbing 已接线；bare=holds；kind 改 source/cert digest、不改 semantic；无 2-field fallback；当前无 preserving certified positive。下一步 EvenCounter，随后第二非 AMM 实例；MiniAmm 最后复用；非 formal/target refinement/release） |
