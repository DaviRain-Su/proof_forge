...
| **COMP-1-CALL-SEM-LAND** | versioned bind table、三叶 emit 与 Solana 支持子集 outer join；保留各族明确 FC | **partial** — Wave 1/2/2a/2b/2c + Wave 3 engineering done；EVM local-output identity follow-on done：EVM/CW exact endpoint、EVM explicit output/runtime digest join + CALL-time code hash；Solana state-bearing single-callee void-call 的 1..8 rows + executable program outer join、program residual 清零与 Mollusk 闭环。EVM deployment proof、Solana/CW identity、Solana unsupported shapes、target inspect static residual、其它 families、formal/C-3 仍开 |
...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。ADR-0053 Wave 3 已于 2026-08-21 闭合 Solana 支持子集的 outer AccountInfo join 与产品 runtime 门；ADR-0036 / ADR-0053 同日已由 owner directive accepted。当前进入 accepted 四目标工程 DoD，按 **EVM → Solana → NEAR → Noir** 对账；EVM 已闭合显式本地 callee output/runtime artifact identity join 与 CALL-time code-hash gate。**不**标 B-CALL-SEM closed：EVM receipt/address provenance/constructor/deployment proof、Solana/CosmWasm identity、Solana unsupported shapes、其它 target families、target static inspect、formal/C-3 仍开。剩余人拍项见 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；不开 XRPL TIME/CALLER，不标 formal done）。完善度队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。
...
3. Solana 残差：
   ...
   - identity digest verification、schedule、generic result-bearing、empty-row/state、multi-callee、mixed frozen-site 继续 FC/open
