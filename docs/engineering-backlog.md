...
| **ALEO-IR** | ProgramV1 → Aleo Instructions 权威物化（去 sole Leo 源） | **in progress（2026-08-08）**：规划见 [`targets/09-aleo-instructions-lowering.md`](targets/09-aleo-instructions-lowering.md)。官方 Leo→Instructions→AVM；权威 = Instructions IR（对标 Psy DPN）。**IR-0/IR-1/IR-2/IR-3/IR-4/IR-5 done**；Next = `IR-6` 产品 primary；Leo 源仍产品过渡。**非** prove/deploy/formal |
...
| **ALEO-IR-2** | Plan→Instructions Counter MVP | **done（2026-08-08）**：`Instructions/LowerPlanV1` — init store(param)+guard、checkedAdd store+return Final、constructor edition；`programFromCapabilityV1` / 手建 Plan ≡ `counterProgramV1` ≡ golden 字节；default/compile 同 Plan→同 Instructions；unsupported FC；产品 primary 仍 Leo 源（IR-6）；**非** 控制流/多叶/prove |
| **ALEO-IR-3** | if/match/bounded for → Instructions | **done（2026-08-08）**：Schema/TextCodec 增 `branch.eq`/`position`/`ternary`；`LowerPlanV1` 通用 Final 语句 lower（if→branch/position、switch→`is.eq` 右嵌套、for 静态 unroll+boundExceeded、`maxIterations>4096` FC）；Counter 金样回归；结构测试 + product Branch；**非** 多叶/产品 primary/prove |
| **ALEO-IR-4** | 多叶 / Map / Option / 窄宽 → Instructions | **done（2026-08-08）**：复用 Leo flatten-to-mapping（`pf_state_{i}` 类型化 value）；`storeAggregate` 全求值再 set；Option UInt64 / Array / Map cap-2 product 结构门；narrow UInt8/16/32 state/param/arith + `typeCast` shift count；Int64/Field residual FC；nested Map 仍 Plan FC；Counter 金样回归；**非** 产品 primary/prove |
| **ALEO-IR-5** | 效果与诚实矩阵 | **done（2026-08-08）**：`checkEffectsHonestyMatrixV1` 先于 validatePlan；Plan `emitEvent`/`callFn`/payload revert 稳定 `ALEO-IR-5:`；product emit/call/schedule/context/assets 路径 FC（resolve 或 Semantic/Plan）；record/pf.assets 零绑定 honesty notes；**无 PARTIAL**；Counter 金样回归；**非** 产品 primary/prove |
