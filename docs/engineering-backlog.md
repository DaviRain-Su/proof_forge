...
| **EXT-ASSETS** | `extension.pf-assets`（L1 portable assets；ADR-0029） | Phase A：payload 冻结 + Typed/Normalize + Quint vault + 产品 demo；Phase B：EVM（payable exact callvalue + full-gas value CALL + `interface-standard` 骨架 + Anvil 门）与 Solana（vault PDA + 幂等 ensure + System CPI 入金 + program-direct 出金 + Mollusk 12/12）；Phase C：CosmWasm（funds exact + BankMsg::Send reply_on=never sync + cw-vm mock）与 NEAR（attached_deposit + transferAsync Promise + near-sandbox 余额观测）已绑定；resolver multi-permit 六行（Quint + EVM×2 + Solana CPI + NEAR + CW）；token/async 全 target FC | **A（08-04）/B/C done（2026-08-05）**；Phase D（TON/Psy/Aleo 单独立项）pending |
