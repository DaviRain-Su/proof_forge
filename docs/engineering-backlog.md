...
| 2026-08-06 | **ADR-0032 U1 Solana 统一 materializer 启动**：sole rail = `solana-sbpf-cpi-elf-v1` 吸收 full body；single-account plan/elf 为过渡 shim；P2 首刀 product body UInt64 sub/mul/div/mod；P2 续 full-body hybrid（零 CPI site + multi-block/Map → LowerSemantic+EmitSbpfAsm，callerPrincipalLeaf）；**MiniAmm cpi-elf product pin** + **admitCaller multi-account layout** + **MiniAmmHybrid Mollusk 11/11** + **P3-b/c skeleton** + **P3-d partial 合成**（BodyCpiIfPay：full-body + empty-meta `sol_invoke` 同 ELF；非 multi-role 成熟度）已绿；multi-role site recipes / Map+CPI 真成熟 / 默认 profile 切换仍后续 |
