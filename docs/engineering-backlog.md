...
| **INV-2** | ADR-0034 通用 L1 Preservation 产品实例 | **in progress**（2026-08-08：foundation + kind plumbing 已接线；**EvenCounter preserving product `check` certified positive GREEN**（`EvenCounterPreservationV1` + ClosedSubjectPin + InlineProofCertifier）；业务主路径 = `PreservationTheoremV1` on product Reference，pin 仅为 golden 加速。下一步：抽出可复用 packaging lemmas / 硬化非 pin 任意合约 author 路径；第二非 AMM 实例作通用性回归（deferred）；MiniAmm 最后；**不** supersede ADR-0027；非 formal/target refinement/release） |
