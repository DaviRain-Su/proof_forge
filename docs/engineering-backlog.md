...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **DPN-1..7 + G5 + R-HARD + G6-DEBUG + G6-RUNTIME + G6-PIN engineering done（2026-08-08）**；**G5 residual closed / allowlist empty**；规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2 / §10。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`；完成条件 = Psy **admit 面**在 DPN 上全编码或 plan-FC（非全 PSL 语法、非全 DPN op）。已闭合至 **G6-PIN**（WideCounter256 VM product 结构 pin）。**仍 open / deferred**：method_id 官方 hash；**全量 dargo package 字节金样有意 deferred**（Counter 为 sole 全量字节金样；宽整数全展开包体过大 + dargo 无 package-only 路径，见规划 §10） |
