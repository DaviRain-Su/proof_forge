...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **G6 engineering closeout done（2026-08-08）** + **RES-METHOD-ID done** + **RES-TOOL-LOCK done**：DPN-1..7 + G5 + R-HARD + G6-DEBUG + G6-RUNTIME + G6-PIN + official method_id SHA-256 + psy-node rev Tool Lock/supply-chain pin；**G5 residual closed / allowlist empty**；产品默认 sole `.dpn.json`；runtime DPN-first plant；WideCounter256 结构 pin。规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2 / §6 / §10。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`。Lane **idle**。**仍 open / deferred（residual only）**：全量 dargo package 字节金样 deferred；package-only dargo execute（`.psy` PARTIAL）；UPS/network/formal；`deployable=false` |
