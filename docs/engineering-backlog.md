...
| **B-CTX-OPEN** | ContextRead 各 target Plan 开放（unixTime/caller） | target leaf | B-ctx 有意钉死八 materializer FC；无 open 项；**需产品决策是否开** | pending |
...
| **B-ctx** | ContextRead 各 target Plan：保持 fail-closed 并补齐负向测 | 八 materializer | **done**（2026-08-02 起：unixTime + caller 的 implemented target materialize decline 钉测；当前 coverage matrix 八 materializer FC，含 CosmWasm/TON） |
...
| **C-5** | Solana 已有 Mollusk；扩 fixture 跟 Normalize 新面 | **ongoing**（14 programs：Counter + 13 fixtures，60 Rust tests 全 active/通过；**MapMini 4/4**、**WideMul 4/4** 与 **PrincipalStore 4/4** 均走真实 ELF+Mollusk；PrincipalStore 固定 `len + 8×UInt64` identity state/param、逐叶 equality 与短值覆盖高位清零，明确非 pubkey/CPI；Option/Context/call 运行覆盖仍待扩） |
...
4. ~~N-CONST-REF~~、~~N-STR-EVENT~~、~~N-FOR-INT~~、~~N-ANON-RESULT~~与~~N-NEST-IDX~~已完成；**剩余串行语言面**：N-CALL-RET（需 schema 决策）→ N-MAP-CONSTRUCT（需 Wire schema 决策）
