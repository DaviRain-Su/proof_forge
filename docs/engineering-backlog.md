...
| **INV-2** | ADR-0034 通用 L1 Preservation 产品实例 | **in progress**（2026-08-09：foundation + kind plumbing + **EvenCounter product GREEN**；**PreservationPackagingV1** 通用 packaging（bf-pack-1/2）；**非 pin author 路径**文档 + `Tests.Semantic.ClosedSubjectPinV1`（bf-unpin-1）。业务主路径 = `PreservationTheoremV1` on product Reference，pin 仅 golden。下一步：Wave2 ZeroCounter bf2-preserve/product（bf2-data GREEN）；MiniAmm 最后；**不** supersede ADR-0027；非 formal/target refinement/release） |
