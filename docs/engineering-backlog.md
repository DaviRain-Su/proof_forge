...
聚合与 Field 子集；EVM/Solana/NEAR/CosmWasm/TON 有工程 runtime 门，Noir 有 compile-only 门，Aleo 有 opt-in locked Leo product compile finalization（仍 non-deployable）+ 独立 host-heavy DevNet/Testnet post-build deploy/receipt wrapper（mainnet/canary 拒绝；formal NetworkProfile 仍 pending），Psy 仍 **registry source-only**（历史默认 profile + 显式 dargo-VM profile 的 UInt128 首片；optional dargo compile + 独立 host-heavy `just psy-runtime` Counter/WideCounter local-VM 菜谱，非 ordinary ci），Quint 仍 source-only（zero-tool `.qnt` executable-model target）。registry = **12 targets / 9 implemented + 3 design-only / 12 resolver rows**（Aleo×2 + Psy×2 + Solana×1 等）；CW：sync 拒、
...
| **ALEO-IR** | ProgramV1 → Aleo Instructions 权威物化（去 sole Leo 源） | **IR-0..IR-7 + G5-MATRIX + G5-HARD + RES-CLEAN + MULTI-GOLDEN + COMPILE-COMPARE + CONST done（2026-08-08）**：规划见 [`targets/09-aleo-instructions-lowering.md`](targets/09-aleo-instructions-lowering.md)。官方 Leo→Instructions→AVM；权威 = Instructions IR（对标 Psy DPN）。产品 primary = Instructions；Leo 源 debug-only / compile dual-write；Counter ≡ golden；residual 桶 0（Int64/Field/pureFn/const true lower + 空 allowlist）；IR-7 runtime honesty **PARTIAL/MISSING**（无 locked snarkVM package-only；`just aleo-runtime` → `PF-TOOLCHAIN-MISSING`）。Lane **idle**。**诚实 residual（deferred only）**：full multi-program leo 字节金样、record custody、full opcode、prove/deploy；`deployable=false`。**非** formal |
| **ALEO-IR-0** | 规划 + pin 策略 | **done（2026-08-08）**：`09-aleo-instructions-lowering.md`；Active 切 Aleo IR；`09-aleo` 指向规划 |
| **ALEO-IR-1** | Instructions Schema + Counter 金样 | **done（2026-08-08）**：`Targets/Aleo/Instructions/{SchemaV1,TextCodecV1}`；金样 `testdata/golden/aleo-instructions-v1/counter.compiled.aleo`（product build + locked Leo 4.0.2）；`Tests.Materialization.AleoInstructionsV1` encode≡golden / decode≡手建 / round-trip；**非** Plan lower |
| **ALEO-IR-2** | Plan→Instructions Counter MVP | **done（2026-08-08）**：`Instructions/LowerPlanV1` — init store(param)+guard、checkedAdd store+return Final、constructor edition；`programFromCapabilityV1` / 手建 Plan ≡ `counterProgramV1` ≡ golden 字节；default/compile 同 Plan→同 Instructions；unsupported FC；**非** 控制流/多叶/prove |
| **ALEO-IR-3** | if/match/bounded for → Instructions | **done（2026-08-08）**：Schema/TextCodec 增 `branch.eq`/`position`/`ternary`；`LowerPlanV1` 通用 Final 语句 lower（if→branch/position、switch→`is.eq` 右嵌套、for 静态 unroll+boundExceeded、`maxIterations>4096` FC）；Counter 金样回归；结构测试 + product Branch；**非** 多叶/prove |
| **ALEO-IR-4** | 多叶 / Map / Option / 窄宽 → Instructions | **done（2026-08-08）**：复用 Leo flatten-to-mapping（`pf_state_{i}` 类型化 value）；`storeAggregate` 全求值再 set；Option UInt64 / Array / Map cap-2 product 结构门；narrow UInt8/16/32 state/param/arith + `typeCast` shift count；Int64/Field residual FC；nested Map 仍 Plan FC；Counter 金样回归；**非** prove |
| **ALEO-IR-5** | 效果与诚实矩阵 | **done（2026-08-08）**：`checkEffectsHonestyMatrixV1` 先于 validatePlan；Plan `emitEvent`/`callFn`/payload revert 稳定 `ALEO-IR-5:`；product emit/call/schedule/context/assets 路径 FC（resolve 或 Semantic/Plan）；record/pf.assets 零绑定 honesty notes；**无 PARTIAL**；Counter 金样回归；**非** prove |
| **ALEO-IR-6** | 产品 primary = Instructions | **done（2026-08-08）**：`EmitIRV1` materialize primary `{id}.aleo` = Instructions（lower 成功 ≡ Counter golden）；query-contract 不变；Leo 源 debug-only `PROOF_FORGE_ALEO_EMIT_LEO=1`/`emitLeoDebug`→`{id}.leo`；compile profile 始终双写 `.leo` 供 Finalize locked-leo **compare** extras；source Finalize note 更新；`deployable=false`；**非** snarkVM / prove/deploy |
| **ALEO-G5-MATRIX** | §3.2 admit 扫描 + FC pins | **done（2026-08-08）**：§3.2 Instructions 现状列 + §3.2.1；Bool/assert/nested Map pins；const 行后由 **ALEO-CONST** 升为 done；原 residual 桶见 G5-HARD |
| **ALEO-G5-HARD** | residual true lower + hard-require | **done（2026-08-08）**：Int64 signed ops + Field BLS12-377 + pureFn callFn inline → Instructions；`isAleoInstructionsG5HardResidualAllowlistV1` 恒 false；Plan admitted 且 IR fail → `ALEO-IR-G5-HARD`（禁 silent Leo-only primary）；Counter golden 仍绿；`deployable=false`；**非** IR-7/prove/formal |
| **ALEO-IR-7** | runtime honesty / optional snarkVM | **done PARTIAL/MISSING（2026-08-08）**：探路 Tool Lock 无 snarkVM/snarkOS；`scripts/aleo_runtime_test.sh` + `just aleo-runtime` 缺工具 `PF-TOOLCHAIN-MISSING`（exit 2；非 ordinary ci）；suite `testIr7RuntimeHonestyNotes`；leo run ≠ package-only Instructions execute；不发明 CLI；`deployable=false`；**非** prove/deploy/formal |
| **ALEO-MULTI-GOLDEN** | 多 fixture Plan→IR 结构/产品钉测 | **done（2026-08-08，engineering）**：`AleoInstructionsV1` `testMultiGolden*` — LoopSum full Example；Accumulator/OptionState/MapMini **admit-surface**（full Example 分别因 reserved `add` / computed view Plan-FC 已钉）；method/mapping/key-op/encode/round-trip；Counter sole full-byte golden 870 B 不变；G5-HARD allowlist 空；**非** multi-program leo 字节金样 / snarkVM / formal；分类见 [`targets/09-aleo-instructions-lowering.md`](targets/09-aleo-instructions-lowering.md) §10 |
| **ALEO-COMPILE-COMPARE** | 小 fixture locked-leo compiled 对照 | **done（2026-08-08，engineering）**：Accumulator **admit-surface**（entry `credit`）product `aleo-leo-4.0.2-u64-compile-v1` → 金样 `testdata/golden/aleo-instructions-v1/accumulator-admit.compiled.aleo`（870 B，SHA-256 `1db88f65…`）；`AleoInstructionsV1` Plan→IR encode ≡ 金样字节 + 可选 live locked-Leo recheck（缺工具 honest skip）；Counter IR-1 金样 `counter.compiled.aleo` 不变；**非** multi-program leo 字节矩阵 / snarkVM / formal；`deployable=false` |
| **ALEO-ADMIT-FIXTURES** | suite-owned admit-surface 产品夹具 | **done（2026-08-08，engineering）**：`Tests/Fixtures/AleoAdmitSurfacesV1.lean` 耐久源（Accumulator `credit`、OptionState entry-only、MapMini put-only）；`AleoInstructionsV1` `testAdmitFixtures*` 产品 select→compile→capability→`programFromCapabilityV1` 成功 + 结构 IR + G5-HARD 空；full Examples Plan-FC 诚实钉（reserved `add`；computed `peek`/`get`）；**不**改 shared Examples；MULTI-GOLDEN/COMPILE-COMPARE 同源 fixture；Counter 870 B 不变；**非** multi-program leo 字节矩阵 / snarkVM / formal |
...
| **ALEO-OPTION-COMPARE** | OptionState admit locked-leo 对照 | **done（2026-08-08，engineering）**：OptionState admit-surface product `aleo-leo-4.0.2-u64-compile-v1` → 金样 `testdata/golden/aleo-instructions-v1/optionstate-admit.compiled.aleo`（1019 B，SHA-256 `ac23dacd…`）；`AleoInstructionsV1` Plan→IR encode ≡ 金样 + 可选 live locked-Leo recheck（缺工具 honest skip）；MapMini 因 Leo Map upsert rewrite（Plan→IR ≠ compiled）保持 structural-only + live rewrite 证据钉，无 mapmini-admit 金样；Counter 870 B 与 Accumulator admit pin 不变；**非** multi-program leo 字节矩阵 / snarkVM / formal；`deployable=false` |
| **ALEO-CONST** | Constant → Instructions 或 plan-FC 钉死 | **done（2026-08-08，engineering）**：`Op.Constant` 与 `Op.Literal` 同 envelope 经 `lowerLiteral` 内联为 Plan literal → Instructions `7u64`/`true`/`3u32` 等操作数（无独立 const IR）；`AleoInstructionsV1.testConstProductLower`；非 envelope 仍 FC；Counter golden 不变；**非** multi-program leo 字节矩阵 / snarkVM / formal |
| **ALEO-RES-CLEAN** | residual honesty closeout | **done（2026-08-08）**：docs/backlog/AGENTS G5/IR-7 诚实收口；sole Counter full-byte golden `counter.compiled.aleo` 保留；multi-program leo 字节金样 / record custody / prove/deploy / full opcode **deferred** 声明；suite `testResidualHonestyNotes`；Lane **idle**；**不**发明 snarkVM CLI；`deployable=false`；**非** formal |
...
| **ALEO-NETWORK-E** | DevNet/Testnet explicit post-build deploy + separate receipt | CLI wrapper/scripts/docs | **done engineering（2026-08-09）**：network engine 不再 build/leo-build，完整 compile-profile OutputSet 先 stable-read private snapshot→product inspect→exact-byte deploy；DevNet funded `--dev-key`，Testnet owner-only key-file exact identity rejoin→inherited FD + pinned snarkOS private executable snapshot；mainnet/canary preflight reject；bounded/reaped `snarkos --wait` + REST visibility；独立 `proof-forge.aleo-deployment-receipt.engineering.v1` 由 owner-safe retained parent/staging FD 发布（`confirmed-partial`/`attempted-unobserved`）；DevNet fresh ledger/loopback/exact owned PID lifecycle，仓库无 hardcoded key；no-network self-tests/CLI pre-spawn rejection+redaction 进 ordinary smoke，real integration host-heavy。**formal N3 NetworkProfile/compatible-build join、locked snarkOS、public Testnet funded run、network UPS 仍 pending；build `deployable=false` 不变** |
...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **G6 engineering closeout done（2026-08-08）** + **RES-METHOD-ID / RES-TOOL-LOCK / RES-CLEAN done**：DPN-1..7 + G5 + R-HARD + G6-DEBUG + G6-RUNTIME + G6-PIN + official method_id SHA-256 + psy-node rev Tool Lock/supply-chain pin + residual honesty cleanup；**G5 residual closed / allowlist empty**；产品默认 sole `.dpn.json`；runtime DPN-first plant；WideCounter256 结构 pin。规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2 / §6 / §10。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`。Lane **idle**。**仍 open / deferred（residual only）**：全量 dargo package 字节金样 deferred；package-only dargo execute（`.psy` PARTIAL）；UPS/network/formal；`deployable=false` |
| **PSY-DPN-RES-METHOD-ID** | 官方 method_id SHA-256 | **done（2026-08-08）**：`genDapenContractFunctionMethodIdV1` = first 4 LE of SHA-256(UTF-8 `name(arg[size],…)`); EmitIR `p{sourceIndex}` size=1；Counter get/increment/initialize ≡ dargo pin；`pinnedMethodIdV1` 仅回归；**非** package-only dargo / full multi-method byte golden |
| **PSY-DPN-RES-TOOL-LOCK** | Tool Lock / supply-chain 镜像 psy-node rev | **done（2026-08-08）**：`SchemaV1.psyNodeDpnAuthorityRevV1` = `79e0b82422ebdd1173a7b4b3751eb3186aad83e5`；`ToolLockV4` 模块注释；`supply-chain/psy-node-dpn-authority.v1.json`；`psy_runtime_test.sh`/`psy_acceptance.sh` 注释；`PsyDpnV1` pin test；**不**改 dargo 0.1.0 asset/executable digests、**不**发明 tools[] psy-node 行（toolchain_assets 拒未知字段）；**非** package-only execute / UPS / formal |
| **PSY-DPN-RES-CLEAN** | residual honesty cleanup + deferred docs | **done（2026-08-08）**：sole full-byte golden `counter-package.v1.json`；duplicate `counter-package-full.v1.json` 删除门槛 + suite 钉不存在；`PsyDpnV1.testResidualHonestyNotes`（golden inventory + method_id ≡ gen_dapen）；docs/tests 重申 package-only execute MISSING（locked dargo 0.1.0）与 multi-method byte golden deferred；**不**发明 package-only CLI、**不**加全量 multi-method 字节金样 |
...
| **PSY-DPN-4** | 多叶/宽整数 → DPN | **done（2026-08-08，engineering；mul/div/shift 见 G5-WIDE）**：`LowerPlanV1` multi-leaf `storeAggregate`/`returnAggregate`→多 `SlotSingle`（sub_slot=fieldIndex+4，WideCounter dargo 证据）；OptionState 产品 Plan→DPN；手建 UInt128 4×UInt32 init/get/add（limbAdd/Select/u32 range）；默认 profile WideCounter FC；Counter 金样仍绿；**非** dargo Option 全量金样、非 formal |
| **PSY-DPN-G5-WIDE** | WideCounter mul/div/shift → DPN | **done（2026-08-08，engineering）**：`bindWideUintMul` schoolbook 8×UInt16（U32And/ShiftRight + Target Mul/Add + overflow assert）；`bindWideUintDivMod` 4×32 restoring 全展开（range + zero-div；无 Felt `/`/`%`）；`bindWideUintShift` 128-step bit walk（shl overflow / count bound）；per-limb bitAnd/Or/Xor U32+CastFelt；`PsyDpnV1` 手建 + WideCounter VM product Plan→DPN；默认 profile 仍 FC；UInt256 同算法；产品结构 pin 见 **PSY-DPN-G6-PIN**；**非** dargo package 字节相等/execute/UPS/formal |
| **PSY-DPN-G5-AGG** | Array/Principal/Bytes/Struct multi-leaf → DPN | **done（2026-08-08，engineering）**：复用 multi-leaf `storeAggregate`/`returnAggregate`→multi `SlotSingle`（sub_slot=`fieldIndex+4`）；Array UInt64 N、Bytes 1..8、Principal wire-identity 9 叶、named Struct flatten 手建 + product Plan→DPN（`PsyDpnV1`）；Nested Map / Map return / Principal return 保持 Plan FC；**非** locked-dargo package 字节相等/execute/UPS/formal |
| **PSY-DPN-G5-MATRIX** | §3.2 admit 矩阵扫描 + FC pins | **done（2026-08-08，engineering）**：§3.2 每 Y/P/F 行 DPN 现状列 + §3.2.1 覆盖表；UInt64 checkedMul/Div/Mod + Bool/assert/revert/bit*/const product DPN pins；**R-NARROW/R-INT/R-SHIFT-BIT/R-PURE/R-HARD 后 residual 桶 0**；payload revertError FC；plan-FC 行引用 PsySource/PsyPfAssets；**非** deletion-gate、**非** formal |
| **PSY-DPN-R-NARROW** | UInt8/16/32 checked arith → DPN | **done（2026-08-08）**：Felt-carried add/sub/mul/div/mod + param range + unsigned compare；产品 dual-write |
| **PSY-DPN-R-INT** | Int64 / narrow Int signed → DPN | **done（2026-08-08）**：Int64 signedCompare/checkedNeg；Int{8,16,32} two's-complement signed add/sub/mul/div/mod/neg/compare（mirror EmitIR）；Int8 product dual-write；Int64 arith 复用 UInt64 checked path；**非** formal/execute |
| **PSY-DPN-R-SHIFT-BIT** | UInt64 shl/shr + checkedBitNot → DPN | **done（2026-08-08）**：`invalidShift: count >= 64` + dargo U32ShiftLeft/Right + CastFelt；checkedBitNot Gte `2^32−1` + Sub mask `2^32−2`；产品 ShiftBit dual-write；**非** formal/execute |
| **PSY-DPN-R-PURE** | pureFn/localCall callFn → DPN inline | **done（2026-08-08）**：callFn 将 pureHelper body inline 进 caller definitions（arg wires 作 params；nested callFn fuel-bounded）；package 省略 pureHelper 顶层方法（匹配 EmitIR free helper）；recursive/effectful pure body FC；product PureDpn dual-write；**非** formal/execute |
| **PSY-DPN-R-HARD** | full hard-require after residual lowers | **done（2026-08-08）**：narrow bitwise/shift/bitNot + Goldilocks Field → DPN（mirror EmitIR）；allowlist 清空；former residual 产品 dual-write；`PsyDpnV1` unit+product pins；**非** G6 deletion、**非** formal |
| **PSY-DPN-G5-HARD** | product honesty：DPN lower fail ≠ silent `.psy`-only | **done（2026-08-08）+ R-HARD empty allowlist**：`isPsyDpnG5HardResidualAllowlistV1` 恒 false；任意 DPN lower 失败稳定 `PSY-DPN-G5-HARD` fail materialize；`buildFromPlanV1` + `PsyDpnV1` pins；`deployable=false`；**非** G6 删 `.psy`、**非** formal |
| **PSY-DPN-5** | Map 等 .psy 破点 → DPN | **done（2026-08-08，engineering）**：dense `Map UInt64 UInt64` cap-8（24 occ/key/val leaves）经 Plan `mapLookup`/`mapUpsert` Select 树 + `storeAggregate` 进 `LowerPlanV1` general builder；`Examples/MapMini` + `Token` product Plan→DPN package 结构门（init 24/25 Sets、put map-full assert+Select、get view Select-merge Option match）；手建 cap-2 lookup/upsert；`maxStateLeavesV1=64`；**绕过** dargo `.psy` return-in-if 破点；Nested Map/Map return 仍 Plan FC；**非** dargo Map 金样/execute/UPS/formal |
| **PSY-DPN-6** | call/emit/context 诚实矩阵 | **done（2026-08-08，engineering PARTIAL+FC）**：emit→`DPNEventRecord`（GetCheckpointId/UserId/ContractId + data）；void call→`InvokeExternalContractFunctionSync` num_outputs=0（FNV QN hash 对齐 EmitIR）；schedule/assets/ContextRead/Commit/nonempty invariant 保持 FC；`PsyDpnV1` pin；**非** Finalize runtime/ordered-event/response/UPS/formal |
| **PSY-DPN-7** | 产品主路径切 DPN | **done（2026-08-08，dual-write transition；G6-DEBUG 收紧 default）**：`EmitIRV1.emitFromIR` 在 Plan→DPN 成功时 emit `{name}.dpn.json` primary；见 **PSY-DPN-G6-DEBUG** |
| **PSY-DPN-G6-DEBUG** | `.psy` debug-only / DPN-primary default | **done（2026-08-08）**：产品默认 **仅** `{name}.dpn.json`；过渡 `{name}.psy` 仅 `emitPsyDebug := true` 或 env `PROOF_FORGE_PSY_EMIT_PSY=1`（Registry/CLI IO）；Finalize 注记更新；`PsyDpnV1` Counter default 无 `.psy` + flag dual-write；EmitIR `.psy` lower **保留** gated；`deployable=false`；**非** 删除 EmitIR.psy 代码、**非** formal |
| **PSY-DPN-G6-RUNTIME** | `psy-runtime` / acceptance DPN-first | **done PARTIAL（2026-08-08）**：`scripts/psy_runtime_test.sh` 先默认 product build（sole `.dpn.json`）+ package-shape + plant `target/<pkg>.json` + method ⊆ post-compile package；`PsyAcceptance`/`psy_acceptance.sh` 同步 plant；**PARTIAL**：locked dargo 0.1.0 无 package-only compile/execute 标志（`execute` 只写 package、始终重编译 `.psy`），local-VM 差分仍 `PROOF_FORGE_PSY_EMIT_PSY=1`；不发明 unsupported dargo flags；`deployable=false`；**非** ordinary ci / formal / UPS |
| **PSY-DPN-G6-PIN** | WideCounter256 product 结构 pin + 全量 dargo 字节金样 defer | **done（2026-08-08，engineering）**：`PsyDpnV1` 钉 `Examples/WideCounter256` 显式 VM profile product Plan→DPN 结构（8-limb init Sets sub_slot 4..11；`u256 mul overflow` / `u256 div by zero` / `u256 mod by zero` / `invalidShift: count >= 256` / `u256 shl overflow` / U32Shift*；get 8 outputs；encode JSON array；default profile FC）；**全量 dargo package 字节金样有意 deferred**（包体膨胀 + locked dargo 无 package-only 抓取；Counter 仍 sole 全量字节金样）；method_id 见 **PSY-DPN-RES-METHOD-ID**；**非** execute/UPS/formal |
...
| **PSY-SCALAR-ABI** | Principal、String、Bytes | **done（2026-08-08）**：`Bytes N`（1..8）→ N×UInt8 Felt leaves（state/param/IndexGet/Set/return `[Felt; N]`）；Principal/String → wire `u32le(len)‖body` 展为 `len`+8×UInt32 body limbs（max 32B payload；state/param/eq-ne leaf-wise）；**非** address/pubkey/Felt scalar；Principal/String return FC（9 leaves > B-RET cap 8）；Bytes 9+ FC；Map 仍 FC；Plan/source pin 已通 |
| **PSY-CONTAINER-ABI** | Map、嵌套容器、aggregate 参数、>8 叶返回 | **done（2026-08-08）**：dense `Map UInt64 UInt64` cap-8（24 occ/key/val Felt leaves；empty + IndexGet→Option + IndexSet upsert + atomic storeAggregate + full-guard）；named Struct/Enum params 预序多叶（≤8）；嵌套 Map / Map return / Map 非 UInt64 K·V / >8 叶 return 证据化 FC（无 dargo 抬 B-RET cap pin） |
| **PSY-INDEX-CAST** | 动态 Array 索引、CheckedCast | **done（2026-08-08）**：Array/Bytes 运行时索引 → `assert indexOutOfBounds` + select 折叠 get/set；字面量索引路径不变。CheckedCast：窄 UInt/Int{8,16,32} 四向转换 + exact `castOutOfRange`（无静默截断）；UInt64/Int64/wide 仍 FC。产品源码尚无 cast 语法/Normalize 发射（共享核 residual）；运行时索引产品路径已 pin |
| **PSY-CONTEXT-COMMIT** | ContextRead、Commit | **done（2026-08-08，证据化 FC）**：`unixTimeSeconds`/`caller`/`blockHeight`/unknown ContextRead 与 Commit 键级诊断 fail closed；电路域无 official dargo public-input/witness 锚点（B-CTX-OPEN）；Commit 禁止 Felt identity 透传（B-COMMIT-ZK）；EnvRead 仍 ADR-0029 zero-binding FC；PsySource pin |
| **PSY-INVARIANT** | nonempty invariants | **done（2026-08-08，证据化 FC）**：nonempty `InvariantDecl`/`.invariant` 键级诊断 fail closed（无 dargo 表面绑定 Semantic predicate closure + `invariantSteps` fuel）；empty invariants 仍开放；inline ADR-0027 check ≠ target Plan；PsySource pin |
...
| **PSY-ASYNC-ASSETS** | async schedule、`pf.assets` | **done（2026-08-08，证据化 FC — 非 capability 开放）**：`schedule`/`effect.asynchronous-workflow` 仍 resolver+Plan FC（不得把 `__invoke_sync` 重命名为 async）；`pf.assets` 五 catalog QN **零绑定**（ADR-0029 Phase D）：带 extension → resolve `PF-REQ-UNSUPPORTED`；无 extension → Plan explicit unbound（deposit/transfer/transferAsync/token* pin，`PsyPfAssetsV1`）；无官方 native vault/deposit/balance 证据前 **admit set 保持 empty**；非 catalog L0 call 仍可降；**非** formal UPS/assets registry |
