...
| **NOIR-IR** | ProgramV1 → ACIR 权威物化（去 sole .nr） | **IR-0..IR-3 + IR-5 + IR-6 + IR-7 done（2026-08-08）**：规划见 [`targets/07-noir-acir-lowering.md`](targets/07-noir-acir-lowering.md)。nargo 1.0.0-beta.26 nargo-assisted ACIR；opt-in dual-write；IR-7 prove honesty **PARTIAL/MISSING**（`barretenberg=null`；`just noir-runtime` → `PF-TOOLCHAIN-MISSING`）。Lane **idle**。**诚实 residual（deferred only）**：IR-4 multi-fixture inventory；真实 backend pin 后 Counter prove 扩展；`deployable=false`。**非** product prove/formal |
