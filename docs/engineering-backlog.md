...
| **B-CTX-OPEN** | ContextRead 各 target Plan 开放（unixTime/caller/blockHeight） | target leaf | **unixTimeSeconds**：EVM `timestamp()`（tag 59）+ NEAR `block_timestamp` ns÷10^9（tag 41）+ CW Env JSON time + TON `blockchain.now()` 已开；**Solana/Noir/Psy/Aleo/Quint 保持 FC**（电路域决策 2026-08-04）。**caller**：**EVM S1 Plan 已开（2026-08-06）** — ADR-0025 `u32le(20)\|\|CALLER` → `callerPrincipalWord` 九叶 + CallerCheck Anvil + Ownable corpus admit；**Solana lane 另 agent 进行中**（CPI `pf_caller`）；NEAR/CW 已另有 S1 提交。**blockHeight**：**EVM S2 leaf 已开**（`number()` / tag 62 + UInt64 range guard）；他 target leaf 仍 FC | **in_progress**（EVM unixTime+caller+blockHeight done；NEAR/CW/TON unixTime；他链 caller/blockHeight residual） |
...
5. **可并行 target leaf（接口冻结后）**：B-COMMIT-ZK / B-CTX-OPEN residual（EVM caller **encoding+Plan** 已 S1 cutover 2026-08-06；NEAR/CW/TON unixTime 已开；Solana ContextRead 全 FC；Solana/NEAR/CW **caller** 与 EVM `blockHeight` 仍需 S1/S2 leaf）
