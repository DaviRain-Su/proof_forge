...
| **COMP-1-CALL-SEM-LAND** | resolver/文档/针，再按 COMP-0 做 EVM 地址、Solana 外层账户、CW `contract_addr` | **partial** — 十三 kind inspect 表面针（human+JSON）+ residual 标签 + ADR-0053 Wave 2 三叶 emit（有表无行 FC）+ Wave 2a 空账户 void CALL FC + ExtFlow/LaterFlow xrpl 针；Wave 2b Solana accounts 仍 pending |
...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。日常工程下一刀 = **CALL-BIND Wave 2b**（Solana nonempty accounts / outer AccountMeta）。Wave 2 emit + Wave 2a 空账户 void CALL FC 已开。**不**标 B-CALL-SEM closed。剩余人拍项已排进 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；**不**自动接受 ADR-0036/0051/0052，**不**开 XRPL TIME/CALLER 叶，**不**标 formal done）。完善度分阶段队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。能力层已闭，见 [`docs/plan/capability-layer-tasks.md`](plan/capability-layer-tasks.md)。对账 `research/28-project-wide-honesty-audit.md`。Formal / 产品决策项仍不进 drain。
