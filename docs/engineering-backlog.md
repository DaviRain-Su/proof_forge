...
| **B-CALL-SEM** | call/schedule capability 与真实平台语义对齐 | 产品决策 + target leaf | **#111 Solana legacy honesty 已接线**；resolver 支持键不得冒充跨平台 call 完成。十三 kind `callScheduleFamilyTagV1` / static residual 已在 inspect 露出但不进 SupportClaim。**ADR-0053 Wave 1/2/2a/2b/2c** 已交付 versioned `--bindings`、三叶 missing-row FC、EVM empty-code void CALL FC、Solana AccountMeta 与 program-level residual。**Wave 3（2026-08-21）** 闭合 Solana 支持子集：state-bearing、单一 generic callee、同步 void CALL、1..8 个 non-alias rows、无 frozen CPI site；Plan/IDL/IR/client 投影 exact outer roles，SBPF 校验 key/signer/writable/executable，传 bound accounts + callee program AccountInfos，产品 caller/callee 过 8 项 Mollusk。该子集 residual=`null`。**Wave 4（2026-08-21）** 令 EVM optional identity 必须由 EVM-only canonical `--binding-evidence` 映射到完整重验的 deployable engineering OutputSet；optional source/semantic + mandatory raw `.bin` SHA-256 对回 bind row，canonical identity-table digest 进入 Plan/caller OutputSet provenance；无 identity 保持历史 Plan bytes。**Solana local-output follow-on（2026-08-21）**：repeatable `--callee-output` 完整 inspect deployable Solana OutputSet，每行按 callee program name + source/semantic/raw `.so` SHA-256 exact join，verified identity 保留进 caller Plan。仍不查 EVM deployment receipt/RPC/code-at-address 或 Solana ProgramData/current ELF；CW identity、Solana unsupported shapes、其它 target family 与 target inspect residual 保持 FC/static | **open**（endpoint + EVM static artifact identity + Solana local artifact identity/支持子集 engineering done；on-chain code identity / unsupported families / static inspect / formal / C-3 仍开） |
...
| **COMP-1-CALL-SEM-LAND** | versioned bind table、三叶 emit、EVM static identity evidence 与 Solana 支持子集 outer join；保留各族明确 FC | **partial** — Wave 1/2/2a/2b/2c + Wave 3/4 engineering done：EVM/CW exact endpoint；EVM identity 对完整重验的 deployable OutputSet + raw `.bin` digest 并进入 Plan/OutputSet provenance；Solana state-bearing single-callee void-call 的 1..8 rows + executable program outer join、local OutputSet/ELF 三 digest exact join + caller Plan retention、program residual 清零与 Mollusk 闭环。EVM code-at-address、Solana ProgramData/current ELF、CW identity、Solana unsupported shapes、target inspect static residual、其它 families、formal/C-3 仍开 |
...
| **COMP-2-SOL-CPI** | generic call-bind 支持子集、local OutputSet/ELF identity 与 Token/ATA binding 已完成；扩大 schedule/result/multi-callee 前另决策 | **partial / expansion-blocked** — Wave 3 product/runtime closure + caller Plan local ELF identity retention done；Token/ATA active catalog 绑定 tracked package-owned loader-v3 ELF 的 exact path/size/SHA-256/source/tag/commit/build-recipe，product bindings/catalog digest 与 product-acceptance 磁盘门已闭；WideDiv/WideDiv256 独立 Rust oracle + Mollusk 及 WideDivDispatch 最远 handler 已闭。ProgramData/on-chain code identity、schedule、generic result-bearing、empty-row/state、multi-callee、mixed-site 与 async 保持 FC/open；扩大须 owner 决策；非 formal/hermetic/package-owner-published |
...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。ADR-0053 Wave 3 已于 2026-08-21 闭合 Solana 支持子集的 outer AccountInfo join 与产品 runtime 门；Wave 4 已将 EVM optional identity 对到完整重验的 static engineering OutputSet/raw `.bin` digest，并进入 Plan / plan digest / caller OutputSet provenance；Solana local-output follow-on 已将完整重验的 local OutputSet/raw `.so` identity exact join 并保留到 caller Plan。ADR-0036 / ADR-0053 同日已由 owner directive accepted。当前进入 accepted 四目标工程 DoD，按 **EVM → Solana → NEAR → Noir** 对账；EVM deployment receipt / block / code-at-address evidence 仍须 owner 冻结，不得由 bind/evidence 两份 caller-supplied input 自证。**不**标 B-CALL-SEM closed：EVM on-chain identity、Solana ProgramData/current ELF、CW identity、Solana unsupported shapes、其它 target families、target static inspect、formal/C-3 仍开。剩余人拍项见 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；不开 XRPL TIME/CALLER，不标 formal done）。完善度队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。
...
2. B-CALL-SEM 产品决策 + EVM 残差（并行于 1 的 leaf；**决策，非 Goal drain**）：
   - ~~EVM opt-in pre-placed 20-byte endpoint binding~~（ADR-0053 Wave 2 done）；~~static engineering OutputSet/raw `.bin` identity evidence~~（Wave 4 done）；无表 hashed-address stub 保持，deployment receipt/block/code-at-address join 仍 open
...
3. Solana 残差：
   ...
   - ~~local OutputSet/ELF identity exact join + caller Plan retention~~（2026-08-21 done）；ProgramData/on-chain code identity、schedule、generic result-bearing、empty-row/state、multi-callee、mixed frozen-site 继续 FC/open
...
| 2026-08-20 | **ADR-0053 Wave 2（emit consume）**：有 `--bindings` 时三叶 generic call/schedule 无行 fail closed；EVM exact endpoint 进入 Plan/digest/build identity 并由 Yul 消费，CW WAT `contract_addr` / Solana empty-meta program id 消费表。无 flag 保持历史 Plan tags / hashed QN / QN stub。表显式下传（不进 capability、无 IO.Ref）。Solana nonempty accounts = Wave 2b；空账户 void CALL = Wave 2a。可选 identity digest verification / inspect residual / B-CALL-SEM / formal / C-3 仍开。 |
...
| 2026-08-21 | **ADR-0053 Wave 3（Solana outer AccountInfo join）**：state-bearing + single generic callee + synchronous void CALL + 1..8 non-alias rows + no frozen CPI sites 的产品子集，把 state 后的 bound accounts 与 executable callee program 纳入 Plan/IDL/IR/client exact role projection；SBPF 精确校验 key/signer/writable/executable，向 `sol_invoke_signed_c` 传 rows+program AccountInfos；product-built caller/callee 过 8 项 Mollusk positive/negative/rollback；支持子集 program residual=`null`。empty-state/schedule/empty-row/multi-callee/mixed-site/generic result-bearing 与 Solana identity verification 保持 FC/open；target inspect 静态；**不**关全 B-CALL-SEM / formal / C-3 / release。 |
| 2026-08-21 | **ADR-0053 Wave 4（EVM static artifact identity evidence）**：新增 EVM-only canonical `proof-forge.call-bind-evidence.v1` / `--binding-evidence`；identity-bearing EVM rows 无 evidence 或无 `artifactSha256` 即 FC。evidence exact 覆盖 callee/address → safe relative engineering output dir；复用 product inspector 重验 sidecars、逐文件 raw-byte SHA-256、exact disk closure、`outputSetDigest` 与 target/deployable，再比较 optional source/semantic + mandatory published `.bin` digest。canonical expected identity-table digest 进入 EVM Plan/planDigest/caller OutputSet provenance；identity-less 保持历史 Plan bytes。**不**查 receipt/RPC/code-at-address；不关 B-CALL-SEM / formal / C-3 / CREATE/CREATE2 / NetworkProfile。 |
| 2026-08-21 | **Solana call-bind local-output identity follow-on**：repeatable `--callee-output` 复用完整 `proof-forge.output.v1` inspector，要求 deployable Solana output + 恰好一个 finalizer-owned `{program}.so`；每个 row 按 callee program name + source/semantic/raw-ELF SHA-256 exact join，verified 三 digest 保留到 caller Plan `callBindProgram` role。runtime 仍只校验 programId/account key/executable/privilege/order，不证明 ProgramData、upgrade authority、当前链上 ELF 或部署过程；EVM 产品 ingress 仍只用 `--binding-evidence`。**不**关全 B-CALL-SEM / formal / C-3 / release。 |
