...
| **B-OPT-STATE** | 九 materializer Option state Plan | target leaf | N-A4 只开 Normalize admit。**NEAR 已开**（2026-08-04 `95289615e`：`Option UInt64` state 复用 Enum 双叶布局（tag+payload 8B KV），none 默认零初始化、none 赋值清零 payload（HostModel+sandbox 双钉），match 走既有 VariantTag/VariantPayload 路径；Option params/非 UInt64 payload 仍 FC）；**EVM 已开**（2026-08-04 `0a46e54d8`：同构 2-slot 布局 + storeAtomic 沿用 B-EVM-MAP-STACK memory-spill 惯例，none 双叶清零，solc strict-assembly 验收 + Token 栈预算不变；Option params 不扩展）；**Solana 已开**（2026-08-04 `42c863d0c`：同构 `slot_tag`/`slot_p0` u64-le 布局 + 多叶原子 store，stale-payload clear 钉死，6 项 Mollusk 测试）；Noir/Psy/Aleo/CosmWasm/TON/Quint 仍 FC | in_progress（NEAR/EVM/Solana done；其余 target 继续） |
| **B-COMMIT-ZK** | Commit × Noir/Psy | target leaf | EVM/Solana/NEAR/Aleo 身份透传已开；Noir/Psy FC | pending |
| **B-CALL-SEM** | call/schedule capability 与真实平台语义对齐 | 产品决策 + target leaf | resolver 过度声明修复中：**Solana 已切真 CPI**（2026-08-04 `ef55e5461`：sync/async 均发 `sol_invoke_signed_c`（空 AccountMeta——无 DSL 账户模型的诚实最小集）、QN 路径 SHA-256 program-id stub、domain-separated method discriminator + LE args；result-bearing call 经 `sol_get_return_data` 读取（短缺 → 0x1006）；Mollusk mock-callee 证实真实 invoke 被发起 + 父状态保持；**文档化缺口**：agave 要求 callee program 账户进外层指令 AccountMeta，完整 CPI 成功需多账户外层布局后续切片）；**EVM 已开 returndata 通道**（2026-08-04：result-bearing sync call → 真 CALL + `returndatasize≥32` 守卫 + 首 word `mload` + UInt64 range check（bare revert 惯例；新 Plan stmt `externalCallResult` schema tag 12）；void/schedule 不变；callee 地址仍为 keccak stub（部署须改写）；Anvil 机制实证：emitter 精确 opcode 序列对 mock callee probe(41)→43、callee revert 整笔回滚；**非** stub 地址与真实部署地址的产品级对齐——那需要部署表决策）；Noir 仍 relation slots、Aleo/Psy 边界待逐项复核 | in_progress（Solana/EVM 切片 done；其余 target 继续） |
