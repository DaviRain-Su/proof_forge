...
| **COMP-1-CALL-SEM-LAND** | versioned bind table、三叶 emit 与 Solana 支持子集 outer join；保留各族明确 FC | **partial** — Wave 1/2/2a/2b/2c + Wave 3 engineering done：EVM/CW exact endpoint；Solana state-bearing single-callee void-call 的 1..8 rows + executable program outer join、program residual 清零与 Mollusk 闭环。identity digest parse-only，Solana unsupported shapes、target inspect static residual、其它 families、formal/C-3 仍开 |
...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。ADR-0053 Wave 3 已于 2026-08-21 闭合 Solana 支持子集的 outer AccountInfo join 与产品 runtime 门；EVM exact endpoint 同时已进入 Plan / plan digest / build identity。ADR-0036 / ADR-0053 同日已由 owner directive accepted。当前进入 accepted 四目标工程 DoD，按 **EVM → Solana → NEAR → Noir** 对账；EVM 可选 identity metadata 的验证先阻塞于 callee artifact / receipt 来源冻结。**不**标 B-CALL-SEM closed：identity metadata verification、Solana unsupported shapes、其它 target families、target static inspect、formal/C-3 仍开。剩余人拍项见 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；不开 XRPL TIME/CALLER，不标 formal done）。完善度队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。
...
2. B-CALL-SEM 产品决策 + EVM 残差（并行于 1 的 leaf；**决策，非 Goal drain**）：
   - ~~EVM opt-in pre-placed 20-byte endpoint binding~~（ADR-0053 Wave 2 done）；无表 hashed-address stub 保持，identity digest verification 仍 open
