...
| **N-A4** | Option state | **done（shared）**（2026-08-02：Normalize/Reference admit Option state/param + default none；当时九 materializer FC。后续 B-OPT-STATE 已开 EVM/Solana/NEAR `Option UInt64` state；其余六 materializer、Option params/非 UInt64/nested 继续 FC） |
...
| **N-ANON-RESULT** | 匿名容器（Array/Map/Option/Bytes）entry/view/fn 返回值 | 共享核 | Normalize/typed Semantic 已接纳四类 anonymous result TypeId；Reference 固定 Array/Bytes/Option/Map canonical valueBytes 与 Array-typed PureCall。**Target ABI 已开全部八 target**（2026-08-03：`Array UInt64 N`(1..8) 按 named struct 同序 flatten、`Option UInt64` tag+payload 双叶，复用 named 多叶路径零新 tag；EVM solc、Solana Mollusk 16B LE、NEAR sandbox 16B LE、Noir nargo 10 包、Psy psyup、TON locked-tolk .fif/.boc、Aleo leo build、CosmWasm cosmwasm-vm 精确 JSON 证据；诚实边界：TON entry FC（async actor 无返回通道）、Aleo view-over-state 经 computed-view 门 FC、Option state 仅 EVM/Solana/NEAR 的 UInt64 子集开放；Map/Bytes state 按 target 非均匀；Map/Bytes 返回、非 UInt64 元素/Int64 容器/嵌套/>8/pureFn 聚合均 FC） | **done**（2026-08-03；`ea74d132a` shared + 八 target lanes；非 formal D2/D4） |
...
| **B-CTX-OPEN** | ContextRead 各 target Plan 开放（unixTime/caller） | target leaf | **unixTimeSeconds 已开 EVM+NEAR**（2026-08-04，用户决策）：EVM `timestamp()` opcode（新 Plan Expr tag 59，UInt64 range 守卫）；NEAR host `block_timestamp()`(ns) ÷10^9 截断（新 Plan Expr tag 41 + 条件 import + HostModel 固定 1700000000）；EVM `context.caller` encoding 已由 ADR-0025 冻结（`u32le(20)\|\|CALLER`）但 **Plan 仍 FC**；Solana/Noir/Psy/Aleo/Quint unixTime 与全部 caller 仍 FC；**2026-08-04 用户决策：Noir/Aleo/Psy unixTime 保持 FC**（电路域值只能作输入进入：私有见证可任意选值使约束通过、公开输入也只证明"程序用了 T"而非"T 是真实链时间"——无锚定的 public-input 注入会教出错误语义模型；待各 target 有锚定设计再评估）；**CW 已开**（2026-08-04 `8bbda226a`：Env JSON `"time"`（cosmwasm-std Timestamp ns 字符串）÷10^9 截断，instantiate/execute/query 入口预解析进共享 i64 global（有界 WAT 子串解析 + 溢出 trap），`"time"` needle 固定 data offset 3000 带发射期不变量 fail-closed；`.seconds()` 本为 u64 故无 range guard）；**TON 已开**（2026-08-04 `6aab0a8ec`：Tolk stdlib `blockchain.now()`（asm NOW；bare `now()` 非 locked 1.4 符号），entry/view 双路径，无 range guard，ClockBox 过 locked tolk→.fif）；CW/TON runtime fixture 接线（cosmwasm-vm mock env / TON sandbox FIXED_NOW 已就绪，fixture 未接）为独立后续 | in_progress（EVM/NEAR/CW/TON unixTime done；caller 待另行决策） |
...
| **B-CALL-SEM** | call/schedule capability 与真实平台语义对齐 | 产品决策 + target leaf | resolver 过度声明修复中：**Solana 已切真 CPI**（2026-08-04 `ef55e5461`：sync/async 均发 `sol_invoke_signed_c`（空 AccountMeta——无 DSL 账户模型的诚实最小集）、QN 路径 SHA-256 program-id stub、domain-separated method discriminator + LE args；result-bearing call 经 `sol_get_return_data` 读取（短缺 → 0x1006）；Mollusk mock-callee 证实真实 invoke 被发起 + 父状态保持（**未演示成功 CPI**——Agave 所需 callee program account 无法由当前单账户外层 ABI 提供，测试固定 invoke 后的 fail-closed）；**文档化缺口**：agave 要求 callee program 账户进外层指令 AccountMeta，完整 CPI 成功需多账户外层布局后续切片）；**EVM 已开 returndata 通道**（2026-08-04：result-bearing sync call → 真 CALL + `returndatasize≥32` 守卫 + 首 word `mload` + UInt64 range check（bare revert 惯例；新 Plan stmt `externalCallResult` schema tag 12）；void/schedule 不变；callee 地址仍为 keccak stub（部署须改写）；Anvil 机制实证：emitter 精确 opcode 序列对 mock callee probe(41)→43、callee revert 整笔回滚；**非** stub 地址与真实部署地址的产品级对齐——那需要部署表决策）；**2026-08-04 残余复核**：Noir resolver 文案已削为 witness-binding relation（电路不执行、证明不 attest，result-bearing 待 response-witness 契约仍 FC）；EVM/Solana `schedule` 明确为同笔交易内同步 fire-and-forget（结果丢弃，匹配 Reference 无 response cursor 契约，与 CW-4 SubMsg 同一接纳纪律，不降 support）；Psy sync 注明仅 `__invoke_sync#<Felt>` 源码面发射（无 VM/proof 验收门）；Aleo 双键+event decline、NEAR 原生 promise、TON 真异步 out-message、CW same-tx SubMsg、Quint 全 FC 均已诚实；残余缺口为独立后续：EVM 部署地址接线（keccak stub）、Solana 外层多账户布局（ADR-0024 lane 取代过渡 stub，成功 CPI 前置）、Noir response-witness 契约 | in_progress（2026-08-04 复核已对齐声明与现实；残余语义缺口闭合前保持 open） |
...
| **B-SOL-MUL** | Solana UInt128/256 schoolbook 多字 mul + fail-closed div/mod | target leaf | SBPF emit `narrowCheckedMul` 128/256 → 真 schoolbook 多字乘（32-bit digit split、lane-ordered carry、高肢 overflow trap）；div/mod 仍 low64 + 高肢零检查 FC（err_mwdiv/err_mwmod）；WideMul 以独立 base-2^64 Rust oracle 验证高肢/跨肢成功与 `0x1001` 溢出全 state 回滚 | **done**（production `9bb6fe1ad`；runtime `de72b46fa`；WideMul 4/4 + current full Mollusk 89/89，2026-08-04；非 formal） |
...
| **C-5** | Solana 已有 Mollusk；扩 fixture 跟 Normalize 新面 | **ongoing**（20 programs：Counter + 19 fixtures，89 Rust tests 全 active/通过（`programs.rs` 74 + `counter.rs` 8 + `cpi.rs` 7）；MapMini/WideMul/PrincipalStore、PairRet/MaybeRet、ArrayRet/OptionRet 保持真实 ELF+Mollusk覆盖；**OptionState 6 项**钉死 BL-29 tag/payload state、none/reset stale-payload clear；**CpiCaller 7 项**钉死真实 `sol_invoke_signed_c`/`sol_get_return_data` 发射与缺外层 callee account 时的 fail-closed。ContextRead 与可成功 CPI 的多账户外层 ABI 仍待扩） |
