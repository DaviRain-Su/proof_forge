...
| **ALEO-IR-6** | 产品 primary = Instructions | **done（2026-08-08）**：`EmitIRV1` materialize primary `{id}.aleo` = Instructions（lower 成功 ≡ Counter golden）；query-contract 不变；Leo 源 debug-only `PROOF_FORGE_ALEO_EMIT_LEO=1`/`emitLeoDebug`→`{id}.leo`；compile profile 始终双写 `.leo` 供 Finalize locked-leo **compare** extras；source Finalize note 更新；`deployable=false`；residual Plan 形状 Instructions FC 时 Leo 仍为 `.aleo` primary（至 G5）；**非** G5 hard-require / snarkVM / prove/deploy |
| **ALEO-G5-MATRIX** | §3.2 admit 扫描 + FC pins | **done（2026-08-08）**：§3.2 Instructions 现状列 + §3.2.1；`AleoInstructionsV1` Bool/assert/const/Int64/Field/pureFn/nested Map pins；residual 桶 Int64/Field BLS12-377/pureFn（禁止假 Y）；**非** G5-HARD |
| **ALEO-G5-HARD** | residual true lower + hard-require | **pending**（Next：Int64/Field/pureFn true lower 优先，再空 allowlist + `ALEO-IR-G5-HARD`） |
| **ALEO-IR-7** | runtime honesty / optional snarkVM | **pending**（可能 evidence MISSING） |
