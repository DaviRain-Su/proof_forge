...
| 2026-08-18 | **B-RET-STR + leftover XRPL/Map-view pin**：十三家 String **恰好 9 叶** identity（state/param/`==`/`!=`/view·entry return；与 Principal 同一 `n==9` 例外，不是把 8 叶 cap 改成 9；TON **仅 view**，entry 聚合仍 FC）。不是 UTF-8 ABI / 不是 address。`Targets.lean` 把 XRPL 补进已 admit 的 MapMini/BytesBox 环，并加 MapViewRet（TON view + 既有 Map-return 家；EVM hashed Map 仍 FC）。remap / AccountID / pubkey / Candid `principal`/`text` / TON entry 聚合 / String const / String match（仅 EVM N-A1）仍 FC |
