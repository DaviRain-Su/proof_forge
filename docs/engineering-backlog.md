...
| **PSY-DPN** | ProgramV1 → DPN 权威物化（去文本 `.psy` 依赖） | **DPN-1..7 + G5-WIDE + G5-AGG + G5-MATRIX engineering done（2026-08-08）**；**G5 residual open**（residual 族 true DPN lower、hard-require DPN、deletion-gate、optional dargo 字节金样）：规划见 [`targets/10-psy-dpn-lowering.md`](targets/10-psy-dpn-lowering.md) §3.2。权威 `psy-node`/`psy_vm` rev `79e0b82…` 的 `DPNFunctionCircuitDefinition`；完成条件 = Psy **admit 面**在 DPN 上全编码或证据化 residual/Plan FC（非全 PSL 语法、非全 DPN op）。已闭合：schema+Counter → if/match/for → multi-leaf/wide → Map → Array/Principal/Bytes/Struct → effects → dual-write → **§3.2 扫描钉测**。**仍 open**：narrow/Int/pureFn/shl residual DPN lower、hard-require DPN、`.psy` deletion-gate、psy-runtime DPN-first、method_id 官方 hash |
