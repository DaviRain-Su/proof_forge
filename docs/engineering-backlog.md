...
Materialized/Finalized + disk closure` 已通；sole Normalize 与**十三个 materializer**（EVM/Solana/NEAR/Noir/
Aleo/Psy/Quint/CosmWasm/TON/Soroban/OpenVM/ICP/XRPL）已覆盖非均匀的多宽算术、控制流、fn、let/for、shift/bitwise、call/schedule、
聚合与 Field 子集；EVM/Solana/NEAR/CosmWasm/TON 有工程 runtime 门，ICP 有 host-optional PocketIC，Noir 有 compile-only 门；Aleo 为 sole `aleo-instructions-v1` zero-tool materializer，Psy 为 sole `psy-dpn-v1` zero-tool materializer；Quint 为 zero-tool `.qnt` executable-model target；Soroban 为 source-only S0（ADR-0044）；OpenVM 默认 `openvm-guest-source-v1` 为 zero-tool guest-source（ADR-0045 O0），opt-in `openvm-guest-elf-v1` 锁定 `cargo-openvm` 2.0.1 build/transpile ELF/VmExe（ADR-0046 O1）；XRPL 默认 `xrpl-bedrock-source-u64-v1` 为 zero-tool source（ADR-0049），opt-in `xrpl-bedrock-wasm-u64-v1` 为 ambient rustc `.wasm` extra（ADR-0050）。registry = **13 targets / 13 implemented + 0 design-only / 17 resolver rows**（EVM×2、Noir×2、OpenVM×2、XRPL×2、其余各一）；CW：sync 拒、
...
| **EXT-CRYPTO** | `extension.crypto`（SHA-256 / Merkle / 签名） | IBC 与大量链上逻辑命脉；capability 矩阵 | **设计钉 done**（RPT-027，2026-08-19 刷新）；S5 sha256 五叶（EVM/Solana/NEAR/TON/Soroban）+ keccak 三叶 + **CRYPTO-B1 sha256Bytes 五叶**（2026-08-19）+ **CRYPTO-C2 Merkle EVM 叶**（2026-08-19，keccak QN）+ **CRYPTO-D2 EVM ecrecover** 均 engineering done；剩余子项（签名叶/Merkle 变体/余 target 叶）均须人拍 |
...
| **B-CALL-SEM** | call/schedule capability 与真实平台语义对齐 | 产品决策 + target leaf | **#111 Solana legacy honesty 已接线**；resolver 支持键不得冒充跨平台 call 完成。十三 kind `callScheduleFamilyTagV1` / static residual 已在 inspect 露出但不进 SupportClaim。**ADR-0053 Wave 1/2/2a/2b/2c** 已交付 versioned `--bindings`、三叶 missing-row FC、EVM empty-code void CALL FC、Solana AccountMeta 与 program-level residual。**Wave 3（2026-08-21）** 闭合 Solana 支持子集：state-bearing、单一 generic callee、同步 void CALL、1..8 个 non-alias rows、无 frozen CPI site；Plan/IDL/IR/client 投影 exact outer roles，SBPF 校验 key/signer/writable/executable，传 bound accounts + callee program AccountInfos，产品 caller/callee 过 8 项 Mollusk。该子集 residual=`null`。**EVM local-output identity follow-on（2026-08-21）**：显式 `--callee-output` 复用完整 output inspect，按 program name + source/semantic/runtime-artifact SHA-256 exact join，runtime code Keccak 进入 CALL-time `EXTCODEHASH` gate。**Solana local-output identity follow-on（2026-08-21）**：同一显式 authority 边界按 program name + source/semantic/ELF-artifact SHA-256 exact join，并把三 digest 保留进 caller Plan；SVM runtime 仍只校验 programId/executable，不证明 ProgramData/ELF。均无网络/RPC。CosmWasm identity 仍 parse-only，EVM receipt/address provenance/constructor/deployment proof、Solana on-chain code identity/unsupported shapes 与其它 target family 边界保持 FC，target inspect residual 仍静态 | **open**（EVM/Solana local artifact identity engineering done；deployment/on-chain code proof / unsupported families / static inspect / formal / C-3 仍开） |
...
| **EVMOZ-001** | 显式 Cancun EVM profile（shared） | **done**（2026-08-03：`evm-yul-solc-0.8.34-cancun-v1` 进入 registry/descriptor/resolver/Tool Lock；2026-08-21 Finalize 切 standard JSON 后仅 Cancun request 加 `evmVersion="cancun"`，两 profile 同源产 creation/runtime；runtime `PF_EVM_PROFILE` → `anvil --hardfork cancun`；默认 legacy v1 不写 evmVersion；同一 solc 0.8.34/Anvil 0.3.0；**非** OZ claim / formal D4 / 工具升级） |
...
| **COMP-1-CALL-SEM-LAND** | versioned bind table、三叶 emit 与 Solana 支持子集 outer join；保留各族明确 FC | **partial** — Wave 1/2/2a/2b/2c + Wave 3 engineering done；EVM local-output identity follow-on done：EVM/CW exact endpoint、EVM explicit output/runtime digest join + CALL-time code hash；Solana local-output identity follow-on done：state-bearing single-callee void-call 的 1..8 rows + executable program outer join、local output/ELF 三 digest exact join + caller Plan retention、program residual 清零与 Mollusk 闭环。EVM deployment proof、Solana on-chain ProgramData/code identity、CW identity、Solana unsupported shapes、target inspect static residual、其它 families、formal/C-3 仍开 |
...
| ID | 项 | 状态 |
|---|---|---|
| **COMP-2-EVM-ADDR** | opt-in pre-placed 20-byte binding 与 local output/runtime identity 已由 ADR-0053 完成；剩 deployment/address provenance、Bool/Int/Bytes returndata；C-3 保持 FC | **partial** — endpoint + local runtime identity done；deployment proof 等仍 pending |
| **COMP-2-SOL-CPI** | generic call-bind 支持子集与 local output/ELF identity 已完成；扩大 schedule/result/multi-callee 前另决策 | **partial** — Wave 3 product/runtime closure + caller Plan local ELF identity retention done；WideDiv/WideDiv256 已由独立 Rust oracle + Mollusk 覆盖 UInt128/256 div/mod 成功与零除 `Custom(0x1001)` 全账户回滚，WideDivDispatch 已执行最远 handler，故该 engineering runtime 项已闭；active classic Token/ATA catalog 已各自绑定固定 Loader-v3 ELF（历史 preactivation absent/false 不改）；ProgramData/on-chain code identity 与 async 保持 FC；非 formal/hermetic |
...
能力层 waves（CAP-1a…5、CAP-X-BYTES、CAP-X-MERKLE）与诚实边界波均 **done（2026-08-19）**。ADR-0053 Wave 3 已于 2026-08-21 闭合 Solana 支持子集的 outer AccountInfo join 与产品 runtime 门；ADR-0036 / ADR-0053 同日已由 owner directive accepted。当前进入 accepted 四目标工程 DoD，按 **EVM → Solana → NEAR → Noir** 对账；EVM 已闭合显式本地 callee output/runtime artifact identity join 与 CALL-time code-hash gate；Solana 已闭合显式本地 callee output/ELF identity join 与 caller Plan retention。**不**标 B-CALL-SEM closed：EVM receipt/address provenance/constructor/deployment proof、Solana ProgramData/on-chain code identity、CosmWasm identity、Solana unsupported shapes、其它 target families、target static inspect、formal/C-3 仍开。剩余人拍项见 [`docs/plan/remaining-owner-waves.md`](plan/remaining-owner-waves.md)（Wave 0–8；不开 XRPL TIME/CALLER，不标 formal done）。完善度队列见 [`plan/completeness-phased-roadmap.md`](plan/completeness-phased-roadmap.md) 与上文 **§12**。
...
2. B-CALL-SEM 产品决策 + EVM 残差（并行于 1 的 leaf；**决策，非 Goal drain**）：
   - ~~EVM opt-in pre-placed 20-byte endpoint binding~~（ADR-0053 Wave 2 done）；无表 hashed-address stub 保持
   - ~~EVM 本地 callee output + runtime artifact identity join + CALL-time code hash gate~~（2026-08-21 done）；receipt/address provenance/CREATE/CREATE2/constructor state/deployment process 仍 open
   - ~~result-bearing 宽于 UInt64 的 returndata~~（2026-08-13 **done**；Bool/Int/Bytes 仍 FC）
   - **hash 结果端序决策（2026-08-19 登记，人拍）**：`pf.crypto.sha256*(UInt256)->UInt256` 的整数级值跨 target 不一致——EVM 返回 BE 字整数（digest 即 BE word），Solana/NEAR 经 runtime 门实测为「digest 原始字节即 LE image」（整数 = LE-read of digest）。同一 source 程序在两约定下得到**不同 UInt256 整数**。NEAR sandbox 针已按实测行为（raw-bytes/LE image）钉绿；EVM 侧 Anvil 针为 BE word。要拍：统一为哪种约定（或文档化「digest 是字节序容器、整数解释随 target」），再决定哪侧发射器/针需要翻转 |
3. Solana 残差：
   - ~~generic call-bind 支持子集 product CPI callee / 外层账户 ABI~~（2026-08-21 Wave 3 done）
   - ~~local output/ELF identity digest verification + caller Plan retention~~（2026-08-21 done）；ProgramData/on-chain code identity、schedule、generic result-bearing、empty-row/state、multi-callee、mixed frozen-site 继续 FC/open
...
| 2026-08-15 | **SOL-0048-OVF-CERT**：同一production artifact + concrete Loader reads + exact fuel 55/56 boundary闭合`increment(UInt64.max, 1)` overflow sparse provider certificate；status/r0=`0x1001`、account不变、return data为空，并接入certified HandlerIR/provider production join。至此D4四条pinned recipe均有dedicated sparse certificate。**不**关闭formal D5/ELF/SVM runtime |
| 2026-08-15 | **SOL-0048-D5-INIT-COMPOSE**：从同一次elaborated source/production compiler恢复initializer的validated Semantic、Reference admission与真实`stepReferenceSliceV1` outcome；Boolean gate把既有Reference→HandlerIR relation与55步certified provider join组合成单一source-derived Reference→provider carrier。checker前提仍保留；increment success/overflow concrete composition待续。**不**关闭无条件formal D5/ELF/SVM runtime |
| 2026-08-15 | **SOL-0048-D5-INC-COMPOSE**：从同一次elaborated source/production compiler恢复`increment(41, 1)`的validated Semantic、Reference admission、logical pre/post state与真实`stepReferenceSliceV1` outcome；Boolean gate把successful checked-add Reference→HandlerIR relation与70步certified provider join组合成单一source-derived Reference→provider carrier。checker前提仍保留；overflow concrete composition待续。D4四条certificate继续保持55/55/70/56步。**不**关闭无条件formal D5/ELF/SVM runtime |
| 2026-08-15 | **SOL-0048-D5-OVF-COMPOSE**：复用同一个private production increment subject，从其validated Semantic/Reference admission恢复`increment(UInt64.max, 1)` exact pre-state与真实arithmetic-overflow outcome；Boolean gate把Reference revert、Handler arithmetic trap/account snapshot与56步certified provider `0x1001`/unchanged-account/empty-return组合成单一carrier，sound theorem实际调用`.referenceJoin`。checker前提仍保留；无第二套semantics/lowering。**不**关闭无条件formal D5/ELF/SVM runtime |
| 2026-08-15 | **SOL-0048-D5-GET-COMPOSE**：从同一次elaborated source/production compiler恢复`get(41)`的validated Semantic、Reference admission、exact logical pre-state与真实nullary `stepReferenceSliceV1` outcome；Boolean gate固定Reference same-state UInt64 return/empty effects、Handler return/account stutter，并通过55步certificate的`.referenceJoin`形成单一carrier。篡改Reference outcome FC。至此四条pinned recipe的concrete composition均闭合；checker前提仍保留，无第二套semantics/lowering。**不**关闭无条件formal D5/ELF/SVM runtime |
| 2026-08-17 | **SOL-0048-D5-STATECELL-CARRIER-RESOURCE-CERTIFICATE**：真实StateCell lowering按production field encoder逐层组合出canonical wire success/resource bound，再与同一次source binding、完整Typed certificate及production lowering通过唯一`normalizeProgramV1`闭合exact carrier。无copied bytes、第二encoder或合约名分支；Codec仅增通用6-element array与1/2-field size seam。**不**关闭compiler identity、Solana Plan/IR、emitter、SHA、unconditional `get`、ELF或SVM runtime |
| 2026-08-17 | **SOL-0048-D5-STATECELL-COMPILER-IDENTITY-CERTIFICATE**：exact canonical carrier经production codec inversion及唯一structure validator恢复同一Semantic data；通用compiler stage-composition seam将唯一normalizer/validator/source hash/semantic hash送入private production mint，StateCell据此闭合`CompiledSemanticV1` identity。通用SHA theorem只证fixed-width，不复制carrier或具体digest；无第二decoder/compiler/hash或合约名分支。**不**关闭production preparation、Solana Plan/IR、emitter equation、assembly SHA、unconditional `get`、ELF或SVM runtime |
| 2026-08-17 | **SOL-0048-D5-STATECELL-PREPARATION-INGRESS-CERTIFICATE**：通用十阶段定理仅从全部真实production equations构造既有preparation并经唯一resolver replay；StateCell同一compiled carrier闭合production validation与private Reference admission。frozen registry/selection因缺compositional kernel seam仍open，whole-registry `decide`不接受。**不**关闭selection、capability、Plan/IR、emitter、SHA、unconditional `get`、ELF或SVM runtime |
| 2026-08-18 | **SOL-0048-D5-TARGET-REGISTRY-SELECTION-CERTIFICATE**：唯一production registry validator/constructor经保持phase precedence的组合式分解、结构递归helper与ASCII canonical insertion sort获得kernel-replayable frozen equations；唯一`resolveBuildSelectionV1`闭合Solana `solana-sbpf-cpi-elf-v1` exact target/profile/kind existential certificate。无copied registry/profile data、第二resolver/checker或额外private selection mint。**不**关闭capability、Plan/IR、emitter、SHA、unconditional `get`、ELF或SVM runtime |
| 2026-08-18 | **SOL-0048-D5-SOLANA-ENGINEERING-CAPABILITY-CERTIFICATE**：唯一17-row support index validator/private mint、registry-root digest、bulk engineering SupportClaim与exact Solana row/claim lookup获得kernel equations，并与既有selection、真实compiled validation及retained requirements通过唯一`resolveEngineeringRequirementsV1`组合。StateCell仅作通用回归实例，无contract-name分支、第二index/resolver/hash/claim/selection/capability或copied digest。**不**关闭Plan/IR、emitter、SHA、unconditional `get`、ELF或SVM runtime |
...
| 2026-08-18 | **Leftover 多波拉齐（pins → B-CONST-STR → N-A1-COPY）**：`Targets.lean` 把 XRPL 补进已 admit 的 public-UInt64 环（Ledger/BoolPredicate/BranchFlow/LoopSum/StateCell/Accumulator）与 MapPrin/容器 FC 针（**不**进 MapPrin positive）。**B-CONST-STR**：十三家 `const GREETING : String := "hi"` 走同一 `n==9` flatten 再 `Op.Constant` 内联成 9 个 UInt64 叶字面量（不是 UTF-8 ABI；Principal const / source literal 仍 FC）。**N-A1-COPY**：十二家复制 EVM String match desugar（scrut 恰好 9 叶 + leaf-wise `==` + 嵌套 `ifThenElse`；Plan `switchOn` 仍仅 UInt64）；TON entry 可 match String（控制流，不是 entry 多叶 return）。Option **UInt64/Int64 param** 十三家已 flatten；Option-of-String 仍 FC。remap / AccountID / pubkey / Candid `principal`/`text` / TON entry 聚合仍 FC |
| 2026-08-18 | **B-RET-PRIN + leftover XRPL pin**：十三家 Principal **恰好 9 叶** view/entry return（`len+w0..w7`；不是把 8 叶 cap 改成 9；TON **仅 view**，entry 聚合仍 FC）。EVM/Psy 同例外开 String 9 叶 return。`Targets.lean` 把 XRPL 补进它已 admit 的 Map/Option/Array/Bytes/Principal return 环。remap / AccountID / pubkey / Candid `principal` / TON entry 聚合仍 FC |
| 2026-08-18 | **Leftover-4 Map B-RET-MAP**：Solana/Noir/Psy entry+view 与 TON **view-only** 开 `Map UInt64 UInt64` / `Map UInt64 Int64` 24 叶返回（不是把 8 叶 cap 改成 24）；四家 Map **param** 同套 occ/key/val flatten。EVM hashed 1-slot 与 TON entry 聚合保持 FC。Int64-key / nested / Map-of-Option / Principal 9 叶返回仍 FC |
| 2026-08-18 | **Aleo Map param + 6-leaf return**：复用既有 cap-2 `flattenTypeLeafSpecsV1`（occ/key/val × 2 = 6 ≤ B-RET 8）。`Map UInt64 UInt64` / `Map UInt64 Int64` entry/view return 与 named param flatten 已开。**不**发明 24 叶 B-RET-MAP。Int64-key / nested / Map-of-Option / EVM Map return 仍 FC |
| 2026-08-18 | **ICP 齐次 Int64 容器 state**：`Option Int64` / `Map Int64 Int64` 复用既有 2 叶 / 24 叶 i64 globals（无 Candid `opt`/`map`）。混合 UInt64-param OptInt / Map UInt64 Int64 仍 FC。Principal return / TON entry 聚合 / Int64-key / UInt128 信封仍 FC |
| 2026-08-18 | **M1–M4 Int64 容器返回 + Bytes return**：十三家 Array Int64 N return（TON 仅 view）；Option Int64 return 2 叶 tag unsigned + payload `isInt`（ICP 当时 construct-only；齐次 state 见上行）；Map Int64 return 24 叶只开 NEAR/CW/Quint/Soroban/OpenVM/ICP/XRPL；Bytes return 补 EVM/Solana/Noir/Aleo/TON-view 五洞。Principal return / TON entry 聚合 / Int64-key / UInt128 信封仍 FC |
...
| 2026-08-15 | **MAT-RET-QS**：B-RET-ABI PairRet view-return 十二 target 补齐。七个 admit；Aleo view-over-state。**T6** 已开 Quint/Soroban/ICP/OpenVM **view** 半边。PairRetEntry Aleo pin 保留。entry named 仍 FC |
...
| 2026-08-15 | **MAT-SRCAUTH-QS**：Plan source-authority rg 扩到十二 facade。十一 facade 自带三 token；Soroban 回退 LowerSemanticV1。forbidden residual-alpha 均为空。**不**第 13 target |
| 2026-08-15 | **MAT-PRIVPARAM-QS**：unused private-param 十二 target 补齐。EVM/Noir + Solana/NEAR/Psy/Aleo/CW/TON admit；Quint/Soroban/ICP/OpenVM public-param envelope FC。**不**改 disclosure |
| 2026-08-15 | **MAT-PRINRET-QS**：Principal view-return 十二 target 补齐。十二个均 FC（EVM pin 保留）。storage admit ≠ ResultKind。**不**开 Principal return / remap |
| 2026-08-15 | **MAT-LEDGER-QS**：rich public-UInt64 Ledger 十二 target 补齐。十二个均 materialize。四 Plan pin 保留。**不**开新 shape |
...
| 2026-08-15 | **MAT-BRANCH-QS**：BranchFlow if/match 十二 target 补齐。八个 admit；当时 Quint/Soroban/OpenVM/ICP single-block envelope FC。**T9a/T9b** 已开四家 if-diamond + `switchOn`。**T9c** 已开 LoopSum 计数 for。不可约 CFG / 臂内 call·emit 仍 FC。四 Plan/IR/file pin 保留 |
...
| 2026-08-15 | **MAT-EXT-QS**：ExtFlow 十二 target 补齐。EVM/Noir admit；十个 named FC（async/sync/event）。四 Plan pin 保留。**不**开 B-CALL-SEM |
| 2026-08-15 | **MAT-BIT-QS**：BitLogic 十二 target 补齐。七个 admit；Aleo Final-only unused-state 与 Quint/Soroban/OpenVM/ICP UInt64-width FC。四 Plan/IR/file pin 保留。**不**开 shift/bitwise |
| 2026-08-15 | **MAT-LOOP-QS**：LoopSum bounded-for 十二 target 补齐。八个 admit；Quint/Soroban/OpenVM/ICP single-block envelope FC。四 Plan/IR pin 保留。**不**开 multi-block/for |
...
| 2026-08-15 | **MAT-ENUM-RET-QS**：MaybeRetBox named Enum entry-return 十二 target 补齐。七个 admit；TON view-only B-RET。**T6** 已开四 envelope **view** 半边（MaybeViewRet）；entry MaybeRetBox 仍 FC。MaybeMark state pin 保留。 |
| 2026-08-15 | **MAT-MAP-RET-QS**：MapRetBox Map UInt64 entry-return 十二 target 补齐。两个 admit（NEAR/CW）；其余十个 named B-RET/container FC。**不**开 Map return ABI。 |
| 2026-08-15 | **MAT-I128-QS**：WideInt128 Int128 十二 target 全 named width FC。**不**开 signed 128/256。 |
| 2026-08-15 | **MAT-I256-QS**：WideInt256 Int256 十二 target 全 named width FC（针与 Int128 相同）。**不**开 signed 128/256。 |
| 2026-08-15 | **MAT-OPT-RET-QS**：OptRetBox Option UInt64 entry-return 十二 target 补齐。七个 admit；TON view-only B-RET。**T6** 已开四 envelope **view** 半边（OptViewRet）；entry OptRetBox 仍 FC。OptBox state pin 保留。 |
| 2026-08-15 | **MAT-ARR-RET-QS**：ArrRetBox Array UInt64 2 entry-return 十二 target 补齐。七个 admit；TON view-only B-RET + 四 envelope Array-pilot FC。**不**开 Array return on decline set。ArrRetEntry Aleo pin 保留。 |
| 2026-08-15 | **MAT-NEST-OPT-QS**：NestOpt Option Option UInt64 十二 target 全 named payload/pilot FC。**不**开 nested Option。 |
| 2026-08-15 | **MAT-ARR-VIEW-RET-QS**：ArrViewRet Array UInt64 2 view-return 十二 target 补齐。七个 admit（含 TON）；Aleo computed-view。**T6** 已开四 envelope **view** 半边。与 ArrRetBox entry 相对。**不**开 Aleo computed view。 |
| 2026-08-15 | **MAT-OPT-VIEW-RET-QS**：OptViewRet Option UInt64 view-return 十二 target 补齐。七个 admit（含 TON）；Aleo computed-view。**T6** 已开四 envelope **view** 半边。与 OptRetBox entry 相对。**不**开 Aleo computed view。 |
| 2026-08-15 | **MAT-ENUM-VIEW-RET-QS**：MaybeViewRet named Enum view-return 十二 target 补齐。七个 admit（含 TON）；Aleo computed-view。**T6** 已开四 envelope **view** 半边。与 MaybeRetBox entry 相对。**不**开 Aleo computed view。 |
| 2026-08-15 | **MAT-NEST-ARR-QS**：NestArr Array Array UInt64 2 2 十二 target 全 named element/pilot FC。**不**开 nested Array。 |
| 2026-08-15 | **MAT-ARR-OPT-QS**：ArrOpt Array Option UInt64 2 十二 target 全 named element/pilot FC。**不**开 Array-of-Option。 |
| 2026-08-15 | **MAT-OPT-ARR-QS**：OptArr Option Array UInt64 2 十二 target 全 named payload/pilot FC。**不**开 Option-of-Array。 |
| 2026-08-15 | **MAT-MAP-OPT-QS**：MapOpt Map UInt64 Option UInt64 十二 target 全 named Map-value/pilot FC。**不**开 Map-of-Option。 |
| 2026-08-15 | **MAT-ARR-BYTES-QS**：ArrBytes Array Bytes 4 2 十二 target 全 named element/pilot FC。**不**开 Array-of-Bytes。 |
| 2026-08-15 | **MAT-MAP-ARR-QS**：MapArr Map UInt64 Array UInt64 2 十二 target 全 named Map-value/pilot FC。**不**开 Map-of-Array。 |
| 2026-08-15 | **MAT-MAP-BYTES-QS**：MapBytes Map UInt64 Bytes 4 十二 target 全 named Map-value/pilot FC。**不**开 Map-of-Bytes。 |
| 2026-08-15 | **MAT-OPT-BYTES-QS**：OptBytes Option Bytes 4 十二 target 全 named payload/pilot FC。**不**开 Option-of-Bytes。 |
| 2026-08-15 | **MAT-OPT-MAP-QS**：OptMap Option Map UInt64 UInt64 十二 target 全 named payload/pilot FC。**不**开 Option-of-Map。 |
| 2026-08-15 | **MAT-MAP-MAP-QS**：MapMap Map UInt64 Map UInt64 UInt64 十二 target 全 named Map-value/pilot FC。**不**开 Map-of-Map。 |
| 2026-08-15 | **MAT-ARR-MAP-QS**：ArrMap Array Map UInt64 UInt64 2 十二 target 全 named element/pilot FC。**不**开 Array-of-Map。 |
...
| 2026-08-15 | **MAT-OPT-FIELD-QS**：OptField Option Field bn254_fr 十二 target 全 named payload/Field FC。**不**开 Option-of-Field。 |
| 2026-08-15 | **MAT-ARR-FIELD-QS**：ArrField Array Field bn254_fr 2 十二 target 全 named element/Field FC。**不**开 Array-of-Field。 |
| 2026-08-15 | **MAT-MAP-FIELD-QS**：MapField Map UInt64 Field bn254_fr 十二 target 全 named Map-value/Field FC。**不**开 Map-of-Field。 |
| 2026-08-15 | **MAT-ARR-STR-QS**：ArrStr Array String 2 十二 target 全 named element/String FC。**不**开 Array-of-String。 |
| 2026-08-15 | **MAT-OPT-STR-QS**：OptStr Option String 十二 target 全 named payload/String FC。**不**开 Option-of-String。 |
| 2026-08-15 | **MAT-MAP-STR-QS**：MapStr Map UInt64 String 十二 target 全 named Map-value/String FC。**不**开 Map-of-String。 |
| 2026-08-15 | **MAT-ARR-BOOL-QS**：ArrBool Array Bool 2 十二 target 全 named element/pilot FC。**不**开 Array-of-Bool。 |
| 2026-08-15 | **MAT-OPT-BOOL-QS**：OptBool Option Bool 十二 target 全 named payload/pilot FC。**不**开 Option-of-Bool。 |
| 2026-08-15 | **MAT-MAP-BOOL-QS**：MapBool Map UInt64 Bool 十二 target 全 named Map-value/pilot FC。**不**开 Map-of-Bool。 |
| 2026-08-15 | **MAT-OPT-INT-QS**：OptInt Option Int64 八 target named payload FC；Quint/Soroban/OpenVM/ICP 走宽度针（异于 Option-pilot）。**不**开 Option-of-Int64。 |
| 2026-08-15 | **MAT-MAP-INT-QS**：MapInt Map UInt64 Int64 八 target named Map-value/pilot FC；Quint/Soroban/OpenVM/ICP 走宽度针（异于 Map-pilot）。**不**开 Map-of-Int64。 |
| 2026-08-15 | **MAT-ARR-INT-QS**：ArrInt Array Int64 2 八 target named element FC；Quint/Soroban/OpenVM/ICP 走宽度针（异于 Array-pilot）。**不**开 Array-of-Int64。 |
| 2026-08-15 | **MAT-MAP-INT-KEY-QS**：MapIntKey Map Int64 UInt64；EVM/Solana 走 key-shape 针（异于 MapInt value）；六家 Map-U64-U64 + Psy pilot；Quint 四家宽度针。**不**开 Int64-key Map。 |
| 2026-08-15 | **MAT-WIDE-INT64-QS**：WideInt64 八 target files-nonempty admit；Quint/Soroban/OpenVM/ICP 走宽度针。**不**开 Int8/16/32。 |
...
| 2026-08-15 | **MAT-MAP-U256-QS**：MapU256 十二 target 全 FC（UInt256 ≠ UInt128）；Aleo/TON 走宽度针。**不**开 Map-of-UInt256。 |
| 2026-08-15 | **MAT-OPT-U128-QS**：OptU128 十二 target 全 FC；Aleo/TON 走宽度针（异于 OptInt payload）。**不**开 Option-of-UInt128。 |
| 2026-08-15 | **MAT-OPT-U256-QS**：OptU256 十二 target 全 FC（UInt256 ≠ UInt128）；Aleo/TON 走宽度针。**不**开 Option-of-UInt256。 |
| 2026-08-15 | **MAT-MAP-U128-KEY-QS**：MapU128Key；EVM/Solana 走 key-shape（异于 MapU128 value）；Aleo/TON 走宽度针（异于 MapIntKey）。**不**开 UInt128-key Map。 |
| 2026-08-15 | **MAT-MAP-U256-KEY-QS**：MapU256Key 十二 target 全 FC（UInt256-key ≠ UInt128-key）；EVM/Solana key-shape；Aleo/TON 宽度针。**不**开 UInt256-key Map。 |
...
| 2026-08-15 | **MAT-ARR-U16-QS**：ArrU16 仅 EVM admit（UInt16 ≠ UInt32）；Aleo/TON Array-U64-element。**不**开其余十一家 Array-of-UInt16。 |
| 2026-08-15 | **MAT-ARR-U8-QS**：ArrU8 仅 EVM admit（UInt8 ≠ UInt16）；Aleo/TON Array-U64-element。窄 unsigned Array 钉完。 |
| 2026-08-15 | **MAT-OPT-U32-QS**：OptU32 十二 target 全 FC；Aleo/TON 走 payload 针（合法宽度，异于 OptU128 宽度针）。**不**开 Option-of-UInt32。 |
| 2026-08-15 | **MAT-OPT-U16-QS**：OptU16 十二 target 全 FC（UInt16 ≠ UInt32）；Aleo/TON payload 针。**不**开 Option-of-UInt16。 |
| 2026-08-15 | **MAT-OPT-U8-QS**：OptU8 十二 target 全 FC（UInt8 ≠ UInt16）；Aleo/TON payload 针。窄 unsigned Option 钉完。 |
| 2026-08-15 | **MAT-MAP-U8-KEY-QS**：MapU8Key 十二 target 全 FC（UInt8-key ≠ UInt16-key）；Aleo/TON Map-U64-U64。窄 unsigned Map-key 钉完。 |
| 2026-08-15 | **MAT-MAP-U16-QS**：MapU16；EVM/Solana value 针（异于 MapU16Key）；Aleo/TON Map-U64-U64。**不**开 Map-of-UInt16。 |
| 2026-08-15 | **MAT-MAP-U8-QS**：MapU8；EVM/Solana value 针（异于 MapU8Key）。窄 unsigned Map-value 钉完。 |
| 2026-08-15 | **MAT-ARR-I32-QS**：ArrI32 十二 target 全 FC；EVM decline（异于 ArrU32 admit）；Aleo/CW/TON 走宽度/narrow-Int（异于 ArrInt element）。**不**开 Array-of-Int32。 |
| 2026-08-15 | **MAT-OPT-I32-QS**：OptI32 十二 target 全 FC；Aleo/CW/TON 走宽度/narrow-Int（异于 OptInt/OptU32 payload）。**不**开 Option-of-Int32。 |
| 2026-08-15 | **MAT-ARR-I16-QS**：ArrI16 十二 target 全 FC（Int16 ≠ Int32）；EVM decline（异于 ArrU16）；Aleo/CW/TON 宽度/narrow-Int。**不**开 Array-of-Int16。 |
| 2026-08-15 | **MAT-ARR-I8-QS**：ArrI8 十二 target 全 FC（Int8 ≠ Int16）；EVM decline（异于 ArrU8）。窄 signed Array 钉完。 |
| 2026-08-15 | **MAT-MAP-I32-QS**：MapI32 十二 target 全 FC；Aleo/CW/TON 走宽度/narrow-Int（异于 MapInt/MapU32 的 Map-U64-U64）。**不**开 Map-of-Int32。 |
| 2026-08-15 | **MAT-OPT-I16-QS**：OptI16 十二 target 全 FC（Int16 ≠ Int32）；Aleo/CW/TON 宽度/narrow-Int。**不**开 Option-of-Int16。 |
| 2026-08-15 | **MAT-MAP-I32-KEY-QS**：MapI32Key；EVM/Solana key-shape（异于 MapI32 value）；Aleo/CW/TON 宽度/narrow-Int（异于 MapIntKey）。**不**开 Int32-key Map。 |
...
| 2026-08-17 | **XRPL-0**：Bedrock WASM / XLS-0101 研究叶落盘（[`targets/16-xrpl.md`](targets/16-xrpl.md) · [`plan/xrpl-bedrock-wasm-gap.md`](plan/xrpl-bedrock-wasm-gap.md) · [`plan/xrpl-bedrock-wasm-tasks.md`](plan/xrpl-bedrock-wasm-tasks.md)）。建议 Q0 = source-only Rust guest；当时**不**进 registry、**不**当默认 Next、**不**混 Hooks/EVM 侧链/OpenVM。 |
| 2026-08-17 | **XRPL-A / ADR-0049**：`TargetId.xrpl` + sole `xrpl-bedrock-source-u64-v1` source-only Q0。registry **13+0**、resolver **16** rows；`{name}.rs` Bedrock 形（`xrpl_wasm_std` / `get_data`/`set_data` / `no_mangle`）；zero-tool Finalize；`deployable=false`。**不**扩 accepted PRD、**不**关 formal、**不**调 rustc/bedrock/AlphaNet。 |
| 2026-08-17 | **XRPL-B / ADR-0050**：opt-in `xrpl-bedrock-wasm-u64-v1`。共享 Plan/`.rs`；Finalize 用 ambient rustc/cargo 编 `wasm32-unknown-unknown` extra `xrpl-build/{name}.wasm`；`xrpl-wasm-std` pin `ffbe88da26df27e59a72b6202883f42f696933cc`；resolver **17** rows；默认 source 仍 zero-tool；`deployable=false`。**不**调 bedrock/AlphaNet/主网。 |
| 2026-08-17 | **XRPL-CAP-D / ADR-0052**：冻 `get_parent_ledger_time`（Ripple Time +946684800）与 `get_current_contract_call().get_account()`；`pf.crypto.sha256` keep-FC（仅 `compute_sha512_half`）。**不开** Plan/IR 叶。 |
...
| 2026-08-16 | **NEAR-INT64-CONTAINERS**：NEAR `Array Int64 N` / `Option Int64` / `Map UInt64 Int64` 跟随 CosmWasm/TON per-leaf `isInt` flatten（Array 全叶 signed；Option tag unsigned + payload signed；Map 仅 val `i%3==2`）。Int8 容器、Int64-key Map、Int64 container return 仍 FC。**不**声称 sandbox / formal / runtime parity |
| 2026-08-16 | **SOL-INT64-CONTAINERS**：Solana `Array Int64 N` / `Option Int64` / `Map UInt64 Int64` 跟随同一 per-leaf `isInt` flatten（cap-8 / 24 叶；Principal Map 仍 UInt64-only）。Int8 容器、Int64-key、Int64 return 仍 FC。Mollusk 不在本切片。**不**声称 formal / runtime parity |
...
| 2026-08-17 | **F-TYPEKEY-RANK-C**：TypeKey Stage A/B/C 工程收口——Normalize `canonicalizeAnonymousTypeRankV1`；六 subject + InvariantABI golden 迁 SPEC 序；结构门 `anonymousRank` 上线；`validateAnonymousTypeUsageClosureV1` 已实现但未接线 StructureV1（fat fixture 待紧）。`cfgOpTypes` 迁 Map/Bool/UInt8/UInt32/Bytes/Option。**不**关闭 TASK-D2-06 / TST-SEM-001 |
| 2026-08-17 | **F-TYPEKEY-USAGE-D（in progress）**：TypeKey Stage D——紧 fixture 后把 `validateAnonymousTypeUsageClosureV1` 接入 StructureV1；控制面 Next 从 CAP-1a 改指 Stage D。**不**关闭 TASK-D2-06 / TST-SEM-001 |
| 2026-08-17 | **F-TYPEKEY-USAGE-D（done）**：StructureV1 `usageClosure` 接线；fixture compact/Core-anchor；Normalize 去掉强制 unused UInt64 envelope；SimpleClosure Bool-only；Wire needle P0/N0/Prec。控制面 Next → B-CALL-SEM / CAP-X-BYTES。**不**关闭 TASK-D2-06 / TST-SEM-001 |
...
| 2026-08-20 | **ADR-0053 Wave 1（parse-only）**：新增 `proof-forge.call-bind.v1` + `build --bindings`；表可解码、可查 QN；产品 EVM/Solana/CosmWasm emit 不变。无 flag 行为不变。`check` 与非三叶拒 flag。**不**关 B-CALL-SEM / formal / C-3 / inspect residual。 |
| 2026-08-20 | **ADR-0053 Wave 2（emit consume）**：有 `--bindings` 时三叶 generic call/schedule 无行 fail closed；EVM Yul / CW WAT `contract_addr` / Solana empty-meta program id 消费表。无 flag 行为不变。表显式下传（不进 capability、无 IO.Ref）。Solana nonempty accounts = Wave 2b；空账户 void CALL = Wave 2a。inspect residual / B-CALL-SEM / formal / C-3 仍开。 |
| 2026-08-20 | **ADR-0053 Wave 2a（empty-account void CALL）**：generic void CALL 在 `extcodesize==0` 时 fail closed；`schedule` / result-bearing / `pf.assets` 不改。inspect residual / Wave 2b / B-CALL-SEM / formal / C-3 仍开。 |
| 2026-08-20 | **ADR-0053 Wave 2b（Solana compile-time AccountMeta）**：nonempty `accounts` 打成编译期 AccountMeta（pubkey + signer/writable，≤8）；empty accounts 仍 empty-meta；syscall r2/r3 仍 0（**不是** outer-instruction account join）。inspect residual / B-CALL-SEM / formal / C-3 仍开。 |
| 2026-08-20 | **ADR-0053 Wave 2c（build program-level residual）**：成功 `build` 露出 `callScheduleResidual`（string 或 JSON `null`）。无 generic call → `null`；EVM/CW 全部 generic call 有精确行 → `null`；Solana 有 generic call 仍 `callee-identity-outer-account-open`。target `inspect <target>` 仍静态 kind 闭表。不进 SupportClaim / manifest / evidence / inspect-output。**不**关 B-CALL-SEM / formal / C-3。 |
| 2026-08-21 | **ADR-0053 Wave 3（Solana outer AccountInfo join）**：state-bearing + single generic callee + synchronous void CALL + 1..8 non-alias rows + no frozen CPI sites 的产品子集，把 state 后的 bound accounts 与 executable callee program 纳入 Plan/IDL/IR/client exact role projection；SBPF 精确校验 key/signer/writable/executable，向 `sol_invoke_signed_c` 传 rows+program AccountInfos；product-built caller/callee 过 8 项 Mollusk positive/negative/rollback；支持子集 program residual=`null`。empty-state/schedule/empty-row/multi-callee/mixed-site/generic result-bearing 与 identity digest verification 保持 FC/open；target inspect 静态；**不**关全 B-CALL-SEM / formal / C-3 / release。 |
| 2026-08-21 | **EVM call-bind local-output identity follow-on**：finalizer 以 locked-solc standard JSON 同源产出 creation `.bin` + deployed `.runtime.bin`；repeatable `--callee-output` 复用完整 `proof-forge.output.v1` inspect，按 callee program name + source/semantic/runtime-artifact SHA-256 exact join，decoded runtime Keccak 进入 bound CALL/schedule 前 `EXTCODEHASH` gate。missing/duplicate/unused/mismatch/symlink/path negatives FC；无 bind hashed-QN 不变。无 RPC/receipt/CREATE/CREATE2/address provenance/constructor/deployment proof；**不**关全 B-CALL-SEM / formal / C-3 / release。 |
| 2026-08-21 | **Solana call-bind local-output identity follow-on**：repeatable `--callee-output` 复用完整 `proof-forge.output.v1` inspect，要求 deployable Solana output + 恰好一个 `{program}.so` finalized-extra；按 callee program name + source/semantic/ELF-artifact SHA-256 exact join，把三 digest 保留进 caller CPI Plan 的 callBindProgram role。runtime 仍只校验表内 programId 对应的 account key + executable；Mollusk 证明 exact outer join/调用/回滚，不证明 ProgramData/upgrade authority/链上 ELF。active classic Token/ATA catalog 已固定 Loader-v3 ELF，历史 preactivation catalog 不改。无 RPC/deployment proof；**不**关全 B-CALL-SEM / formal / C-3 / release。 |
...
| **TON-5** | TON schedule sandbox 差分（ScheduleFlow） | **done**（2026-08-03；`678f78a9e`：`runtime-tests/ton` ScheduleFlow fixture + `schedule.test.js`；`later()` 产出恰好一条 internal out-message（bounce=false、value=0、dest stub、op=0x6f31304c、query_id=0、pre-increment arg），父状态仍 +1；Counter 无 createMessage 回归；当时全套 **10/10**，现 `@ton/sandbox` 语料 **16/16**（locked tolk + companion fift）；engineering differential，非主网/formal） |
