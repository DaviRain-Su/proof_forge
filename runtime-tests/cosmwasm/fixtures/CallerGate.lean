...
-- setOwner rebinds owner to the current sender. View has no sender and
-- must not read context.caller (Plan FC). Production-code-generated
-- test fixture only — not a mainnet contract.
