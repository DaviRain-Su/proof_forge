//! CosmWasm mock-runtime differential for NarrowCounter (UInt8 multi-width).
//!
...
fn narrow_instance() -> common::CwInstance {
    assert_abi_schema_if_present("NarrowCounter");
    make_instance("NarrowCounter")
}

/// init(7) → get == 7; state leaf is 8-byte LE with value in low byte.
#[test]
fn narrow_counter_init_and_query() {
...
#[test]
fn narrow_counter_increment_and_query() {
    let mut instance = narrow_instance();
...
#[test]
fn narrow_counter_overflow_traps_state_unchanged() {
    let mut instance = narrow_instance();
    expect_response_ok(instantiate_ok(
        &mut instance,
...
#[test]
fn narrow_counter_accepts_u8_max() {
    let mut instance = narrow_instance();
    expect_response_ok(instantiate_ok(
        &mut instance,
...
#[test]
fn narrow_counter_param_out_of_range_traps() {
    let mut instance = narrow_instance();
...
#[test]
fn narrow_counter_increment_param_out_of_range_traps() {
    let mut instance = narrow_instance();
    expect_response_ok(instantiate_ok(
        &mut instance,
