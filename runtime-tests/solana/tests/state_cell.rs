//! Mollusk runtime differential for the product StateCell ELF (S3a).
//!
//! Reference authority: `Tests/Semantic/ReferenceV1.lean` `testStateCellReferenceSlice`
//! (init → increment → get → arithmeticOverflow on UInt64 max+1) and
//! `ProofForgeV2/Examples/StateCell.lean`.
//!
//! Env (required; no hard-coded product paths):
//! - `PROOF_FORGE_STATE_CELL_OUT` — complete published StateCell output tree.
//!
//! Both `StateCell.so` and `StateCell.sbpf-plan` are resolved from exact manifest
...
fn state_cell_fields() -> [StateField; 1] {
    single_field("count")
}

fn state_cell_state(initialized: bool, count: u64) -> Vec<u8> {
    state_data(&state_cell_fields(), initialized, &[count])
}

fn assert_state_cell_plan() {
    assert_discriminators_match_plan(
        &state_cell_plan_bytes(),
...
#[test]
fn state_cell_initialize_sets_count() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    let disc = instruction_discriminator("initialize", 1);

    let ix = build_ix(program_id, state_key, &disc, &[5], true, true);
    let account = state_account(&program_id, state_cell_state(false, 0));
...
#[test]
fn state_cell_increment_updates_and_returns() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let account = state_account(&program_id, state_cell_state(true, 5));
...
#[test]
fn state_cell_get_returns_count() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let account = state_account(&program_id, state_cell_state(true, 8));
    let expected_return = 8u64.to_le_bytes();
...
#[test]
fn state_cell_increment_overflow_custom_0x1001() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let pre = state_cell_state(true, u64::MAX);
...
#[test]
fn state_cell_unknown_discriminator_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let account = state_account(&program_id, state_cell_state(true, 0));
...
#[test]
fn state_cell_wrong_owner_check_fails() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let account = state_account(&foreign, state_cell_state(true, 8));
    assert_custom1_preserves_exact_accounts(&mollusk, &ix, &[(state_key, account)]);
}
...
#[test]
fn state_cell_initialize_missing_signer_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_increment_not_writable_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_double_initialize_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let account = state_account(&program_id, state_cell_state(true, 1));
    assert_custom1_preserves_exact_accounts(&mollusk, &ix, &[(state_key, account)]);
}

/// Entry (mutate) on uninitialized header fails closed.
#[test]
fn state_cell_increment_uninitialized_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_get_uninitialized_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_malformed_marker_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    let disc = instruction_discriminator("get", 0);
    let ix = build_ix(program_id, state_key, &disc, &[], false, false);
    let mut data = state_cell_state(false, 7);
...
#[test]
fn state_cell_short_account_data_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_long_account_data_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    let disc = instruction_discriminator("get", 0);
    let ix = build_ix(program_id, state_key, &disc, &[], false, false);
    let mut data = state_cell_state(true, 3);
...
#[test]
fn state_cell_zero_accounts_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let account = state_account(&program_id, state_cell_state(true, 4));
    assert_custom1_preserves_exact_accounts(&mollusk, &ix, &[(state_key, account)]);
}

/// Two distinct accounts → num_accounts == 2 fails closed.
#[test]
fn state_cell_two_distinct_accounts_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let state = state_account(&program_id, state_cell_state(true, 4));
    let other = state_account(&program_id, state_cell_state(true, 0));
...
#[test]
fn state_cell_duplicate_account_meta_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_instruction_len_0_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_instruction_len_7_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
#[test]
fn state_cell_instruction_short_after_disc_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
/// Exact positive length already covered by state_cell_initialize/increment/get.
/// Trailing bytes after exact ABI length fail exact instruction_data_len.
#[test]
fn state_cell_instruction_trailing_bytes_custom_1() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
...
/// End-to-end chain matching the Reference StateCell trace shape.
#[test]
fn state_cell_reference_trace_chain() {
    assert_state_cell_plan();
    let program_id = Pubkey::new_unique();
    let mollusk = make_state_cell_mollusk(&program_id);
    let state_key = Pubkey::new_unique();
    ...
    let result = mollusk.process_and_validate_instruction(
        &init_ix,
        &[(state_key, state_account(&program_id, state_cell_state(false, 0)))],
        &[
            Check::success(),
            Check::account(&state_key)
                .data(&state_cell_state(true, 7))
                .build(),
        ],
    );
...
fn product_artifacts_present() {
    ...
    .expect("StateCell ELF binding");
    assert!(bytes.len() > 64, "StateCell.so too small: {}", bytes.len());
    assert_eq!(&bytes[..4], b"\x7fELF", "StateCell.so is not ELF");
    assert!(!state_cell_plan_bytes().is_empty(), "plan missing or empty");
    assert_state_cell_plan();
    assert_ne!(layout_marker(&state_cell_fields()), 0);
}
