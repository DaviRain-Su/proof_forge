...
static pf_tq_handoff_expectation_v2 pf_tq_test_expectation(
    pf_tq_handoff_principal_v2 principals[3]
) {
    static const char *const principal_ids[3] = {
        "principal-architecture", "principal-quality", "principal-security"
    };
    static const char *const key_ids[3] = {
        "key-architecture", "key-quality", "key-security"
    };
    static const char *const public_keys[3] = {
...
static int pf_tq_test_run(
    ...
    if (strcmp(mode, "--validate") == 0) {
        ...
        if (pf_tq_handoff_join_descriptor_v2(
                &handoff, &descriptor, error, sizeof(error)) != 0 || error[0] != '\0') {
