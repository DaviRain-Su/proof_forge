...
def ekind(k):
    if k not in ("Holds","Preserving"): raise E("proof kind must be Holds or Preserving")
    return null(f"ProofKind.{k}")
def eproof(inv,kind,th): return etag("ProofDecl",[eident(inv),ekind(kind),eqid(th)])
def dtag(b,o):
    n,o=ru32(b,o)
...
def dkind(b,o):
    tag,o=dtag(b,o)
    m={"ProofKind.Holds":"Holds","ProofKind.Preserving":"Preserving"}
    if tag not in m: raise E(f"unknown proof-kind tag '{tag}'")
    o=dfc(b,o,tag,0); return m[tag],o
def dproof(d,n,b,o):
    o=head(b,o,"ProofDecl","proof-decl",3); _,n=charge(d,n)
    inv,o=dident(b,o); kind,o=dkind(b,o); th,o=dqid(b,o); return ("proof",inv,kind,th),n,o
...
G=[
 ...
 ("proof",eproof("safe","Holds",["Proofs","safe"]),("proof","safe","Holds",("Proofs","safe")),1),
 ("proof",eproof("safe","Preserving",["Proofs","safe"]),("proof","safe","Preserving",("Proofs","safe")),1),
]
...
def reenc(v):
    ...
    return eproof(v[1],v[2],list(v[3]))
def setfc(raw,fc):
...
def self_check():
    ...
    expect("source name component must contain 1..240 UTF-8 bytes", lambda: dproof(2,4,etag("ProofDecl",[u32(0),ekind("Holds"),earr([eident("Only")])]),0)); nb+=1
    expect(QID_ERR, lambda: dproof(2,4,etag("ProofDecl",[eident("safe"),ekind("Holds"),earr([eident("Only")])]),0)); nb+=1
    expect("unknown proof-kind tag 'ProofKind.Bogus'", lambda: dproof(2,4,etag("ProofDecl",[eident("safe"),null("ProofKind.Bogus"),eqid(["Proofs","safe"])]),0)); nb+=1
    expect("tag 'ProofKind.Holds' must declare 0 fields", lambda: dproof(2,4,etag("ProofDecl",[eident("safe"),etag("ProofKind.Holds",[b"\x00"]),eqid(["Proofs","safe"])]),0)); nb+=1
    expect("tag 'ProofDecl' must declare 3 fields", lambda: dproof(2,4,etag("ProofDecl",[eident("safe"),eqid(["Proofs","safe"])]),0)); nb+=1
    raw=G[0][1]+b"\x00"; v,nodes,o=dstate(3,4,raw,0)
    if v[0]!="state" or o!=len(G[0][1]) or nodes!=2 or o==len(raw): raise SystemExit("trail")
    nb+=1
    if (npos,nfc,nb)!=(15,14,45): raise SystemExit(f"inventory {npos} {nfc} {nb}")
    print("reference_source_ast_decl_decode_v1: ok 15 14 45")
