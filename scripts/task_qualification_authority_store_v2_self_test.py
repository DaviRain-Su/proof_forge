...
This focused RED matrix freezes only the two production black-box surfaces
accepted by ADR-0021:

1. ``task_qualification_protected_adapter.protect_taskqualification_v1`` with
   exactly seven required positional-only parameters; and
...
not define. Protocol vectors in this file are test-owned arithmetic KATs only:
...
At this tests-only commit the accepted seven-parameter entrypoint is absent, so
all production black-box cases are RED for the stable reason ``API-ABSENT``.
The test-owned protocol KATs must already pass; otherwise the runner reports a
preflight failure rather than disguising a broken fixture as RED.
