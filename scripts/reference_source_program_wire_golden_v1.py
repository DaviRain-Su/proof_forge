#!/usr/bin/env python3
"""Independent full-tag ProgramV1 wire golden emitter/checker for D1-PA-125."""
...
SCHEMA = "proof-forge.source-program-wire-golden-prerequisite.v1"
SCOPE = "constructed-validated-programv1-no-frontend"
CASE_ID = "full-tag-valid-v1"
MODULE = ["Golden"]
PROGRAM_IDENTITY = ["Golden", "FullTag"]
PACKAGE = Path("testdata/golden/source-program-v1/full-tag-v1")
...
def build_fixture():
    ...
    all_expressions = [zero, maximum, false_value, true_value, unicode_value, place_expr,
                       ctor_empty, ctor_many, *unary, *binary, local_empty, local_many, match_expr]
    ...
    init_body = block([
        ...
        node("Stmt.Emit", ("event", ident("Changed")), ("args", ArrayValue(all_expressions))),
        node("Stmt.Return", ("value", option(match_expr))),
        node("Stmt.Return", ("value", option())),
        node("Stmt.Call", ("call", external_call(["Peer", "call"], all_expressions))),
        node("Stmt.Schedule", ("call", external_call(["Peer", "later"], []))),
    ])
    ...
    items = [
        node("StateDecl", ("visibility", visibility("Public")), ("name", ident("state")),
        ...
        node("InvariantDecl", ("name", ident("safe")), ("predicate", match_expr)),
...
def expected_package():
    root = build_fixture()
    observed_tags = set()
    canonical = qualified(MODULE) + qualified(PROGRAM_IDENTITY, 2) + encode(root, observed_tags)
    visits = canonical_visits(root)
    ...
    require(len(WIRE_TAGS) == 84 and observed_tags == WIRE_TAGS,
            f"wire tag inventory: {sorted(WIRE_TAGS - observed_tags)} / {sorted(observed_tags - WIRE_TAGS)}")
    require(len(NODE_TAGS) == 57 and node_tags == NODE_TAGS, "node tag inventory")
    require(len(EDGE_PAIRS) == 63 and edges == EDGE_PAIRS, "edge inventory")
    require(len(paths) == len(set(paths)), "node paths are not unique")
    items = root.fields[1][1]
    ...
    require(tagged_field_u16(root, "Type.UInt", 0) == WIDTHS, "UInt widths")
    require(tagged_field_u16(root, "Type.Int", 0) == WIDTHS, "Int widths")
    require({0, 4096}.issubset(tagged_field_u32(root, "Type.Array", 1)), "Array lengths")
    require({0, 4096}.issubset(tagged_field_u32(root, "Type.Bytes", 0)), "Bytes lengths")
    require(tagged_field_u32(root, "Stmt.For", 3) == {0, 4096}, "For bounds")
    require(tagged_field_bytes(root, "Literal.Bool", 0) == {b"\x00", b"\x01"},
            "Bool false/true")
    require(tagged_field_bytes(root, "Literal.Integer", 0) ==
            {u256le(0), u256le(MAX_U256)}, "Integer zero/max")
    array_sizes = {len(value.items) for value in walk_values(root)
                   if isinstance(value, ArrayValue)}
    require(0 in array_sizes and 1 in array_sizes and any(size > 1 for size in array_sizes),
            "empty/single/multiple arrays")
    options = option_coverage(root)
    ...
    require(any(isinstance(value, Tagged) and value.tag == "FieldDecl" and
                value.fields[0][1] == ident("café") for value in walk_values(root)),
            "NFC Unicode Ident coverage")
    require(string("café") in tagged_field_bytes(root, "Literal.String", 0),
...
def package_paths(root):
    ...
    return directory, directory / MANIFEST, directory / CANONICAL


def validate_checked_in(root):
    expected_binary, expected_document = expected_package()
    directory, manifest_path, canonical_path = package_paths(root)
    require(directory.is_dir(), f"missing golden package directory: {PACKAGE}")
    require({path.name for path in directory.iterdir()} == {MANIFEST, CANONICAL},
            "golden package must contain exactly manifest.json and canonical.bin")
...
def emit(root):
    canonical, document = expected_package()
    _directory, manifest_path, canonical_path = package_paths(root)
...
def main(argv):
    ...
    try:
        if argv == ["--emit"]:
            emit(root)
            print("reference_source_program_wire_golden_v1: emitted 1 84 57 63")
            return 0
        if argv == ["--self-check"]:
            validate_checked_in(root)
            print("reference_source_program_wire_golden_v1: ok 1 84 57 63")
            return 0
        print("usage: reference_source_program_wire_golden_v1.py --emit|--self-check",
              file=sys.stderr)
        return 2
    except (OSError, RuntimeError, ValueError) as error:
        print(f"reference_source_program_wire_golden_v1: FAIL: {error}", file=sys.stderr)
        return 1
