#!/usr/bin/env python3
"""Run the CMP-3 ReentrancyGuard differential against independent native references."""
...
SCENARIO_PATH = REPO_ROOT / "testkit/differential/reentrancy-guard/scenario.v1.json"
REFERENCE_ROOT = REPO_ROOT / "testkit/differential/reentrancy-guard/references"
PRODUCT_SOURCE = REPO_ROOT / "Examples/Product/ReentrancyGuard.lean"
...
STEPS: tuple[tuple[str, str], ...] = (
    ("initial-locked", "locked"),
    ("release-while-unlocked", "release"),
    ("state-after-failed-release", "locked"),
    ("acquire", "acquire"),
    ("locked-after-acquire", "locked"),
    ("acquire-while-locked", "acquire"),
    ("state-after-failed-acquire", "locked"),
    ("release", "release"),
    ("final-locked", "locked"),
)

EXPECTED_LOCK: tuple[int, ...] = (0, 0, 0, 1, 1, 1, 1, 0, 0)
ERRORS: Mapping[str, tuple[str, Mapping[str, str]]] = {
    "release-while-unlocked": ("assertion", {"kind": "lock-not-held"}),
    "acquire-while-locked": ("assertion", {"kind": "reentrant-call"}),
}
INTERFACES: Mapping[str, Mapping[str, Any]] = {
    "locked": {"entrypoint": "locked", "mutability": "view", "params": (), "returns": "u64"},
    "acquire": {"entrypoint": "acquire", "mutability": "call", "params": (), "returns": "unit"},
    "release": {"entrypoint": "release", "mutability": "call", "params": (), "returns": "unit"},
}
...
def direct_artifact(path: Path, target: str) -> dict[str, Any]:
    ...
    if mismatches:
        raise PilotError(f"{path.relative_to(REPO_ROOT)} is not a direct ReentrancyGuard artifact: {mismatches}")
    return document
...
def no_legacy_sidecars(root: Path) -> None:
    ...
    if forbidden:
        ...
        raise PilotError(f"direct ReentrancyGuard output contains forbidden legacy sidecars: {names}")


def context(family: str, implementation: str, native_height: int | None = None) -> RunnerContext:
    return RunnerContext(
        accounts=(
            LogicalAccount("contract", f"reentrancy-guard.{implementation}.{family}", ("contract",)),
...
def validate_raw_step(runner_name: str, index: int, raw: Mapping[str, Any]) -> None:
    ...
    expected_lock = EXPECTED_LOCK[index]
    if raw.get("lock") != expected_lock:
        raise PilotError(
            f"{runner_name}: step {step_id} lock={raw.get('lock')}, expected {expected_lock}"
        )
    expected_return = expected_lock if action == "locked" else None
    if raw.get("returnLocked") != expected_return:
        raise PilotError(
            f"{runner_name}: step {step_id} returned {raw.get('returnLocked')}, expected {expected_return}"
        )
...
def normalized_result(
    ...
    for index, raw in enumerate(raw_steps):
        ...
        return_value = (
            NormalizedValue.u64(int(raw["returnLocked"])).to_json()
            if action == "locked"
        ...
        observations = {
            ...
            "state": {"lock": NormalizedValue.u64(int(raw["lock"])).to_json()},
            "balances": {},
            "events": [],
            "externalActions": [],
            "interface": {
    ...
    return RunnerResult(
        scenario_id="portable-reentrancy-guard-primary-triad",
...
def evm_locked(
    ...
    output = run(
        [cast, "call", "--rpc-url", rpc, address, f"locked()({return_type})"],
        env=env,
    ).strip()
    ...
    for step_id, action in STEPS:
        ...
        if action == "locked":
            resource = parse_number(
                run(
                    [cast, "estimate", "--rpc-url", rpc, "--from", ALICE_EVM, address, "locked()"],
                    env=env,
                ).strip()
            )
            return_locked = evm_locked(cast, rpc, address, return_type, env)
            native_error = None
        else:
        ...
        raw_steps.append(
            {
                ...
                "success": success,
                "error": native_error,
                "returnLocked": return_locked,
                "lock": evm_locked(cast, rpc, address, return_type, env),
                "events": [],
                "externalActions": [],
                "resource": resource,
            }
...
def compile_native_evm(solc: str, output: Path, reference: Mapping[str, Any]) -> str:
    ...
    if f"Version: {version}" not in version_output:
        raise PilotError(f"ReentrancyGuard EVM reference requires solc {version}; got:\n{version_output}")
    output.mkdir(parents=True, exist_ok=True)
    source = REPO_ROOT / reference["source"]["path"]
    document = json.loads(run([solc, "--combined-json", "abi,bin", str(source)]))
    matches = [value for key, value in document.get("contracts", {}).items() if key.endswith(":ReentrancyGuard")]
    if len(matches) != 1:
        raise PilotError(f"solc returned {len(matches)} ReentrancyGuard contracts")
    contract = matches[0]
    ...
    if functions != set(INTERFACES):
        raise PilotError(f"native Solidity ReentrancyGuard ABI functions drift: {sorted(functions)}")
    if any(entry.get("type") == "event" for entry in abi):
        raise PilotError("native Solidity ReentrancyGuard unexpectedly declares events")
...
def build_pf_evm(output: Path, env: Mapping[str, str], cast: str) -> tuple[str, dict[str, Any]]:
    output.mkdir(parents=True, exist_ok=True)
    bytecode = output / "ReentrancyGuard.bin"
    artifact_path = output / "ReentrancyGuard.proof-forge-artifact.json"
    run(
        [
    ...
    init_path = output / "ReentrancyGuard.init.bin"
    if not init_path.is_file():
        raise PilotError("ProofForge EVM direct build did not produce ReentrancyGuard.init.bin")
    ...
    try:
        for _ in range(100):
            if process.poll() is not None:
                ...
                raise PilotError(f"Anvil exited before ReentrancyGuard execution: {stderr}")
...
def run_solana_binary(elf: Path, runner_name: str, env: Mapping[str, str]) -> RunnerResult:
    stdout = run(
        [
            ...
            "-p", "proof-forge-testkit-harness-solana", "--bin", "reentrancy_guard_differential", "--",
            str(elf), runner_name,
        ],
        env=env,
        timeout=300,
    ...
    if document.get("schema") != "proof-forge.native-reentrancy-guard.solana.v1":
    ...
    raw_steps = [
        {
            ...
            "error": item["error"],
            "returnLocked": item["returnU64"],
            "lock": item["state"]["lock"],
            "events": [],
            "externalActions": [],
...
def build_pf_solana(output: Path, env: Mapping[str, str]) -> tuple[Path, dict[str, Any]]:
    ...
    if not elf.is_file():
        raise PilotError("ProofForge Solana direct build did not produce ReentrancyGuard.so")
    no_legacy_sidecars(output)
    return elf, artifact
...
def build_native_solana(
    ...
    run(
        [
            cargo_build_sbf, "--manifest-path", "benchmarks/native/solana/reentrancy-guard/Cargo.toml",
...
def decode_near_locked(payload_hex: str | None, native_json: bool, label: str) -> int:
    if payload_hex is None:
        raise PilotError(f"{label}: expected locked return bytes")
    ...
    if value not in (0, 1):
        raise PilotError(f"{label}: observed invalid lock value {value!r}")
    return int(value)
...
def near_lifecycle(
    ...
    for index, (step_id, action) in enumerate(STEPS):
        ...
        if main["call"] != action or state_call["call"] != "locked":
            raise PilotError(f"{runner_name}: NEAR call grouping drift at {step_id}")
        if state_call["abort"] is not None:
            raise PilotError(f"{runner_name}: locked query aborted after {step_id}: {state_call['abort']}")
        if main["logs"] or state_call["logs"] or main["actions"] or state_call["actions"]:
            raise PilotError(f"{runner_name}: portable ReentrancyGuard unexpectedly created NEAR side effects")
        raw_steps.append(
            {
                "id": step_id,
                "action": action,
                ...
                "error": main["abort"],
                "returnLocked": (
                    decode_near_locked(main["return"], native_json, f"{runner_name}:{step_id}")
                    if action == "locked"
                    else None
                ),
                "lock": decode_near_locked(
...
def run_near(
    ...
    run(
        [
            ...
            "testkit/compare/near/reentrancy-guard/Cargo.toml",
        ],
        env=native_env,
        timeout=1200,
    )
    native_wasm = (
        REPO_ROOT
        / "testkit/compare/near/reentrancy-guard/target/wasm32-unknown-unknown/release/pf_near_sdk_reentrancy_guard_reference.wasm"
    )
    ...
    pf_wasm = pf_output / "reentrancyguard.wasm"
    if not pf_wasm.is_file():
        raise PilotError("ProofForge NEAR direct build did not produce reentrancyguard.wasm")
...
def compare_family(
    ...
    if not report["semanticMatch"]:
        raise PilotError(f"{family} ReentrancyGuard differential did not semantically match: {report['comparison']}")
    if report["observationCoverage"]["missing"]:
        raise PilotError(f"{family} ReentrancyGuard differential has incomplete coverage")
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("build/differential/reentrancy-guard"))
    ...
    summary = {
        "schema": "proof-forge.differential.reentrancy-guard-evidence.v1",
    ...
    print(f"differential-reentrancy-guard: ok ({output.relative_to(REPO_ROOT)}/evidence.v1.json)")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"differential-reentrancy-guard: {error}", file=sys.stderr)
        raise SystemExit(1)
