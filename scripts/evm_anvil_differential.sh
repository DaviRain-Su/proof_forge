...
#   5. TipJar / TokenJar / EnvReadJar / CallerCheck / BlockHeightCheck / Sha256Check / Keccak256Check / MerkleVerifyCheck companions
...
# Sha256Check: ADR-0031 SYS-S5-EVM pf.crypto.sha256 → precompile 0x02.
...
if [[ -x "$root/scripts/evm_sha256_anvil_smoke.sh" ]]; then
  echo "evm-anvil-differential: companion Sha256Check pf.crypto.sha256 smoke (build hard-fail; profile=$expected_profile_wire)" >&2
  PF_EVM_PROFILE="$evm_profile" bash "$root/scripts/evm_sha256_anvil_smoke.sh" || {
    echo "evm-anvil-differential: Sha256Check smoke failed (hard)" >&2
    exit 1
  }
elif [[ -f "$root/scripts/evm_sha256_anvil_smoke.sh" ]]; then
  echo "evm-anvil-differential: Sha256Check smoke present but not executable (hard)" >&2
  exit 1
else
...
# MerkleVerifyCheck: CAP-X-MERKLE-EVM pf.crypto.merkleVerifyKeccak256 D=2
# unrolled sorted-pair keccak256(0, 64). Product build/runtime assertions
# are hard; missing locked tools are handled inside the host-optional
# companion (exit 0). Engineering Anvil pin only — not formal C-3 / ICS-23.
if [[ -x "$root/scripts/evm_merkle_verify_anvil_smoke.sh" ]]; then
  echo "evm-anvil-differential: companion MerkleVerifyCheck merkleVerifyKeccak256 smoke (build hard-fail; profile=$expected_profile_wire)" >&2
  PF_EVM_PROFILE="$evm_profile" bash "$root/scripts/evm_merkle_verify_anvil_smoke.sh" || {
    echo "evm-anvil-differential: MerkleVerifyCheck smoke failed (hard)" >&2
    exit 1
  }
elif [[ -f "$root/scripts/evm_merkle_verify_anvil_smoke.sh" ]]; then
  echo "evm-anvil-differential: MerkleVerifyCheck smoke present but not executable (hard)" >&2
  exit 1
else
  echo "evm-anvil-differential: note: MerkleVerifyCheck companion script missing (skip leg)" >&2
fi
