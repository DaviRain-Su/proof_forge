#!/usr/bin/env python3
"""Development matrix for the ADR-0021 unique seqpacket lineage owner.

The positive case creates exactly one fixed-FD socketpair, configures both
buffers, forks, selectively retains one endpoint per process, performs an
ordinary static exec for the adapter, and directly inspects its exact inherited
FD set and endpoint identity through proc while kernel SCM_CREDENTIALS are
observed on the service endpoint.  These are development kernel facts, not a
formal custody ceremony or task evidence.
"""
...
_ROOT = _HERE.parent
_CASES = (
    "positive-exec-lineage",
    "invalid-api",
    "allocation-order",
    "occupied-fixed-fd",
    "reversed-endpoints",
    "manifest-channel",
    "prepare-before-select",
    "select-repeat",
    "prepare-repeat",
    "wrong-role",
    "buffer-drift",
    ...
    "service-before-exec",
    "service-passcred-already",
    "endpoint-expected-flags",
    "lineage-flags",
    "lineage-inode",
    "lineage-alias",
)
...
def _sources() -> list[Path]:
    return [
        _HERE / "task_qualification_socket_v2_driver.c",
        _HERE / "task_qualification_socket_v2.c",
...
def _compile(binary: Path, *, sanitizer: bool) -> None:
    ...
    if completed.returncode != 0 or completed.stdout:
        raise AssertionError(
            f"socket {'sanitizer' if sanitizer else 'static'} compile failed: "
...
def _inspect_static(binary: Path) -> None:
    ...
    if (not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1 or
        ...
        raise AssertionError("socket driver is not an ordinary single-link ELF")
    try:
        ...
        raise AssertionError("socket driver unexpectedly has security.capability")
    completed = subprocess.run(
    ...
    if completed.returncode != 0 or completed.stderr:
        raise AssertionError(f"socket ELF inspection failed: {completed.stderr!r}")
...
def _analyze_sources() -> None:
    ...
    if completed.returncode != 0 or completed.stdout or completed.stderr:
        raise AssertionError(
            f"socket static analysis failed: stdout={completed.stdout!r} "
...
def main() -> int:
    ...
    if platform.system() != "Linux" or platform.machine() != "x86_64":
        print("task-qualification socket self-test: requires Linux x86_64")
        return 1
    parent = arguments.scratch_root
    if parent is not None:
        parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix="pf-tq-socket-v2-",
        ...
        try:
            ...
            print(f"task-qualification socket self-test: PRECHECK-FAIL: {exc}")
            return 1
    for result in results:
        print(result)
    passed = sum(result.passed for result in results)
    print(f"task-qualification socket self-test: {passed}/{len(results)} passed")
