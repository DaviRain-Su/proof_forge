#!/usr/bin/env bash
# TransferSol local path via `pf`: build + offline verify + Mollusk test.
# No public RPC. Replaces ad-hoc cargo test wiring in justfile.
set -euo pipefail
...
if [[ -z "${PROOF_FORGE_SOLANA_CLIENT:-}" || ! -x "$PROOF_FORGE_SOLANA_CLIENT" ]]; then
...
rm -rf "$out"
echo "solana-transfer-sol-local: pf build → $out"
"$PF" build Examples/TransferSol.lean \
  --module Examples.TransferSol \
  --target solana \
  -o "$out"
echo "solana-transfer-sol-local: pf verify"
"$PF" verify -t solana --artifact "$out" --adapter transfer-sol-v1
echo "solana-transfer-sol-local: pf test (Mollusk TransferSol specialty)"
"$PF" test -t solana --artifact "$out"
echo "solana-transfer-sol-local: ok"
